...
where $A = \sqrt{2/3}$ is the off-democratic projection of any generation axis (§7.1). This is suggestive of a "double Higgs-projection" mechanism: the $U$-even component of the $O(\lambda^2)$ charged-lepton perturbation acquires a projection factor $A^2$ at each end of an inter-generation vertex (total $2A^2 = 4/3$), while the $U$-odd component does not. The same projection geometry produces $\sin\theta_{13} = A^2\lambda$ at $O(\lambda)$ via a single-vertex projection. A complete first-principles derivation of $b_{23} = (4/3)(b_{12}+b_{13})$ from explicit lattice perturbation theory on the cubic-lattice Yukawa structure remains an open task; the relation is presented here as a structural input supported by the geometric argument above and tested by the empirical sum rule.
