...
*Proof sketch.* (a) Direct counting; the map is well-defined and respects natural cubic-group action on spatial indices. (b) For $\gamma_5$ identification: $\varepsilon_{3D}^2 = 1$ matches $\gamma_5^2 = 1$ in Euclidean signature, and $\{\varepsilon_{3D}, \gamma_i\} = 0$ follows from $\varepsilon_{3D}(x \pm \hat i) = -\varepsilon_{3D}(x)$ via direct computation: $(\gamma_i \varepsilon_{3D} \chi)(x) = \eta_i(x)[\varepsilon_{3D}(x+\hat i)\chi(x+\hat i) - \varepsilon_{3D}(x-\hat i)\chi(x-\hat i)]/2 = -\varepsilon_{3D}(x)(\gamma_i \chi)(x)$. The matrix structure (sublattice grading acts diagonally with opposite signs, like $\gamma_5$ in chiral basis, not off-diagonally like $\gamma_0$) confirms $\gamma_5 \leftrightarrow \varepsilon_{3D}$ rather than $\gamma_0 \leftrightarrow \varepsilon_{3D}$. The $\gamma_0$ identification follows from $\gamma_0 = \pm \gamma_5 \gamma_1 \gamma_2 \gamma_3$ in the standard Clifford algebra. (c) Direct: $\varepsilon_{4D}(x_0, \vec x) = (-1)^{x_0}\varepsilon_{3D}(\vec x)$. (d) Verified by checking the staggered shift transformation acts compatibly under the isomorphism of (a). $\square$
