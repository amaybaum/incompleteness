...
All three SM gauge couplings are reproduced through a chain with one structural input ($1/\alpha_0 = 23.25$) and three parameters of the threshold function ($\delta_0$, $A$, $B$) fixed by the three observed couplings. The structural predictions in this sector are:

**(i) The structural value $1/\alpha_0 = 23.25$** from the one-loop staggered vacuum polarization (§6.1). This is computed analytically, has no free parameters, and is the foundational input for the rest of the chain.

**(ii) The universality of $\delta_0$** — the $C_2$-independent part of the threshold must be the same for all three gauge groups by construction (no $C_2$ dependence ⇒ no group dependence at this order). This structural claim is the reason a single parameter $\delta_0$ appears in all three rows, rather than three independent parameters $\delta_0^{\text{U(1)}}, \delta_0^{\text{SU(2)}}, \delta_0^{\text{SU(3)}}$. The universality reduces the parameter count in the threshold function by two, making the log form a genuinely three-parameter fit rather than a five-parameter fit.

**(iii) The consistency of the 2-loop VP computation with the empirically determined $\delta_0$.** The U(1) row fixes $\delta_0 = 10.02$ empirically. The 2-loop staggered VP computation from first principles gives $\delta_0^{(2L)} = 8.0 \pm 2$, with the natural 3-loop convergence range $9.6$–$10.4$ bracketing the empirical value. This is a non-trivial consistency check: a first-principles calculation agrees with the fitted value within its expected convergence range.

The $C_2$-dependent threshold parameters $(A, B) = (8.3, 5.59)$ are fixed by matching to the observed SU(2) and SU(3) couplings at $M_Z$, given the structurally computed $1/\alpha_0$ and the empirically fixed $\delta_0$. In the current formulation, this makes the SU(2) and SU(3) couplings retrodictions rather than strict predictions. Deriving $(A, B)$ from a two-loop lattice PT computation of the $C_2$-dependent threshold (an open computational task) would upgrade both entries to strict first-principles predictions; the current chain provides the structural form and fixed points required for such a derivation to be tested against.
