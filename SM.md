...
*Remark (scope: $\bar\theta$ versus flavor-basis CP violation).* Theorem 21 establishes $\bar\theta = 0$, which is the *static* CP-odd combination $\theta + \arg\det(Y_u Y_d)$ — the quantity coupling to QCD instantons and generating a neutron electric dipole moment. It does *not* forbid the CKM and PMNS CP-violating phases that are observed in flavor-changing processes. The distinction is that $\bar\theta$ is a property of the determinant $\det(Y_u Y_d)$, while CKM CP violation (encoded in the Jarlskog invariant $J \approx 3 \times 10^{-5}$) arises from the *basis mismatch* between the up-type and down-type Yukawa mass eigenstates. T-invariant Yukawas are simultaneously real in some basis — specifically, each Yukawa matrix is real in its own mass basis — but the up-type and down-type mass bases generically differ by a unitary transformation, and when this transformation has complex entries that cannot be absorbed by field redefinitions, the resulting CKM matrix contains an irreducible CP-violating phase. This is the standard SM picture [16, §6.3]: real mass-basis Yukawas plus a basis mismatch are together compatible with a nonzero Jarlskog. The framework's T-invariance at the substrate level forces each Yukawa to be individually diagonalizable to real form (fixing $\bar\theta = 0$), but does not force the two mass bases to coincide. The basis mismatch is a solution-specific property of the particular bijection $\varphi$, analogous to the absolute scale of fermion masses (§7.6) and the specific eigenvalues of the Yukawa matrices: the framework derives the structural form but does not predict the basis-alignment parameters. The CKM and PMNS CP phases are therefore compatible with — though not derived from — the framework's structural commitments, and are listed as solution-specific inputs in §8.3 and in the pre-registered falsification conditions ([Substratum, §6.4, "what does not falsify"]).
