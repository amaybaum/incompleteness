...
- $\delta_0 = 10.0$: the $C_2$-independent threshold. In the current formulation, $\delta_0$ is determined empirically by the U(1) row (see below). A 2-loop staggered VP computation was implemented as a consistency check; an audit of that implementation (see `PI_S_CONVENTION_AUDIT.md` in the repository) found that the numerical scalar used as the induced-propagator denominator in the code is not the $\Pi_s$ defined above, and the resulting ratio exhibits strong IR-regulator dependence (factor $\sim 7$ variation across $m \in [0.01, 0.20]$) characteristic of a scheme-locked quantity rather than a physical $\delta(1/\alpha)$ shift. The 2-loop result is therefore held in reserve pending a rebuilt calculation with the correct $\Pi_s$ convention. A natural 3-loop bound provides the structural plausibility check:
...
- Subleading contributions. The F$^3$ correction has been propagated through the bubble (session 2026-04-23l): the sensitivity is $|\Delta(A \cdot B)/(A \cdot B)| \approx 8.4 \cdot |c_{F^3}|$, extracted from a $c_{F^3}$ sweep at $N = 8, 12, 16, 20$ with $1/N^2$ extrapolation. Under the continuum 1-loop fermion-induced value $c_{F^3} \approx -1/(360\pi^2) \approx -2.8 \times 10^{-4}$, the shift is $\approx 0.24\%$ — negligible within the extrapolation uncertainty. Under a conservative upper bound (naive $N_f = 6$ flavor multiplication, $c_{F^3} \approx -1.7 \times 10^{-3}$), the shift is $\approx 1.4\%$. The raw lattice value $c_{F^3} \approx -0.034$ extracted in an earlier session is in a different normalization (a $\sim 120\times$ discrepancy vs the continuum Wilson coefficient was noted in that extraction) and is not directly applicable in the bubble integration without a normalization-matching step. The dominant remaining subleading contribution is lattice $V_{4g}$ cos-factor corrections to the tadpole's $p^2$-coefficient (absent in the continuum $V_{4g}$ used here), estimated at $\pm 4$–$5\%$.
...
**Open work.** Separating $A$ from $B$ individually requires the next-order coefficient of $(C_2 g_0^2)^2$ in the expansion of the gauge self-energy. In the OI induced theory, this coefficient is structurally suppressed at strict 2-loop: the $C_A^2$ channel receives only negligible Faddeev-Popov ghost contributions, with induced 3- and 4-gluon vertices first appearing at 3-loop order (no tree-level gauge self-interactions exist in the induced theory). A clean separation of $A$ from $B$ therefore requires 3-loop lattice perturbation theory or non-perturbative methods, both beyond the current scope. Under the one-parameter constraint $A \cdot B = 48$ alone (§6.2.1), fitting the remaining freedom to the observed SU(2) and SU(3) couplings gives $(A, B) \approx (8.07, 5.95)$, numerically close to the suggestive structural hypothesis $(A, B) = (\dim \mathrm{adj}\,\mathrm{SU}(3), N_f) = (8, 6)$ — both consistent with the unconstrained fit $(8.3, 5.59)$ within the $\pm 3\%$ systematic uncertainty on $A \cdot B$. No rigorous derivation of individual $A$ and $B$ is yet available. The F$^3$ correction has been propagated (see uncertainty bullet above); the remaining systematic component is dominated by lattice $V_{4g}$ cos-factor corrections, whose inclusion would reduce the $\pm 4$–$5\%$ residual systematic.
...
**(iv) Consistency check with the 2-loop VP.** The U(1) row fixes $\delta_0 = 10.02$ from observation, given (i). A first-principles 2-loop staggered VP computation was implemented as a cross-check; an audit (see `PI_S_CONVENTION_AUDIT.md`) found that the original implementation used a scheme-locked $\Pi_s$ convention. A rebuilt calculation is in progress. The consistency of the rebuilt 2-loop value with empirical $\delta_0$ is an open question and is held in reserve; the U(1)-row determination is the primary source of $\delta_0$ in the current framework.
