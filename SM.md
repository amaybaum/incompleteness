...
The explicit derivation of the amplitude-level formula from the cubic-lattice Yukawa structure remains an open OI-specific calculation, structurally analogous to the deferred OI-vertex 1-loop computation for $K = 1/2$ (§7.5) in being mixed-layer — substratum cubic-group structure combined with electroweak-symmetry-breaking emergent machinery. The structural square-root relation is supported by the empirical match at $0.05\sigma$ but is not yet derived to theorem grade from first principles.
