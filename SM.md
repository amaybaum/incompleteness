...
The cycle ergodicity assumption deserves explicit acknowledgment. Pure linear dynamics on a cubic lattice is *integrable*, not chaotic: the lattice frequencies $\omega(\vec{k}) = 2\sin(|\vec{k}|/2)$ are not linearly independent over $\mathbb{Q}$, so orbits fill a sub-torus of the energy shell rather than the full shell. The Gaussianization argument therefore has two natural interpretations: (way out i) the cubic-lattice + finite-set discretization introduces small but non-zero non-linearity (e.g., from rounding rules in the bijection update), providing the chaos needed for full-shell sampling; or (way out ii) the "Gaussian on energy shell" claim is shorthand for "Gaussian on the sub-torus actually sampled," which is a smaller phase space and would modify the substratum-pseudofermion covariance match. Way out (i) is the standard interpretation for lattice-substrate frameworks; we adopt it implicitly. The detailed cycle-ergodicity question for OI's specific bijection orbit structure remains a Level 3 residual in the bridge-gap classification of §7.5.
...
**Over-determination of $n_{\text{gen}} = 3$.** The matching condition $K_{\text{loop}} \cdot (n_{\text{gen}} - 1) = 1$ (forced by $m_{\text{match}} = \lambda g_0^2$ with prefactor unity from power counting) combined with $K_{\text{loop}} = 1/2$ (Argument 1, staggered convention) forces $n_{\text{gen}} - 1 = 2$, i.e., $n_{\text{gen}} = 3$. This is an independent structural argument for the framework's three-generation structure, complementing the cubic decomposition $T_1 \oplus E \oplus A_1$ of Theorem 7. For any other $n_{\text{gen}}$ in the discrete enumeration $\{2, 3, 4, \ldots\}$ with $K_{\text{loop}} = 1/2$ unchanged, the matching condition would be inconsistent. The framework's commitment to three generations is therefore over-determined: the geometric argument (Theorem 7: $T_1$ has dimension 3) and the operational argument (matching consistency) converge on $n_{\text{gen}} = 3$ from independent mechanisms.
...
The framework's prediction $m_{\text{match}} = \lambda \cdot g_0^2$ requires $K = 1/2$. This value is established at Layer 1 by the structural identity above, with Layer 2(a)-tractable verification provided by:

**Argument 1 (Staggered convention).** The standard staggered fermion action uses the symmetric kinetic term $S_{\text{kin}} = \tfrac{1}{2}\sum_{x,\mu}\eta_\mu(x)\bar\chi(x)[U_\mu(x)\chi(x+\hat\mu) - U_\mu^\dagger(x-\hat\mu)\chi(x-\hat\mu)]$, with the gauge vertex $V_\mu = (g_0/2)\eta_\mu(x)[U_\mu + U_\mu^\dagger]$. The factor $1/2$ from this symmetrization propagates as a universal kinematic prefactor in one-loop bilinear renormalization. This is established in the canonical staggered perturbation theory references: Patel & Sharpe [33] derive the standard one-gluon vertex $V_\mu \propto -ig\cos(q'_\mu + k_\mu/2)(\gamma_\mu \otimes 1)$ (their Eq. 17) — the same form as the OI vertex $V_j = ig_0\mu T^\alpha\cos(k_j + p_j/2)$ — and the two-gluon (tadpole) vertex with explicit factor $-i\tfrac{1}{2}g^2$ (their Eq. 18) tracing directly to the symmetrization. The full one-loop renormalization of staggered scalar density bilinears computed there and in subsequent work [Constantinou-Costa-Panagopoulos and others] shows the $1/2$ propagating consistently through to the renormalization coefficients $Z_S, Z_P$.

The key point: **OI uses the standard staggered convention for the vertex normalization**, not a different one. The OI's taste-decomposed Coleman-Weinberg construction inherits the same one-loop kinematic structure as standard staggered $Z_S$ calculations, including the $1/2$ prefactor. The OI's K = 1/2 is therefore not an independent OI prediction but the value forced by the staggered convention OI uses; verifying it explicitly within OI amounts to checking that no OI-specific modification (e.g., the matching mass $\mu$ insertion in the vertex) breaks the inheritance from standard staggered. The structural argument is: same vertex form (cited from Patel-Sharpe Eq. 17), same symmetrization (cited from Patel-Sharpe action setup), same loop kinematic structure → same K. The matching mass $\mu$ enters the OI calculation as an overall factor (it sets the scale at which the matching is performed, not the dimensionless loop coefficient), so it does not modify K.
...
$K = 1/2$ is therefore established at Layer 1 by the representation-theoretic identity $K = 1/(n_{\text{gen}} - 1)$ (which forces $K = 1/2$ for the framework's required $n_{\text{gen}} = 3$), with Layer 2(a)-tractable verification provided by Argument 1 (inheritance from standard staggered convention), Consistency check 2 (definitional symmetric averaging), and Consistency check 3 (empirical exclusion of natural-fraction alternatives).

Within the four-layer framing (§8.3), the $K = 1/2$ coefficient is now Layer 1 closed at the structural-identity level, with the remaining open question being the Layer 2(a)-tractable verification that the OI's specific taste-decomposed Coleman–Weinberg potential reproduces $K = 1/(n_{\text{gen}} - 1)$ at one loop. This parallels the §7.3 Cond 2 stratification: both predictions have a Layer 1 representation-theoretic core (Cond 2's $4A^2 = 4(d-1)/d$ and D10's $K = 1/(n_{\text{gen}} - 1)$) plus a Layer 2(a)-tractable substrate-loop verification piece. The deferred OI-specific 1-loop computation produces useful data either way: confirmation strengthens the framework, while a different result would localize the discrepancy in the loop kinematic structure rather than the structural identity itself.

**Residual gap.** With Argument 1 reframed as inheritance from the standard staggered convention rather than independent OI derivation, the structural support for $K = 1/2$ is substantially strengthened: K = 1/2 is the value forced by the staggered vertex normalization OI uses, established in published staggered perturbation theory [33], and the matching mass $\mu$ enters as an overall scale rather than modifying the dimensionless loop coefficient. The remaining gap is verifying explicitly that the OI's specific taste-decomposed Coleman-Weinberg construction reproduces the standard staggered calculation at the relevant level of detail (no convention drift, no missing diagram). A fully explicit one-loop calculation within the OI lattice setup — computing the diagram directly with the OI vertex $V_j = ig_0\mu T^\alpha\cos(k_j + p_j/2)$ and confirming it reduces to the standard staggered $Z_S$ structure — would close this gap definitively.

The per-vertex $\cos^2$ symmetrization producing the $1/2$ factor is structurally robust to the 3D-vs-4D dimensional change: the identity $\cos^2(k/2) = (1+\cos(k))/2$ is dimension-independent per direction, so the factor $1/2$ is preserved in OI's spatial-only vertex form. Four bridge gaps in the SχPT inheritance are addressed below; their current status:
...
- **ᵇ** *Layered prediction (m_b/m_τ).* The matching scale $m_{\text{match}} = \lambda \cdot g_0^2$ is structurally derived in the taste-decomposed Coleman–Weinberg potential (§7.5): the b-quark is identified with the democratic eigenstate $\hat{h}$ of the cubic-symmetric taste-breaking operator (from §7.1, §7.2 representation theory), giving diagonal taste-breaking $c_{bb} = (n_{\text{gen}} - 1) \lambda = 2\lambda$. Combining with the natural matching condition at condensate scale $\Sigma$ and a loop kinematic coefficient $K$, the prediction $m_{\text{match}} = \lambda g_0^2$ requires $K = 1/(n_{\text{gen}} - 1) = 1/2$ — a representation-theoretic structural identity (Layer 1 closed) paralleling Wolfenstein $A^2 = (d-1)/d$. Layer 2(a)-tractable verification: the staggered kinetic-term symmetrization plus empirical exclusion of natural alternatives within $K \in \{1/4, 1/2, 1, 2\}$ ($K = 1$ excluded at $35\%$, $K = 1/4$ at $24\%$); see §7.5 for the structural detail. The MC-extracted value $m_{\text{match}}^{\text{MC}} = 0.1234 \pm 0.0002$ agrees with the derived $\lambda g_0^2 = 0.1217$ within $1.4\%$ (NLO truncation). The central-value match $m_b/m_\tau = 0.5\sigma$ is at $L = 32$; realistic systematic uncertainty including finite-volume and regulator effects is $\sim 1$–$2\%$ (§7.5). Within the four-layer framing (§8.3), $K = 1/2$ is now Layer 1 closed at the structural-identity level, with the Layer 2(a)-tractable residual being whether the OI's specific taste-decomposed Coleman-Weinberg one-loop calculation reproduces $K = 1/(n_{\text{gen}} - 1)$ explicitly.
