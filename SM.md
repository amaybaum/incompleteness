...
**(iv) Consistency check with the 2-loop VP.** The U(1) row fixes $\delta_0 = 10.02$ from observation, given (i). A first-principles 2-loop staggered VP computation was implemented as a cross-check, but the implementation was found to use a scheme-locked $\Pi_s$ convention. The consistency of a correctly-implemented 2-loop value with empirical $\delta_0$ is an open question; the U(1)-row determination is the primary source of $\delta_0$ in the current framework.
...
Twenty-two quantitative observables from a $d = 3$ cubic lattice with spacing $\epsilon = 2\,l_P$, classified by input status: **S** = unconditional parameter-free structural prediction (Layer 0 or Layer 1; see §8.3); **C** = conditional structural prediction, conditional on a stated structural input pending derivation (§7.3 Cond 2); **L** = layered conditional prediction combining structural form with one Layer-2 substrate input or Layer-3 empirical input (see §8.3); **M** = one-input mass/parameter chain; **E** = uses explicit empirical input; **P** = phenomenological/ansatz input; **R** = retrodiction. Items classified as **C** or **L** carry footnotes specifying the conditional input and the open derivation (if any) that would close the gap to **S**.
