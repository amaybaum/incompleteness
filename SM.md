...
C2: the wave equation is a Hamiltonian system — it conserves energy and preserves phase-space volume. In a closed finite-dimensional Hamiltonian system no true relaxation occurs (the spectrum is purely imaginary); what sets the memory timescale is the *dephasing* rate of hidden-sector correlations under ETH, not a decay rate from a spectral gap. The C2 necessity proof [Main, §3.4] establishes that fast hidden-sector dephasing (small $\tau_B$) drives the marginal dynamics toward Markovianity on accessible timescales, while slow dephasing (large $\tau_B$) preserves the non-Markovian returns that produce P-indivisibility. For the cubic-lattice substratum with a cosmological-horizon partition, the hidden-sector dephasing time $\tau_B$ is set by cosmological scales while laboratory $\tau_S$ is many orders of magnitude shorter, placing the system deep in the slow-bath regime ($\tau_S \ll \tau_B$) regardless of partition geometry.
