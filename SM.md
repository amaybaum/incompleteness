...
**Theorem 28** (Local gauge invariance from background independence). *Background independence of the coupling structure, combined with the dynamics selection theorem, implies local gauge invariance.*

*Proof.* (i) Background independence (Ontology §7.4) requires the coupling matrix $M$ to be state-dependent: $M = M(\mathbf{n}, \hat{e}_j)$ varies from link to link. The wave equation becomes $\phi_a(\mathbf{n}, t+1) = \sum_j M_{ab}(\mathbf{n}, \hat{e}_j)[\phi_b(\mathbf{n}+\hat{e}_j, t) + \phi_b(\mathbf{n}-\hat{e}_j, t)] - \phi_a(\mathbf{n}, t-1)$, with $M$ now a link variable.

(ii) The dynamics selection theorem (Theorem 13) and cubic group decomposition (Theorem 11) are constraints on the *local* structure of the coupling graph: the cubic symmetry acts at each site independently. At every site, $M(\mathbf{n}, \hat{e}_j)$ has $K = 6$ components with eigenvalue multiplicities $(3, 2, 1)$.

(iii) The commutant of $M(\mathbf{n}, \hat{e}_j)$ at each site is $\mathrm{U}(3) \times \mathrm{U}(2) \times \mathrm{U}(1)$: it has dimension 14 = 9 + 4 + 1, determined by the multiplicities alone, not by the specific eigenvalues. Since $M$ varies from site to site, the commutant acts independently at each site. Under a site-dependent transformation $G(\mathbf{n}) \in \mathrm{U}(3) \times \mathrm{U}(2) \times \mathrm{U}(1)$:

$$\phi(\mathbf{n}) \to G(\mathbf{n})\,\phi(\mathbf{n}), \qquad M(\mathbf{n}, \hat{e}_j) \to G(\mathbf{n})\,M(\mathbf{n}, \hat{e}_j)\,G(\mathbf{n}+\hat{e}_j)^{-1}$$

The wave equation is invariant under this transformation. This is local gauge invariance.

(iv) The link variable $M(\mathbf{n}, \hat{e}_j)$ transforms as a gauge connection. The plaquette product $P(\mathbf{n}, j, k) = M(\mathbf{n}, \hat{e}_j)\,M(\mathbf{n}+\hat{e}_j, \hat{e}_k)\,M(\mathbf{n}+\hat{e}_k, \hat{e}_j)^{-1}\,M(\mathbf{n}, \hat{e}_k)^{-1}$ transforms in the adjoint: $P \to G(\mathbf{n})\,P\,G(\mathbf{n})^{-1}$. Therefore $\mathrm{Re\,Tr}(P)$ is gauge-invariant — it is the Wilson plaquette action [9], now derived rather than postulated. $\square$

*Remark.* The remaining structural gap is continuum-limit universality: the proof that the lattice gauge theory with gauge group $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ flows to the correct continuum QFT under the renormalization group. This is supported by decades of numerical evidence but not rigorously proved — it is the Yang-Mills mass gap problem. This gap is shared with all lattice QCD, not specific to this framework.
...
| 4 | Gauge group = commutant of $M$ | Thm. 6, Thm. 28 | $\mathrm{U}(n_1) \times \cdots$ (local) | Theorem (background independence → local gauge invariance) |
...
**Status summary.** The primary derivation chain (steps 0–6, 4a, 10, 12–13, 14a, 15, 16a–b) is proved end-to-end, from the wave equation to SU(3) × SU(2) × U(1) with three chiral generations, one Higgs doublet, unique hypercharges, and θ̄ = 0 at all energy scales. Local gauge invariance is derived from background independence (Theorem 28), not postulated. Steps 7–9, 11, and 14b are propositions that provide independent confirmations of results already established on the primary route (N_c = 3 from step 4a, chirality from step 10, N_gen = 3 from step 14a). No genuine gaps remain in the primary chain. The sole remaining structural gap — continuum-limit universality — is shared with all lattice QFT.
...
**Open:** (i) Continuum limit of lattice gauge theory — universality is supported by numerical evidence but not rigorously proved; a gap shared with all of lattice QCD (essentially the Yang-Mills mass gap problem). (ii) Generation identification — the fermion count is proved; the representation assignment is a unique consistency argument, not a geometric derivation. (iii) Taste-breaking phenomenology — whether specific patterns in $\varphi$ reproduce the observed mass ratios.
...
The filter chain from $(S, \varphi)$ to the Standard Model contains *no empirical inputs beyond the two structural properties* (bounded coupling and statistical isotropy). The chain's steps vary in rigor: the bottom-up results (wave equation → KG → staggered fermions → chiral symmetry, $K = 2d = 6$, gauge multiplicities $(3, 2, 1)$, local gauge invariance from background independence, $\theta = 0$, detailed balance at all scales) are at theorem level. The top-down constraints (asymptotic freedom, minimality of $N_c$, chirality from the trace-out) are at proposition level — well-motivated physical arguments, not pure mathematics, but redundant with the primary chain's proved results. The identification of fermion sectors with SM generations involves anomaly cancellation uniqueness (proved). The continuum-limit universality that connects the lattice theory to the SM is supported by extensive numerical evidence but shares the status of all lattice QCD — it is the only remaining structural gap. These distinctions matter: the framework's credibility rests on precision about what is proved and what is motivated.
