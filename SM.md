...
The framework as written derives the gauge group $SU(3) \times SU(2) \times U(1)$ from the commutant construction (Theorems 5, 7, 14) and uses anomaly cancellation (Theorem 15) as a uniqueness check on the SM matter assignment, given a chiral $SU(2)$ structure. The matter representations and hypercharge assignments are derived — via the link-carrier construction of §4.7.X.2 and anomaly cancellation (Theorem 15) — rather than taken as input. The framework's claim on the fermion sector is therefore: *the SM gauge group is forced by first principles; the SM matter content, its gauge representations, and its hypercharges are the unique anomaly-free consequence of placing matter on links in the link-carrier space $V_K \otimes V_K^*$.* The residual gap is narrower: the absolute generation count (why $K = 2d$ admits exactly three triplet tastes rather than more) is not derived from first principles and is noted as the surviving open problem.
...
#### 4.7.X.1 Partial resolution: gauge bosons from the Schur complement, and a level-confusion diagnosis for the fermion sector

The Schur-complement trace-out used in Theorem 13 to establish chirality contains more structural information than that theorem extracts. Pursuing it carefully yields a clean retrodiction of the gauge boson content of the Standard Model from $\mathrm{End}(V_K)$, and simultaneously sharpens the open fermion-embedding problem of §4.7.X into a precise and tractable form.
...
This reframing is consistent with — and arguably required by — the way Theorem 13 already handles chirality. That proof treats the gauge group as acting on $V_K$ *coupled to* the spinor/sublattice grading, not as a pure internal rotation on a 6-dimensional space: SU(3) is vector-like because color commutes with the grading, SU(2) is chiral because its action survives only on the left-handed external legs after the trace-out. Theorem 13 is already implicitly working at the observer level, where the gauge action is induced rather than literal. §4.7.X.1 simply makes this commitment explicit and applies it to the matter sector.

**The asymmetry is structural, not embarrassing.** The reframing also explains *why* the gauge sector closes from $\mathrm{End}(V_K)$ while the matter sector does not close from $V_K$. Gauge bosons are properties of the coupling matrix $M$ itself — they live at the bijection level, where the direct-sum decomposition of $V_K$ is the literal structure and $\mathrm{End}(V_K)$ is the literal kernel produced by the Schur complement. Fermions are observer-level objects: they are what an embedded observer reports when it coarse-grains lattice deviations into single-particle states. The two sectors live at two different layers of the framework, and the layer separation is exactly what "QM is emergent" means. The asymmetry between a closed gauge-boson retrodiction and an open fermion-embedding question is therefore not a defect; it is the expected signature of a framework in which gauge fields are structural and matter is emergent.

**The open problem, restated sharply.** The open problem of §4.7.X is therefore not "how does one extract $(\mathbf{3}, \mathbf{2})$ from $V_3 \oplus V_2 \oplus V_1$" — that question is well-posed only under the level confusion identified above, and under that confusion the answer is "one cannot." The correct successor question is:

> **Open problem (§4.7.X.1).** Characterize the coarse-graining map from bijection-level configurations of $(S, \varphi)$ to the observer-level single-fermion Hilbert space $\mathcal{H}_{\text{obs}}$, such that the induced action of the bijection-level commutant $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ on $\mathcal{H}_{\text{obs}}$ realizes exactly one Standard Model generation per $T_1$ staggered taste, with the chirality assignments fixed by Theorem 13.

This is a concrete research program rather than a vague structural worry. It connects the matter-sector embedding to the same coarse-graining machinery that [Main] uses to derive emergent QM in the first place, and it predicts that the resolution should look uniform across the three $T_1$-taste generations established in Theorem 10. The escape hatches flagged in §4.7.X — bimodule constructions, link- or plaquette-level gauge actions, or fermions placed outside the gauge-defining carrier — are all candidate realizations of this coarse-graining map rather than alternatives to it.

#### 4.7.X.2 Closure: Matter on Links
...
**Status of §4.7.X.** With the closure above, the open problem of §4.7.X is resolved within the existing framework, not via a new construction but via a corrected reading of where the matter carrier lives. The remaining caveat noted earlier — that the assignment of SM representations to tastes is a uniqueness argument via anomaly cancellation rather than a direct geometric derivation — stands unchanged and is the honest residual scope. What is no longer open is the algebraic question of how cross-charged representations like $(\mathbf{3}, \mathbf{2})$ fit into the framework at all.
