...
**Pre-registration.** Flavor-sector CP phases are listed as solution-specific inputs in §8.3 and in the pre-registered falsification conditions ([Substratum, §6.4, "what does not falsify"]). They join the absolute fermion mass scale $m_s$ and the specific neutrino Yukawa eigenvalues in the class of quantities the framework treats as properties of the particular $\varphi$. The framework makes no commitment to any specific value of $\delta_{\text{CKM}}$, $\delta_{\text{PMNS}}$, the Jarlskog $J$, or the Wolfenstein parameters $(\rho, \eta)$, and the measurement of these quantities does not bear on the framework's structural status. This is the honest scope boundary of the $\bar\theta = 0$ result.

**Attempted structural derivation retired.** A framework-internal derivation of the CKM phase via a hypothesized T-parity assignment on link-carrier orientations was investigated and ruled out. The relevant obstacle is that the substrate-level T-symmetry of Theorem 17 is stronger than the physical Dirac-level T-symmetry — M is real at the substrate level, and the complex structure of the emergent Yukawas comes from the γ-matrix representation of the staggered-to-Dirac reconstruction (which is fixed) rather than from any orientation-dependent feature of the lattice. Producing the CKM phase as a structural prediction would require extending the framework's axiomatization with new structural commitments not present in A1–A6. Such an extension is outside the scope of the current construction and is noted as a potential direction for future research alongside the rigorous ν-derivation (Substratum §7).
