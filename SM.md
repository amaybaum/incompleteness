...
- $\delta_0 = 10.0$: the $C_2$-independent threshold. The 2-loop staggered VP computation with the induced gauge propagator $D(p) = 1/(p^2_{\text{lat}} \cdot \Pi_s)$ — an 8-dimensional lattice integral evaluated on $N^4$ grids up to $N = 12$ with Richardson extrapolation — gives $\delta_0^{(2L)} = 8.0 \pm 2$ from the four contributions: self-energy insertion ($+3.9$), vertex correction ($+2.7$, by Ward identity), momentum-dependent $\Pi_s$ correction ($+0.8$), and sails (crossed) diagram ($+0.63$, computed directly and N-independent to 5 digits across $N = 6, 8, 10, 12$). The required value $\delta_0 = 10.0$ exceeds this by $\Delta = 2.0$, consistent with the natural 3-loop range:
  ...
  *3-loop bound.* The non-perturbative coupling at the staggered VP scale gives an effective expansion parameter $\sim g_0^2/\pi \approx 0.17$. Standard lattice perturbation theory in this regime has 2-loop $\to$ 3-loop ratios of $0.20$–$0.30$. The observed 1-loop $\to$ 2-loop ratio is $\delta_0^{(2L)}/\delta_0^{(1L)} \approx 8.0/4.7 \approx 1.7$, indicating the series is non-asymptotic but not divergent. The expected 3-loop contribution is $0.20$–$0.30 \times 8.0 = 1.6$–$2.4$. The required $\Delta = 2.0$ sits squarely in the middle of this natural range.
...
The spread across estimators is $\sim 2$, dominated by the linear $1/N^2$ fit lying slightly higher than the Shanks family. The Shanks iterates (levels 2, 3, 4) are stable at $47.9$, capturing the subleading-$1/N^2$ corrections that the naive linear fit misses. Taking the iterated-Shanks consensus as central and the Shanks-vs-$1/N^2$ spread as uncertainty:

$$\boxed{A \cdot B = 48.0 \pm 1.5 \quad (\pm 3\%)}$$

against the fitted value $A \cdot B = 46.4$, agreement at the $3\%$ level.
...
**Open work.** Separating $A$ from $B$ individually requires computing the next-order coefficient of $(C_2 g_0^2)^2$ in the expansion of the gauge self-energy — a substantial additional calculation and a direct test of whether the log-resum functional form $A \ln(1 + B \cdot C_2 g_0^2)$ is quantitatively accurate beyond the leading linear term. Including the F$^3$ subleading piece of the induced $\Gamma_3$ via $c_{F^3}$ would further reduce the current $\pm 5\%$ systematic uncertainty on $A \cdot B$.
...
**Status of the rows.** U(1) uses one fitted parameter ($\delta_0$) against one observable; the structural content is (i) together with the consistency check of (iv). SU(2) and SU(3) use two additional fitted parameters ($A$, $B$) against two additional observables; their structural content is carried by the universality constraint (ii) and the log-form constraint (iii). The SU(2) and SU(3) rows, as originally fit, were retrodictions. The product $A \cdot B$ (the leading coefficient in the $C_2 g_0^2$ expansion) is now computed from 2-loop lattice PT in the induced gauge theory (§6.2.1, giving $A \cdot B = 48.0 \pm 1.5$), agreeing with the fit ($46.4$) at the $\sim 3\%$ level. This upgrades the SU(2) and SU(3) entries to first-principles predictions at $\sim 3\%$ precision. Separating $A$ from $B$ individually remains open.
...
The chain in §6.3 reproduces all three SM gauge couplings at $M_Z$ through the structural input $1/\alpha_0 = 23.25$ (computed analytically from the one-loop staggered VP, §6.1), the universal threshold $\delta_0 = 10.02$ (empirically fixed by the U(1) row, consistent with the 2-loop VP computation $8.0 \pm 2$ plus natural three-loop corrections), and the $C_2$-dependent threshold parameters $(A, B) = (8.3, 5.59)$ (matched to the observed SU(2) and SU(3) couplings at $M_Z$, with the product $A \cdot B = 46.4$ independently computed from first principles at $A \cdot B = 48.0 \pm 1.5$; see §6.2.1). The structural predictions in this sector are: (i) the specific value $1/\alpha_0 = 23.25$, (ii) the universality of $\delta_0$ across gauge groups, (iii) the consistency of the 2-loop VP computation with the empirically determined $\delta_0$, and (iv) the first-principles determination of $A \cdot B$ agreeing with the fitted value at the $\sim 3\%$ level. Separating $A$ from $B$ individually remains an open task.
