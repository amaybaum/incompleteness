...
The neutrino mass mechanism (Dirac vs. Majorana, presence or absence of $\nu_R$) is not fixed by the lattice structure as currently understood and is left open. The absolute generation count is addressed in §4.7.X.2: the uniqueness of the $K = 2d = 6$ minimizer locks the count at three as a derived consequence rather than an assignment.

#### 4.7.X.1 Partial resolution: gauge bosons from the Schur complement, and a level-confusion diagnosis for the fermion sector

The Schur-complement trace-out used in Theorem 13 to establish chirality contains more structural information than that theorem extracts. Pursuing it carefully yields a clean retrodiction of the gauge boson content of the Standard Model from $\mathrm{End}(V_K)$, and simultaneously sharpens the fermion-embedding question of §4.7.X into a precise and tractable form — a form that §4.7.X.2 then answers via the link-carrier construction.
...
**The asymmetry is structural, not embarrassing.** The reframing also explains *why* the gauge sector closes from $\mathrm{End}(V_K)$ while the matter sector does not close from $V_K$. Gauge bosons are properties of the coupling matrix $M$ itself — they live at the bijection level, where the direct-sum decomposition of $V_K$ is the literal structure and $\mathrm{End}(V_K)$ is the literal kernel produced by the Schur complement. Fermions are observer-level objects: they are what an embedded observer reports when it coarse-grains lattice deviations into single-particle states. The two sectors live at two different layers of the framework, and the layer separation is exactly what "QM is emergent" means. The asymmetry between a closed gauge-boson retrodiction and a fermion-embedding question that requires a layer-corrected treatment is therefore not a defect; it is the expected signature of a framework in which gauge fields are structural and matter is emergent. §4.7.X.2 shows how the layer-corrected treatment resolves both the embedding and the generation count.

**The question, restated sharply — and answered in §4.7.X.2.** The open question of §4.7.X is therefore not "how does one extract $(\mathbf{3}, \mathbf{2})$ from $V_3 \oplus V_2 \oplus V_1$" — that question is well-posed only under the level confusion identified above, and under that confusion the answer is "one cannot." The correct successor question is:

> **Question (§4.7.X.1, answered by §4.7.X.2).** Characterize the coarse-graining map from bijection-level configurations of $(S, \varphi)$ to the observer-level single-fermion Hilbert space $\mathcal{H}_{\text{obs}}$, such that the induced action of the bijection-level commutant $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ on $\mathcal{H}_{\text{obs}}$ realizes exactly one Standard Model generation per $T_1$ staggered taste, with the chirality assignments fixed by Theorem 13.

This question connects the matter-sector embedding to the same coarse-graining machinery that [Main] uses to derive emergent QM in the first place, and it predicts that the resolution should look uniform across the three $T_1$-taste generations established in Theorem 10. The escape hatches flagged in §4.7.X — bimodule constructions, link- or plaquette-level gauge actions, or fermions placed outside the gauge-defining carrier — are all candidate realizations of this coarse-graining map. §4.7.X.2 shows that the link-carrier construction is the realized answer: placing matter on links where the carrier is $V_K \otimes V_K^*$ dissolves the direct-sum obstruction, and the absolute generation count is simultaneously locked by the uniqueness of $K = 2d = 6$ (Step 3).
