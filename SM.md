...
*Remark (measure reduction, open).* Theorem 2 is an operator identity: it identifies $D_{\text{st}}^2$ with $-\tfrac14\Box_{\text{lat}}$ acting on lattice fields. For the standard staggered ChPT machinery used in §6 and §7.5 to apply with its conventional measure factors (e.g., the $1/(16\pi^2 f^2)$ measure factor of Aubin–Bernard SχPT), one needs a stronger statement: that the substratum's path-integral measure on real scalar bijections $\varphi: S \to S$ reduces, under the Susskind change of variables, to the standard Grassmann–Berezin measure on staggered fermion fields $(\bar\chi, \chi)$ in the relevant correlator sector. This measure-equivalence statement is not derived in the present version of the framework; the operational dynamical-fermion code (§7.5 inputs) uses the standard Berezin pseudofermion measure with the equivalence assumed implicitly. Establishing the measure equivalence as an explicit theorem — or characterizing the conditions under which it holds approximately (e.g., for chirally-protected observables in the weak-coupling regime) — is open work that would strengthen the bridge from §4 to §6, §7.
...
**Note on the perturbative form.** The integrand above is hybrid in dimensional structure: the loop integration $\int d^4q$ and the lattice propagator $D(q) = \sum_\nu \sin^2(q_\nu) + m^2$ run over four directions (consistent with the substratum's 3+1D Lorentzian structure), while the vertex factors $\cos^2(q_\mu)$ are spatial-only ($\mu \in \{1,2,3\}$, consistent with the framework's "gauge fields on spatial links only" structure — there is no time-direction gauge connection in the substratum). The result is a perturbative form that is neither standard 4D staggered VP (which has $\cos^2$ on all four directions) nor standard 3D PT (which would have a 3D loop integral): it is OI-specific. At one loop, this hybrid form gives a finite, well-defined integral converging to $\Pi_s \to 0.3084$ on a robust plateau (verified analytically and numerically). At two loops, the framework's analysis (§6.2) finds that the hybrid form's strict 2-loop result for the universal threshold $\delta_0$ vanishes at large $N$ (the dominant scalar-bubble integrand falls as $1/N^{3.09}$), which is why $\delta_0 \approx 10$ is treated as empirical input rather than derived. A proof that the hybrid form is a consistent perturbative scheme — closing at 1-loop with the standard analogue, and giving the observed asymptotic behavior at higher loops — would strengthen the structural foundation of the gauge coupling chain.
...
**Note on the dynamical-fermion MC.** A separate set of dynamical-fermion HMC simulations is used to extract empirical inputs to the §7.5 mass relations (notably the chiral condensate $\Sigma \approx 0.20$ and the matching-scale verification $m_{\text{match}}^{\text{MC}} = 0.1234 \pm 0.0002$). These simulations use the standard Haar measure on $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ together with the standard staggered pseudofermion Berezin measure, run in the weak-coupling regime $\beta \in \{11.10, 7.40, 3.70\}$. The relevant question is whether these standard-measure observables agree with the corresponding observables in the OI framework's induced $(\mathbb{Z}/q\mathbb{Z})$-pushforward measure (which the present section identifies as the framework's gauge measure). At the level of $\Sigma$ specifically, both measures concentrate near the identity at weak coupling — Haar via $\beta \gg 1$ giving $U \approx I + O(1/\sqrt\beta)$, and the $(\mathbb{Z}/q\mathbb{Z})$-pushforward via $q \gg 1$ giving the framework's "fraction of nontrivial link values scales as $1/q$" — so the leading-order $\Sigma$ values plausibly agree. Establishing this agreement explicitly (or characterizing the $O(1/\beta)$ versus $O(1/q)$ corrections) would strengthen the bridge between the framework's perturbative apparatus (§6.1, in the $q$-independent sector) and the dynamical-fermion observables used in §7.5. This bridge is open work; the empirical agreement of $m_{\text{match}}^{\text{MC}}$ with the predicted $\lambda g_0^2$ at 1.4% is consistent with the approximate-agreement scenario.
...
**Audit-identified bridge gaps in the SχPT inheritance.** A May 2026 audit of the K = 1/2 inheritance examined the specific transitions between OI's substratum / operational lattice setup and the standard 4D staggered ChPT framework cited above. The audit verified that the per-vertex cos² symmetrization producing the $1/2$ factor is structurally robust to the 3D-vs-4D dimensional change (the identity $\cos^2(k/2) = (1+\cos(k))/2$ is dimension-independent per direction). However, four bridge gaps in the inheritance were identified that are not currently derived in the framework documents and would benefit from explicit treatment:

(i) *Substratum measure reduction.* Theorem 2 establishes operator factorization $D_{\text{st}}^2 = -\tfrac{1}{4}\Box_{\text{lat}}$, but the corresponding reduction of the substratum's path-integral measure (on real scalar bijections) to the standard staggered Berezin measure is not explicitly derived. The operational dynamical-fermion code uses the standard Berezin pseudofermion measure, with the equivalence to the substratum measure assumed implicitly.

(ii) *3D operational MC vs 4D SχPT dimensional bridge.* OI's MC code is purely 3D Euclidean (gauge configurations are static spatial snapshots), whereas the standard SχPT machinery cited above (Aubin–Bernard, Patel–Sharpe, [33]) is 4D Euclidean. The bridge — using 3D Euclidean static-property MC observables (Σ ≈ 0.20) as inputs to 4D SχPT calculations — is the standard lattice-QCD assumption that vacuum expectation values factorize through the time direction; this is plausible but is asserted rather than derived for OI's 3+1D Lorentzian substratum.
...
(iv) *3D parity vs 4D parity for chirality.* Operationally, chirality is identified with $\varepsilon_{3D}(x) = (-1)^{x_1+x_2+x_3}$, while Theorem 13 derives chirality from $\varepsilon_{4D}(x) = (-1)^{\sum x_\mu}$ with $\eta \in \{0,1\}^{d+1}$. The two are algebraically related ($\varepsilon_{4D} = (-1)^{x_0} \cdot \varepsilon_{3D}$), and the staggered $U(1)$ shift symmetry that underlies Manifestation 7's two-loop diagnosis is preserved in both setups; the explicit bridge between the operational 3D-parity assignment and Theorem 13's 4D-parity derivation has not been spelled out.

These four gaps are documentation gaps rather than physics gaps — the inherited conventions are approximately right (as the empirical 0.4% match on $m_b/m_\tau$ corroborates) and the per-vertex $K = 1/2$ result is structurally robust. Closing them in the framework documents would upgrade the K = 1/2 inheritance from "asserted by analogy" to "derived from OI first principles," and the same audit findings apply to the other §7 mass-cluster predictions (D11, $m_\tau$, etc.) that share the SχPT machinery. None of these gaps changes the closure path for D10's terminal value, which remains the two-loop SχPT calculation specializing Panagopoulos-Spanoudes [Panagopoulos-Spanoudes 2017] to the OI cubic-group setting.
...
Twenty-two quantitative observables from a $d = 3$ cubic lattice with spacing $\epsilon = 2\,l_P$, classified by input status: **S** = strictly parameter-free structural; **M** = one-input mass/parameter chain; **E** = uses explicit empirical input; **P** = phenomenological/ansatz input.

| Prediction | Formula | Value | Observed | Match | Input |
|------------|---------|-------|----------|-------|-------|
| $1/\alpha_1(M_Z)$ | lattice + RGE | 59.00 | 59.00 | $< 0.1\%$ | **E**† (fixes $\delta_0$) |
...
**Solution-specific properties** are determined by the particular bijection φ within the equivalence class. These include: gauge couplings (set by the eigenvalues μ_c, μ_w of M), fermion masses (set by taste-breaking in φ at $\mathcal{O}(\epsilon^2)$), mixing angles (set by the Yukawa structure of φ), and the CP-violating phases in CKM and PMNS (set by the basis mismatch between up-type and down-type Yukawa mass eigenstates, which is itself a property of φ). The CP phases are explicitly compatible with Theorem 21: the theorem establishes $\bar\theta = 0$ (the determinant-phase combination) but does not constrain the basis-alignment structure that produces the Jarlskog invariant (see §5.4 for the full scope statement). These are analogous to the mass $M$ in the Schwarzschild solution: GR derives the functional form of the metric but does not predict which $M$ describes a particular black hole. The OI framework derives the SM's structure but does not predict which φ describes our universe.
