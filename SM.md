...
The expansion parameter for the $C_2$-dependent threshold is $C_2 \cdot g_0^2 = C_2 \times 4\pi\alpha_0 = 1.08$ (SU(2)) and $1.62$ (SU(3)) — both $\mathcal{O}(1)$. Finite-order perturbation theory converges too slowly: using known Wilson-action plaquette coefficients, 3-loop PT explains only $\sim$26% of the threshold at SU(3). The all-orders result is obtained from pure-gauge Monte Carlo simulation at the induced coupling $\beta = 2N_c/g_0^2$. At SU(3) with $\beta = 11.1$, the plaquette expectation value $\langle P \rangle = 0.806$ (from a multi-hit Metropolis simulation of the standard Wilson plaquette action on a $4^4$ lattice; see [`oi_lattice_code/gauge/metropolis_plaquette.c`](oi_lattice_code/gauge/metropolis_plaquette.c)); at SU(2) with $\beta = 7.4$, $\langle P \rangle = 0.783$. The non-perturbative threshold extracted from these plaquettes is well parameterized by a logarithmic resummation:
...
All three SM gauge couplings are reproduced through a chain in which the universal threshold $\delta_0$ is **over-determined by the data**. The structural input is $1/\alpha_0 = 23.25$ (one number, exact, from §6.1). The framework's structural prediction is that $\delta_0$ is universal — the $C_2$-independent part of the threshold must be the same for all three gauge groups by construction (no $C_2$ dependence ⇒ no group dependence at this order). This prediction is tested by the three rows of the table independently determining $\delta_0$:

$$\delta_0^{\text{U(1)}} = 10.02, \qquad \delta_0^{\text{SU(2)}} = 9.99, \qquad \delta_0^{\text{SU(3)}} = 10.00$$

Agreement to $0.4\%$ across three rows with very different $C_2$-dependent thresholds (0, 16.17, 19.13) and very different RG running ($+25.73$, $-19.88$, $-43.93$). The consistency of these three independent determinations is the framework's prediction in the gauge coupling sector — most theories do not predict a universal matching threshold across gauge groups. The two-loop staggered VP computation gives $\delta_0^{(2L)} = 8.0 \pm 2$ from first principles (§6.2), and the natural 3-loop convergence range $9.6$–$10.4$ brackets the over-determined value.

The $C_2$-dependent threshold parameters $(A, B) = (8.3, 5.59)$ are extracted from two pure-gauge MC plaquette measurements at the OI couplings ($\beta_2 = 7.4$, $\beta_3 = 11.1$) — physical inputs from independent lattice computations, not fits to observed M_Z couplings. The chain therefore has **zero free parameters in the M_Z couplings sector**: $1/\alpha_0$ is structurally computed, $(A, B)$ come from independent MC measurements, and $\delta_0$ is over-determined by the data with consistency at $0.4\%$.
...
The chain in §6.3 has **no free parameters in the M_Z couplings sector**. The structural input $1/\alpha_0 = 23.25$ is computed analytically. The threshold parameters $(A, B) = (8.3, 5.59)$ are extracted from two pure-gauge MC plaquette measurements at the OI couplings $\beta_2 = 7.4$ and $\beta_3 = 11.1$ — physical inputs from independent lattice computations. The universal threshold $\delta_0$ is over-determined by the three rows: U(1), SU(2), SU(3) independently give $\delta_0 = 10.02, 9.99, 10.00$ — agreement to $0.4\%$. The framework's prediction in this sector is the consistency of these three determinations, which non-trivially confirms the structural claim that $\delta_0$ is universal across gauge groups. The 2-loop staggered VP computation gives $\delta_0^{(2L)} = 8.0 \pm 2$ from first principles, with the natural 3-loop convergence range bracketing the over-determined value at $9.6$–$10.4$. A direct 3-loop computation would replace the convergence bound with an exact value, but is not required for the chain's parameter-free status — the over-determination by three independent rows already pins $\delta_0$ to better than $0.5\%$.
...
|------------|---------|-------|----------|-------|-------|
| $1/\alpha_1(M_Z)$ | lattice + RGE | 59.00 | 59.00 | $< 0.1\%$ | **S**† |
| $1/\alpha_2(M_Z)$ | lattice + RGE | 29.57 | 29.57 | $< 0.1\%$ | **S**† |
| $1/\alpha_3(M_Z)$ | lattice + RGE | 8.47  | 8.47  | $< 0.1\%$ | **S**† |
...
- **†** The three SM gauge couplings at $M_Z$ are reproduced with no free parameters in this sector (§6.3). The structural input $1/\alpha_0 = 23.25$ is computed analytically; the threshold parameters $(A, B)$ come from independent MC plaquette measurements; the universal threshold $\delta_0$ is over-determined by the three rows of the table independently giving $\delta_0 = 10.02, 9.99, 10.00$ (agreement to $0.4\%$). The framework's prediction is the consistency of these three independent determinations, which non-trivially confirms the structural claim that $\delta_0$ is universal across gauge groups. The 2-loop staggered VP gives $\delta_0^{(2L)} = 8.0 \pm 2$ from first principles, with the natural 3-loop convergence range $9.6$–$10.4$ bracketing the over-determined value.
...
**Parameter-accounting summary:** Of the 22 observables: **16 are strictly parameter-free structural predictions** (**S**), including all three gauge couplings at $M_Z$ (where $\delta_0$ is over-determined by the data, not fitted); 3 form a one-input mass/parameter chain (**M**, with $m_e$ and $m_\mu$ from $m_\tau$, and $m_H$ from $m_t$); 1 uses an explicit empirical input (**E**, the Jarlskog phase $\eta$); and 2 use a phenomenological input (**P**, the down-sector $Q$ relation). No parameters are fit to the predicted quantities.
...
- The pure-gauge plaquette measurements in §6.2 — $\langle P \rangle_{\mathrm{SU}(2)} = 0.783$ at $\beta_2 = 7.4$ and $\langle P \rangle_{\mathrm{SU}(3)} = 0.806$ at $\beta_3 = 11.1$, both on a $4^4$ lattice — were obtained from a multi-hit Metropolis simulation of the standard Wilson plaquette action. The utility (`oi_lattice_code/gauge/metropolis_plaquette.c`) is shipped with this paper and validated against literature reference values at SU(2) $\beta = 2.3$ and $\beta = 20$, and SU(3) $\beta = 5.7$ and $\beta = 6.0$.
