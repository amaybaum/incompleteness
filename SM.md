...
*Remark (measure reduction).* Theorem 2 is an operator identity: it identifies $D_{\text{st}}^2$ with $-\tfrac14\Box_{\text{lat}}$ acting on lattice fields. For the standard staggered ChPT machinery used in §6 and §7.5 to apply with its conventional measure factors (e.g., the $1/(16\pi^2 f^2)$ measure factor of Aubin–Bernard SχPT), one needs a stronger statement: that the substratum's path-integral measure on real scalar bijections $\varphi: S \to S$ reduces, under the Susskind change of variables, to the standard Grassmann–Berezin measure on staggered fermion fields $(\bar\chi, \chi)$ in the relevant correlator sector. This measure-equivalence statement is not derived in the present paper; the dynamical-fermion code uses the standard Berezin pseudofermion measure, with the equivalence assumed implicitly. Establishing the measure equivalence as an explicit theorem — or characterizing the conditions under which it holds approximately (e.g., for chirally-protected observables in the weak-coupling regime) — is open work.
...
(i) *Substratum measure reduction.* Theorem 2 establishes operator factorization $D_{\text{st}}^2 = -\tfrac{1}{4}\Box_{\text{lat}}$, but the corresponding reduction of the substratum's path-integral measure (on real scalar bijections) to the standard staggered Berezin measure is not derived (cf. §4.2 *Remark*). The dynamical-fermion code uses the standard Berezin pseudofermion measure, with the equivalence to the substratum measure assumed implicitly.

(ii) *3D operational MC vs 4D SχPT dimensional bridge.* The dynamical-fermion code is purely 3D Euclidean (gauge configurations are static spatial snapshots), whereas the standard SχPT machinery cited above (Aubin–Bernard, Patel–Sharpe, [33]) is 4D Euclidean. The bridge — using 3D Euclidean static-property MC observables (Σ ≈ 0.20) as inputs to 4D SχPT calculations — is the standard lattice-QCD assumption that vacuum expectation values factorize through the time direction; this is plausible but is asserted rather than derived for OI's 3+1D Lorentzian substratum.

(iii) *3D BZ pairing vs 4D Dirac-matrix presentations of the taste structure.* Theorem 8's proof uses 3D BZ pairing ($2^d = 8$ corners pair under $\eta \leftrightarrow 1-\eta$); Theorem 9's proof uses 4D Dirac matrices ($I_4$, four-component spinors). The two presentations are equivalent under the identification "3D BZ corners × sublattice parity = 4D effective hypercube," but this equivalence is not stated as an explicit theorem in §4.

(iv) *3D parity vs 4D parity for chirality.* Operationally, chirality is identified with $\varepsilon_{3D}(x) = (-1)^{x_1+x_2+x_3}$, while Theorem 13 derives chirality from $\varepsilon_{4D}(x) = (-1)^{\sum x_\mu}$ with $\eta \in \{0,1\}^{d+1}$. The two are algebraically related ($\varepsilon_{4D} = (-1)^{x_0} \cdot \varepsilon_{3D}$), and the staggered $U(1)$ shift symmetry that underlies the two-loop structure of $K_S$ is preserved in both setups; the explicit bridge between the operational 3D-parity assignment and Theorem 13's 4D-parity derivation is open.

The same gaps apply to the other §7 mass-cluster predictions (D11, $m_\tau$, etc.) that share the SχPT machinery. None of them changes the closure path for $K_S$, which remains the two-loop SχPT calculation specializing Panagopoulos–Spanoudes [Panagopoulos-Spanoudes 2017] to the OI cubic-group setting.
