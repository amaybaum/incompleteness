...
The explicit derivation of the amplitude-level formula from the cubic-lattice Yukawa structure remains an open OI-specific calculation (Direction 11 in the framework's research-direction documentation; structurally analogous to Direction 10's K=1/2 derivation in being mixed-layer — substratum cubic-group structure combined with electroweak-symmetry-breaking emergent machinery). The structural square-root relation is supported by the empirical match at $0.05\sigma$ but is not yet derived to theorem grade from first principles.
...
Twenty-two quantitative observables from a $d = 3$ cubic lattice with spacing $\epsilon = 2\,l_P$, classified by input status: **S** = strictly parameter-free structural; **C** = conditional structural (depends on a stated open assumption); **L** = layered conditional (structural form combined with a Layer-2 substrate input or Layer-3 empirical input — see §8.3 four-layer framing); **M** = one-input mass/parameter chain; **E** = uses explicit empirical input; **R** = retrodiction; **P** = phenomenological/ansatz input.
...
| $\sin^2\theta_{12}$ | $1/3 - 1/(4\pi^2)$ | 0.3080 | 0.3085 ± 0.0073 (Capozzi 2025 post-JUNO global) | $0.07\sigma$ | **C**ᶠ |
...
- **ᵉ** *Layered conditional structural prediction (m_u/m_d).* The structural form $m_u/m_d = \sqrt{\theta_0} = \sqrt{2/9}$ uses the same quadratic Casimir $C_2 = 2$ of the $T_1$ representation that produces the Koide angle $\theta_0 = 2/9$, with the square root following from amplitude-vs-intensity transformation between intra-doublet and inter-generation channels (§7.2). A more explicit derivation from the taste-decomposed Yukawa structure (Direction 11) is the natural follow-up; the empirical match at $0.05\sigma$ provides strong support pending closure.

- **ᶠ** *Conditional structural prediction (sin²θ_12, sin²θ_23).* The value follows unconditionally given Condition 2 of §7.3, which is currently a structural input pending derivation from cubic-lattice Yukawa operator content. The empirical match ($\sin^2\theta_{12}$ at $0.07\sigma$ to the post-JUNO global fit; sum rule $2\sin^2\theta_{12} + \sin^2\theta_{23} = 7/6$ at $0.6\sigma$) tests Cond 2 directly. The Layer 0 component (TBM zeroth-order value) and Layer 1 component ($\lambda^2$ scale) are unconditional. See §8.3 four-layer framing; see §7.3 "Sharpening the open task" for the substrate-level question Cond 2 reduces to.

**Parameter-accounting summary (post-audit, v1.10.0):** Of the 22 observables: **10 are unconditional parameter-free structural predictions** (**S**, Layer 0 or Layer 1: items $1/\alpha_0$, $A \cdot B$ cross-check, Wolfenstein $A$, $|V_{cb}|$, $m_d/m_s$ via GST, Cabibbo $\lambda$, Koide $Q$, Koide angle $\theta_0$, $\sin^2\theta_{13}$, $\lambda(M_{\text{Pl}}) = 0$, top-mass IR fixed point); **2 are conditional structural predictions** (**C**, conditional on Condition 2 of §7.3: $\sin^2\theta_{12}$ and $\sin^2\theta_{23}$); **3 are layered conditional predictions** (**L**, structural form combined with one Layer-2 substrate input or Layer-3 empirical input: $m_u/m_d$ pending Direction 11, $m_b$ inheriting $Q_{\text{down}}$'s phenomenological status, $m_b/m_\tau$ pending Direction 10); 2 are retrodictions in the gauge coupling sector (**R**, where $(A, B)$ are currently matched to observation rather than computed); 1 fixes the universal threshold $\delta_0$ from the U(1) coupling (**E**); 3 form a one-input mass/parameter chain (**M**); 1 uses an explicit empirical input (**E**, the Jarlskog phase $\eta$). No parameters are fit to the predicted quantities in any of the **S**, **C**, or **L** classes; the conditional/layered classifications reflect explicit dependence on stated structural inputs (Cond 2, Direction 10/11) or phenomenological inputs ($Q_\text{down}$) rather than free parameter fitting.

The pre-audit count of 13 strictly structural predictions reduces to 10 unconditional + 5 conditional/layered = 15 in the structural-or-near-structural class. The audit refines classification precision; substantive content (the 22 numerical predictions and their empirical matches) is unchanged.
