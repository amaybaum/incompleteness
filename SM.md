...
All three SM gauge couplings are reproduced through a chain that has **one effective free parameter**: $\delta_0$. The structural input is $1/\alpha_0 = 23.25$ (one number, exact, from §6.1). The universal threshold $\delta_0$ is constrained by three independent considerations: (i) the U(1) row requires $\delta_0 = 10.02$, (ii) the 2-loop staggered VP computation gives $\delta_0^{(2L)} = 8.0 \pm 2$ from first principles (§6.2), and (iii) the natural 3-loop range $9.6$–$10.4$ (from standard lattice perturbative convergence in this coupling regime) brackets the required value. The convergence of these three constraints — empirical, perturbative, and convergence-bounded — on $\delta_0 \approx 10$ is non-trivial. The $C_2$-dependent threshold parameters $(A, B) = (8.3, 5.59)$ are extracted from two pure-gauge MC plaquette measurements at the OI couplings ($\beta_2 = 7.4$, $\beta_3 = 11.1$) — physical inputs, not fits to observed M_Z couplings. With $1/\alpha_0$ structural and $\delta_0$ constrained as above, the SU(2) and SU(3) entries in the table are predictions of the chain MC plaquette $\to$ lattice-to-MS-bar matching $\to$ SM RG running, with no further adjustable parameters. They match observation at $<0.1\%$ and $0.3\%$ respectively.
...
The chain in §6.3 has one effective free parameter, $\delta_0$, constrained by three independent considerations: (i) the U(1) row requires $\delta_0 = 10.02$, (ii) the 2-loop staggered VP computation gives $\delta_0^{(2L)} = 8.0 \pm 2$ from first principles, and (iii) the natural 3-loop range from standard lattice perturbative convergence brackets $9.6$–$10.4$. All three converge on $\delta_0 \approx 10$. The remaining inputs are structural ($1/\alpha_0 = 23.25$, analytic) or physical lattice measurements ($\langle P \rangle_{SU(2)} = 0.783$ at $\beta_2 = 7.4$ and $\langle P \rangle_{SU(3)} = 0.806$ at $\beta_3 = 11.1$, from which $A$ and $B$ are extracted). The SU(2) and SU(3) M_Z values are then predictions of the chain with no further adjustable parameters. A direct 3-loop computation would replace the convergence bound with an exact value; this is the natural next step in tightening the calculation.
