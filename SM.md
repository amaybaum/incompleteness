...
**Structural identification of $(A, B)$ as a $(2^d, 2d)$ factorization.** The candidate factorization $(A, B) = (I_{\text{loop}}, N_f) = (8, 6)$ identified in the Open work below admits a sharper structural reading. The two enumerations underlying $I_{\text{loop}} = 8$ — the $2^d = 8$ Brillouin-zone corners of the cubic lattice (cf. §4.7.1.2 Step 3) and the $4$ staggered tastes $\times\,2$ sublattices — are equivalent: they describe the same lattice momentum-space structure in two different bases ($4 \times 2 = 2^3 = 2^d$ for $d = 3$). Combined with $N_f = K = 2d = 6$ from coupling-degree minimization (Theorem 6, §4.5), this yields the structural factorization

$$(A, B) = (2^d,\; 2d), \qquad A \cdot B = 2^{d+1} \cdot d,$$

which gives $(8, 6)$ and $A \cdot B = 48$ for $d = 3$, matching the unconstrained one-parameter fit $(8.07, 5.95)$ within $1\%$ and the independently-derived $A \cdot B = 48.0 \pm 1.5$ exactly. The asymmetric assignment ($A$ inheriting the lattice-geometric BZ-corner factor $2^d$, $B$ inheriting the matter count $2d$) follows from the analytic structure of the resummed threshold $A \ln(1 + B \cdot C_2 g_0^2)$: matching the geometric-series iteration of fermion bubbles connected by gauge propagators forces $B^n / n \propto N_f^n / n$ at $n$-loop order (each additional loop contributing another fermion-bubble factor of $N_f$), which fixes $B = N_f$ and therefore $A = I_{\text{loop}}$. Within the four-layer framing of §8.3, $B = 2d$ is Layer 1 closed via Theorem 6, while $A = 2^d$ is Layer 2(a)-tractable: full closure requires verification that the BZ-corner factor appears explicitly in the OI 1-loop integral structure, which is offline lattice-PT scope. This closure pattern — Layer 1 product or ratio combined with Layer 2(a)-tractable individual coefficients — parallels the Cond 2 stratification of §7.3 and the D10 $K = 1/(n_{\text{gen}} - 1)$ derivation of §7.5. The factorization $(A, B) = (2^d, 2d)$ derives from lattice geometry (BZ-corner enumeration plus Theorem 6 matter count), which distinguishes this structural identity from the cubic-group representation-theoretic family — Wolfenstein $A^2 = (d-1)/d$ (§7.1), Cond 2 ratio $4(d-1)/(2d)$ (§7.3), D10 $K = 1/(n_{\text{gen}} - 1)$ (§7.5) — that closes the framework's other Layer 2(a)-tractable predictions.
...
The chain in §6.3 reproduces all three SM gauge couplings at $M_Z$ through the structural input $1/\alpha_0 = 23.25$ (computed analytically from the one-loop staggered VP, §6.1), the universal threshold $\delta_0 = 10.02$ (empirically fixed by the U(1) row; a 2-loop VP rebuild indicates $\delta_0$ is not reproduced by strict 2-loop PT and is accordingly treated as empirical input), and the $C_2$-dependent threshold parameters $(A, B) = (8.3, 5.59)$ (matched to the observed SU(2) and SU(3) couplings at $M_Z$, with the product $A \cdot B = 46.4$ independently derived at $A \cdot B = 48.0 \pm 1.5$; see §6.2.1). The structural predictions in this sector are: (i) the specific value $1/\alpha_0 = 23.25$, (ii) the universality of $\delta_0$ across gauge groups, and (iii) the independent derivation of $A \cdot B$ agreeing with the fitted value to $\sim 3\%$. Separating $A$ from $B$ individually admits the Layer 2(a)-tractable structural factorization $(A, B) = (2^d, 2d) = (8, 6)$ for $d = 3$ (§6.2.1), with $B = 2d$ Layer 1 closed via Theorem 6 (matter count) and $A = 2^d$ Layer 2(a)-tractable via the BZ-corner count.
...
where $\eta = 0.348$ [20] is the solution-specific CP-violating parameter. This gives $J = 3.02 \times 10^{-5}$ (observed: $(3.08 \pm 0.13) \times 10^{-5}$, $0.5\sigma$). The structural suppression factor $12\pi^6 \approx 11{,}537$ is purely geometric. In structural-form notation, the prefactor admits the cross-family decomposition $1/(12\pi^6) = (d-1)/(d \cdot 2^d \cdot \pi^{2d})$ for $d = 3$: the off-democratic factor $(d-1)/d$ inherited from Wolfenstein $A^2$ combines with the lattice-geometric factor $1/2^d$ from $\lambda^{2d}$ and the BZ-volume normalization $1/\pi^{2d}$.
...
**Over-determination of the Layer 2 cluster.** The discretization-determined character above has a quantitative consequence under continuum-limit irrelevance of most discretization parameters. Of the discretization-scheme choices listed above, $L \to \infty$ removes lattice-size dependence, $\tau \to 0$ removes timestep dependence (the lattice spacing $\varepsilon = 2 l_p$ is itself structurally fixed by the thermal self-consistency of [GR §3-4], so $\tau = \varepsilon/c$ is not a free parameter), and boundary conditions become irrelevant for bulk observables. The field-quantization granularity is gauge under $\mathcal{G}_{\rm sub}$ — different granularities producing identical observables when the coupling structure is preserved (the $q$-gauge theorem, [GR §3.2]) — and is therefore not a physical parameter. The remaining parameter is the rounding rule, which determines the effective non-linearity supporting cycle ergodicity (per §4.2 *Remark*). Under the natural assumption of orbit-level Markovian-equivalence, the rounding rule contributes one effective continuum parameter (the variance of its orbit-level noise) by central-limit-style reasoning; higher moments correspond to higher-derivative or higher-power operators that are RG-irrelevant in the IR continuum limit, and a non-zero mean would constitute a relevant linear-in-field tadpole that would spoil the wave-equation dynamics and is therefore structurally excluded. The Markovian-equivalence assumption has been numerically tested at small lattice (cubic $L \in \{4, 6, 8, 12\}$, quantization $N_{\text{levels}} \in \{100, 500\}$, time series of length $T = 5000$): the integrated autocorrelation time of the rounding noise is $\tau_{\rm int} = 0.50 \pm 0.03$ across all tested parameters, indistinguishable from the white-noise value $\tau_{\rm int} = 0.5$, and a sanity-check synthesis with AR(1) noise at correlation $\phi = 0.5, 0.9$ correctly recovered the predicted $\tau_{\rm int} = 1.5, 9.5$ — so the test is sensitive to non-Markovian behavior if present. Non-Markovian generalizations — long-range autocorrelation, field-dependent rounding, or site-dependent structure — can add up to two further parameters, bracketing the total count at one (natural) to three (generic). The Layer 2 quantities the framework currently identifies — the Cond 2 ratio $b_{23}/(b_{12}+b_{13}) = 4(d-1)/(2d) = 4/3$ (§7.3), the D11 alignment producing $m_u/m_d = \sqrt{\theta_0}$ (§7.2), the $K_S$ subleading contribution to $m_b/m_\tau$ (§7.5), and the $\delta_0$ universal threshold (§6.2) — constitute at least four numerical Layer 2 inputs. The system is therefore over-determined four-to-one in the natural Markovian-equivalent setting, and at worst four-to-three under generic non-Markovian extensions. Mutual consistency among the four empirical matches — Cond 2 at 0.07σ on the post-JUNO $\sin^2\theta_{12}$ global fit, $m_u/m_d$ at 0.05σ, $K_S$ at $9\sigma$ from zero with empirical value $0.0072$, $\delta_0 = 10.02$ matching $\text{Tr}[Y^2] = 10$ at $0.2\%$ — is therefore a non-trivial test of the Layer 2 interpretation: the framework predicts these are not independent, so functional consistency confirms the discretization-determined structure while inconsistency would localize a specific structural failure. Making the consistency check quantitative — computing each Layer 2 quantity as an explicit function of the same rounding-rule parameter set — requires substrate-loop calculations beyond the current scope; the over-determination structure itself is a structural prediction, falsifiable in principle once that calculation is performed.
