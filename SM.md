...
A *regular lattice*. Any bounded-degree graph with statistical isotropy suffices for emergent QM and the gauge-group existence theorem; the *specific* Bravais structure (simple cubic vs. FCC/BCC/HCP) is separately required for the quantitative predictions of §7 and is addressed in §4.6. A *specific alphabet size q*. All proofs work for any q ≥ 2; no prediction depends on q. A *specific dimensionality d*. The proofs work for any d ≥ 1; self-consistency selects d = 3 (§3.2). The *wave equation*. It is derived from center independence, isotropy, and linearity (§4.1), each of which follows from the structural properties listed above.
...
**Remark (robustness and cubic specificity).** Theorem 7 uses the character table of the octahedral group $O$, the exact symmetry of the simple cubic (SC) lattice. The decomposition $6 = T_1 \oplus E \oplus A_1$ has a two-part robustness structure that is worth making explicit, because one half is often over-stated and the other half is worth owning as a positive predictive commitment.

*What is robust.* The *integer* multiplicities $(3, 2, 1)$ are topologically stable against small continuous perturbations of the coupling graph that preserve local $O$ symmetry. The eigenvalue multiplicities of $M$ are integers, and Theorem 5 maps integer multiplicities to unitary group factors. Small perturbations — breaking *exact* $O$ symmetry to *statistical* $O$ symmetry of the kind one would find on a bounded-degree random graph approximating $\mathbb{Z}^3$ at large scales — perturb the eigenvalues of $M$ continuously but cannot change integer multiplicities without crossing a codimension-1 boundary in parameter space. Likewise, $K = 2d$ (Theorem 6) is integer-stable under quasi-isometric deformations that preserve the spectral dimension.

*What is not robust: the Bravais-lattice commitment.* The $(3, 2, 1)$ decomposition is specific to the discrete $O$ action on the six NN link vectors $\{\pm\hat e_j\}$ of the simple cubic lattice. Under the *continuous* rotation group $\mathrm{O}(3)$, the same six vectors span a representation equivalent to two copies of the vector representation — decomposition $\mathbf{3} \oplus \mathbf{3}$, multiplicities $(3, 3)$ — because $\mathrm{O}(3)$ does not preserve the discrete set $\{\pm\hat e_j\}$ as a distinguished orbit. The $E$ and $A_1$ factors emerge only because $O \subset \mathrm{O}(3)$ acts discretely on these specific six vectors. An alternative d=3 Bravais lattice with a different NN coordination — face-centered cubic (FCC, 12 NN), body-centered cubic (BCC, 8 NN), or hexagonal close-packed (HCP, 12 NN with $D_{6h}$ point group) — would produce a different link-decomposition, different multiplicities, and therefore a different gauge group. The framework's commitment to SC is not framing decoration; it is load-bearing at the predictive level.

*Concrete demonstration: the Cabibbo angle.* The prescription of §7.1 — $\lambda = 1/|X_i - X_j|$ where $X_j$ are the three $T_1$ BZ corners in the same (reciprocal-space) units — extends to any Bravais lattice. Using natural units in which the nearest-neighbor spatial distance equals unity on each lattice (so that the spatial wave equation has equal propagation speed across lattice types), the corresponding calculation on the three natural candidate Bravais lattices gives:

| Lattice | Point group | Coord. no. | Triplet separation | Cabibbo $\lambda$ | vs. observed |
|---|---|---|---|---|---|
| **SC** | $O_h$ | 6 | $\pi\sqrt{2}$ | $\mathbf{0.2251}$ | $0.04\%$ ($0.12\sigma$) |
| FCC | $O_h$ | 12 | $2\pi$ | $0.1592$ | 29% off ($98\sigma$) |
| BCC | $O_h$ | 8 | $\pi\sqrt{6}$ | $0.1299$ | 42% off ($142\sigma$) |

Observed $\lambda = 0.2250 \pm 0.00067$ [20]. The SC prediction matches; the FCC and BCC predictions from the same prescription miss by tens of σ. HCP would differ further still because its point group ($D_{6h}$, 24 elements) has a genuinely different irrep structure from $O_h$. The 22 quantitative observables of §7 — built on $\lambda$, on the $T_1$-Casimir Koide angle $\theta_0 = C_2/d^2 = 2/9$, on the $A_4 \subset O$ TBM baseline of PMNS, and on the $K = 2d = 6$ coupling-degree minimum — are all specific to SC in the same way.

*The cubic commitment as a falsifiability feature.* The framework does not claim robustness against the choice of d=3 Bravais lattice; it claims SC. A reader who finds $\lambda = 0.2250$ mysterious on SC must also find FCC and BCC's $0.1592$ and $0.1299$ mysterious in their own terms, and must explain why observation lands on the SC value. The cubic specificity is therefore a positive predictive differentiator rather than a hidden assumption: the framework's 22 structural retrodictions distinguish SC from FCC/BCC/HCP at high confidence, and the framework would be straightforwardly falsified if any cosmological or high-energy observation required a non-SC Bravais structure (e.g., FCC-specific discrete-symmetry signatures in cosmic-ray anisotropies, or predictions from cubic-specific SM parameters drifting outside the SC prediction as experimental precision sharpens).
...
The source code under `oi_lattice_code/`, together with the run drivers and analysis scripts, is archived on Zenodo with concept DOI [10.5281/zenodo.19060318](https://doi.org/10.5281/zenodo.19060318) (the version of record for this paper is 1.4.4, [10.5281/zenodo.19482992](https://doi.org/10.5281/zenodo.19482992)) and is also available on GitHub at <https://github.com/amaybaum/incompleteness>. The code is released under the MIT License. Gauge configurations at the OI coupling $\beta_3 = 11.1$ and the raw $Z_S(m)$ measurement outputs underlying the §7.5 prediction are available from the author on request.
