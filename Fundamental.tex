...
\subsubsection{What remains undetermined}\label{what-remains-undetermined}}

The framework determines the SM's \emph{structure} but not its \emph{parameters}. Gauge couplings depend on the eigenvalues μ\_c, μ\_w of M (with μ\_y = 1 derived). Fermion masses arise from taste-breaking at \(\mathcal{O}(\epsilon^2)\). Mixing parameters have magnitudes set by φ.

\textbf{Open problems.} (i) \emph{Continuum-limit universality} --- the proof that the lattice gauge theory flows to the correct continuum QFT. This is shared with all lattice QFT (essentially the Yang-Mills mass gap problem). (ii) \emph{Generation identification} --- the fermion count (three) is proved; the assignment of specific representations to specific tastes is a uniqueness argument from anomaly cancellation, not a geometric derivation. (iii) \emph{Taste-breaking phenomenology} --- whether specific patterns in φ reproduce the observed fermion mass ratios and mixing angles.
...
\textbf{Remark} (Continuum extension). The lattice-level reconstruction extends to the full continuum SM under lattice QFT universality --- supported by decades of numerical evidence but not rigorously proved (the Yang-Mills mass gap problem). This gap is shared with all lattice QFT.
