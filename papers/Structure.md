...
The framework developed in [Main, SM, GR, Substratum] establishes — under the reconstruction theorem's stated conditions — a uniquely fixed **local lattice/gauge residue** modulo the substratum gauge group $\mathcal{G}_{\rm sub}$ within OI's specific structural class of finite deterministic bijection systems. H-Bell is separately required for a Bell-inclusive completion to coexist with that local sector, and the Bell-inclusive completion is not proved unique. This paper addresses two layers of structural commitments that lie beyond this core technical content.
...
In a separate program [Main, SM, GR, Substratum], the Incompleteness of Observation (OI) framework approaches fundamental physics from a different direction. The starting point is the observation that quantum mechanics is the marginal-statistics description used by an embedded observer with finite distinguishable states; the trace-out over the hidden sector generates the reduced observable law — universally admitting the quantum representation — from a deterministic classical substratum. The framework develops two technical results:

**Reconstruction theorem (Theorem 23, [Substratum §3]).** Given E1–E7 together with A1–A6, M1-T, the hidden-sector mixing hypothesis for the C2 necessity direction, and Lemma 24.1's semigroup-transfer/completeness step, the **local propagating lattice/gauge sector** is fixed up to the stated substratum-gauge freedoms: $d=3$, $K=6$, the $(3,2,1)$ decomposition, $SU(3)\times SU(2)\times U(1)$ and the associated local representation data, with the qualifications stated in [SM], and $\hbar=c^3\epsilon^2/(4G)$. M1-B is a separate Bell input. A single measurement-independent deterministic completion containing both that local sector and the Bell-violating composite exists only conditional on H-Bell, now the curvature-stability requirement for preparation-indexed state-dependent adjacency. Neither H-Bell nor Lemma 24.1 establishes uniqueness of the Bell-inclusive full completion.
...
The negative answer to Question 1 (Part I) does not exhaust the structural relationship between OI and string theory. OI's $\mathcal{G}_{\rm sub}$ classifies observationally-equivalent substrata *within OI's structural class* — substrata satisfying A1-A6. The reconstruction theorem establishes uniqueness of the local reconstructed residue within this class, not uniqueness of the Bell-inclusive completion and not uniqueness across structural classes.
...
(C2) **Record persistence.** The written record must survive to readback — in our universe realized by a slow bath, the hidden-sector correlation time much longer than the observer's measurement timescale (one sufficient route).

(C3) **Sufficient capacity.** The hidden sector must have information capacity enough to store the interaction history.

(C4) **History readback.** The interaction must read back what was written — the load-bearing memory condition.

A substratum $(S, \varphi)$ is *observer-admitting* if there exists a partition $V \subset S$ such that $(S, \varphi, V)$ satisfies C1–C4, and *observer-lacking* otherwise (formal Definition 9.9 in §9.6). Level B commits to: the space of substrata is structurally partitioned into observer-admitting and observer-lacking subsets; most substrata are observer-lacking (random permutations on finite sets typically lack the slow-mixing, sufficient-capacity, coupled-subsystem structure required); the framework's content applies only to observer-admitting substrata.
...
Combined with the empirical inputs and modeling assumptions stated in Theorem 23, these determine — conditional on C2-mixing and Lemma 24.1's completeness — the **local lattice/gauge residue** up to $\mathcal{G}_{\rm sub}$. H-Bell separately conditions Bell-inclusive existence, whose uniqueness is open (Theorem 23 in [Substratum]).

A1–A6 are not the same as observer-admission C1–C4; they operate at a different hierarchy level. C1–C4 are conditions for *any* embedded observer to exist (Level B); A1–A6 are conditions for membership in *OI's specific* universality class (Level D). Some relations: A1 (finiteness) is related to but distinct from C3 (capacity); A2 (determinism) is a Level D commitment that selects deterministic-substratum members of a universality class — observers can in principle exist on stochastic dynamics. Other A's (A3–A6) are pure class-specific: required for OI's specific gauge-structure derivation, not necessary for observer-admission.
...
- *Level G2 × Level D predictions* — SM gauge group, the $1 \oplus 3$ candidate-sector count (three physical generations under H-spin'), anomaly-free hypercharges. Forced uniquely in OI through cubic-group commutant; shared with any SM-reproducing universality class.
...
- *[Main]:* Levels A and B (observation axiom; embedded-observer characterization via C1–C4)
...
This theorem underwrites OI's claim that A5 (linearity) is forced rather than assumed, but conditionally: the forcing runs through *amplitude-scale* gauge (the unobservability of the absolute field scale), which [Substratum §4, Theorem 24, *Status*] classifies as an adjoined operational input rather than a theorem ([SM §2.7]) — its independent warrant by the observer architecture being an open, referee-grade question. Linearity is therefore forced *given* that operational principle, not unconditionally. Non-linear dynamics over $\mathbb{Z}/q\mathbb{Z}$ generically produce $q$-dependent observable physics ($x^2 \bmod 3$ and $x^2 \bmod 5$ have different algebraic structure), violating the gauge equivalence. The theorem operates within the discrete alphabet structure; it does not address whether $q \to \infty$ limit produces continuum dynamics or what role real-valued degrees of freedom play.
...
The honest assessment: OI naturally produces horizon holography (the $A/4$ entropy law and the holographic bound) but does not naturally produce full AdS/CFT-style correspondence as an emergent feature. A is a structural consequence of the partial-trace observational structure; the latter requires specific compactification-dependent content that OI's substratum does not directly provide.
...
Part II §9.6 formalizes a structural distinction between *observer-admitting* substrata — those that admit a partition satisfying C1–C4 — and *observer-lacking* substrata, where no such partition exists. The distinction operates at the level of the substratum's bijection structure, prior to any observer being instantiated. This section examines how the distinction reframes the relationship between OI and the string landscape.
...
**The translation of C1–C4 to string-theoretic structure.** The structural conditions for observer admission [Main §3] are formulated for finite deterministic substrata. Translating them to string-theoretic compactifications requires identifying analogs in string-theoretic structure:
...
- *C2 (memory persistence)* requires the written record to persist to readback, the slow bath one realization: the hidden-sector correlation time much longer than the observer's measurement timescale. In string-theoretic terms: the cosmological structure must have a stable phase with horizon timescales $\tau_B \sim H^{-1}$ much longer than laboratory timescales $\tau_S$. AdS compactifications without de Sitter structure, or compactifications with rapidly mixing cosmological structure, fail C2.

- *C3 (sufficient capacity)* requires the hidden sector to have sufficient information capacity to store the interaction history. In string-theoretic terms: the cosmological horizon's information capacity (set by the holographic bound at $\sim 10^{122}$ for our observed universe) must be sufficient. Compactifications with horizons of insufficient capacity fail C3.
...
- *Rigorous translation of C1–C4 to string-theoretic structure.* The translations above are plausible but not formal. Working out a rigorous version for specific compactification classes (heterotic, Type II, F-theory) would require detailed analysis specific to each class.
...
- A unitary realization: a hidden sector $\mathcal{H}_H$, a reference state $\rho_H$, and a unitary $U$ on $\mathcal{H}_V \otimes \mathcal{H}_H$ lifting $\Phi$ to a unitary dynamics (Definition 9.2; when $\rho_H$ is pure the associated isometry is the Stinespring dilation)

The substratum is captured by the realization; the observer is captured by the visible-sector algebra and the CPTP channel. Equivalence between embedded observers is equivalence of the channel and algebra structure, with the realization being gauge relative to the fixed reference preparation.
...
*for some hidden-sector reference state $\rho_H$. The triple $(\mathcal{H}_H, \rho_H, U)$ is a* unitary realization *of $\Phi$. When $\rho_H = |0\rangle\langle 0|$ is pure, the map $V_{U,0}: \psi \mapsto U(\psi \otimes |0\rangle)$ is an isometry $\mathcal{H}_V \to \mathcal{H}_V \otimes \mathcal{H}_H$ satisfying $\Phi(\rho) = \mathrm{Tr}_H[V_{U,0}\, \rho\, V_{U,0}^\dagger]$, and it is this isometry — not $U$ — that is a Stinespring dilation of $\Phi$.*

**Remark (why the distinction is load-bearing).** A unitary realization carries strictly more data than the channel constrains, in two separate ways, and both matter below. First, when $\rho_H$ is pure the channel sees $U$ only through its action on $\mathcal{H}_V \otimes \mathbb{C}|0\rangle$; the action of $U$ on the rest of $\mathcal{H}_V \otimes \mathcal{H}_H$ is entirely free. Second, when $\rho_H$ is mixed the realization remains a realization rather than a Stinespring dilation, and its hidden-sector dimension is not the Kraus rank — it can be strictly *smaller*. The completely depolarizing qubit channel has Kraus rank $4$, yet $\rho_H = \mathbb{1}/2$ with $U = \mathrm{SWAP}$ realizes it on a two-dimensional hidden sector. This is not an edge case for OI: §10.4's construction uses the uniform mixed reference state, so OI's own $(\mathcal{H}_H, \rho_H, U_\varphi)$ is a unitary realization and not a minimal Stinespring dilation, and the uniqueness statements below must not be read as applying to it directly.
...
A structural feature important for the universality-class framework is the *many-to-one* relationship between substratum-with-observer systems and their embedded observers. Many distinct substratum-with-observer quintuples produce the same embedded-observer triple; the additional data is the unitary realization $(\mathcal{H}_H, \rho_H, U)$. The uniqueness available here is at the level of the Stinespring isometry, and it is worth separating the algebraic content from what it does and does not say about $U$.

**Proposition 9.7a** (Minimal Kraus uniqueness). *Let $\mathcal{H}_V$ be finite-dimensional and let*
$$\Phi(\rho) = \sum_{i=1}^{r} A_i \rho A_i^\dagger = \sum_{j=1}^{s} B_j \rho B_j^\dagger$$
*with $\{A_i\}_{i \le r}$ and $\{B_j\}_{j \le s}$ each linearly independent. Then $r = s$ and there is a unique unitary $W \in \mathrm{U}(r)$ with*
$$B_j = \sum_i W_{ji} A_i .$$

*Proof.* Vectorize: the map $\rho \mapsto \sum_i A_i \rho A_i^\dagger$ determines, and is determined by, the positive semidefinite operator $M = \sum_i |a_i\rangle\langle a_i|$ where $|a_i\rangle = \mathrm{vec}(A_i)$. Linear independence of the family says the $|a_i\rangle$ are linearly independent, so $r = \mathrm{rank}\,M = s$ and both families are bases of $\mathrm{ran}\,M$. Writing $X$ and $Y$ for the matrices with those columns, $XX^\dagger = M = YY^\dagger$ with $X, Y$ of full column rank forces $Y = X W^{\mathsf T}$ for a unique unitary $W$. $\square$

Linear independence is exactly minimality, and it is not removable: without it the family size is not even determined. For any $3 \times 2$ isometry $M$ the family $C_k = \sum_i M_{ki} A_i$ realizes the same channel with three members rather than two, and no unitary relates families of different sizes.

**Proposition 9.7b** (Minimal pure-reference dilation uniqueness). *Let $(\mathcal{H}_H^{(1)}, |0_1\rangle\langle 0_1|, U^{(1)})$ and $(\mathcal{H}_H^{(2)}, |0_2\rangle\langle 0_2|, U^{(2)})$ be pure-reference unitary realizations of the same channel $\Phi$, and let $V_a \psi = U^{(a)}(\psi \otimes |0_a\rangle)$ be the associated Stinespring isometries. If both are minimal — $\overline{\mathrm{span}}\{(A \otimes \mathbb{1})V_a \psi : A \in \mathcal{B}(\mathcal{H}_V),\ \psi \in \mathcal{H}_V\} = \mathcal{H}_V \otimes \mathcal{H}_H^{(a)}$ — then $\dim \mathcal{H}_H^{(1)} = \dim \mathcal{H}_H^{(2)}$ and there is a unique unitary $W: \mathcal{H}_H^{(1)} \to \mathcal{H}_H^{(2)}$ with*
$$V_2 = (\mathbb{1}_V \otimes W)\, V_1, \qquad \text{equivalently} \qquad U^{(2)}(\psi \otimes |0_2\rangle) = (\mathbb{1}_V \otimes W)\, U^{(1)}(\psi \otimes |0_1\rangle) \ \ \forall \psi .$$
*No equality of the full unitaries away from the reference subspaces follows.*

*Proof.* Minimality of a Stinespring isometry is linear independence of its Kraus family, so this is Proposition 9.7a transported through $A_i = (\mathbb{1}_V \otimes \langle i|)V_a$. $\square$

The final clause is not a hedge. The channel constrains $U^{(a)}$ only on $\mathcal{H}_V \otimes \mathbb{C}|0_a\rangle$, and the action elsewhere is free: with $\mathcal{H}_V = \mathcal{H}_H = \mathbb{C}^2$, the realizations $U^{(1)} = \mathrm{CNOT}$ and $U^{(2)} = \mathrm{CNOT}\,(\mathbb{1} \otimes Z)$ produce the same channel, share the same hidden sector, and are both minimal — yet minimality forces $W = \mathbb{1}$, so no unitary conjugation carries one to the other. Any statement of the form $U^{(2)} = (\mathbb{1}_V \otimes W) U^{(1)} (\mathbb{1}_V \otimes W^\dagger)$ is therefore false as an identity of operators.

**Remark (the conjugation form).** After additionally choosing $W$ so that $W|0_1\rangle = |0_2\rangle$ — an admissible normalization, not a consequence — the relation of Proposition 9.7b can be written as a conjugation on the reference subspace. It still does not extend to an equality of the full operators.

**Remark (why [Substratum] Lemma 24.1 can say more).** That lemma does assert a conjugation of the dilating unitaries, $\hat\varphi' = (\mathbb{1}_V \otimes W_B)\,\hat\varphi\,(\mathbb{1}_V \otimes W_B)^{-1}$, and this is consistent with the restriction here rather than in tension with it. Proposition 9.7b fixes a single channel and a single reference preparation, so nothing constrains $U$ off $\mathcal{H}_V \otimes \mathbb{C}|0\rangle$. Lemma 24.1 instead fixes the whole family $\{\Phi_t\}_{t \ge 0}$ generated by one unitary and works on the cyclic boundary sector $\mathcal{C}_B = \overline{\mathrm{span}}\{\hat\varphi^{\,t}(\mathcal{C}_V \otimes \mathrm{supp}\,\omega_H)\}$ — the subspace the dynamics reaches from the visible factor. There is no off-reference remainder there by construction: cyclicity is exactly the statement that the reference subspace generates everything, which is what upgrades an isometry statement to an operator statement. The price is that the conclusion holds on $\mathcal{C}_V \otimes \mathcal{C}_B$ and not on a larger hidden sector, which is the deep-sector remainder that lemma isolates as generator (iii). Single-channel uniqueness of the kind stated here cannot substitute for it, and the completeness argument of [Substratum §4] correctly invokes the semigroup form.

These propositions give the universality-class framework's many-to-one structure its precise form: at the level of minimal pure-reference realizations, what the embedded observer determines is the Stinespring *isometry*, up to hidden-sector unitary. That unitary is the algebraic counterpart of OI's $\mathcal{G}_{\rm sub}$ generator (i) (hidden-sector relabeling); non-minimality is the counterpart of generator (iii) (deep-sector enlargement); and the extension of the isometry to a full tensor-product unitary is a third freedom with no generator, because it is invisible to the channel.

**Corollary 9.8** (Substratum description as gauge). *The embedded-observer triple $(\mathcal{H}_V, \mathcal{A}_V, \Phi)$ does not determine a unique substratum realization. For a minimal pure-reference representative it determines the Stinespring isometry only up to hidden-sector unitary; the extension of that isometry to a full tensor-product unitary is additionally unconstrained off the prepared reference subspace; and non-minimal realizations introduce further enlargement freedom. The universality-class data is the embedded-observer triple.*

So the gauge freedom in the substratum description has at least three pieces — hidden-sector unitary, unitary-extension freedom off the prepared subspace, and non-minimal enlargement — where the earlier formulation named only the first and third.

**Scope (relative to the fixed-reference observational interface).** The second piece is gauge only because Definition 9.2 always prepares the same $\rho_H$. The off-reference action of $U$ is invisible to $\Phi$ for that reason and no other. If the hidden sector became independently preparable, that action would become observable and would cease to be gauge. The claim is therefore not that the substratum has less structure than the channel, but that the channel, at a fixed reference preparation, does not see it.

This sharpens the structural-realism claim. What is invariant under the universality-class framework's gauge structure is the embedded-observer triple; the substratum description is gauge data relative to the fixed-reference interface. OI's framework treats the equivalence class modulo $\mathcal{G}_{\rm sub}$ as ontologically real; the universality-class framework treats the embedded-observer triple as ontologically real.
...
OI's reconstruction theorem (Theorem 23 of [Substratum]) states that the empirical/structural inputs uniquely fix, under the theorem's stated conditions, the **local propagating lattice/gauge residue** modulo $\mathcal{G}_{\rm sub}$. In universality-class language, this is a uniqueness result for the local OI residue characterized by the relevant partial-trace features plus A1-A6; it is not a theorem that one Bell-inclusive full substratum uniquely populates the class.

The reconstruction theorem's content is that the stated inputs select one **local lattice/gauge residue** modulo $\mathcal{G}_{\rm sub}$ within OI's structural class. H-Bell separately conditions whether a measurement-independent Bell-violating composite can inhabit the same ontology, and neither H-Bell nor Lemma 24.1 makes that Bell-composite structure unique.

### 10.2 What OI does and does not establish at the universality-class level

OI's reconstruction theorem establishes uniqueness of the **local reconstructed residue** within the OI structural class. It does not establish Bell-inclusive uniqueness even within that class, and it does not establish anything about *other* universality classes. Part I (§§3-7) is explicit on this: the equivalence-class extension to string-theoretic substrata fails because string-theoretic substrata are in different universality classes (substrata violating A2 and A5 are not in OI's class).
...
OI's structural-realism claim (per [Substratum §1.1]) is scoped to the **reconstructed local residue modulo** $\mathcal{G}_{\rm sub}$. The universality-class framework refines this: what is class-specific is that local residue within OI's class; what is universality-class-level is the partial-trace observational pattern. Bell-inclusive completion data remain an additional underdetermined layer.
...
**The channel.** The CPTP channel encoding visible-sector dynamics is generated by the substratum bijection $\varphi$ traced over the hidden sector. As a unitary realization (Definition 9.2):
$$\Phi^{\rm OI}(\rho) = \mathrm{Tr}_H\big[U_\varphi \, (\rho \otimes \rho_H^{\rm OI}) \, U_\varphi^\dagger\big]$$
where $U_\varphi$ is the unitary lift of the bijection $\varphi$ on $\mathcal{H}_V^{\rm OI} \otimes \mathcal{H}_H^{\rm OI}$ ([Substratum §3.1]), and $\rho_H^{\rm OI}$ is the uniform reference state on the hidden sector (the OI-canonical choice from the maximal-entropy invariant-measure lemma of [Main §1.2]). That reference state is *mixed*, so this triple is a unitary realization rather than a Stinespring dilation, and Propositions 9.7a–9.7b do not apply to it directly: obtaining a Stinespring isometry from it requires first purifying $\rho_H^{\rm OI}$, which enlarges the hidden sector. At the EFT scale, $\Phi^{\rm OI}$ encodes:
...
- *Non-perturbative structural features.* Instanton-sum closure consistent with $T$-invariance and detailed balance.
...
| Bekenstein-Hawking $1/4$ coefficient | Conditional on H-slope with the horizon and frame conditions; no direct empirical test (GW250114 constrains the area theorem) | [GR §2, §8.5, GW250114] |
...
| 22 SM observables, classified (seven parameter-free structural on the empirically selected cubic substratum) | Sharpest entries sub-percent; classes per [SM §7.6] | [SM §7.5] |
...
### 12.5 The no-GUT structural prediction

The framework predicts that no grand-unified gauge structure exists at any energy scale. This prediction has three layers, established jointly by [SM §4.6], [SM §6.4], and [SM §6.1] and brought together in [SM §6.7]:

(A) *Group-theoretic.* The SM gauge group emerges directly from cubic-commutant decomposition; no intermediate large gauge group exists at any scale.

(B) *Coupling-level.* Gauge couplings do not unify. They are split at the cutoff by $C_2$-dependent thresholds (induced-gauge-theory structure), with pure SM running below $M_{\mathrm{Pl}}$ continuing the splitting.
...
**Connection to the unified naturalness pattern.** The no-GUT prediction is an instance of a broader pattern in the framework: substratum-level structural symmetries dissolve naturalness problems by forcing emergent parameter values that would otherwise be free. The instances catalogued in [SM §8.8] are: (i) *strong CP* — an open instance: substratum T-invariance narrows $\theta$ to $\{0,\pi\}$ and gives reciprocal transition counts but does not fix $\bar\theta$ ([SM §5], H-top and H-det); (ii) *hierarchy problem* — singlet-taste chiral symmetry provides composite-Higgs-style protection of the Higgs mass against cutoff-scale corrections ([SM §8.8], partial resolution); (iii) *cosmological constant* — trace-out's information compression ratio dissolves the $10^{122}$ discrepancy ([GR §6]). The no-GUT structural prediction extends this pattern: rather than requiring a GUT mechanism to explain gauge-coupling unification (and inheriting all the GUT scenarios' associated empirical commitments), OI's direct cubic-commutant gauge emergence sidesteps the unification puzzle structurally. Three of the four — hierarchy, cosmological constant, gauge structure — are dissolved by the same kind of substratum-level structural mechanism, and strong CP is narrowed but not dissolved by it, giving the framework a unified treatment of what standard physics treats as four independent open problems requiring four independent solutions.
...
#### 13.1.1.1 Converged comparison protocol

A specific protocol for executing Steps 2-4 against a chosen compactification has been worked out as preparation for offline computational follow-up. The protocol is:

**Compactification choice: heterotic line-bundle Calabi-Yau in the Anderson-Gray-Lukas-Palti class.** Specifically, the Schoen-quotient model $X = \tilde{X}/(\mathbb{Z}_3 \times \mathbb{Z}_3)$ with five-line-bundle SM model [Anderson-Gray-Lukas-Palti 2011-2014]. This compactification was selected after considering three candidates (heterotic line-bundle on Schoen quotient; type IIA Pati-Salam on intersecting D6-branes; F-theory $SU(5)$ with hypercharge flux) on multiple criteria: 4D EFT characterization completeness (heterotic AGLP has multi-loop Yukawa structure characterized in the literature); three-generation match (all three candidates produce three generations, but through structurally distinct mechanisms — see below); pure SM gauge group at IR (all three reach SM after appropriate breaking); bundle-cohomology computational tractability; phenomenological literature density (heterotic AGLP has the most extensive literature). The Schoen-$\mathbb{Z}_3 \times \mathbb{Z}_3$ model dominates on most criteria and was selected over its alternatives.

**Comparison scale: below the Stueckelberg $U(1)$ mass (~$10^{16}$ GeV).** Heterotic line-bundle compactifications produce an extra Stueckelberg-massive $U(1)$ via the Green-Schwarz mechanism (anomaly cancellation in heterotic with line bundles); typical Stueckelberg masses in AGLP models are $\sim 10^{16}$ GeV, set by the compactification scale. Below this mass, the heterotic effective theory has only the SM gauge group, matching OI's structure at the IR. The comparison must therefore be conducted at scales below the Stueckelberg mass to ensure both sides have matching gauge structure. This restricts the universality-class claim to the IR EFT (scales $M_Z \lesssim \mu \lesssim 10^{16}$ GeV) but does not prevent the comparison from proceeding.

**Comparison test: Yukawa textures.** Both frameworks reach three generations by structurally distinct mechanisms (heterotic uses bundle-cohomology index of the relevant cohomology groups on the CY, an entirely topological invariant; OI uses the cubic group's $T_1$ irrep with multiplicity 3 from the spatial nearest-neighbor structure). Three generations is therefore not a discriminator; both frameworks pass this test by different routes. The natural quantitative test is whether the *Yukawa textures* arising from the cubic-group reduction in OI ([SM §7]) match the textures arising from bundle-cohomology computations on the Schoen-$\mathbb{Z}_3 \times \mathbb{Z}_3$ heterotic model. OI's Yukawa textures are characterized in [SM §§7.1-7.3] (CKM matrix, charged-lepton masses, PMNS angles); the heterotic Yukawa textures are characterized in [Anderson-Gray-Lukas-Palti 2013-2014]. The comparison reduces to: do these textures agree at the scale below the Stueckelberg mass?

**Verdict criteria.** The comparison produces one of three verdicts: (i) Yukawa textures agree at the comparison scale (suggesting OI and Schoen-$\mathbb{Z}_3 \times \mathbb{Z}_3$ are in the same algebra-channel-pair universality class at the IR EFT level); (ii) textures disagree on dimension-4 (renormalizable) operators (frameworks not in same class even at IR); (iii) textures agree on renormalizable operators but disagree on higher-dimensional Wilson coefficients (frameworks share leading-order SM but differ on subleading structure — the expected outcome per §13.1.1's three-outcome decomposition).

**A subtle point about scale-choice.** The comparison-scale specification matters for the verdict. Below the Stueckelberg mass, both frameworks have just SM, and the comparison tests Yukawa textures. Between the Stueckelberg mass and $M_{\mathrm{Pl}}$, the heterotic theory has an extra $U(1)$ + GUT-related structure that OI does not have; the algebras differ at this scale. A comparison at the *higher* scale would yield a *different-classes* verdict not because of fundamental disagreement but because of the integrating-out structure. The honest comparison protocol must specify the scale, and the IR EFT scale (below Stueckelberg mass) is the natural choice for testing whether OI and the heterotic compactification can coexist in the same universality class at all.

**Status of execution.** The protocol is fully specified at the structural level. Executing it requires: (i) extracting OI's Yukawa-texture predictions in a form directly comparable to bundle-cohomology output; (ii) extracting AGLP's Yukawa-texture predictions for the Schoen-$\mathbb{Z}_3 \times \mathbb{Z}_3$ model in matching form; (iii) running the *-isomorphism check at the IR EFT level; (iv) running the channel-conjugacy check on the dilation structures (substratum-to-EFT for OI; heterotic-UV-with-Stueckelberg-integrated-out for the compactification). Steps (i) and (ii) are literature-extraction work; (iii) and (iv) are the operator-algebra computations. All four steps are deferred to offline computational follow-up.

**Connection to the no-GUT structural finding.** The Stueckelberg $U(1)$ mass at $\sim 10^{16}$ GeV is the heterotic compactification's natural intermediate scale, set by the compactification structure. This is the string-theoretic analog of the seesaw scale (per Q8 in §13.3). The comparison scale's choice — at or below this Stueckelberg mass — implicitly settles the comparison's relationship to Q8: at the IR EFT level, OI and heterotic may share universality class even though OI lacks a structural intermediate scale (per Q10's negative resolution in §13.3). Universality-class matching at the IR is consistent with structurally-distinct intermediate-scale physics, with the structural difference being absorbed into the channel rather than being a barrier to algebra-isomorphism. This refines the §13.1.1 expected-outcome decomposition: outcome (b) is the working expectation (algebras agree at IR EFT; channels differ at intermediate scales).
...
### 13.3 Open questions on intermediate-scale physics

The structural investigation of OI's no-GUT prediction (developed in [SM §6.7] and [SM §8.7]) surfaced several substantive open questions that bear on the framework's predictive content but require further work to develop. They are catalogued here as research directions; their development is deferred to future work.

**Q8.** *The neutrino mass scale puzzle.* The framework's [SM §8.4] predicts Majorana neutrinos via the dimension-5 Weinberg operator $(LH)(LH)/\Lambda_{\mathrm{seesaw}}$, with $\Lambda_{\mathrm{seesaw}}$ treated as a Tier 3 (solution-specific) parameter. The observed neutrino mass scale $m_\nu \sim 10^{-3}$ eV requires $\Lambda_{\mathrm{seesaw}} \sim 10^{16}$ GeV — coincidentally the GUT scale. In standard GUT scenarios, this scale is structurally explained by gauge-coupling unification. In OI's no-GUT framework, no GUT scale exists structurally; the natural cutoff is $M_{\mathrm{Pl}} \sim 10^{19}$ GeV, which would give $m_\nu \sim 10^{-6}$ eV — three orders of magnitude too small. The framework currently treats $\Lambda_{\mathrm{seesaw}}$ as solution-specific input rather than deriving it. *Open question:* does the framework have an intermediate scale between $M_Z$ and $M_{\mathrm{Pl}}$ that arises structurally from the substratum's staggered-taste structure or other lattice-level features, and that could naturally produce $\Lambda_{\mathrm{seesaw}} \sim 10^{16}$ GeV? Speculative candidates include scales associated with the staggered-taste mass spectrum (where different tastes have different couplings $|\mu_\Gamma| = 1, |\mu_X| = 1/3$ per [SM §4.7] Theorem 8b), or scales associated with the boundary-layer thickness in the visible-vs-hidden sector partition.

**Q9.** *The hierarchy problem in OI's composite-Higgs structure.* The framework's [SM §7.4] predicts $\lambda(M_{\mathrm{Pl}}) = 0$ as a structural consequence of the Higgs being a composite scalar (the singlet taste). This prediction is empirically supported at $0.6\sigma$ and identifies the Higgs sector as composite at the substratum level. Standard composite-Higgs scenarios in BSM phenomenology dissolve the hierarchy problem via chiral-symmetry protection (the Higgs as a pseudo-Nambu-Goldstone boson of an approximate chiral symmetry). *Partial resolution* (developed in [SM §8.8]): the framework structurally implements an analog of this protection through the singlet-taste chiral symmetry. The $\mu = +1$ and $\mu = -1$ singlet tastes are paired under chirality at the substratum level (exact discrete symmetry); the Higgs vev spontaneously breaks this pairing in the emergent EFT. The Higgs is the order parameter of the broken chirality, with mass protected against cutoff-scale corrections by Goldstone's theorem.

The framework *derives* cutoff insensitivity — no $O(M_{\mathrm{Pl}}^2)$ corrections to $m_H^2$, only explicit-breaking corrections of order $g^2 f^2 / (16\pi^2)$ — together with the compositeness scale $f \sim M_{\mathrm{Pl}}$ being structurally forced. The framework *does not derive* the Higgs vev value $v \sim 246$ GeV, which remains Tier 3 (solution-specific). The boundary: the framework derives why $m_H$ is not at the cutoff (chiral protection); it does not derive why $v$ is at the electroweak scale rather than at the compositeness scale.

Quantitatively, the framework's partial resolution differs from "no resolution" by a factor of $\sim 10^{34}$ in the required fine-tuning. Standard SM without chiral protection requires fine-tuning $m_H^2$ to 34 decimal places; the framework's mechanism requires only that the Tier 3 input $v$ be at the electroweak scale, with $m_H$ following from $\lambda(M_{\mathrm{Pl}}) = 0$ + RG running.

*Open question:* is there a structural mechanism in OI that derives $v \ll f$, or is $v$ genuinely solution-specific? Two candidate paths: (i) deriving $v \ll f$ from substratum structure through a mechanism currently not in the framework; (ii) accepting $v$ as solution-specific, with the structural-vs-solution-specific partition being part of the framework's content. The framework's current commitment is (ii): $v$ sits in the solution-specific category alongside CP phases, generation-specific Yukawa magnitudes, and other parameters whose values are not forced by the structural derivation chain.

**Q10.** *Possible unified resolution of Q8 and Q9.* Both Q8 (neutrino mass scale) and Q9 (Higgs hierarchy) point to a single underlying question: does OI's substratum produce a structural intermediate scale between $M_Z$ and $M_{\mathrm{Pl}}$ that's not a GUT scale but plays the roles that GUT scales play in standard scenarios — setting the seesaw scale for neutrino masses and protecting the Higgs mass via chiral mechanisms? **Tentative resolution:** negative on the analysis conducted here. The framework's substratum-level scales are essentially $M_{\mathrm{Pl}}$ (lattice cutoff) and emergent scales below $M_Z$; candidates for intermediate scales (staggered-taste mass gap, boundary-layer thickness, $\delta_0 = \mathrm{Tr}[Y^2]$ coincidence, $C_2$-dependent threshold effective scales, cosmological-horizon-induced scales, emergent dimensional transmutation) all yield scales at $M_{\mathrm{Pl}}$ modulo $O(1)$ factors or scales below $M_Z$, but none naturally at $\sim 10^{16}$ GeV. The seesaw scale and the Higgs mass therefore remain as Tier 3 (solution-specific) parameters in the framework, set by the specific bijection $\varphi$ rather than by structural derivation. *Implication:* OI's predictions on intermediate-scale physics are limited to what can be derived from $M_{\mathrm{Pl}}$ + EW-scale physics, with no claim of structural intermediate-scale derivation. This is consistent with the framework's existing treatment of $\Lambda_{\mathrm{seesaw}}$ as Tier 3 (per [SM §8.4]) and represents an honest scoping rather than a hidden deficiency. The negative resolution sharpens the structural-vs-solution-specific distinction: many parameters that GUT-style frameworks treat as derivable from unification structure are, in OI, genuinely solution-specific. This *may* be revisited if a structural mechanism overlooked in the chat-tractable analysis is identified by future work, but the chat-tractable conclusion is that no such mechanism is currently apparent in the framework's existing structure.

**Q11.** *Quantitative baryogenesis in a spectrum with no right-handed neutrino.* The framework's substratum-level baryon-number conservation (per [SM §8.7]) coexists with sphaleron-mediated $B+L$ violation in the emergent EFT. But the OI spectrum contains no right-handed neutrino — anomaly cancellation forces exactly five representations per generation and the singlet taste is the Higgs ([SM §4.9, §8.4]) — so standard type-I thermal leptogenesis, which requires heavy right-handed Majorana neutrinos, does not directly apply. *Open question:* the full quantitative calculation — connecting whatever ultraviolet physics generates the dimension-5 Weinberg operator (the $\Lambda_{\mathrm{seesaw}}$ scale of Q8) to the resulting baryon asymmetry $\eta = n_B/n_\gamma \approx 6 \times 10^{-10}$ — is open, and the mechanism is not fixed by the framework. Resolution depends partly on Q8: without a structural derivation of the Weinberg-operator scale, the baryogenesis prediction inherits the same status (Tier 3 / solution-specific).

**Q12.** *Integrated treatment of non-perturbative emergence.* [SM §8.7]'s table of substratum-level structural precursors mapping to emergent-level non-perturbative SM features (T-invariance → $\theta = 0$; topology of induced gauge action → sphalerons; bipartite checkerboard → chiral fermions and anomalies; block-diagonal $M$ → substratum $B$ conservation) provides a unified pattern that organizes the framework's treatment of non-perturbative SM physics. *Open question:* is this mapping exhaustive, or are there non-perturbative SM features (e.g., specific instanton-density distributions, specific non-perturbative QCD vacuum structure, specific anomaly-inflow mechanisms) that lack identified substratum-level precursors? A complete cataloging would either confirm the framework's structural completeness on this dimension or identify gaps requiring further development.

These five open questions (Q8-Q12) represent the framework's substantive research frontier as of the no-GUT investigation. Each is a candidate research direction for future development; together they form a coherent set focused on whether OI's substratum has the structural richness needed to resolve standard SM open problems (neutrino masses, hierarchy, baryogenesis, non-perturbative completeness) without invoking new physics beyond what the framework already commits to.

### 13.4 Limits of the approach
...
**Limit 3: Depends on continuum / discrete bridge.** The formalization assumes that substratum descriptions, regardless of discreteness, yield visible-sector reduced laws admitting CPTP-channel representation with operator-algebra structure — representability is universal for finite laws ([Main §3.4]); operational closure beyond it is a model hypothesis, not a framework property. For OI's discrete substratum and string theory's continuum substratum this is plausible (both yield quantum-representable observable-level descriptions). For substrata whose visible-sector observable structure is not of this form, the framework may not apply.

**Limit 4: The foundational thesis is unfalsifiable on its own; the content lives in the model of the incompleteness.** The framework's name and starting point — that what is known is reached through observation, which is necessarily incomplete (an embedded observer cannot access the total state) — is, read as a bare thesis, not a falsifiable physical claim. It is close to a restatement of the empirical stance common to all of science: every theory is a description constructed from inside, from partial access. So the bare thesis cannot be wrong, and therefore cannot by itself predict anything. All of the framework's predictive content lives one level down, in the *specific model* of the incompleteness: the C1–C4 architecture (a fast visible sector reading and writing to a slow, high-capacity hidden register), the cubic lattice, the wave-equation dynamics, and the ETH-genericity of the hidden sector. Each of these is either an empirically-anchored selection or a structural stipulation (catalogued for the SM sector in [SM §8.3, *Derivation status: the three tiers*]; for the reconstruction in [Substratum §3, Refinement]). The framework's power and its circularity are accordingly the same structure seen from two sides: it is a maximally tight compression of known physics onto a small set of commitments — establishing that the Standard Model's discrete structure, the gravitational thermodynamics, and the Born-rule statistics are facets of one object rather than independent inputs (a genuine claim about the unity of physics) — while those commitments are themselves chosen to recover the physics already observed. Reading "observation is incomplete" as the *source* of the predictions overstates the framework; reading it as the *organizing principle* under which a large body of physics compresses to one structure is the defensible claim, and the one this paper's universality-class machinery is built to make precise. This limit is not a defect to be removed by further work — it is intrinsic to any framework whose foundation is the empirical stance itself — and stating it plainly is what distinguishes a structural-realist compression from a claimed derivation of the world from nothing.
...
## 14. Cross-framework audit of observational structures
...
We proceed feature by feature. For each, we characterize the structural feature in OI's framework, identify the analogous mechanism in string-theoretic emergence, and assess whether the feature transports across frameworks. Four feature classes are examined: Born-form quantum probability structure, channel-level unitary dilation, memory/non-Markovian marginal dynamics with divisibility tracked separately, and commutant gauge-invariance.
...
**OI's framing.** At the finite observable-law layer, OI supplies an exact fixed-basis Born-form representation rather than a parameter-free derivation of the quadratic probability functional [Main §3]. A finite reversible realization lifts to a permutation unitary, and with $Q_{\mathrm{fb}}$ defined by the dictionary $p=|U|^2$, tracing over the hidden register reproduces the visible transition law exactly. The important scope distinction is that the construction does **not select** the exponent: permutation-unitary entries are $0$ or $1$, so $|U|^p$ gives the same transition table for every $p>0$. For a general quantum representation the visible density matrix $\rho_V = \text{Tr}_H |\psi\rangle\langle\psi|$ gives $p(a)=\text{Tr}(\Pi_a\rho_V)$ in the usual way; identifying that full coherent observable/instrument structure simultaneously across all interventions is part of the operational-lifting frontier. OI's proved contribution at this layer is therefore the exact Born-form representation of classical deterministic substrata, not an independent selection theorem for why nature chooses the quadratic dictionary.
...
**Transport assessment.** The **Born-form representation structure** transports straightforwardly: once a framework is represented quantum mechanically and reduced by partial trace, subsystem probabilities use the standard quadratic trace rule. The distinction is that OI constructs an exact fixed-basis quantum representation starting from a classical reversible substratum, whereas string theory assumes the quantum formalism from the outset. This comparison should not be read as an OI derivation of the exponent $2$ itself; Main §3.1/§3.4 makes explicit that the permutation construction is exponent-blind and that the dictionary's physical origin is a separate frontier item.

The shared-invariants reading is therefore representation-theoretic rather than a new Born-rule uniqueness theorem. The quadratic Born rule itself is empirically tested at the $10^{-3}$–$10^{-4}$ level via multi-path interferometry [Sinha2010, Söllner2012], with ongoing precision improvements through ultracold-atom platforms [Kanthak2025] (further discussion in §15.5).
...
### 14.3 Memory, non-Markovianity, and recurrence-scale indivisibility

**OI's framing.** OI's C4 readback condition is first a statement of *conditional memory*: in general $P(a_3\mid a_2,a_1) \neq P(a_3\mid a_2)$ because hidden-sector correlations retain information about visible history [Main §2.3, §3.4]. That property is not identical to P-indivisibility. In Main's divisibility sense, P-indivisibility means that for some pair of rooted visible maps there is no stochastic propagator that divides the later map through the earlier one. What OI proves for one fixed finite reversible representative is more specific: C4 supplies a non-permutation rooted map, finite recurrence later returns the visible map to the identity, and a non-permutation stochastic map has no stochastic inverse; hence an indivisibility witness occurs somewhere in the recurrence cycle. Bell is a separate constraint: a response-complete deterministic completion that is both measurement-independent and ontically parameter-independent obeys $|S_{\rm CHSH}|\le2$, so Bell-violating OI composites retain measurement independence and relinquish ontic parameter independence while preserving operational no-signaling [Main §3.3]. Thus C4 memory is the temporal mechanism, recurrence supplies the proved indivisibility witness, and neither memory nor indivisibility is a Bell selector.

**String-theoretic analog.** Holographic effective field theories explicitly classify modes by their Markovianity. The sound scalar in compressible relativistic fluids dual to AdS planar black holes is a non-Markovian field with Markovianity index $3-d$ for a $d$-dimensional fluid. The holographic Schwinger-Keldysh formalism [HolographicSK2024, HolographicNonHydro2024] derives effective field theories that include non-Gaussian noise and non-Markovian dynamics as features of the boundary effective description, arising from integrating out bulk degrees of freedom. This supplies a close analog of the *memory/non-Markovian* part of OI's mechanism.

The structural parallel is therefore at the memory level: in both frameworks, restricting attention to a subset of degrees of freedom can produce history-dependent effective dynamics when the integrated-out degrees of freedom retain correlations across time. Partial trace or coarse-graining by itself does not force this outcome; a memoryless hidden sector can produce a Markovian reduced law.

**Transport assessment.** The memory/non-Markovian mechanism transports across frameworks. The stronger P-indivisibility property does **not** follow merely from the cited holographic non-Markovianity statements, and is not claimed here for string theory without a separate divisibility analysis. OI's recurrence-scale witness uses additional structure that the cross-framework analogy does not automatically supply: one fixed finite reversible representative plus C4 readback.

This distinction is important for the Bell comparison as well. Holographic non-Markovian effective theories provide an analog of hidden-memory dynamics, not by themselves a proof of the standard quantum Bell correlation set. In OI, a response-complete deterministic completion that combines the pointwise graph cone with a measurement-independent pre-setting ensemble is Bell-local and satisfies $|S_{\rm CHSH}|\le2$; quantum Bell violation therefore requires the separately stated ontic-parameter-dependent branch, with operational no-signaling retained. Hidden memory and stochastic indivisibility alone do not supply that branch.

The memory mechanism has independent theorem-level corroboration. [BrandnerMeyer2025] establishes a rigorous weak-memory regime for autonomous linear evolution equations with hidden-degree-of-freedom coupling, providing a controlled framework for the non-Markovian visible dynamics OI invokes. [TomizukaTakeda2026] constructs a structurally similar hidden-model decoherence picture independently, motivated by classical–quantum dynamics in gravity. [Barandes2026]'s stochastic-quantum theorem concerns indivisible stochastic processes and unitary quantum systems; Main §3.4 retains that result for the compressed form and interpretation under the translation hypothesis, while keeping indivisibility distinct from conditional-memory non-Markovianity and keeping fixed-basis representability internal. The framework's central technical move therefore sits within an established theorem-level neighborhood without identifying these different notions by definition (§15.1 develops this in detail).
...
|---|---|---|
| Born-form quantum probability structure | Yes at the representation/formalism level | OI exhibits an exact fixed-basis representation; string theory starts from quantum formalism; the comparison does not select the exponent $2$ |
| Channel-level unitary dilation | Yes | Different dilation mechanisms (Stinespring vs holographic) |
| Memory / divisibility | Memory mechanism: yes; P-indivisibility: not established cross-framework | OI has C4 memory and a recurrence-scale indivisibility theorem; holographic non-Markovianity alone does not prove stochastic non-divisibility |
...
The first two features transport directly at the representation/dilation level. The third transports at the memory/non-Markovian mechanism level, while OI's recurrence-scale P-indivisibility uses additional finite-recurrence structure and is not inferred for string theory here. The fourth transports as a *pattern* (gauge invariance via commutant) but not as a *specific output*.

This refines recovery (v) substantively. Partial trace is an architectural mechanism, not a theorem that every reduced process is memory-bearing. Once a quantum probability dictionary is in place, reduced probabilities have the usual Born form; once a visible map is CPTP, a unitary dilation is available. By contrast, non-Markovian memory requires retained hidden correlations, and P-indivisibility requires a further divisibility argument. OI supplies those extra ingredients through C4 and finite recurrence for a fixed representative; the cross-framework audit must check them separately elsewhere.

The picture that emerges is therefore narrower than a universal four-feature derivation: **observation by restriction supplies a common interface pattern, while memory, divisibility, and substratum-symmetry outputs depend on additional structure**. OI and string theory share important representation/dilation architecture and can share hidden-memory mechanisms; they do not thereby become equivalent at the full operational level.

This conclusion has implications for the universality-class question developed in Part II below. Embedded-observer universality classes should be characterized by a vector of properties rather than by “partial trace” alone: which quantum-probability representation is used, which dilation structure exists, whether the discarded sector retains operational memory, whether the reduced dynamics is actually indivisible, and which gauge structures the substratum admits. OI and string theory can intersect on several coordinates while reaching them through different mechanisms.

### 14.6 Implications for the OI-string relationship

The cross-framework analysis of §14.1–14.5 establishes that recovery (v) — shared observational structure — has substantive content, but it also fixes the scope of that claim. The commonality is strongest at the representation/dilation interface and becomes conditional once memory, divisibility, or substratum symmetry is involved.

This refines the conclusion of §7.8 and §14.5: OI and string theory are not equivalent at the substratum level (per §6), and they do not share *all* observational features. They share a structural pattern of restricting a larger description to an observable sector; within quantum formulations this carries the standard probability formalism and admits unitary dilation. Whether the reduced dynamics is memory-bearing depends on correlations retained in the discarded sector, and whether it is P-indivisible requires a separate divisibility theorem. OI proves the latter at recurrence scale from C4 plus a fixed finite reversible representative; no corresponding conclusion for string theory is inferred merely from holographic non-Markovianity.

This intersection is non-trivial but should not be overstated. A framework that already uses quantum states and partial trace inherits the standard quantum probability rule for its subsystem and the usual dilation architecture. It does **not** automatically inherit non-Markovianity, P-indivisibility, Bell causal locality, or OI's specific commutant output. Those require additional dynamical, statistical, or symmetry structure.

The constructive content of recovery (v) is therefore: **the restriction/partial-trace interface is a locus of shared architecture, not by itself a derivation of every observational feature audited here**. This connects to OI's "observation occurs" principle while preserving Main's distinction between universal representation, C4 memory, recurrence-scale indivisibility, and the still-conditional operational-local lift.
...
**Memory / divisibility.** Spin-foam dynamics on visible-sector observables, after integrating over hidden internal-edge data, plausibly produces non-Markovian effective dynamics. The mechanism is structurally similar to OI's trace-out: the visible-sector marginal can carry memory of substratum-level dynamics through correlations established in earlier spin-foam evolution. This supports a plausible memory/non-Markovian analog, but it does not by itself establish P-indivisibility in Main's stochastic-propagator sense. Both the existence of the effective memory law and any stronger divisibility failure require technical work; this is the most underdeveloped of the four feature classes for LQG.
...
**LQG transport summary.** The quantum-probability formalism, channel-level unitary-dilation pattern, and commutant gauge-invariance pattern are present in the comparison. The memory/divisibility feature is only plausible at the memory level; P-indivisibility remains open. LQG therefore shares several interface-level structures with OI without yet matching OI's recurrence-scale indivisibility result.
...
**Memory / divisibility.** Causal-set coarse graining can plausibly produce history-dependent effective dynamics when fine-grained causal structure retains correlations. That is structurally similar to OI's hidden-memory mechanism. The comparison supports a plausible memory/non-Markovian analog; it does not establish P-indivisibility without a separate divisibility analysis.
...
**Causal sets transport summary.** The commutant/order-invariance pattern transports most directly. Quantum-probability and unitary-dilation structure depend on the QSG formulation, while memory is plausible and P-indivisibility remains unaudited. Causal sets are therefore related at several structural levels but do not presently match the full OI feature vector.
...
**Memory / divisibility.** Integrating over high-momentum modes in an RG flow can generate non-local-in-time or memory-bearing effective interactions. This gives a strong mechanism-level analog of OI's visible-hidden memory picture, with the split operating in momentum space rather than the spatial substratum. The existence of such non-Markovian effective structure does not by itself certify P-indivisibility in Main's divisibility sense; that stronger property would require a separate propagator analysis.
...
**Asymptotic safety transport summary.** Standard quantum probability and unitarity are inherited from the QFT framework, the RG split supplies a strong memory-mechanism analog, and gauge-invariance is preserved. The comparison does not establish P-indivisibility from non-Markovianity alone. Mechanistically this remains one of the closest cross-program parallels to OI, but not an all-four-feature theorem.
...
| Program | Born-form / quantum probability | Unitary dilation | Memory / indivisibility | Commutant gauge |
|---|---|---|---|---|
| String theory | Yes (built in) | Yes | Non-Markovian modes; indivisibility not established here | Pattern; specific differs |
| OI | Yes (represented) | Yes | C4 memory; indivisible at recurrence scale | $SU(3)\times SU(2)\times U(1)$ |
| LQG | Yes (built in) | Yes | Memory plausible; indivisibility open | Pattern; specific differs |
| Causal sets | Open (QSG) | Structurally | Memory plausible; indivisibility open | Pattern; trivial gauge |
| Asymptotic safety | Yes (built in) | Yes | Non-Markovian EFT mechanisms; indivisibility not established here | Pattern; specific differs |

Three observations:

*The restriction-interface features are robust, but at different logical strengths.* Quantum formulations carry their probability rule and admit unitary dilation; several programs also exhibit or plausibly generate memory-bearing effective dynamics when degrees of freedom are integrated out. The stronger P-indivisibility statement does not follow from non-Markovianity alone and is proved in OI here only at recurrence scale from C4 plus a fixed finite reversible representative. Thus §15.3's universality claim should be read as an architectural transport claim, not as a theorem that partial trace forces every feature in the table. For OI specifically the split is sharper than "integrating out generates memory": partial trace alone does not force memory — the memory-free reversible witnesses are exactly the visible-decoupled maps, whose visible dynamics carries no hidden dependence. But in the uniform full-product finite reversible setting, the $T$-level form of (C1) is a non-permutation one-step visible matrix, and then memory follows in general rather than only by census: [Main §3.4]'s non-permutation-witness lemma gives P-indivisibility at recurrence scale, while every first-order Markov process is P-divisible. Equivalently, the visible factor of a finite deterministic bijection has zero asymptotic entropy rate (its entire history is fixed by one of finitely many initial substratum states), whereas a stationary first-order Markov chain with non-permutation $T$ has $H(X_{t+1}\mid X_t)>0$. That second route needs less than uniformity and less than $|V|=2$: it needs a $\varphi$-invariant prior — which makes the visible process stationary — giving positive weight to some visible state whose $T$-row is non-degenerate. Uniform-on-the-full-product is one such prior, so the ensemble censused above is a special case, and the theorem covers the non-uniform invariant priors that [Main §3.4]'s arbitrary-prior extension admits (check 10, `equivalence_recovery_probes.py`, with a prior supported on a visible-decoupled block as the control that correctly does not fire). The sharp form is worth stating on its own: the visible factor of a finite reversible substratum has zero entropy rate under every invariant prior, so whatever randomness an embedded observer records is bounded by $\log|\mathcal{C}_V \times \mathcal{C}_H|$ and is exhausted at recurrence scale — the same scale at which the indivisibility witness appears, and for the same reason. The exhaustive checks are therefore regressions of that theorem, not its proof: at $|V|=2$ and $|H|=2,3,4$, respectively, all $16/16$, $648/648$, and $39{,}168/39{,}168$ $T$-level-C1 bijections carry visible memory; the $|H|=4$ complement consists of exactly $1{,}152=2(4!)^2$ visible-decoupled bijections (checks 7–9, `equivalence_recovery_probes.py`). The $648$ denominator coincides with [Main §3.1]'s entanglement-breaking census but the diagnostic is different: that census classifies channels as measure-and-prepare, while this one classifies visible processes as memory-bearing, and the two must not be merged.
...
The cross-program audit refines §13.1.4's per-condition A1–A6 analysis by identifying which observational structures recur even where the structural conditions differ. The evidence supports a shared *interface architecture* — coarse graining/restriction, quantum-probability form where quantum formalism is present, dilation structure, and possible hidden-memory effects — but not a blanket derivation of P-indivisibility or of the Born exponent from A1, A3, A4, A6 plus the SM target. Those stronger properties must be established by the program-specific dynamics. Section 14 is therefore a transport audit, not an independent derivation theorem.
...
**Stinespring dilation as universal device.** The Stinespring representation theorem [Stinespring1955] guarantees that every CPTP channel admits a unitary dilation. §9.2 (Definition 9.2) and §10.4's channel construction invoke this, with the qualification recorded there: OI's own realization uses a mixed reference state, so what it exhibits is a unitary realization rather than a Stinespring dilation, and the uniqueness statements of Propositions 9.7a–9.7b apply to the pure-reference isometry rather than to it. The recent literature has refined Stinespring dilations into a working tool for quantum channel learning and metrology, including channel tomography techniques [RandomStinespring2025] that provide protocols for distinguishing channels via their dilations — the operational analog of the §13.1.1 four-step recipe's channel-conjugacy step.
...
**Cosmological tests of the framework's GR-emergence cluster.** [DESI_DR2_2025] reports rejection of the cosmological constant at 2.8σ–4.2σ depending on the supernovae dataset, with potential phantom crossing at $z \approx 0.5$ and a 2σ+ detection of nonzero neutrino masses in extended-parameter cosmology. The framework's [GR §7] carries dark energy in Type II running-vacuum form — the functional form the local expansion shared by every smooth model, the content the Type II channel and the magnitude; the [Bertini2025] direct fit to DESI DR2 + DES-Y5 + Planck 2018 reports $\nu = -(2.5 \pm 1.3) \times 10^{-4}$, consistent with zero at 2σ; the framework's expected magnitude $|\nu| \sim 10^{-32}$ (conditional on the Type II reading) is itself indistinguishable from zero at this precision, so the two are consistent. DESI Year 5 results, expected within the publication horizon, will be discriminating.
...
**Black-hole entropy.** GW250114 (LIGO/Virgo/KAGRA, September 2025) confirmed Hawking's area theorem at $99.999\%$ confidence — classical horizon dynamics the framework preserves; the $1/4$ coefficient of [GR §5], conditional on H-slope with the horizon and frame conditions, has no direct empirical test. The observation is a non-discriminating consistency for the cosmological-horizon application of the partial-trace mechanism.
