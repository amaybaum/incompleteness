...
**Q11.** *Quantitative leptogenesis from OI's RH neutrino sector.* The framework's substratum-level baryon-number conservation (per [SM §8.7]) coexists with sphaleron-mediated $B+L$ violation in the emergent EFT, supporting standard cosmological baryogenesis through thermal leptogenesis with the framework's predicted Majorana neutrino sector. *Open question:* the full quantitative calculation — connecting the framework's right-handed neutrino mass scale (Q8) to the resulting baryon asymmetry $\eta = n_B/n_\gamma \approx 6 \times 10^{-10}$ via the standard washout-factor machinery — is open. Resolution depends partly on Q8: without a structural derivation of the seesaw scale, the leptogenesis prediction inherits the same status (Tier 3 / solution-specific).
