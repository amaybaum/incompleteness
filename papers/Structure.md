...
**Reconstruction theorem (Theorem 23, [Substratum §3]).** Given the empirical inputs E1–E7 (unitary QM, Bell violations, finite boundary entropy, spatial isotropy, propagating gravity, stable matter, boundary-entropy concordance) together with the structural assumptions A1-A6 (finiteness, determinism, bounded coupling, center independence, linearity, background independence), together with the M1 modeling premise (Stage 1's P-indivisible route), the equivalence class $[(S, \varphi)]/\mathcal{G}_{\text{sub}}$ is uniquely determined — conditional on the hidden-sector mixing hypothesis for the C2 necessity direction ([Main §3.4]; ETH its physical motivation) and on Lemma 24.1's semigroup-transfer/completeness step — as a finite deterministic bijection on a $d=3$ cubic lattice with $K = 6$ internal components, gauge group $SU(3) \times SU(2) \times U(1)$, three generations (physical reading conditional on H-spin', [SM §4.7]; chirality on H-χ', [SM §4.8]), anomaly-free hypercharges, $\bar\theta = 0$ (kinematic level; conditional on the open fork of [SM §5.5]), and $\hbar = c^3 \epsilon^2/(4G)$.
...
Combined with empirical inputs E1–E4 (unitary QM, Bell violations, finite boundary entropy, spatial isotropy), these, with the M1 modeling premise, uniquely determine — conditional on C2-mixing and Lemma 24.1's completeness — the equivalence class $[(S, \varphi)]/\mathcal{G}_{\rm sub}$ at the lattice level (Theorem 23 in [Substratum]).
...
This distinction is important for the Bell comparison as well. Holographic non-Markovian effective theories provide an analog of hidden-memory dynamics, not by themselves a proof of the standard Bell causal-local quantum correlation set. In OI, graph causal cones and operational setting screening remain separately load-bearing for that conclusion.
