...
**OI's framing.** OI derives the Born rule (probability $|\psi|^2$ for measurement outcomes) from the embedded-observer trace-out [Main §3]. The visible-sector density matrix $\rho_V = \text{Tr}_H |\psi\rangle\langle\psi|$ produces probabilities for visible-sector observables via $p(a) = \text{Tr}(\Pi_a \rho_V)$, where $\Pi_a$ is the projector onto outcome $a$. The Born rule is not postulated as a separate axiom; it follows from the partial-trace structure of the observer's marginal description. As with the compressed form of the emergent description, this is an *identification via the Barandes stochastic-quantum correspondence* under its translation hypothesis rather than a parameter-free derivation ([Main §3.4]) — the representation's existence internal: the $|\psi|^2$-structure enters with that correspondence, and OI's specific contribution is to show it is forced even for a classical, deterministic substratum via the Stinespring lift.
...
**Transport assessment.** The Born rule transports straightforwardly. Both frameworks use the same partial-trace structure to extract probabilities for subsystem observables. The distinction is that OI *exhibits the Born rule as forced* by substratum-level structure (classical bijection lifted to Stinespring dilation, traced down to observer marginal, with the quantum $|\psi|^2$-structure entering via the Barandes correspondence), while string theory *assumes* the Born rule as a feature of its quantum-mechanical apparatus.
