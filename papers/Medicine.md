...
The Observational Incompleteness (OI) framework [1] provides such a principle. Originally developed for fundamental physics — where it proves that the description of any embedded observer with partial access to a deterministic system is necessarily non-Markovian — quantum for the P-indivisible subclass, under the translation hypothesis (T) — the framework's core theorem is abstract and scale-independent. It identifies four conditions under which any fast subsystem necessarily exhibits non-Markovian dynamics:
