...
The Observational Incompleteness (OI) framework [1] provides such a principle. Originally developed for fundamental physics — where it proves that quantum mechanics is the necessary description of any embedded observer with partial access to a deterministic system — the framework's core theorem is abstract and scale-independent. It identifies four conditions under which any fast subsystem necessarily exhibits non-Markovian dynamics:
