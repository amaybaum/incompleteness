...
for p in c1_cp_scope_probes c2_mixing_probes c3_dilation_probes \
         ...
         c4_backflow_probes review4_probes hspin_labels_probes hchi_selectivity_probes intervention_probes; do
  if python3 "$p.py" >/dev/null 2>&1; then echo "PASS  $p"
  else echo "FAIL  $p"; FAIL=1; fi
done
[ $FAIL -eq 0 ] && echo "all eleven suites green" || echo "BATTERY FAILED"
exit $FAIL
