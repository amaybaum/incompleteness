...
INVARIANTS = [
    ...
    # ---- b149 families: review-56 classes ----
    (r"places P and NP inside BQP|P and NP are both inside BQP|Both P and NP are inside BQP",
     "NP vs BQP is an open containment; only P ⊆ BPP ⊆ BQP is known (b149)",
     [],
     "the framework's BQP characterization places P and NP inside BQP"),
    (r"conjecture to theorem and",
     "the ECT is a theorem of the framework TOGETHER WITH the §14.4 model, never unconditional (b149)",
     [],
     "the ECT reframed from empirical conjecture to theorem and"),
    (r"theorem-grade commitment",
     "the qubit-wall is a complexity-assumption-dependent proposal, not entailed by Main (b149)",
     [],
     "which is a theorem-grade commitment of the corpus"),
    (r"full unitary group through composition",
     "finite gate sets are approximately universal (dense), never the full unitary group (b149)",
     [],
     "generate the full unitary group through composition"),
    # ---- b149 families: review-56 interior-conditionality classes ----
    (r"reframes the ECT from empirical conjecture to \*theorem\*",
     "the ECT is a CONDITIONAL theorem under §14.4's model, never a bare structural consequence (b149)",
     [],
     "reframes the ECT from empirical conjecture to *theorem*"),
    (r"would (falsify|refute) the framework at the foundational level",
     "super-BQP falsifies the conjunction with the model, sharpest at the bridge; the framework only if every bridge repair fails (b149)",
     [],
     "would refute the framework at the foundational level"),
    (r"ceiling is structural rather than technological",
    ...
    # ---- b154 families: review-61 sentence-level proof classes ----
    (r"Every observer operation is a CPTP map",
     "unconditioned operations are CPTP; conditioned branches CP trace-nonincreasing (quantum instruments, b154)",
     [],
     "Every observer operation is a CPTP map on the density matrix"),
    (r"channels (form|coincides? with) BQP",
     "channels are transformations; BQP is a language class — use the circuit-simulation implication (b154)",
     [],
     "the efficiently and uniformly implementable channels form BQP"),
    (r"corresponds to projective measurement followed by post-selection",
     "state preparation is an explicit efficient-preparation model hypothesis, charged its success cost (b154)",
     [],
     "State preparation corresponds to projective measurement followed by post-selection"),
    # ---- b155 families: review-62 propagation classes ----
    (r"identifies BQP with the class of operations",
     "BQP is a class of problems/languages, never of operations; use decidable-by-computations form (b155)",
     [],
     "identifies BQP with the class of operations available to embedded observers"),
    (r"QM emergence theorem's toolkit construction|quantum toolkit \(from the QM emergence theorem",
     "the representation is Main's; the controllable gate toolkit is the model's (b155)",
     [],
     "the framework's quantum toolkit (from the QM emergence theorem of Chapter 1)"),
    (r"constitut(e|ing) a falsification\.",
     "every falsification names the framework-plus-model conjunction, sharpest at the bridge (b155)",
     [],
     "with any demonstrated super-BQP capability constituting a falsification."),
    # ---- b156 families: review-63 classes ----
    (r"BQP-BPP",
     "retired corpus-wide (reversed-string survivor of b151): task-specific quantum advantage / conjectured separation (b156)",
     [],
     "the BQP-BPP gap is largest for problems"),
    (r"complete box|none negotiable",
     "the account is a four-axis PROGRAM: width complexity-assumption-dependent, rate/memory queued (b156)",
     [],
     "the complete box of physical computation — every face with a number or a queued derivation, none negotiable"),
    (r"experimental test of the framework's foundational characterization",
     "the BEC experiment tests the proposed mechanism, never the characterization theorems (b156)",
     [],
     "the cleanest near-term experimental test of the framework's foundational characterization theorem"),
    (r"no known exponential quantum advantage",
    ...
    # ---- b164 families: review-71 corpus-architecture closure ----
    (r"hidden sector (to )?evolve much more slowly|far more degrees of freedom than the visible",
     "C2 = persistence (slowness one route); C3 = capacity (N_H >> N_V a regime, not the condition) (b164)",
     [],
     "requires that the hidden sector evolve much more slowly than the visible sector"),
    (r"and hence QM",
     "P-indivisibility diagnoses the memory sector; nothing is 'hence QM' (b164; Main's 'indivisible, hence non-Markovian' is the true temporal direction and stays)",
     [],
     "the information backflow that defines P-indivisibility and hence QM"),
    (r"produce QM as the unique|produces the emergent description in the first place",
     "trace-out produces the reduced observable law; Q_fb representability universal, non-unique (b164)",
     [],
     "allows the trace-out to produce QM as the unique embedded-observer description"),
    (r"C1–C4 — supplied by the imported correspondence",
     "the representation is internal; Barandes supplies the COMPRESSED form under (T), never the gate (b164)",
     [],
     "under C1–C4 — supplied by the imported correspondence"),
    (r"Bell-inequality violations are P-indivisible",
     "Bell violation is spatial/operational; temporal P-indivisibility is the framework's ROUTE, not a generic identity (b164)",
     [],
     "Bell-inequality violations are P-indivisible, hence non-Markovian a fortiori"),
    # ---- b165 families: review-72 closure classes (broad) ----
    (r"slow-bath memory|slow-bath timescale separation|slow \(C2|C3 \(high|C3 \(large|C3: \$?N_H|C3 condition — \$?N_H|C3 requires large hidden-sector capacity|high-capacity \(C3|coupled to a slow, high-capacity hidden sector|\*\*Slow bath\.\*\*",
     "C2 = persistence, C3 = capacity; slow bath and N_H>>N_V are realization regimes only (b165; the exception covers Main:128's n_H->inf limit description, not a definition)",
     [r"independent conditions in statement"],
     "C2 (slow-bath memory), and C3 (high hidden-sector capacity)"),
    (r"C1–C4 ⇒ QM|necessarily experiences P-indivisible|hence imply the stochastic process is P-indivisible|necessarily non-Markovian whenever|is the \*necessary description\*|P-indivisible at the recurrence scale\)",
     "no C1–C4 ⇒ QM, no C4 ⇒ P-indivisibility, no generic Bell ⇒ temporal P-indivisibility (review4_probes.py; b165; appendix-B's proof-structure recurrence parenthetical is the certified C1+finiteness content)",
     [r"structurally equivalent to a unitary"],
     "the *sufficiency* direction (C1–C4 ⇒ QM) is unconditional"),
    (r"generates (emergent )?(QM|quantum mechanics)|whose cost is quantum mechanics|how quantum mechanics emerges at all|produce QM at the observable level",
     "trace-out produces the reduced observable law; representability is universal, never generated by the trace-out (b165)",
     [],
     "the trace-out over the hidden sector generates emergent QM"),
    # ---- b167 families: review-74 semantic census (arrow + prose forms) ----
    (r"Bell violations? → P-indivisibility|QM ⇒ C[23]|QM → C[23]",
     "no arrow-notation Bell=>P-indivisibility or QM=>C2/C3; the table states the certified equivalence (b167)",
     [],
     "(Bell violations → P-indivisibility)"),
    (r"C2 requires the hidden sector's correlation time|fails C2 because its hidden sector has no slow modes",
     "C2 = record persistence; slowness is one realization route (b167)",
     [],
     "C2 requires the hidden sector's correlation time"),
    (r"necessarily P-indivisible|produces P-indivisibility and consequently|trace-out that produces the emergent quantum",
     "P-indivisibility requires a measured witness, never automatic from conditions; trace-out produces the reduced description (b167)",
     [],
     "their dynamics is necessarily P-indivisible"),
    # ---- b168 families: review-75 proposition-level closure ----
    (r"slow-bath condition",
     "C2 is persistence; the slow bath is a realization — name it as such (b168; the continuum derivation's own regime hypothesis in Main/appendix-B is excepted by its weak-coupling window)",
     [r"small conditioned-Hamiltonian spread"],
     "The slow-bath condition C2 ensures that information"),
    (r"framework predicts[^.\n]{0,50}is P-indivisible|prediction: P-indivisible",
     "theorem-level consequence is memory/non-Markovianity; P-indivisibility needs a measured witness (b168)",
     [],
     "the framework predicts that the qubit's noise structure is P-indivisible"),
    (r"classical stochastic rather than quantum",
     "C-failure changes the memory sector, never representability: memoryless vs memory-bearing (b168)",
     [],
     "emergent dynamics is classical stochastic rather than quantum"),
    (r"C2: slow bath|slow bath with capacity \(C2",
     "definitional C2-as-slowness retired; persistence with the slow bath one realization (b168)",
     [],
     "(C1: non-trivial coupling, C2: slow bath, C3: high-capacity hidden sector"),
    # ---- b169 families: review-76 classes ----
    (r"requires the hidden-sector correlation time to be much longer",
     "C2 requires persistence; the correlation-time form is the slow-bath realization (b169)",
     [],
     "requires the hidden-sector correlation time to be much longer than the observer's measurement timescale"),
    (r"unitary structure of QM forces|Bell violations force C1",
     "no inverse QM/Bell => C-condition statements; certification runs through the modeling premise and the memory phenomenology (b169)",
     [],
     "the unitary structure of QM forces C2 and C3"),
    (r"produces the emergent quantum description",
     "trace-out produces the reduced observable description; representation universal (b169)",
     [],
     "produces the emergent quantum description"),
    (r"load-bearing only for the necessity direction",
     "the correspondence carries the compressed form under (T); necessity results are internal per frozen Main (b169)",
     [],
     "the correspondence is load-bearing only for the necessity direction"),
    # ---- b170 families: review-77 Theorem-22 proof-honesty ----
    (r"correlations persist indefinitely",
     "energy conservation does not protect record ACCESSIBILITY from phase mixing; persistence is a hypothesis here (b170)",
     [],
     "The hidden sector has spectral gap Δ = 0: correlations persist indefinitely"),
    (r"No visible-sector process of sub-exponential duration",
     "saturation claims require the no-reuse/compression hypothesis Main does not assume; use the per-process data-processing bound (b170)",
     [],
     "No visible-sector process of sub-exponential duration saturates the hidden sector's memory"),
    (r"C2 is still satisfied",
     "representability does not depend on C2; only the effective-Hamiltonian stationarity changes (b170)",
     [],
     "the emergent description remains quantum mechanical (C2 is still satisfied)"),
    # ---- b171 families: review-78 classes ----
    (r"CP-indivisibility implies non-monotonic trace distance",
     "system-only trace-distance revival is sufficient evidence, not a necessary consequence; the complete witness is ancilla-assisted (Main §3.4; b171)",
     [],
     "By the Breuer-Laine-Piilo criterion, CP-indivisibility implies non-monotonic trace distance"),
    (r"Together, C2 and C3 guarantee",
     "readback (C4) is the load-bearing condition; storage and capacity without it do not suffice (b171)",
     [],
     "Together, C2 and C3 guarantee that P-indivisibility is strong"),
    (r"C1 ✓, C2 ✓, C3 ✓ \|",
     "condition check-lists include C4; three-of-four is the certified counterexample class (b171)",
     [],
     "| C1 ✓, C2 ✓, C3 ✓ |"),
    # ---- b172 families: review-79 classes ----
    (r"proved to exhaust",
     "Theorem 24's exhaustiveness is conditional on Lemma 24.1's semigroup transfer, itself deferred to specialist verification (b172)",
     [],
     "four explicit families of generators proved to exhaust"),
    (r"QM \*?requires\*? C1, C2, C3, C4",
     "the necessity result concerns accessible memory, never QM membership; Q_fb is universal (b172)",
     [],
     "QM *requires* C1, C2, C3, C4"),
    (r"C1–C4-determined quantum-classical",
     "the boundary is memoryless-to-memory-bearing (backflow), never quantum-vs-classical membership (b172)",
     [],
     "the sharp C1–C4-determined quantum-classical boundary"),
    (r"conditions C1–C4 that produce P-indivisibility",
    ...
    # ---- b282: Selection-retirement provenance guards + Bell-mechanism correction ----
    (r"Bell (?:violations?|data)[^.\n]{0,100}(?:enter|entering|through|force|forcing|certif(?:y|ies|ied))[^.\n]{0,90}P[- ]?indivis",
     "Bell violation does not imply temporal P-indivisibility; Stage-1 M1-T and M1-B are separate after the Bell correction (b282)",
     [r"not.{0,45}P[- ]?indivis", r"does not.{0,45}P[- ]?indivis"],
     "Bell violations enter through the framework's P-indivisible joint-dynamics route"),
    (r"(?:[Ee]xact|[Ss]tandard) Bell causal locality[^.\n]{0,90}(?:requires|uses)[^.\n]{0,50}(?:setting )?screening|Bell locality additionally requires operational setting screening",
     "the adopted deterministic Bell branch is not Bell-local: measurement independence is retained and ontic parameter independence is relinquished (b282)",
     [],
     "Exact Bell causal locality additionally requires operational setting screening"),
    (r"Bell[- ]inequality violations?[^.\n]{0,100}(?:because|through)[^.\n]{0,80}(?:P[- ]?indivis|indivisible joint)",
     "indivisibility is not the deterministic Bell-violation mechanism; ontic parameter dependence is (b282)",
     [r"not.{0,45}(?:P[- ]?indivis|indivisible)"],
     "Bell-inequality violations occur through P-indivisible joint dynamics"),
    (r"nonlocal theory with a local ontology|ontology and (?:the )?dynamics are local[^.\n]{0,80}Bell",
     "the Bell-violating deterministic completion is ontically nonlocal in the parameter-independence sense (b282)",
     [],
     "the framework can survive the verdict a nonlocal theory with a local ontology"),
    (r"finite dimensionality[^.\n]{0,30}(?:—|-|is )?established|finite predictive (?:dimension|rank)[^.\n]{0,70}(?:completion)[^.\n]{0,40}(?:automatic|follows|established)",
     "stagewise/fixed-representative finiteness does not establish finite predictive rank of an exact operational completion (Selection P83/C85; b282)",
     [],
     "finite dimensionality — established"),
    (r"(?:mixed )?ancilla[^.\n]{0,80}can always be (?:further )?purified|reversible dilation[^.\n]{0,90}(?:therefore|hence|implies)[^.\n]{0,50}purification",
     "hidden reversible dilation is not the operational purification axiom; existence and essential uniqueness remain completion-level obligations (Selection P41/P42; b282)",
     [],
     "a mixed ancilla can always be further purified"),
    (r"(?:graph locality|causal cones?|tensor[- ]product structure)[^.\n]{0,100}(?:implies|forces|establishes|derives)[^.\n]{0,50}local tomography",
     "current composite primitives do not imply local tomography; Selection O3-T36 supplies an explicit holistic-kernel countermodel (b282)",
     [],
     "graph locality establishes local tomography"),
]
