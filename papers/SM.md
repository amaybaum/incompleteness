...
*A present-day constraint on the smearing coefficient.* The radiative-stability discussion above is usually framed as an open theoretical problem awaiting the characterization of the coarse-graining map. It is worth recording that it already has a mild *empirical* handle. The smearing mechanism, if it operates, leaves a residual dimension-six Lorentz-violating operator with a coefficient $c_6$ at the natural scale $\Lambda \sim M_{\text{Pl}}$ (the lattice cutoff): the dispersion takes the form $E^2 = m^2 + k^2\,[\,1 + c_6\,(k/M_{\text{Pl}})^2\,]$ in the isotropic part. Ultra-high-energy observations now constrain exactly this quadratic ($n=2$) coefficient: the absence of neutrino splitting for the KM3NeT event KM3-230213A gives $\Lambda_{n=2} \gtrsim 1.1\times10^{19}$ GeV $\approx M_{\text{Pl}}$ (Satunin, arXiv:2502.09548), and the consistency of the GZK suppression in the Auger and HiRes proton spectra bounds the proton coefficient at $\delta_{n=2} \lesssim 10^{-23}$ at Lorentz factor $\sim 2\times10^{11}$. In the framework's parameterization these require $c_6 \lesssim \mathcal{O}(1)$ — that is, an order-unity coefficient at the lattice scale is right at the current frontier rather than excluded, and a smearing mechanism that left $c_6 \gg 1$ would already be falsified. This is the first present-day empirical constraint on the radiative-Lorentz sector: it does not establish that the smearing is sufficient (the open problem remains the computation of $c_6$ from the coarse-graining map), but it bounds the answer from above and converts part of the question from purely theoretical to observational. The corresponding *differentiating* signature — the $\ell=4$ cubic-harmonic angular dependence of this residual, aligned to the cosmic rest frame (the rest-frame-selection lemma, [Main §3.5]) and correlated across species — is not constrained by these isotropic analyses and is the subject of forward prediction F14 ([appendix-A §A.5]).
