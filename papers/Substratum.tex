...
\textbf{Remark (amplitude-scale gauge --- the field-value-scale freedom).} One freedom invoked elsewhere in the framework is \emph{not} one of (i)--(iv) as stated and is named here as the single authoritative home: \emph{amplitude-scale invariance}, the rescaling of the field value \(\phi \mapsto \lambda\phi\) (for \(\lambda\) a unit) together with shifts \(\phi \mapsto \phi + c\) --- the \textbf{additive-automorphism (affine) relabellings} of the alphabet \(\mathbb{Z}/q\mathbb{Z}\), at fixed \(q\). It is the field-value-scale analogue of the alphabet-\emph{size} freedom (ii), and is the proper subgroup of the site-wise relabellings in (i) that preserves not only the coupling graph but the field-amplitude dispersion \(\omega(k)\) --- i.e. the emergent Hamiltonian. Two derivations invoke it: the linearity equivalence of [SM §4.1] (amplitude-independent dispersion \(\iff\) affine dynamics) and the \(SU(N)\) reduction of Stage 2(d). \emph{Status.} Amplitude-scale gauge is \textbf{not} derivable from (i)--(iv) as written: generator (i) over-counts (its full \(|S|!\) includes dispersion-altering nonlinear relabellings, which would make linearity gauge-non-invariant and so vacate rather than establish it), and generator (ii) is alphabet-\emph{size} only. It is therefore adjoined as an explicit principle, and the results that invoke it are conditional on it. Its motivation is that the alphabet is the cyclic \emph{group} \(\mathbb{Z}/q\mathbb{Z}\), so its additive automorphisms are the natural structure-preserving relabellings; this is weaker than assuming linear dynamics (a nonlinear \(F\) on the group is still a function on it). Whether amplitude-scale gauge is \emph{further} derivable, or whether grounding the gauge group on it begs the linearity it then implies, is the open structural question on the gauge sector --- flagged for external review.
