...
Theorem 23 reconstructs the lattice-level equivalence class
\([(S,\varphi)]/\mathcal G_{\rm sub}\) from its stated empirical inputs
and structural assumptions, conditional on the named modeling and
completeness premises; Theorem 24 identifies the observables-preserving
kernel, conditional on Lemma 24.1's semigroup-transfer completeness
step. The synthesis claim is therefore structural rather than
rhetorical: the same finite deterministic object supports the visible
stochastic/quantum projection, the boundary-thermodynamic gravitational
projection, and the lattice gauge projection. The traditional QM--GR
``unification problem'' is correspondingly recast as a category error
between two projections of one object, while the theorem statuses and
remaining conditional steps are kept explicit.
...
causal cones. What remains conditional is only the strongest operational
reading --- one common standard local quantum instrument/composite
theory for the whole coherent intervention family, together with
operational setting screening in Bell experiments. The projection from
...
sector. The additional empirical input is the observed memory/Bell
structure, interpreted within the framework through the modeling premise
M1: the relevant joint law belongs to the history-readback/indivisible
sector. {[}Main §2.3{]} proves that genuine C4 readback on a fixed
finite representative forces global indivisibility, while {[}Main
§3.3{]} separates graph-causal propagation from the still-explicit
operational setting-screening premise used for Bell locality. By the
reverse-direction lemma at the stochastic-process level ({[}Main
§3.2{]}) --- applicable here because the one-step matrices, as
ancilla-uniform marginals of unistochastic matrices, are doubly
stochastic --- any such P-indivisible process on a finite configuration
space admits a realization as marginalization of a deterministic
...
the dependency graph additionally supplies exact causal cones. Full
equality with the standard local coherent instrument/composite theory
remains the narrower operational-lifting statement of {[}Main §3.4{]},
and Bell locality additionally requires operational setting screening as
stated in {[}Main §3.3{]}. Applied to a cubic lattice with the wave
...
Bell correlations separate dynamical locality from operational
screening. Two visible sites prepared jointly may carry correlated
hidden-sector configurations, while the dependency graph still enforces
an exact causal cone: before the cones meet, a remote intervention
cannot dynamically alter the local substratum state {[}Main, §3.3{]}. To
promote that graph-local dynamics to Bell causal locality, the setting
variables must additionally be screened from preparation-relevant hidden
data. Under exact screening the causal-local indivisible construction
obeys Tsirelson; with imperfect screening Main gives a quantitative
correction. Thus the coupling graph supplies propagation locality, not
by itself the full statistical factorization condition.
...
\item
  \emph{Tsirelson/setting-screening test.} For the causal-local
  indivisible sector with exact operational setting screening,
  \(S_{\mathrm{CHSH}}\le 2\sqrt{2}\); with residual setting-hidden
  correlation \(I_{\mathrm{MI}}\), Main §3.3 gives the explicit
  robustness allowance. A confirmed excess beyond that allowed bound
  would falsify the corresponding causal-local/screening claim. Bare
  C1--C4 or graph locality alone are not asserted to enforce Tsirelson.
\item
