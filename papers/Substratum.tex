...
(C1--C4 ⇒ QM) is unconditional, so the framework's positive predictive
content does not rest on ETH. ETH is required to argue that QM
\emph{forces} the slow-bath structure, not to argue that the slow-bath
structure produces the quantum phenomenology. A referee skeptical of ETH
may therefore read the reconstruction's uniqueness as holding within the
class of ETH-satisfying (equivalently, non-integrable, non-MBL) hidden
sectors --- a class the framework argues is generic at \(d = 3\) rather
than special.
...
(slow-bath memory is required for the non-Markovian returns that produce
quantum interference) and C3 (high hidden-sector capacity is required
for the data-processing bound to accommodate observed information
backflow).
...
(memory persistence), C3 (high-capacity hidden sector), and C4 (history
readback), the embedded observer's compressed description is necessarily
memory-bearing (non-Markovian) --- its fixed-basis quantum
