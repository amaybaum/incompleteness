...
uniqueness claim is therefore substantive in exactly the dimensions that
carry physical content (the discrete structural invariants) and silent
in exactly the dimensions that do not (the gauge directions). It is the
opposite of tautological: a tautological theorem would fix nothing
specific, whereas this one fixes a particular finite list of integers
and groups that the framework then matches against the observed Standard
Model.
...
It is worth being explicit about the epistemic structure here, because a
reasonable objection is that the framework wants the lattice to be
fundamental (so the mass-gap problem can be set aside) while also using
continuum perturbation theory to extract the numbers that match data
(which would seem to make the continuum limit load-bearing after all).
The position is consistent, and stating it precisely matters. The
framework's claim is that the lattice theory is the complete physical
content and its predictions are exact \emph{at the lattice level}. The
comparison to experiment is then a comparison between two objects: the
lattice theory's prediction extracted via continuum PT (a controlled
...
approximation and the continuum-QFT data fit, and the framework's claim
is precisely that the approximation error in the first object is
\(\sim 30\) orders of magnitude below the experimental precision of the
second --- so the lattice-vs-continuum gap cannot be the source of any
observed agreement or disagreement at accessible energies. The mass-gap
problem is genuinely not load-bearing for this: it concerns whether a
continuum limit \emph{exists as a rigorous mathematical object}, whereas
the framework needs only that continuum PT is an accurate
\emph{calculational approximation} to the lattice theory's predictions
at low energy, which is the standard and well-tested status of
lattice-to-continuum perturbative matching and does not require the
existence of the rigorous limit. The framework does not have it both
ways: the lattice is fundamental, the continuum is a calculational tool
with a quantified error, and the experimental comparison is honest about
being a comparison of the tool's output against continuum-reported data.
...
(transition probabilities \(T_{ij}(t)\) for all \(t\), the emergent
Hamiltonian up to D-gauge, and \(\hbar\)). The completeness claim is
that \(g\) factors through (i)--(iv). The proof proceeds by decomposing
the data that \(g\) must preserve into a complete list of
substratum-level determinants and showing each is governed by one of the
four families.
...
\(\Phi_t\) invariant, which by Stinespring uniqueness applied to the
channel family (not a single channel) is a hidden-sector partial
