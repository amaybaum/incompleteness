...
minimality jointly with the condensate-stabilizer accounting); and (ii)
the \emph{substratum gauge group} \(\mathcal{G}_{\text{sub}}\) (Theorem
24), with four explicit families of generators proved to exhaust all
...
the Higgs as composite, the \((3,2,1)\) gauge structure, and
\(\bar\theta = 0\) follow as forced consequences {[}SM{]}. The
framework's hierarchical structural realism and its relationship to
other unification programs are developed in the fifth core paper
{[}Structure{]}.
...
doublet, anomaly-free hypercharges, \(\bar\theta = 0\) (kinematic level,
{[}SM{]} Theorem 21; the same premise removes CKM CP --- the open fork
of {[}SM §5.5{]}), and \(\hbar = c^3 \epsilon^2/(4G)\).}
...
on H-χ', {[}SM §4.8{]}), one Higgs doublet \((1, 2, +1/2)\), unique
hypercharges, and \(\bar\theta = 0\). Because the derivation is unique
--- no free parameters, no model-building choices, no landscape of
alternatives --- every independent experimental confirmation of the
Standard Model's gauge structure is retroactive evidence for the
derivation chain that produces it. The Standard Model is the most
...
conditions both necessary and sufficient (the C2-necessity step
conditional on the hidden-sector mixing hypothesis, {[}Main §3.4{]}; ETH
is its physical motivation). Applied to a cubic lattice with the wave
...
generations (chirality conditional on H-χ', {[}SM §4.8{]}) and the Higgs
as a composite scalar in the singlet staggered taste ({[}SM, §4.7{]},
Theorems 8--11), and anomaly-free hypercharges ({[}SM, §4.9{]}, Theorems
14--15). Twenty-two quantitative predictions follow ({[}SM, §7{]}),
including the Cabibbo angle from a single Brillouin-zone distance, the
Koide angle from a cubic-group quadratic Casimir, all three PMNS angles
within \(1.1\sigma\) (vs NuFIT 6.0), six fermion masses from one
empirical input to better than \(1\%\), and all three SM gauge couplings
at \(M_Z\) ({[}SM, §6.3{]}).
...
condensate-stabilizer accounting). The substratum gauge group identifies
the kernel of this inverse map and shows that the Standard Model gauge
group is its visible-sector shadow. Together, these two results
establish that the framework's emergence of quantum mechanics
