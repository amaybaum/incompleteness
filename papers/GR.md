...
*Reconciliation with the internal component count.* The counting $\eta = 1/\epsilon^2$ is one boundary mode per cell, whereas the substratum carries $K = 2d = 6$ internal components per site ([SM §4.5]). The $q$-gauge argument above covers the number of internal *states* per mode (the alphabet size), not the number of components, so the reconciliation is separate: what enters the horizon entropy is the number of field degrees of freedom that couple *across* the partition through a single boundary (normal-direction) link, which is not the same as the number of components carried at a site. Three candidate counts are available — the raw component count ($6$), the gauge-invariant flux count (one per $(3,2,1)$ block, i.e. $3$), and the scalar-field-value count crossing a single link ($1$) — and the entropy-relevant one is fixed by the rank of the cross-link coupling $M(\mathbf{n}, \hat e_{\text{normal}})$ restricted to the partition surface. The construction takes this count to be $1$, consistent with the single scalar field value transmitted per link; a computation of that rank on the OI coupling matrix would confirm it or replace it with $\kappa_m$. The dependence of the downstream results on this choice is asymmetric and worth stating: the Bekenstein-Hawking coefficient is *independent* of it — writing $\eta = \kappa_m/\epsilon^2$, the gap equation gives $\hbar = c^3/(4G\eta)$ and $S = \eta A = A/(4 l_p^2)$ with the $\kappa_m$ cancelling, so the $1/4$ is a fixed point of the construction — while the discreteness scale carries it as $\epsilon = 2\sqrt{\kappa_m}\, l_p$, so a count $\kappa_m \neq 1$ would rescale $\epsilon$ and, through it, the lattice cutoff at which [SM §6] defines $1/\alpha_0$. The reconciliation is therefore a determinate open computation whose outcome leaves the entropy law untouched and adjusts the discreteness scale and the coupling-sector cutoff identification; it is recorded here rather than resolved.
