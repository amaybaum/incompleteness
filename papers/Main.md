...
**Remark (a withdrawn claim).** Earlier versions asserted that condition (C1) — $T$ not a
permutation — suffices for $\Phi$ to be non-entanglement-breaking, on the grounds that
$\Phi(|i\rangle\langle i|)$ then has rank $\geq 2$ and some off-diagonal input has non-zero
image. Both inferences are invalid and the conclusion is false. Output rank is unrelated to
the measure-and-prepare property: the completely depolarizing channel has maximal output
rank and is entanglement-breaking. For a counterexample within the present construction take
$|V|=|H|=2$ and $\varphi(x,h)=(h,x)$. Then $T_{ij}=1/2$ for all $i,j$, which
is not a permutation, yet $\Phi(\rho)=\mathrm{Tr}_H[U_\varphi(\rho\otimes I/2)U_\varphi^\dagger]=I/2$
for every $\rho$ — a constant channel, manifestly measure-and-prepare. Exhaustive enumeration
of all bijections on $V\times H$ shows the failure is generic rather than exceptional: for
$|V|=|H|=2$ every (C1)-satisfying $\varphi$ gives an entanglement-breaking $\Phi$, and for
$|V|=2,|H|=3$ this holds for $612$ of $648$. The property (C1) constrains the *diagonal* of
$\Phi$, whereas entanglement-breaking is a statement about $\mathrm{id}\otimes\Phi$ acting on
half of an entangled pair; no list of single-system matrix elements can decide it.

What is true, and what the construction actually delivers, is a geometric statement.

**Definition.** Fix a partition with visible region $R\subseteq\Lambda$. Write
$\partial^{-}R=\{x\in R:\exists\,z\notin R,\ z\sim x\}$ for the inner boundary and
$\partial^{+}R=\{y\notin R:\exists\,z\in R,\ z\sim y\}$ for the outer boundary. Call a hidden
configuration *recording* if the induced map from visible inputs to hidden outputs is
injective — that is, if the hidden sector's final state determines which visible input
occurred.

**Lemma (boundary bound).** *For the nearest-neighbour reversible update $u'=Au+v$, $v'=u$ of
§1.4, the hidden output depends on the visible input only through $\partial^{-}R$, and the
visible output depends on the hidden input only through $\partial^{-}R$. Consequently*
$$\operatorname{rank}M_{HV}\ \leq\ |\partial^{+}R|,\qquad
  \operatorname{rank}M_{VH}\ \leq\ |\partial^{-}R| .$$

*Proof.* For $y\notin R$ the component $v'_y=u_y$ is a hidden input and carries no visible
dependence, while $u'_y=\sum_{z\sim y}u_z+v_y$ involves a visible input only if some
$z\sim y$ lies in $R$, i.e. only if $y\in\partial^{+}R$. Hence $M_{HV}$ has non-zero rows only
among $\{u'_y:y\in\partial^{+}R\}$. Dually, for $x\in R$ the component $v'_x=u_x$ is a visible
input, and $u'_x$ involves a hidden input only if some $z\sim x$ lies outside $R$, i.e. only if
$x\in\partial^{-}R$. $\square$

The content is one-step locality: a single update moves information one lattice site, so the
two sectors can inspect each other only across the cut. The argument uses no property of the
coefficient ring, and holds verbatim for $\mathbb{Z}_q$; it also survives non-linearity, since
for any nearest-neighbour rule the hidden output is a function of the visible input restricted
to $\partial^{-}R$.

**Theorem (coherence criterion).** *Write $\kappa=\dim\ker M_{HV}$ for the number of visible
directions invisible to the hidden sector, and $w=\operatorname{rank}M_{VH}$. Then $\Phi$
preserves coherence exactly between visible states differing by an element of $\ker M_{HV}$,
and*
$$\Phi\ \text{is non-entanglement-breaking}\iff w<\kappa .$$

*Proof.* $\Phi(|i\rangle\langle i'|)$ vanishes unless $M_{HV}(i-i')=0$, and on the surviving
blocks the output is averaged over translations by $W=\operatorname{im}M_{VH}$. Thus $\Phi$ is
a uniform random-Weyl channel over the group generated by the $2^{\,s-\kappa}$ phase operators
dual to $\ker M_{HV}$ and the $2^{w}$ translations in $W$, where $s=\dim V$. Because the
configurations $(0,v_R)$ lie in $\ker M_{HV}$ — the hidden output does not involve $v_R$ — and
$M_{VV}$ carries them onto the full $u'$-subspace, which contains $W$, that group is abelian.
A uniform mixture over an abelian group of Weyl operators is a stabilizer mixture, and its Choi
state is separable precisely when the group is maximal isotropic, i.e. when
$(s-\kappa)+w\geq s$. $\square$

**Corollary (decoherence is a boundary effect).** *If $|\partial^{-}R|+|\partial^{+}R|<2|R|$
then $\Phi$ is non-entanglement-breaking.* Indeed $\dim V=2|R|$, so the Lemma gives
$w+(\dim V-\kappa)\leq|\partial^{-}R|+|\partial^{+}R|<\dim V$, which is $w<\kappa$. For a
$d$-dimensional region of linear size $\ell$ one has $|\partial R|=O(\ell^{d-1})$ against
$2|R|=2\ell^{d}$, so the condition holds for every $\ell>2d$: in $d=3$, any visible region more
than roughly six sites across carries coherence.

Three features deserve emphasis. First, the criterion contains no reference to $|H|$: a hidden
sector of $10^{122}$ states is no more decohering than a small one, because it can still read
only the boundary. Second, it explains the counterexample above — the swap has no bulk, every
visible site lying on the cut, so it decoheres completely; the same holds for the one- and
two-site regions, which are all boundary. Third, the surviving coherence is a bulk quantity
while the decoherence is an area quantity, the same bookkeeping that governs the horizon
entropy of Chapter 7.

*Status.* The Lemma and Corollary are proved. The Theorem's group-size threshold is the
standard separability criterion for stabilizer mixtures; it has been verified here against
Choi-matrix ground truth in every configuration tested — all subspace pairs in $\mathrm{GF}(2)^3$
satisfying the abelian condition, and all visible-block partitions of rings up to $L=10$ — and
the abelian condition itself is proved above for the update of §1.4. The extension to
non-linear nearest-neighbour rules currently rests on the boundary factorization alone, which
supplies the invisible directions but not yet the sufficiency argument.
