...
The $A_1$ taste is the composite Higgs; the three $T_1$ tastes are the fermion generations; $T_2$ and $A_2$ are additional scalars [2]. The gauge coupling at the lattice scale is $1/\alpha_0 = 23.25$ (a convergent one-loop integral), with universal threshold $\delta_0$ doubly constrained by U(1) and a two-loop staggered VP integral (consistent at $\sim 1\sigma$). Pure-gauge MC plaquettes at the OI couplings $\beta_2 = 7.4$ and $\beta_3 = 11.1$ determine the $C_2$-dependent threshold. Combined with SM RG running, the chain reproduces all three SM gauge couplings at $M_Z$ via a one-parameter consistency test [2].
