...
**Q9.** *The hierarchy problem in OI's composite-Higgs structure.* The framework's [SM §7.4] predicts $\lambda(M_{\mathrm{Pl}}) = 0$ as a structural consequence of the Higgs being a composite scalar (the singlet taste). This prediction is empirically supported at $0.6\sigma$ and identifies the Higgs sector as composite at the substratum level. Standard composite-Higgs scenarios in BSM phenomenology dissolve the hierarchy problem via chiral-symmetry protection (the Higgs as a pseudo-Nambu-Goldstone boson of an approximate chiral symmetry). *Partial resolution* (now developed in [SM §8.8]): the framework structurally implements an analog of this protection through the singlet-taste chiral symmetry. The $\mu = +1$ and $\mu = -1$ singlet tastes are paired under chirality at the substratum level (exact discrete symmetry); the Higgs vev spontaneously breaks this pairing in the emergent EFT. The Higgs is the order parameter of the broken chirality, with mass protected against cutoff-scale corrections by Goldstone's theorem. **However, chiral protection alone is partial**: it bounds the Higgs mass by explicit breaking (gauge/Yukawa couplings squared times the breaking scale) but does not explain why the Higgs vev $v$ sits 17 orders of magnitude below the compositeness scale $f \sim M_{\mathrm{Pl}}$. The full hierarchy resolution requires additionally explaining $v \ll f$, which the framework currently treats as Tier 3 (solution-specific). *Open question:* is there a structural mechanism in OI that derives $v \ll f$ rather than leaving it as solution-specific input? Speculative candidates (trace-out-induced fractional-power suppression connecting the Higgs hierarchy to the cosmological-constant compression) are flagged for future investigation; the framework does not currently commit to them.
