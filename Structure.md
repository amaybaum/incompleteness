...
**Reconstruction theorem (Theorem 23, [Substratum §3]).** Given the empirical inputs E1-E6 (unitary QM with Bell violations, finite boundary entropy, spatial isotropy and homogeneity, observed Standard Model structure), the structural conditions A1-A6 (finiteness, determinism, bounded coupling, center independence, linearity, background independence) follow as consequences, and the substratum is uniquely determined as a finite deterministic bijection $(S, \varphi)$ on a $d=3$ cubic lattice with $K = 6$ internal components, gauge group $SU(3) \times SU(2) \times U(1)$, three chiral generations, anomaly-free hypercharges, $\bar\theta = 0$, and $\hbar = c^3 \epsilon^2/(4G)$.
...
Combined with empirical inputs E1–E4 (unitary QM with Bell violations, finite boundary entropy, spatial isotropy, observed SM structure), these uniquely determine the equivalence class $[(S, \varphi)]/\mathcal{G}_{\rm sub}$ at the lattice level (Theorem 23 in [Substratum]).
...
The hierarchy nests: Level A $\supset$ Level B $\supset$ Level C $\supset$ Level D. Each deeper level is a more specific commitment.
...
The hierarchy nests: G1 $\subset$ G2 $\subset$ G3 $\subset$ G4. Each broader level absorbs more degrees of freedom. The trace-out projects higher levels onto lower: trace-out of $\mathcal{G}_{\rm sub}$ produces the SM gauge group as image; restriction to Hamiltonian projects G2 onto G1.
...
The structural conditions, as established in [Substratum §3.1] following the V1 reformulation [Substratum §3.1], are: A1 (finiteness), A2 (determinism / bijection), A3 (bounded coupling degree), A4 (center independence), A5 (linearity), A6 (background independence). All six are forced by the empirical inputs E1–E5 combined with the framework's gauge structure ([SM §2.7] $q$-gauge invariance, [Substratum §4] Theorem 24 classical-substratum descent). For matrix models to be candidate representatives of OI's equivalence class, the same conditions must hold for the matrix-model description.
...
To establish definitively whether OI and string theory are in the same universality class, several technical questions must be addressed:
...
**Question 2: Do the algebras agree?** OI's emergent QFT has a specific algebra of observables determined by its cubic-lattice structure. String-theoretic compactifications produce algebras determined by the compactification specifics. Are these algebras *-isomorphic for SM-reproducing constructions, or merely structurally similar (sharing partial-trace features but differing on specific algebraic content)?

**Question 3: Do the channels agree?** Beyond the algebra structure, the channel $\Phi$ encodes the dynamics on visible-sector states. Does OI's $\Phi$ agree with the string-theoretic $\Phi$ on identical SM gauge group plus identical particle content, or does it differ on detailed dynamical features (e.g., the specific RG flow, specific anomaly content, specific instanton effects)?

These are substantial technical questions. Each would require detailed comparison of OI's framework predictions with specific string-theoretic compactifications producing the same observable target. Part III §14.5 establishes the structural-features-of-trace-out level; the universality-class question requires going further to algebra-channel-level comparison.
...
[Lehnert2025] K. Lehnert, "Hitchhiker's Guide to the Swampland", arXiv:2509.02632 (2025).
...
[Substratum] A. Maybaum, "Substratum Reconstruction and the Substratum Gauge Group" (2026). Zenodo DOI 10.5281/zenodo.19060318.

[Main] A. Maybaum, "Main paper" (2026). [Section to be filled in.]

[SM] A. Maybaum, "Standard Model from the Substratum" (2026). [Section to be filled in.]

[GR] A. Maybaum, "General Relativity from the Substratum" (2026). [Section to be filled in.]

[Mackey1963] G. W. Mackey, "Mathematical Foundations of Quantum Mechanics" (1963).
...
[HigherPointBFSS2025] A. Biggs, A. Herderschee, "Higher-point correlators in the BFSS matrix model", arXiv:2503.14685 (2025).

[HolographicSK2024] Y. Bu et al., "Holographic Schwinger-Keldysh field theory of SU(2) diffusion", arXiv:2205.00195 (2024).
