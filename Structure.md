...
### 13.3 Open questions surfaced by the no-GUT structural investigation
...
**Q9.** *The hierarchy problem in OI's composite-Higgs structure.* The framework's [SM §7.4] predicts $\lambda(M_{\mathrm{Pl}}) = 0$ as a structural consequence of the Higgs being a composite scalar (the singlet taste). This prediction is empirically supported at $0.6\sigma$ and identifies the Higgs sector as composite at the substratum level. Standard composite-Higgs scenarios in BSM phenomenology dissolve the hierarchy problem via chiral-symmetry protection (the Higgs as a pseudo-Nambu-Goldstone boson of an approximate chiral symmetry). *Partial resolution* (developed in [SM §8.8] with sharpened scope): the framework structurally implements an analog of this protection through the singlet-taste chiral symmetry. The $\mu = +1$ and $\mu = -1$ singlet tastes are paired under chirality at the substratum level (exact discrete symmetry); the Higgs vev spontaneously breaks this pairing in the emergent EFT. The Higgs is the order parameter of the broken chirality, with mass protected against cutoff-scale corrections by Goldstone's theorem.

*Sharpened scope* (per [SM §8.8] expanded treatment): the framework *derives* (i) cutoff insensitivity — no $O(M_{\mathrm{Pl}}^2)$ corrections to $m_H^2$, only explicit-breaking corrections of order $g^2 f^2 / (16\pi^2)$ — and (ii) compositeness scale $f \sim M_{\mathrm{Pl}}$ structurally forced. The framework *does not derive* (iii) the Higgs vev value $v \sim 246$ GeV, which remains Tier 3 / solution-specific. The boundary is therefore: *the framework derives why $m_H$ is not at the cutoff* (chiral protection); *it does not derive why $v$ is at the electroweak scale rather than at the compositeness scale* (Tier 3 input). The first is the substantive content of the hierarchy problem in standard SM phenomenology; the framework provides a structural answer. The second is a residual question about $v$'s specific magnitude.

*Distinguishing claim*: the framework's partial resolution differs from "no resolution" by a factor of $\sim 10^{34}$ in the required fine-tuning. Standard SM without chiral protection requires fine-tuning $m_H^2$ to 34 decimal places; OI's chiral protection mechanism requires only that the Tier 3 input $v$ be at the electroweak scale, with $m_H$ following from $\lambda(M_{\mathrm{Pl}}) = 0$ + RG running.

*Open question:* is there a structural mechanism in OI that derives $v \ll f$ rather than leaving it as solution-specific input? Two candidate paths: (i) deriving $v \ll f$ from substratum structure through a mechanism currently not in the framework — speculative candidates include trace-out-induced fractional-power suppression connecting the Higgs hierarchy to the cosmological-constant compression, flagged for future investigation but not committed to; (ii) accepting that $v$ is genuinely solution-specific, with the framework's responsibility being to identify which parameters are structural and which are not, rather than to derive every parameter from structure. The framework's current commitment is (ii).
