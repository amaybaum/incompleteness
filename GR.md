...
A potential additional contribution from the substratum mode count is constrained by the boundary architecture established in §3.1 and §5: boundary modes are catalogued by spatial location alone (one degree of freedom per cell of area $\epsilon^2$), with no inherent frequency assignment from the spatial counting alone. The dynamical dispersion $\omega(\mathbf{k})$ on this 2D spatial lattice reduces to $\omega = c|\mathbf{k}|$ at sub-cutoff wavenumbers $|\mathbf{k}| \ll 1/\epsilon$ — the requirement for the trace-out's emergent QFT to reproduce 2D continuum field theory at long wavelengths (§6.1) — and the standard density of states $dN/d\omega \propto \omega$ follows. At the cosmological horizon frequency $\omega \sim H \ll c/\epsilon$, mode contributions are suppressed by the spatial-vs-temporal scale ratio $(H\epsilon)^2 \sim 10^{-122}$, well below the emergent-QFT magnitude. This suppression is robust against UV-cutoff dispersion details (van Hove singularities at the band edge change $\mathcal{O}(1)$ factors but not the scale ratio at $\omega \sim H$), so the framework's prediction $|\nu| \approx |\nu_{\rm QFT}|$ is structural rather than dispersion-specific. A measurement $|\nu| \sim 10^{-4}$ would require a *structurally different* mode architecture — modes evenly distributed in $\log \omega$ from $H$ to the cutoff, rather than dynamically derived from spatial-lattice dispersion — and would falsify the spatial-lattice mode-counting architecture, not the dispersion choice within it. The framework's prediction therefore reduces to $|\nu| \approx |\nu_{\rm QFT}|$ — observationally indistinguishable from zero at all foreseeable precision, including DESI Year 5.
