...
**Dimensional-estimate attempt (retracted as quantitative prediction).** Earlier versions (1.6.5–1.8.x) presented the following as a best-effort qualitative prediction. The natural framework-internal small parameters available for a correction coefficient are $\Omega_B \approx 0.05$, $1/\mathcal{N}_0 = 1/\ln(c/\epsilon H_0) \approx 1/140$, and combinations thereof. A dimensional argument — that $\nu$ parameterizes a correction involving both baryonic back-reaction on the boundary entropy ($\Omega_B$) and spectral dilution over the available frequency decades ($1/\mathcal{N}_0$) — gives

$$|\nu| \sim \Omega_B / \mathcal{N}_0 \sim 3 \times 10^{-4}$$

with a sign argument from $\Omega_B(H)$ decreasing over cosmic time pointing to $\nu < 0$. On audit (see Retraction note below for the second retraction), this dimensional argument inherits the same un-derived spectral-uniformity assumption that motivated the v1.6.4 retraction: the $1/\mathcal{N}_0$ "spectral dilution" factor imports the bulk emergent-QFT decade structure (modes from $H$ to $1/\epsilon$ with $\mathcal{N}_0$ logarithmic decades) and applies it as a suppression on boundary modes (which §3.1 and §5 count by spatial location, one mode per $\epsilon^2$ cell, with no inherent frequency assignment). Direct mode-counting under the natural 2D field-theoretic distribution $dN/d\omega \propto \omega$ on the boundary gives modes-per-decade concentrated at the UV cutoff, with the fraction near $H$ scaling as $(H\epsilon)^2 \sim 10^{-122}$ — vastly below the emergent-QFT contribution. The sign argument relied on equipartition with the "uniform fraction → vacuum" identification, which gives $\nu \approx 0.84$ and is independently ruled out at $\gtrsim 6000\sigma$ by Bertini.

The specific magnitude $|\nu| \sim 3 \times 10^{-4}$ is therefore retracted as a quantitative prediction. The dimensional argument's *content* survives only as a possibility *if* additional substratum structure produces a log-uniform mode-per-decade distribution on the cosmological horizon — structure not currently derivable from §3-§5.
...
The dimensional argument in the preceding paragraph ($|\nu| \sim \Omega_B/\mathcal{N}_0 \sim 3 \times 10^{-4}$, retracted on audit) was offered through versions 1.6.5–1.8.x as suggesting a substantially larger magnitude matching Bertini's central value. Audit (session 3 of the 2026-04-25 derivation arc) established that the $1/\mathcal{N}_0$ factor smuggles in a log-uniform mode-per-decade boundary-mode distribution that is not derivable from §3-§5. Without that additional substratum structure, the framework's clean prediction is dominated by the emergent-QFT contribution above. Whether such substratum structure exists — i.e., whether the cosmological horizon's boundary modes have a log-uniform spectral distribution as opposed to the natural 2D field-theoretic distribution that would concentrate them at the UV cutoff — is the open structural question whose resolution would re-open the dimensional argument. DESI Year 5 precision ($\sim 1\%$ on $\nu$) will settle the empirical side regardless: a measurement $|\nu| \gtrsim 10^{-4}$ at high significance would require the substratum structure; a null at the same precision would confirm the framework's clean prediction.

*Scope of the prediction.* The framework's rigorous claim on $\nu$ is its *functional form* (Type II RVM, derived from boundary-entropy architecture §7.1–7.2) and its emergent-QFT magnitude $|\nu_{\rm QFT}| \sim 10^{-32}$, which is observationally indistinguishable from zero. The specific magnitude $|\nu| \sim 3 \times 10^{-4}$ from the framework-internal dimensional argument is retracted (above; second Retraction note below). The framework's clean prediction is therefore $|\nu| \approx |\nu_{\rm QFT}|$, observationally indistinguishable from zero at all foreseeable precision, including DESI Year 5. Bertini's $|\nu| \sim 10^{-4}$ measurement at $2\sigma$ from zero is consistent with this clean prediction (because the measurement is itself consistent with zero at $2\sigma$); a future high-confidence detection of $|\nu|$ at the $10^{-4}$ level would require additional substratum machinery not currently in §3-§5.
...
with the $\Lambda$CDM limit at the $2\sigma$ boundary of the posterior. This is consistent at $2\sigma$ with the framework's emergent-QFT prediction ($\nu \approx 0$). The dimensional-argument estimate $|\nu| \sim 3 \times 10^{-4}$ is retracted (see Retraction notes); the framework's clean prediction is the emergent-QFT value. An earlier pre-DESI canonical RVM fit by Gómez-Valent and Solà [16] returned $\nu = (1.34 \pm 0.38) \times 10^{-3}$ with $3.5\sigma$ preference over $\Lambda$CDM — positive sign and an order larger in magnitude; the migration from this value to Bertini's as the data improved from pre-DESI to DESI DR2 is itself informative. DESI Year 5 ($\sim 1\%$ precision on $\nu$) provides the decisive test: a null at that precision confirms the framework's clean prediction; a high-confidence detection of $|\nu|$ at the $10^{-4}$ level would require additional substratum structure (specifically, log-uniform mode-per-decade boundary-mode distribution) not currently derivable from §3-§5.
...
**Second retraction (2026-04-25).** Versions 1.6.5–1.8.x replaced the v1.6.4 ansatz with a revised dimensional argument $|\nu| \sim \Omega_B/\mathcal{N}_0 \sim 3 \times 10^{-4}$, switching the prefactor from $\Omega_\Lambda$ to $\Omega_B$ (the latter framework-derived via §7.3's Clausius identity $S_{\rm struct}/S_{\rm total} = \Omega_B$) and noting that the sign argument from $\Omega_B(H)$ decreasing gives $\nu < 0$, consistent with Bertini's central value. Direct mode-counting audit established that the new dimensional argument inherits the same un-derived $1/\mathcal{N}_0$ "spectral dilution" structure as the v1.6.4 ansatz: §3.1 and §5 count boundary modes spatially (one mode per $\epsilon^2$ cell, with no inherent frequency assignment), while $\mathcal{N}_0$ is the bulk emergent-QFT decade count, not a property of the boundary. Five candidate spectral identifications were tabulated: equipartition ($|\nu| \sim 0.84$, ruled out at $\gtrsim 6000\sigma$), all-modes-at-$H$ ($|\nu| \sim \Omega_B = 0.05$, ruled out by ΛCDM limit), linear $dN/d\omega = $ const ($|\nu| \sim 10^{-62}$), 2D phonon $dN/d\omega \propto \omega$ ($|\nu| \sim 10^{-122}$), and log-uniform $dN/d\omega \propto 1/\omega$ ($|\nu| \sim 3 \times 10^{-4}$). Only the log-uniform spectrum reproduces the dimensional argument's magnitude, but this density is non-physical (diverges at $\omega \to 0$ unless cut off; doesn't match natural field-theoretic distributions). The natural 2D distribution gives $|\nu|$ many orders of magnitude below the emergent-QFT contribution. The specific magnitude $|\nu| \sim 3 \times 10^{-4}$ is therefore retracted on the same grounds as the v1.6.4 retraction. The structural prediction (Type II RVM functional form) and the emergent-QFT magnitude $|\nu_{\rm QFT}| \sim 10^{-32}$ are retained as the framework's clean prediction. The dimensional argument's physical content — that baryonic back-reaction at $\Omega_B$ scale and spectral structure together set $\nu$ — survives as a possibility *if* additional substratum structure produces log-uniform boundary-mode distribution; that structure is not currently derivable from §3-§5 and remains an open structural question.
...
The Type II running-vacuum form for dark energy is the gravitational sector's most distinctive *structural* prediction: the functional form is forced by boundary-entropy architecture (§7.1), the Planck-scale UV cutoff is forced by the gap equation (§3.2, §4), and matter conservation plus Bianchi consistency fixes the coupled running of $G(H)$ and $\rho_{\rm vac}(H)$. The *magnitude* of $\nu$ is given at theorem level by the self-consistent emergent-QFT calculation: $|\nu_{\rm QFT}| \sim (M_{\rm SM}/M_{\rm Pl})^2 \sim 10^{-32}$, indistinguishable from zero. Earlier framework-internal dimensional arguments ($\Omega_\Lambda/(2\mathcal{N}_0) \sim 2.5 \times 10^{-3}$ in v1.6.4, $\Omega_B/\mathcal{N}_0 \sim 3 \times 10^{-4}$ in v1.6.5–1.8.x) suggested substratum-level contributions at observable magnitude; both have been retracted (§7.1 Retraction notes) on the same audit finding — the $1/\mathcal{N}_0$ "spectral dilution" factor smuggles in a log-uniform mode-per-decade boundary-mode distribution that is not derivable from §3-§5's spatial mode counting (one mode per $\epsilon^2$ cell, with no inherent frequency assignment). The natural 2D field-theoretic distribution gives modes-per-decade concentrated at the UV cutoff, contributing $|\nu|_{\rm substratum}$ many orders of magnitude below $|\nu_{\rm QFT}|$. The framework's clean prediction is therefore $|\nu| \approx |\nu_{\rm QFT}|$, observationally indistinguishable from zero. The Bertini et al. (2025) measurement $\nu = -(2.5 \pm 1.3) \times 10^{-4}$ is consistent with this prediction at $2\sigma$ (where Bertini is itself consistent with zero). DESI Year 5 ($\sim 1\%$ precision on $\nu$) provides the decisive empirical test: a null confirms the clean prediction; a high-confidence detection at the $10^{-4}$ level would require additional substratum structure (log-uniform mode-per-decade distribution at the cosmological horizon) not currently derivable from §3-§5, which would re-open the dimensional argument as a structural derivation. The functional form (Type II RVM) is structurally protected and is the framework's pre-registered prediction.
...
**Falsifiability.** The *theorem* would be falsified by a failure of both the stochastic-quantum correspondence and the Stinespring construction for the class of processes generated here — a possibility excluded by the independent proofs in [Main, §3.1] and Appendix A. The *cosmological application* (Type II RVM functional form as a structural prediction) would be falsified by definitive observational demonstration that dark energy deviates from the $\nu$-parameterized Type II form — e.g., a high-confidence detection of strong $w(z)$ variation incompatible with $w = -1 + \mathcal{O}(\nu)$. The clean magnitude prediction ($|\nu| \approx |\nu_{\rm QFT}| \sim 10^{-32}$, indistinguishable from zero) admits a stratified falsification structure: a measurement $|\nu| \gtrsim 10^{-4}$ at high significance would require log-uniform boundary-mode spectral structure beyond §3-§5 (recoverable only if such structure is derived); a measurement $|\nu| > 10^{-3}$ at high significance would also exceed the retracted dimensional-argument range and would constitute a structural challenge to the magnitude bracket without falsifying the Type II functional form itself. The *dark-sector concordance* (§7.2) would be falsified by detection of dark matter particles accounting for the full structured dark sector, or by evidence that the dark sector's gravitational effects are unrelated to the cosmological boundary geometry.
