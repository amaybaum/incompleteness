...
**Note on v1.10.0 (audit-revised edition).** This revision applies findings from the framework's internal derivation audit. The substantive changes in [GR]:

- **§7.3 Step 2 reclassification.** The MOND acceleration scale $a_0 = cH/6$ and the dependent predictions (baryonic Tully-Fisher, crossover radius $r_M$ in §7.4) are reclassified from unconditional structural to *layered conditional* (**L**) pending an explicit derivation of the $1/6$ geometric factor. The structural form (entropy displacement + Jacobson conversion + spherical redistribution geometry) walks end-to-end; the specific $1/6$ factor is presented in compressed form, with explicit derivation deferred to Direction 12 in companion research-direction documentation (~1 session, ~80% probability of clean closure to **S**). The empirical predictions and their matches are unchanged.

- **§7.2 three-d=3-selectors remark.** A new paragraph following the general-$d$ formula $\rho_s/\rho_{\rm crit} = 2/(d-1)$ explicitly notes the three independent $d=3$ selectors in the framework — substrate coupling-degree minimization, Bravais-lattice uniqueness for the SM gauge group, and the present boundary-entropy filter — and observes that they derive $d=3$ from very different physical considerations (substrate dynamics, crystallographic representation theory, cosmological boundary thermodynamics).

- **§7.3 minor refinements.** The Bullet Cluster paragraph adds a brief acknowledgment that the relaxation timescale identification $\tau_{\rm relax} \sim H^{-1}$ is itself a Tier 2 derived quantity. The RAR scatter paragraph distinguishes *intrinsic* (zero) from *observational* (set by measurement uncertainty) scatter explicitly.

The §3 Tier 1 results (ℏ derivation, BH 1/4 coefficient, CC dissolution) are unchanged; the §7.1 Type II RVM dark energy form and the §7.2 dark-sector corollary are unchanged. Appendix A theorems are unchanged. The framework-wide audit identified no fatal gaps across all five primary documents.
...
This boundary-entropy filter is one of three independent $d=3$ selectors in the framework, each derived from very different physical considerations: (i) substratum coupling-degree minimization (Theorem 6 in [SM §4.5], $K = 2d$ gives $K=6$, the unique minimizer of internal coupling degree on a bounded-degree graph with polynomial growth exponent $d$); (ii) Bravais-lattice uniqueness for the SM gauge group (Theorem 7b in [SM §4.6], the simple cubic lattice in $d=3$ is the unique 3-dimensional Bravais lattice producing $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ AND the observed Cabibbo angle); (iii) the present boundary-entropy filter, $\rho_s/\rho_{\rm crit} = 2/(d-1) = 1$ only at $d=3$. The three derive $d=3$ from substrate dynamics, crystallographic representation theory, and cosmological boundary thermodynamics — three very different physical considerations producing the same dimensional commitment.
...
**Step 2: The critical acceleration (derived, layered conditional).** The displaced entropy redistributes over the volume between the matter and the horizon. The Jacobson mechanism converts the entropy gradient into additional radial acceleration. The crossover between Newtonian gravity and the entropic effect, accounting for the volume-to-surface ratio of spherical redistribution in de Sitter ($1/6$), gives:

$$a_0 = \frac{cH}{6} \approx 1.2 \times 10^{-10} \; \text{m/s}^2$$

This is the MOND critical acceleration. (Milgrom's empirical value: $a_0^{\text{obs}} \approx 1.2 \times 10^{-10}$ m/s².)

**Audit-revised classification.** Following the framework's internal derivation audit, $a_0 = cH/6$ and the dependent predictions (baryonic Tully-Fisher, crossover radius $r_M$) are reclassified from unconditional structural to *layered conditional* (**L**) pending an explicit derivation of the $1/6$ geometric factor in the Step 2 chain above. The structural form (entropy displacement from Step 1 + Jacobson conversion + spherical redistribution geometry) walks end-to-end; the specific numerical factor $1/6$ — emerging from the volume-to-surface ratio $V/A = L/3$ for spherical redistribution in de Sitter combined with a halving from the dark-gravity-potential derivation — is presented in compressed form here. An explicit derivation walking through each step is the natural follow-up (Direction 12 in research-direction documentation; ~1 session, ~80% probability of clean closure restoring the **S** classification). The empirical match (Milgrom's $a_0 \approx 1.2 \times 10^{-10}$ m/s² to better than 0.5%) provides strong empirical support pending closure of Direction 12.
...
| MOND acceleration scale (**L**)ᵃ | $a_0 = cH/6 \approx 1.2 \times 10^{-10}$ m/s² | $a_0^{\text{obs}} \approx 1.2 \times 10^{-10}$ m/s² | exact (pending D12) |
| Baryonic Tully-Fisher (**L**)ᵃ | $v^4 = G M_B \cdot cH/6$ | SPARC data | within $0.13$ dex RAR scatter |
| Rotation curve crossover (**L**)ᵃ | $r_M = \sqrt{6 G M_B/(cH)}$ | Milky Way: $\sim 17$ kpc | Jiao et al. 2023, $3\sigma$ |
| High-$z$ rotation curves | More baryon-dominated with $z$ | Genzel et al. $z = 0.9$–$2.4$ | consistent at $> 3\sigma$ |

**Footnote ᵃ (L classification).** Layered conditional Tier 2 prediction. The structural form follows from §7.3 Step 1 (entropy displacement, derived from Clausius relation + Generalized Second Law Theorem A.6) + Step 2 (Jacobson conversion of entropy gradient to dark-gravity acceleration) + the geometric factor $1/6$ from spherical redistribution in de Sitter. The $1/6$ factor is presented in compressed form in §7.3 Step 2; an explicit derivation walking through $V/A = L/3$ for spherical redistribution combined with the dark-gravity-potential halving is the natural follow-up (Direction 12 in research-direction documentation; ~1 session, ~80% probability of clean closure to **S**). The empirical match (Milgrom's $a_0 \approx 1.2 \times 10^{-10}$ m/s² to better than 0.5%; Milky Way Keplerian decline at ~17 kpc per Jiao et al. 2023 at 3σ) provides strong empirical support pending closure.
