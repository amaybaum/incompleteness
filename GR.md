...
**Type II identification.** The derivation above places the framework in the *Type II* class of RVM implementations [11]. In the Type II structure, matter is locally conserved and the effective gravitational coupling $G(H)$ runs mildly with cosmic expansion; the Bianchi identity is preserved by the combined running of $\rho_{\rm vac}(H)$ and $G(H)$. This is the structure implied by §7.1's horizon-mode derivation and §7.2's dark-sector corollary — where matter-boundary interaction is purely gravitational via the metric, not via a direct source term exchange with cold dark matter. The sign $\nu_{\rm OI} > 0$ corresponds in Type II to $G$ *increasing* with cosmic expansion (per Bianchi consistency). By contrast, *Type I* RVM implementations assume $G = G_N$ fixed and introduce a direct vacuum-DM exchange term $Q = 3\nu H \rho_{\rm dm}$; these are physically distinct models sharing the same functional form for $\rho_{\rm vac}(H)$.

DESI data [12, 13] report evolving dark energy at $2.8\sigma$–$4.2\sigma$. The most recent multi-variant analysis [14] of DESI DR2 + Planck PR4 + supernovae explicitly tests several RVM variants, finding the "flipped RVM" and "RVM with threshold" point to significant evidence of dynamical dark energy at a level comparable to the more flexible $w_0w_a$CDM parameterization — DESI favours the specific running-vacuum functional form this framework predicts. The 2026 analysis in [14], however, runs only Type I implementations (where matter exchanges energy with vacuum via DM); Type I fits return $|\nu| \sim 10^{-3}$ with variant-dependent sign, broadly in the expected magnitude range but not a direct test of OI's Type II prediction. Pre-DR2 Type II RVM fits [11] give $|\nu|$ of order $10^{-3}$ consistent with $\nu_{\rm OI} = 2.45 \times 10^{-3}$ in sign and order of magnitude. A direct Type II fit to DESI DR2 would be the decisive comparison and does not yet exist in the literature. DESI Year 5 ($\sim 1\%$ precision on $\nu$) provides the decisive long-term test.
