...
The dimensional argument in the preceding paragraph ($|\nu| \sim \Omega_B/\mathcal{N}_0 \sim 3 \times 10^{-4}$) suggests a substantially larger magnitude, matching Bertini's central value. If that estimate is correct, it reflects a substratum-level contribution to the $H^2$ running of the emergent Friedmann equation beyond what the emergent QFT calculates — the substratum being a classical discrete structure, its contribution is not captured by one-loop QFT machinery and requires genuinely new formalism to derive rigorously. Whether such a contribution exists at $\sim 10^{-4}$ magnitude, or whether Bertini's central value reflects a statistical fluctuation around the $\nu \approx 0$ emergent-QFT prediction, is an open question that DESI Year 5 precision ($\sim 1\%$ on $\nu$) will decisively settle.

*Scope of the prediction.* The framework's rigorous claim on $\nu$ is its *functional form* (Type II RVM, derived from boundary-entropy architecture §7.1–7.2). Its *magnitude* is not currently derivable from the OI architecture at theorem level: the emergent QFT sector predicts $|\nu| \sim 10^{-32}$, the framework-internal dimensional argument suggests $|\nu| \sim 3\times 10^{-4}$, and a substratum-level contribution beyond emergent QFT would require non-QFT machinery not yet developed. The ΛCDM limit ($\nu = 0$) is consistent with all of this, as is Bertini's $|\nu| \sim 10^{-4}$ measurement at $2\sigma$. Decisive discrimination awaits DESI Year 5.
...
with the $\Lambda$CDM limit at the $2\sigma$ boundary of the posterior. This is consistent at $2\sigma$ with both the framework's emergent-QFT prediction ($\nu \approx 0$) and its framework-internal dimensional estimate ($|\nu| \sim 3 \times 10^{-4}$). An earlier pre-DESI canonical RVM fit by Gómez-Valent and Solà [16] returned $\nu = (1.34 \pm 0.38) \times 10^{-3}$ with $3.5\sigma$ preference over $\Lambda$CDM — positive sign and an order larger in magnitude; the migration from this value to Bertini's as the data improved from pre-DESI to DESI DR2 is itself informative. DESI Year 5 ($\sim 1\%$ precision on $\nu$) provides the decisive test. The framework's rigorous prediction is the Type II functional form; definitive discrimination between the emergent-QFT value ($\nu \approx 0$), the dimensional-estimate magnitude ($|\nu| \sim 10^{-4}$), or a substantially larger value that would falsify the framework awaits that precision.
...
| Dark energy form (structural) | Type II RVM (functional form derived; magnitude not derived at theorem level) | Bertini 2025 $\nu = -(2.5\pm 1.3) \times 10^{-4}$ | consistent at $2\sigma$ (with ΛCDM limit) |
...
The Type II running-vacuum form for dark energy is the gravitational sector's most distinctive *structural* prediction: the functional form is forced by boundary-entropy architecture (§7.1), the Planck-scale UV cutoff is forced by the gap equation (§3.2, §4), and matter conservation plus Bianchi consistency fixes the coupled running of $G(H)$ and $\rho_{\rm vac}(H)$. The *magnitude* of $\nu$ is not currently derivable from OI architecture at theorem level: the self-consistent emergent-QFT calculation gives $|\nu| \sim (M_{\rm SM}/M_{\rm Pl})^2 \sim 10^{-32}$ (indistinguishable from zero), while a framework-internal dimensional argument ($\Omega_B/\mathcal{N}_0$) suggests $|\nu| \sim 3 \times 10^{-4}$ — the latter would require a substratum-level contribution to the $H^2$ running beyond what the one-loop QFT machinery of curved-spacetime field theory captures, because the substratum is a classical discrete structure rather than a quantum field. Both estimates are consistent with the Bertini et al. (2025) measurement $\nu = -(2.5 \pm 1.3) \times 10^{-4}$ (consistent with zero at $2\sigma$). The sign is likewise not derived at this level. DESI Year 5 ($\sim 1\%$ precision on $\nu$) will decisively distinguish $|\nu| \ll 10^{-4}$ (favoring the emergent-QFT prediction) from $|\nu| \sim 10^{-4}$ (favoring the dimensional estimate, requiring substratum contribution). A first-principles derivation of the magnitude from OI architecture alone remains an open technical item, and is now understood to require non-QFT machinery for the classical discrete substratum not yet developed.
