...
**Best-effort qualitative prediction.** The natural framework-internal small parameters available for a correction coefficient are $\Omega_B \approx 0.05$, $1/\mathcal{N}_0 = 1/\ln(c/\epsilon H_0) \approx 1/140$, and combinations thereof. A dimensional argument — that $\nu$ parameterizes a correction involving both baryonic back-reaction on the boundary entropy ($\Omega_B$) and spectral dilution over the available frequency decades ($1/\mathcal{N}_0$) — gives

$$|\nu| \sim \Omega_B / \mathcal{N}_0 \sim 3 \times 10^{-4}$$

The *sign* follows from the direction of the displacement. As the universe expands, $\Omega_B(H) = \Omega_B^{(0)}(H_0/H)^2 [(H/H_0)^2 - \Omega_\Lambda]/\Omega_m$ decreases from $\sim 0.16$ in matter domination to $\sim 0.05$ today, so the uniform (cosmological-constant-like) fraction of boundary entropy grows over cosmic time. Past dark energy is therefore smaller than present dark energy, which translates to $\nu < 0$ in the Friedmann coefficient. Combined qualitative prediction: $\nu \sim -3 \times 10^{-4}$, with factor-of-a-few uncertainty absorbed into the un-derived DE/DM split dynamics.

**Field-theoretic estimate.** An independent cross-check comes from the Shapiro-Solà β-function formula for the vacuum-energy running coefficient in QFT on curved spacetime [33]:

$$\nu = \frac{1}{48\pi^2}\sum_i a_i \frac{M_i^2}{M_{\rm Pl}^2}$$

where $M_i$ are heavy-particle masses, $a_i$ are spin-dependent coefficients of order unity, and the sum runs over quantized matter fields. The OI framework's gap equation (§3.2, §4) fixes the UV cutoff at $\epsilon = 2 l_p$, corresponding to a UV energy scale $M_{\rm UV} = \hbar c/\epsilon = M_{\rm Pl,full}/2 \approx 6 \times 10^{18}$ GeV — i.e.\ Planck-scale. For heavy fields at this scale, $(M_{\rm UV}/M_{\rm Pl,\,reduced})^2 \approx 6$ and the formula gives $|\nu| \sim 10^{-2} |a_{\rm tot}|$. Explicit computations for Standard-Model-extended field content [34] find the net $|a_{\rm tot}| \approx 0.01$–$0.1$ from partial cancellation between bosonic and fermionic contributions, yielding $|\nu| \sim 10^{-4}$–$10^{-3}$ — consistent with the dimensional estimate above at order-of-magnitude level and directly consistent with Bertini's measurement.

*Scope of the prediction.* The foregoing estimates are not a rigorous derivation of $\nu$ from OI architecture alone; they combine (a) OI's derived Type II RVM functional form and UV cutoff $\epsilon = 2 l_p$ with (b) standard QFT-in-curved-spacetime machinery that applies to any Type II RVM framework. OI's specific contributions are the *origin* of the Type II form (boundary-entropy architecture, §7.1–7.2) and the Planck-scale UV (gap equation, §3.2–4); the *magnitude* $|\nu| \sim 10^{-4}$ is a generic expectation for Planck-UV Type II RVM with SM field content, not an OI-only prediction. Exact $\Lambda = \text{const}$ ($\nu = 0$) is formally consistent with OI's three architectural commitments but would require fine-tuned exact cancellation of all β-function contributions across SM field content, which is not the generic expectation. Decisive observational discrimination between $\nu = 0$ and $|\nu| \sim 10^{-4}$ awaits DESI Year 5 precision.
...
| Dark energy coefficient (structural) | $|\nu| \sim 10^{-4}$ (Type II RVM) | Bertini 2025 fit to DESI DR2 + DES-Y5 + Planck | consistent at $< 1\sigma$ |
...
The prediction of Type II running-vacuum dark energy with $|\nu| \sim 10^{-4}$ is the gravitational sector's most distinctive output: the structural form is forced by boundary-entropy architecture (§7.1), and the magnitude is grounded on two independent routes — the framework-internal combination $\Omega_B/\mathcal{N}_0 \sim 3 \times 10^{-4}$ on a dimensional argument, and the Shapiro-Solà β-function formula applied at OI's Planck-scale UV cutoff. The prediction agrees with the Bertini et al. (2025) direct RG-corrected $\Lambda$CDM fit to DESI DR2 + DES-Y5 + Planck 2018 [15] at better than $1\sigma$ in order of magnitude. The sign is not uniquely determined by current architecture (depends on boson/fermion balance in the β-function sum); Bertini's measurement prefers negative but is consistent with zero at $2\sigma$. The falsification path is explicit: DESI Year 5 returning $|\nu| > 10^{-3}$ at high confidence would rule out the framework's Type II RVM prediction by exceeding the Shapiro-Solà upper bound for SM-content field theory at Planck UV. A full first-principles derivation of the magnitude from OI architecture alone — upgrading the dimensional argument to a theorem — remains an open technical item.
...
**Falsifiability.** The *theorem* would be falsified by a failure of both the stochastic-quantum correspondence and the Stinespring construction for the class of processes generated here — a possibility excluded by the independent proofs in [Main, §3.1] and Appendix A. The *cosmological application* would be falsified by definitive absence of dark energy evolution in Type II RVM form, or by a high-confidence measurement of $\nu > +1.5 \times 10^{-4}$ (wrong sign and outside the framework's dimensional estimate) or $|\nu| > 10^{-3}$ (an order of magnitude larger than the qualitative prediction). The *dark-sector concordance* (§7.2) would be falsified by detection of dark matter particles accounting for the full structured dark sector, or by evidence that the dark sector's gravitational effects are unrelated to the cosmological boundary geometry.
