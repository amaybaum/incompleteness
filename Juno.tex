...
We derive the parameter-free prediction $\sin^{2}\theta_{12}$ = 1/3 $-$ 1/($4\pi^{2}$) = 0.3080
from cubic-group flavor structure, with no parameter fit to the value of
$\sin^{2}\theta_{12}$. The first JUNO measurement of reactor neutrino oscillations
reports $\sin^{2}\theta_{12}$ = 0.3092 $\pm$ 0.0087, with the post-JUNO global fit
tightening this to $\sin^{2}\theta_{12}$ = 0.3085 $\pm$ 0.0073, matching the prediction at
0.$07\sigma$. The same construction gives $\sin^{2}\theta_{13}$ = 4/($18\pi^{2}$) = 0.02252 and
$\sin^{2}\theta_{23}$ = 1/2 + 1/($2\pi^{2}$) = 0.5507, each consistent with NuFIT 6.0
{[}14{]} within 1.$1\sigma$, and forces the exact sum rule 2$\sin^{2}\theta_{12}$ + $\sin^{2}\theta_{23}$ =
7/6 --- a clean discriminator from the TM1 and TM2 patterns currently
analyzed against JUNO. The angle predictions follow from a discrete $\mu$-$\tau$
...
Cabibbo angle $\lambda$ = 1/($\pi\sqrt{2}$) = 0.22508, set by the cubic Brillouin-zone
geometry, sets the perturbation scale and matches the observed value to
0.04\%. JUNO's design-lifetime precision of $\pm$0.0014 will test the
...
within 1.$1\sigma$ of the NuFIT 6.0 best fits {[}14{]} and forces the exact sum
rule
...
value 0.22500 $\pm$ 0.00067 {[}13{]} to 0.04\%. Second, the deviations from
tribimaximal (TBM) mixing at O($\lambda^{2}$) are controlled by a discrete $\mu$-$\tau$
...
lattice consistent with the SU(3) $\times$ SU(2) $\times$ U(1) gauge structure and the
observed Cabibbo angle (Theorem 7b). The same cubic structure that
produces the 1/($4\pi^{2}$) correction here also produces a number of related
...
\bottomrule\noalign{}
\endlastfoot
Cabibbo angle $\lambda$ & 1/($\pi\sqrt{2}$) = 0.22508 & 0.22500 $\pm$ 0.00067 {[}13{]} &
0.04\% \\
Koide angle $\theta_{0}$ (charged leptons) & $C_{2}/d^{2}$ = 2/9 & 0.222204 $\pm$ 0.000010
{[}13{]} & 0.02\% \\
Solar mixing $\sin^{2}\theta_{12}$ (this work) & 1/3 $-$ 1/($4\pi^{2}$) = 0.30798 & 0.3085 $\pm$
0.0073 {[}8{]} & 0.$07\sigma$ \\
PMNS sum rule & 2$\sin^{2}\theta_{12}$ + $\sin^{2}\theta_{23}$ = 7/6 & 1.178 $\pm$ 0.020 & 0.$6\sigma$ \\
\end{longtable}
...
symmetries: the same parameter $\lambda^{2}$ = 1/($2\pi^{2}$) and the same projection
factor $A^{2}$ = 2/3 enter multiple observables at sub-percent precision,
...
six-element generator set \{$\pm\hat{e}_{1}$, $\pm\hat{e}_{2}$, $\pm\hat{e}_{3}$\} --- six minimal-length
...
matching the observed value $\lambda$\_obs = 0.22500 $\pm$ 0.00067 {[}13{]} to
0.04\%. Squaring,
...
agreeing with the PDG value A = 0.826 $\pm$ 0.012 at 0.$23\sigma$. The combination
...
to the (1,3) element of V\_$\ell^{\dagger}$ U\_TBM. The observation that both $\Delta_{12}\equiv\sin^{2}\theta_{12}-$ 1/3 and $\Delta_{23}\equiv\sin^{2}\theta_{23}-$ 1/2 are O($\lambda^{2}$) (not O($\lambda$)) further
...
\textbf{V\_$\ell$ = I + $\lambda A_{1}$ + $\lambda^{2}$($A_{2}$ + $1/2A_{1}^{2}$) + O($\lambda^{3}$)}
...
Direct computation (taking V\_$\ell$\^{}T = exp(-($\lambda A_{1}$+$\lambda^{2}A_{2}$)) acting on the
columns of U\_TBM, using the explicit form $A_{1}^{2}$ = diag(-4/9, -2/9, -2/9)
+ (2/9)($e_{2}\otimes e_{3}$ + $e_{3}\otimes e_{2}$) for $A_{1}$ = ($\sqrt{2}/3$)($E_{12}- E_{13}$)) gives the deviations
from TBM:
...
\textbf{(iii) $\Delta_{23}$ depends only on $b_{23}$, not on the U-even combination.}
The explicit calculation gives $|U_\mu 3|^{2}$ = $1/2$ + $\lambda^{2}$($b_{23}-$
2/9) before the $\cos^{2}\theta_{13}$ dressing; after dressing, $\sin^{2}\theta_{23}$ = $1/2$ + $\lambda^{2}b_{23}$
exactly. The U-even combination ($b_{12}$+$b_{13}$) does not contribute to $\Delta_{23}$ at
O($\lambda^{2}$): the contributions that would enter through the (V\_$\ell$\^{}T)$_{2}\nu$
matrix elements all arise at O($\lambda^{3}$) or higher in the calculation of
$|U_\mu 3|^{2}$.
...
explicit results $\Delta_{12}$ = $-$(2/3)($b_{12}$+$b_{13}$)$\lambda^{2}$ and $\Delta_{23}$ = $b_{23}\lambda^{2}$ from §3.3:

\textbf{$2\Delta_{12}$ + $\Delta_{23}$ = ($b_{23}-$ (4/3)($b_{12}$+$b_{13}$)) $\lambda^{2}$}

Hence Cond 2 is \emph{equivalent} to the statement that the TBM sum rule
2$\sin^{2}\theta_{12}$ + $\sin^{2}\theta_{23}$ = 7/6 --- which holds at zeroth order from cubic
geometry, since 2(1/d) + 1/(d$-$1) = (3d$-$2)/(d(d$-$1)) = 7/6 for d = 3 ---
continues to hold at O($\lambda^{2}$). The Wilson-coefficient relation and the
angle-level sum rule are the same content under different
normalizations.

\textbf{Structural protection of the sum rule by $A_{1}$ alone (Layer 1).}
With $A_{2}$ = 0, direct calculation gives 2$|U_e2|^{2}$ +
$|U_\mu 3|^{2}$ = 7/6 + $\lambda^{2}${[}$-$14/27{]} + ($\cos^{2}\theta_{13}$ corrections).
Dividing by $\cos^{2}\theta_{13}$ = 1 $-$ (4/9)$\lambda^{2}$ (which itself follows from $\sin^{2}\theta_{13}$ =
$A^{4}\lambda^{2}$, a Layer 1 quantity) precisely cancels the $-$14/27 offset:

**(2$\sin^{2}\theta_{12}$ + $\sin^{2}\theta_{23}$)\textbar\_\{$A_{2}$=0\} = 7/6 + O($\lambda^{4}$)**

--- that is, the sum rule is preserved by $A_{1}$ alone at all orders up to
$\lambda^{2}$. This cancellation is non-trivial and specific to $A_{1}'s$ Layer 1-forced
form: the magnitude ($\sqrt{2}/3$) is fixed by $\sin\theta_{13}$ = $A^{2}\lambda$ (§3.2), and the
U-odd character is fixed by the requirement $\Delta_{12}$, $\Delta_{23}$ = O($\lambda^{2}$) rather than
O($\lambda$). With those two constraints, the cancellation between the $-$14/27
contribution from $A_{1}^{2}$-induced shifts and the +14/27 contribution from
the $\cos^{2}\theta_{13}$ dressing is automatic. The TBM sum rule is therefore
\emph{structurally protected} against $A_{1}'s$ perturbation by cubic
geometry alone --- no substrate input required for this component.

Cond 2 reduces to the question of whether $A_{2}$ preserves the sum rule it
was protected for under $A_{1}$, or breaks it. The answer depends on the
...
(Sec. 2.2). The identity $A^{2}$ = (d$-$1)/d = $C_{2}(T_{1})/d$ for d = 3 makes this a
...
counting on $T_{1}\otimes T_{1}\to T_{2}$ in the supporting framework {[}18{]} shows that
this $T_{2}$ direction is determined by the substrate's specific Yukawa
...
produced above changes the layer status of the prediction. The $A_{1}$
component is Layer 1: the sum rule is structurally protected against
$A_{1}'s$ perturbation by cubic geometry, with no substrate input. The $A_{2}$
component reduces to a sharply specified open question --- \emph{why
does $b_{23}/$($b_{12}$+$b_{13}$) take exactly the value 4/3 = $2A^{2}$?} --- which is Layer
2(a)-tractable rather than the prior Layer 2(b) ``arbitrary $T_{2}$ direction
...
direction in $T_{2}$-space; substrate calculations targeting this specific
ratio admit definite outcomes. The empirical match at 0.$07\sigma$ on $\sin^{2}\theta_{12}$
...
= $2A^{2}$ is consistent with this picture; whether it follows rigorously
...
$b_{23}$\textsuperscript{sub/($b_{12}$}sub+$b_{13}$\^{}sub) at leading order in the
hierarchy m\_e/m\_$\tau$ takes the value 4/3 = 4(d$-$1)/(2d), and the sharp
...
been completed. Empirical agreement at 0.$07\sigma$ on $\sin^{2}\theta_{12}$ is, in this
...
of neutrino oscillation data until Daya Bay measured a nonzero $\theta_{13}$
...
family that accommodates the observed $\theta_{13}$ but predicts different values
for the other two angles. Most existing TBM-modification proposals start
from a discrete \emph{flavor symmetry} --- typically $A_{4}$, $S_{4}$, or a $Z_{2}$
...
excluded at \textgreater3.$5\sigma$ while TM1 remains viable; Zhang {[}10{]}
...
(Comparing against post-JUNO global fit 0.3085 $\pm$ 0.0073. TM1 uses
...
$\pm$0.998, close to maximal. Here, $\delta$\_CP is identified with the only
...
versus the observed 2(0.3085) + 0.561 = 1.178 $\pm$ 0.020 (using the
...
0.02252. The NuFIT 6.0 best fit (normal ordering) is 0.02195 $\pm$ 0.00056,
...
$\sin^{2}\theta_{23}$ = 0.$561^{+0}\cdot ^{012}_{-0}$.$_{015}$, agreeing at 0.$74\sigma$. The prediction lies in
...
A $\pm$0.0014 ultimate JUNO error on $\sin^{2}\theta_{12}$ then implies a $\pm$0.0028 error on
...
$\sin^{2}\theta_{23}$ & 1/2 + 1/($2\pi^{2}$) = 0.5507 & 0.$561^{+0}\cdot ^{012}_{-0}$.$_{015}$ & 0.74 \\
\end{longtable}
...
ultimate-precision uncertainty of $\pm$0.0014 and roughly half of JUNO's
first-result uncertainty of $\pm$0.0087. The current 0.$07\sigma$ agreement
...
established in Sec. 3.4: an $A_{1}$ component closed at Layer 1 (the TBM sum
rule is structurally protected against the $A_{1}$ perturbation by the
$\cos^{2}\theta_{13}$ dressing alone, requiring no substrate input), and an $A_{2}$
component reducing to the sharp question why $b_{23}/$($b_{12}$+$b_{13}$) takes the
specific value 4/3 = $2A^{2}$ in the framework's bijection. The reactor angle
$\sin^{2}\theta_{13}$ is independent of Condition 2 (the $A_{2}$ coefficients cancel from
$|U_e3|^{2}$) and remains Layer 1 unconditional structural.
The angle predictions $\sin^{2}\theta_{12}$ and $\sin^{2}\theta_{23}$ are layered: their structural
form is forced by cubic-group representation theory and the Layer 1
closure of the $A_{1}$ component, while the specific 4/3 coefficient is a
Layer 2(a)-tractable open question --- sharply specified rather than a
generic Layer 2(b) ``alignment in $T_{2}$-space'' --- concerning a single
...
staggered Yukawa structure on $T_{1}$ BZ corners, both the leading and
subleading channels produce equal off-diagonal $Y^{(2)}$ entries by cubic
...
form lacks. The 0.$07\sigma$ match is therefore a test of the bijection's
specific $T_{2}$-channel alignment.
...
{[}18, §7.7{]}. This places the sum rule's 0.$6\sigma$ match in a broader
...
permutation freedom. The specific 4/3 = $2A^{2}$ coefficient required by
...
produces the $b_{23}/$($b_{12}$+$b_{13}$) = 4/3 ratio from the $C_{2}(T_{1})$ Casimir structure
would close Condition 2 to Layer 2(a). The 0.$07\sigma$ empirical match at
$\sin^{2}\theta_{12}$ and 0.$6\sigma$ at the sum rule are tests of this discretization scheme
...
lifetime, JUNO is projected to reach $\sin^{2}\theta_{12}$ = 0.3092 $\pm$ 0.0014, a 6$\times$
...
fit to $\sin^{2}\theta_{12}$. The same construction predicts $\sin^{2}\theta_{13}$ and $\sin^{2}\theta_{23}$
within 1.$1\sigma$ of the NuFIT 6.0 best fits {[}14{]}, and forces the exact
sum rule 2$\sin^{2}\theta_{12}$ + $\sin^{2}\theta_{23}$ = 7/6, which distinguishes the prediction
from the TM1, TM2, and TM3 column-preservation patterns. The reactor
angle $\sin^{2}\theta_{13}$ = 4/($18\pi^{2}$) is unconditional structural (Layer 1 in {[}18,
§8.3{]}), based on the representation-theoretic identity $A^{2}$ = (d$-$1)/d =
$C_{2}(T_{1})/d$ for vector reps. The solar and atmospheric angle predictions
are layered: their structural form is forced by cubic-group
representation theory and the Layer 1 closure of the $A_{1}$ component (Sec.
3.4), while the specific 4/3 coefficient in Condition 2 ($b_{23}$ =
(4/3)($b_{12}$+$b_{13}$)) is a Layer 2(a)-tractable open question --- sharply
specified as the substrate ratio
$b_{23}$\textsuperscript{sub/($b_{12}$}sub+$b_{13}$\^{}sub) at leading order in the
m\_e/m\_$\tau$ hierarchy. The empirical match at 0.$07\sigma$ on $\sin^{2}\theta_{12}$ and at 0.$6\sigma$
...
cubic-group framework. The Cabibbo scale $\lambda^{2}$ = 1/($2\pi^{2}$) entering the
