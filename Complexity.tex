...
the d = 3 lattice {[}2, §4.2{]}. The emergent QFT inherits Lorentz
invariance, microcausality, and a stable vacuum from the underlying
emergent field-theory construction {[}2, §3.1{]} --- these are the
conditions of the spin-statistics theorem (Streater--Wightman). The
theorem then gives the Pauli exclusion principle: each orbital holds at
most 2 electrons (spin up and spin down). The shell capacities follow:
...
generically, structurally permitting a mass hierarchy with light
fermions (electron, u and d quarks). The existence of \emph{some} mass
hierarchy is structural --- the staggered lattice's coupling matrix has
eigenvalues at different scales. The specific \emph{scale} of light
fermion masses relative to the QCD scale, and whether the lightest
fermions fall in the chemistry-viable range, is parametric (§3 handles
the viable range).
...
structure. (See §3 for the parametric question of which temperature
ranges actually have a liquid phase in our universe.)
...
The joint viable fraction, treating the parameters as independent for
illustrative purposes, is roughly:

\[f_{\text{viable}} \sim 0.6 \times 0.65 \times 0.5 \times 0.8 \approx 16\%\]

Genuine correlations between these parameters in the SM (e.g.,
\(\alpha_s\) and the strong CP scale, or \(\alpha\) and \(m_e/m_p\) via
QED corrections) could enlarge or shrink this fraction; a fully rigorous
joint estimate would require a coupled analysis. The independent
estimate is offered as an order-of-magnitude indicator: the viable
fraction is \(\mathcal{O}(10\%)\), not \(\mathcal{O}(10^{-50})\).

The ``log-fraction'' computation depends on a choice of ``available
range'' for each parameter --- the orders-of-magnitude window over which
the parameter could conceivably vary while keeping other framework
structure intact. The values used above (3--6 orders per parameter)
follow standard fine-tuning literature {[}6, 7, 8, 9, 10{]} but are not
derived from the framework. Different reasonable priors give similar
order-of-magnitude results (the viable fraction stays in the 5--30\%
range), but the precise number is not parameter-free.
...
processes and \(H = (1 + \alpha)/2\) for \(1/f^\alpha\) noise. A
striking empirical observation, consistent with the framework's
prediction of P-indivisibility wherever C1--C3 are satisfied, is that
single-entity systems show universal \(H \approx 0.79 \pm 0.03\)
(excluding myoglobin's deeper-hierarchy outlier). The derivation of
\(H(C3)\) from the characterization theorem's dependence on
hidden-sector dimension --- which would convert this empirical
observation into a parameter-free framework prediction --- is an open
research question.
