...
\textbf{The design principle.} In natural materials, \(\tau_S / \tau_B\) is fixed by the chemistry. In engineered metamaterials, it can be tuned. Candidate platforms:

\emph{Molecular junctions.} A single molecule bridging two metallic electrodes, with the molecule's conformational degrees of freedom as the hidden sector. By choosing molecules with slow conformational dynamics (large, floppy side chains: \(\tau_B \sim \mu\)s) and fast electronic transport (\(\tau_S \sim\) fs), ratios of \(\tau_S / \tau_B \sim 10^{-9}\) are achievable. The prediction: the junction's I-V curve shows history-dependent conductance --- the current depends on the recent voltage history, not just the instantaneous voltage.

\emph{Cold-atom lattices.} Ultracold atoms in optical lattices, with one species forming the ``system'' and another forming the ``bath.'' The bath species' tunneling rate (tuned by lattice depth) sets \(\tau_B\); the system species' tunneling rate sets \(\tau_S\). The ratio is continuously tunable from \(\sim 1\) (Markovian) to \(\sim 10^{-3}\) (strongly P-indivisible). The prediction: at large \(\tau_B / \tau_S\), the system species' momentum distribution shows non-thermal features --- signatures of information backflow from the bath species.

\emph{Photonic crystals with embedded quantum dots.} Quantum dots in a photonic bandgap material, with the photonic crystal modes as the hidden sector. The photonic density of states can be engineered to have a gap (suppressing fast decay, increasing \(\tau_B\)) while maintaining coupling (C1). The prediction: the quantum dot's fluorescence shows non-exponential decay with partial revivals --- the photonic crystal stores and returns excitation energy on the engineered timescale.

\hypertarget{the-broader-principle}{%
\subsubsection{8.9 The broader principle}\label{the-broader-principle}}

The framework identifies a design space that standard quantum engineering does not recognize: the space of C1--C3 architectures. Current technology either (a) minimizes coupling to fight decoherence (standard qubits) or (b) accepts the natural coupling and works within its constraints (standard materials science). The framework opens a third option: (c) \emph{engineer} the coupling, bath timescale, and capacity to produce desired P-indivisible dynamics.
...
\subsubsection{8.10 Quantitative performance estimates}\label{quantitative-performance-estimates}}
...
\subsubsection{8.11 Existing experimental evidence}\label{existing-experimental-evidence}}
