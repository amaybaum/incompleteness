/*
 * k6_hmc_v2.c — K=6 HMC with dynamical staggered fermions
 *
 * Systematic build:
 *   Phase 1: Gauge-only HMC (verify integrator)
 *   Phase 2: Add fermion force (verify dH ~ O(dt²))
...
/* ---- Gauge action S_g = -β Σ Re Tr(plaq) ---- */
...
static void compute_gauge_force(M3 *frc3,M2 *frc2,double *frc1,double b3,double b2,double b1){
    for(int s=0;s<VOL;s++)for(int mu=0;mu<DIM;mu++){
        ...
        /* U(1): F = β₁ * Im(e^{iθ} * staple*) = β₁ * [sinθ * Re(staple) - cosθ * Im(staple)] */
        /* dS/dθ = β₁ * sin(θ) * staple_re - β₁ * cos(θ) * staple_im */
        /* Wait: S_U1 = -β₁ Σ cos(plaq). dS/dθ_{s,μ} = β₁ Σ sin(θ_{s,μ} + phases) */
        /* = β₁ [sin(θ)*staple_re_cos + cos(θ)*staple_re_sin] */
        /* Simpler: F = -dS/dθ = β₁*(cos(θ)*staple_im - sin(θ)*staple_re) */
        /* Actually for S = -β cos(θ+φ), dS/dθ = β sin(θ+φ) = β[sinθ cosφ + cosθ sinφ] */
        double th=U1[idx];
...
static void compute_force(M3 *frc3,M2 *frc2,double *frc1,double mass,double b3,double b2,double b1){
    /* Zero force arrays */
    memset(frc3,0,VOL*DIM*sizeof(M3));
    memset(frc2,0,VOL*DIM*sizeof(M2));
    memset(frc1,0,VOL*DIM*sizeof(double));
    /* Gauge force */
    compute_gauge_force(frc3,frc2,frc1,b3,b2,b1);
    /* Fermion force (chi must be current) */
    cg_solve(chi,phi,mass);
    compute_fermion_force(frc3,frc2,frc1,mass);
}

/* ---- HMC trajectory ---- */
static double do_hmc(double mass,double b3,double b2,double b1,int nstep,double tau,int *acc){
    double dt=tau/nstep;
    ...
    refresh_pseudofermion(mass);

    /* Initial H */
    double H0=kinetic_energy()+gauge_action(b3,b2,b1)+fermion_action(mass);
    ...
    compute_force(frc3,frc2,frc1,mass,b3,b2,b1);
    for(int i=0;i<VOL*DIM;i++){
    ...
    /* Final H */
    double H1=kinetic_energy()+gauge_action(b3,b2,b1)+fermion_action(mass);
    double dH=H1-H0;
...
/* ---- Measure condensate ---- */
static double measure_pbp(double mass){
    int N=VOL*NK;
...
int main(int argc,char **argv){
    ...
    fprintf(stderr,"\n  k6_hmc_v2: K=6 HMC with dynamical fermions\n");
    fprintf(stderr,"  L=%d mass=%.4f β=(%.1f,%.1f,%.1f) nstep=%d τ=%.2f\n\n",L,mass,b3,b2,b1,nstep,tau);
    ...
    for(int sw=0;sw<200;sw++){
        for(int s=0;s<VOL;s++)for(int mu=0;mu<DIM;mu++){
            ...
            {double So=-(b2/NC2)*m2_retr(m2_mul(U2[s*DIM+mu],m2_staple(s,mu)));double a1=gauss01()*eps,a2=gauss01()*eps,a3=gauss01()*eps;M2 h;h.e[0][0]=cx(1,a3);h.e[0][1]=cx(a2,a1);h.e[1][0]=cx(-a2,a1);h.e[1][1]=cx(1,-a3);M2 Uo=U2[s*DIM+mu];U2[s*DIM+mu]=m2_proj(m2_mul(h,Uo));double Sn=-(b2/NC2)*m2_retr(m2_mul(U2[s*DIM+mu],m2_staple(s,mu)));if(Sn-So>0&&drand01()>=exp(-(Sn-So)))U2[s*DIM+mu]=Uo;}
    ...
    fprintf(fp,"# k6_hmc_v2 L=%d mass=%.6f beta=(%.1f,%.1f,%.1f) nstep=%d tau=%.2f\n",L,mass,b3,b2,b1,nstep,tau);
    fprintf(fp,"# traj  acc  dH         pbp         P3      P2      P1\n");
    fflush(fp);

    /* HMC */
    int total_acc=0; double pbp_sum=0,pbp_sq=0; int nmeas=0;
    for(int t=0;t<Ntraj+Nthm;t++){
        int acc;
        double dH=do_hmc(mass,b3,b2,b1,nstep,tau,&acc);
        total_acc+=acc;
        plaquettes(&p3,&p2,&p1_val);

        double pbp_val=0;
        if(t>=Nthm){
            pbp_val=measure_pbp(mass);
            pbp_sum+=pbp_val;pbp_sq+=pbp_val*pbp_val;nmeas++;
            fprintf(fp,"%5d  %d  %+12.4f  %12.6f  %.4f  %.4f  %.4f\n",
                    t-Nthm,acc,dH,pbp_val,p3,p2,p1_val);
            fflush(fp);
        }

        if(t%5==0||t==Ntraj+Nthm-1){
            fmt_time(difftime(time(NULL),T0),tb);
            double pm=(nmeas>0)?pbp_sum/nmeas:0;
            double pe=(nmeas>1)?sqrt(fabs(pbp_sq/nmeas-pm*pm)/(nmeas-1)):0;
            fprintf(stderr,"  traj %3d/%d [%s] %s dH=%+.3f P3=%.3f P2=%.3f P1=%.3f",
                    t+1,Ntraj+Nthm,tb,acc?"ACC":"REJ",dH,p3,p2,p1_val);
            if(nmeas>0) fprintf(stderr," <pbp>=%.4f±%.4f",pm,pe);
            fprintf(stderr,"\n");
        }
    }

    double pm=pbp_sum/nmeas,pe=sqrt(fabs(pbp_sq/nmeas-pm*pm)/(nmeas-1));
    fmt_time(difftime(time(NULL),T0),tb);
    fprintf(stderr,"\n  DONE (%s). acc=%d%% <pbp>=%.6f±%.6f\n\n",tb,(100*total_acc)/(Ntraj+Nthm),pm,pe);
    fprintf(fp,"# RESULT: <pbp>=%.6f +/- %.6f acc=%d%%\n",pm,pe,(100*total_acc)/(Ntraj+Nthm));
    fclose(fp);
