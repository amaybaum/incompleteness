...
**On completeness.** The four generators are *sufficient* for the uniqueness claim of Lemma 23.0: any two reconstructions consistent with E1–E4 + A1–A6 are related by some composition of (i)–(iv). Whether they are *exhaustive* — that is, whether $\mathcal{G}_{\text{sub}}$ contains additional independent generator families beyond (i)–(iv) — is a separate question that this paper does not settle. Candidate fifth families (time reversal $\varphi \to \varphi^{-1}$ where the framework's observables are time-symmetric; reparametrization of the deep hidden sector beyond enlargement; visible-sector emergent global phase) plausibly reduce to compositions of (i)–(iv), but a positive proof would require demonstrating directly that any observables-preserving transformation lies in the group generated by (i)–(iv), independently of Theorem 23's uniqueness claim. This direct demonstration is open. The looser statement — that $\mathcal{G}_{\text{sub}}$ is *at least as coarse* as the four-generator quotient — is what is established here and what Theorem 23 actually requires; full coincidence of $\mathcal{G}_{\text{sub}}$ with the four-generator quotient is conjectural.
