...
The characteristic acceleration scale of the partition boundary is its surface gravity $\kappa = cH$. When the baryonic gravitational acceleration $g_b(r) = GM_b/r^2 \gg cH$, the baryonic field dominates and the boundary entropy's influence is a small perturbation. The effect is expected to become significant when $g_b \lesssim cH$ — the regime where the boundary's own acceleration scale competes with the baryonic field. This identifies a transition radius $r_t \sim \sqrt{GM_b/(cH)} \sim 5$ kpc for a Milky Way–mass galaxy. The transition scale $cH$ is intrinsic to the partition geometry, not a free parameter; but the precise onset profile requires a quantitative force law beyond the scope of the axioms (see §9.4).
...
**Theorem (Dark gravity).** *Under Axioms 1–4 and conditions (C1)–(C3), the cosmological partition produces, in addition to emergent quantum mechanics, additional gravitational acceleration with the following properties: (i) no source in the emergent QFT (§9.2); (ii) effective energy density $\rho_{\text{crit}}$ (§9.1); (iii) persistent entropy displacements producing history-dependent gravitational structure (§9.3).*

*Proof.* Each property is proved in the referenced subsection. $\square$

Properties (i)–(iii) are derived from the axioms. Two further properties follow from dimensional reasoning rather than proof: (iv) the effect is expected to become significant at baryonic accelerations $g_b \lesssim cH$, because $cH$ is the partition boundary's characteristic acceleration scale (§9.2); and (v) because the horizon area evolves cosmologically ($A = 4\pi c^2/H^2$), the transition scale $cH$ inherits cosmological evolution. The dimensional expectation is well-motivated — $cH$ is the only acceleration scale intrinsic to the partition — but the precise onset profile and interpolation between regimes require a quantitative force law beyond Axioms 1–4, deferred to future work.

**Falsifiability.** The dark gravity prediction would be falsified by definitive detection of dark matter particles (contradicting (i)), or by gravitational anomalies uncorrelated with the baryonic field (contradicting (i) and (iii)).
...
**A second application (Part III).** The same axioms and conditions that produce quantum mechanics independently predict additional gravitational effects: the boundary entropy's thermal energy equals $\rho_{\text{crit}}$ (derived, not assumed), its gravitational influence has no operator in the emergent QFT, and entropy displacements persist through P-indivisibility. Dimensional reasoning identifies the partition boundary's surface gravity $cH$ as the characteristic scale at which the effect becomes significant. That the framework produces a second, qualitatively distinct phenomenon from the same premises provides evidence that it is capturing real physical structure.
