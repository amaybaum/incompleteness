...
**Tier 1 (Part I, from axioms alone).** P-indivisibility of the reduced dynamics (§2.3), the characterization theorem establishing QM $\iff$ embedded observation under C1–C3 (§3.3), partition-relativity (§1.4), and the deep-sector cardinality gauge corollary (§5.2). These follow from Axioms 1–4 and combinatorial arguments; disputing them requires finding a mathematical error.
...
**Tier 3 (consistency checks and parametric predictions).** The dark-sector concordance (~95% of $\rho_{\text{crit}}$, §8.3), the dark energy parameter $\nu_{\text{OI}}$ (§8.1), and the GW echo timescale (§8.2). These are derived consequences, not independent derivations; they would be falsified by observation but do not carry the same deductive weight as Tiers 1–2.
...
These three layers — associativity, the generalized second law, and the emergent Page curve — define a research program for nested and evolving partitions. §8.2 treats GW echoes as a specific case; the general theory requires further development. Laboratory tests may be possible in analogue gravity systems where the hidden sector capacity is tunable.
