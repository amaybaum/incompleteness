...
**Finiteness condition.** The effective configuration space of the visible sector is finite-dimensional. A discreteness scale $\epsilon$ provides a finite minimal cell volume. In Part I, $\epsilon$ is a free parameter; its value is determined *a posteriori* by self-consistency (§6), which pins $\epsilon = 2\,l_p$ exactly. In Part II, the finiteness condition follows from the partition geometry: the cosmological horizon has finite area $A = 4\pi c^2/H^2$, bounding the number of coupled modes to $A/\epsilon^2$. This condition enters the proof at a single point: the stochastic-quantum correspondence (§3.1), which requires a finite configuration space. Independent support comes from holographic arguments that the Hilbert space of quantum gravity is locally finite-dimensional [27].
