...
**Independent corroboration of the trace-out mechanism.** The technical move from a deterministic substrate to non-Markovian visible dynamics via the trace-out of inaccessible degrees of freedom — central to both routes — is now established at theorem level in the open systems literature. Brandner [47, 48] proves that for autonomous linear evolution equations, integrating out inaccessible degrees of freedom (whether external environments or internal subsystems) yields well-defined non-Markovian visible dynamics in a controlled weak-memory regime, with explicit error bounds and a convergent perturbation scheme. This is the mathematical apparatus the present construction relies upon, derived independently for general open systems. Direct experimental demonstrations of the mechanism include Mehl et al. [50], who showed that hidden slow degrees of freedom in a colloidal system produce non-Markovian visible dynamics violating naive Markovian fluctuation theorems, and Gröblacher et al. [51], who observed non-Markovian Brownian motion in a macroscopic micromechanical oscillator. On the quantum side, Kim [49] shows that monitored quantum systems are formally quantum hidden Markov models, with a rigorous correspondence to classical hidden Markov models for setups involving Haar-random unitaries and measurements — the same formal structure as the stochastic-quantum bridge. The framework's central technical move is therefore standard rather than speculative.
...
where $r_+ = (GM/c^2)(1 + \sqrt{1-a^2})$ is the Kerr outer horizon radius, with $M$ understood as the source-frame mass. For GW250114 ($M_{\text{source}} = 62.7\,M_\odot$, $a/M = 0.68$): $\Delta t_{\text{source}} = 48.9$ ms in the source frame. The observed echo delay in detector data is redshifted by $(1+z)$, giving $\Delta t_{\text{obs}} = (1+z)\,\Delta t_{\text{source}}$; for GW250114 ($z = 0.09$), $\Delta t_{\text{obs}} = 53.2$ ms. Equivalently, plugging the detector-frame mass $M_{\text{det}} = (1+z)\,M_{\text{source}} = 68.1\,M_\odot$ into the formula above directly yields $\Delta t = 53.2$ ms (with a small $\ln(1+z)/\ln(r_+/2l_p) \sim 0.1\%$ correction). All matched-filter calculations against actual detector strain data must use detector-frame quantities. For a non-spinning $30\,M_\odot$ remnant: $\Delta t = 26.8$ ms. The logarithmic dependence on $\epsilon$ makes the timescale robust ($\sim 0.1$ ms shift within the $\sim 53$ ms scale [23]).
...
This gives a first-echo amplitude $A_1 = \mathcal{R} \times \Gamma \approx 0.45$ relative to the ringdown peak strain. The predicted matched-filter signal-to-noise ratio in a single detector is $\text{SNR}_{\text{echo}} \approx \sqrt{\Sigma}\,A_1\,\text{SNR}_{\text{rd}}$, where $\text{SNR}_{\text{rd}}$ is the (per-detector) ringdown matched-filter SNR for the dominant $(2,2,0)$ mode and $\Sigma = \sum_n (1-\Gamma)^{2(n-1)} \approx 1/(2\Gamma - \Gamma^2) \approx 1.55$ accounts for the coherent contribution of subsequent echoes; numerically, $\sqrt{\Sigma}\,A_1 \approx 0.56$. For GW250114, the per-detector ringdown SNR for $(2,2,0)$ alone is $\sim 8$–$12$ in each of H1 and L1 (the network ringdown SNR including all modes is higher), giving a per-detector echo SNR of approximately $4$–$7$. Coherent two-detector network analysis (with antenna-pattern weighting and time-delay alignment) gains a further factor $\sim \sqrt{2}$, giving a coherent network echo SNR of $\sim 6$–$10$. These numbers are at or just above the standard detection threshold of $5$, so a *single-event* test is marginal; a clean detection requires either a higher-SNR ringdown event, coherent multi-detector analysis, or coherent stacking across several BBH events. Existing model-independent echo searches on GW250114 find no statistically significant evidence [45], which is consistent with both the OI prediction (because of the search-template mismatch discussed below) and with the framework being incorrect.

**Why model-independent searches do not constrain this prediction.** Beyond the marginal single-event sensitivity discussed above, there is a second reason existing searches do not test the OI prediction: a morphology mismatch between the typical search template and the OI signal. The Wu et al. [45] search targets long-lived quasi-monochromatic oscillations persisting after the ringdown — effectively a damped sinusoid with $\tau_{\text{LL}} \gg \tau_{\text{QNM}}$. The OI echo is a *train of short bursts* — each lasting only $\tau \sim 4$ ms (the physical QNM damping time, in the detector frame), separated by $\sim 50$ ms of silence, with alternating phase $(-1)^n$. The fitting factor between the OI template and a single damped sinusoid depends strongly on the template's assumed damping time: for $\tau_{\text{LL}} \sim \tau_{\text{QNM}}$, the best overlap is $\text{FF} \approx 0.83$; for $\tau_{\text{LL}} \sim 10\,\tau_{\text{QNM}}$, $\text{FF} \approx 0.50$; and for $\tau_{\text{LL}} \sim 10^2\,\tau_{\text{QNM}}$ (the regime targeted by model-independent searches for long-lived echo-type signals), $\text{FF} \approx 0.19$. The long-lived QNM search therefore recovers only $\sim 19\%$ of the optimal SNR against the OI template against which it would otherwise be best matched — a $5.3\times$ loss relative to a dedicated OI matched filter. Combined with the marginal predicted single-event SNR, the long-lived QNM search would expect to see effective SNR $\lesssim 2$ even if the OI prediction is correct, well below detection threshold. The non-detection is therefore consistent with the OI prediction, but it also does not strongly constrain it. A dedicated matched-filter search using the OI template is required for a meaningful test.

| Parameter | Formula | $M_{\text{source}} = 62.7\,M_\odot$, $a/M = 0.68$, $z = 0.09$ |
|---|---|---|
| $M_{\text{det}}$ | $(1+z)\,M_{\text{source}}$ | $68.1\,M_\odot$ |
| $\Delta t$ (observed) | $(r_+/c)\ln(r_+/2l_p)$ with $M_{\text{det}}$ | 53.2 ms |
| $f_{\text{QNM}}$ (observed) | Berti+2006 fit with $M_{\text{det}}$ | 250 Hz |
| $\tau$ (observed) | Berti+2006 with $M_{\text{det}}$ | 4.08 ms |
...
| Comb teeth within QNM line | $(1/\tau)/(1/\Delta t)$ | $\approx 13$ |
| Fitting factor vs long-lived QNM | $\text{FF} = \max_{\tau_{\text{LL}},f,\phi} \langle h_{\text{OI}} \mid h_{\text{LL}} \rangle$ | $0.19$ (long-lived); up to $0.83$ ($\tau_{\text{LL}} \sim \tau_{\text{QNM}}$) |
| Per-detector predicted echo SNR | $\sqrt{\Sigma}\,A_1\,\text{SNR}_{\text{rd},(2,2,0)}$ | $\sim 4$–$7$ (range from $\text{SNR}_{\text{rd}} \sim 8$–$12$) |
| Coherent network echo SNR | $\sim \sqrt{2} \times$ per-detector | $\sim 6$–$10$ |

**Detection prospects.** The predicted echo amplitude ($\sim 45\%$ of ringdown) is substantial, but the signal's comb structure and phase alternation make it nearly orthogonal to long-lived QNM templates (fitting factor as low as $0.19$ depending on the assumed search-template damping time). Three search strategies can recover the full power: (i) matched filtering with the one-parameter OI template (overall amplitude as the only unknown, everything else fixed by the detector-frame remnant mass and spin; predicted per-detector SNR $\sim 4$–$7$ for GW250114, coherent network SNR $\sim 6$–$10$), (ii) auto-correlation of the post-ringdown signal at lag $\Delta t$, or (iii) a comb filter at spacing $1/\Delta t$. All three are feasible with publicly released O4 strain data, but a single high-SNR event is at the threshold of single-event detectability. Coherent stacking across multiple BBH events with similar remnant masses, or analysis with next-generation detectors (Einstein Telescope, Cosmic Explorer, where the matched-filter echo SNR for typical BBH mergers would exceed 100), would make detection unambiguous.
...
**Observational status.** Model-independent searches for post-merger echoes in GW150914, GW231226, and GW250114 find no statistically significant evidence [45]. As discussed above, this non-detection does not constrain the OI prediction, both because the predicted single-event matched-filter SNR is only marginally above threshold and because the search templates are mismatched against the OI comb morphology (recovering as little as $\sim 19\%$ of the optimal SNR). A dedicated matched-filter search using the OI template at the detector-frame remnant mass and spin is required to test the prediction. A preliminary single-detector matched-filter analysis on public LVK GW250114 strain data, using the OI template with $M_{\text{det}} = 68.1\,M_\odot$, $a/M = 0.68$, locally-normalized SNR statistics to handle non-stationary detector noise, and a $\pm 5$ ms timing window around the predicted echo arrival, returns no detection above the $5\sigma$ threshold in either H1 or L1 individually, with healthy noise diagnostics ($\text{bg\_std} \approx 1.0$ in both detectors) and a per-detector $5\sigma$ upper limit on the echo amplitude of $A_1 \lesssim 7 \times 10^{-22}$ strain.\footnote{This upper limit is approximately a factor of 1.5 above the standard predicted echo amplitude assuming detector-frame ringdown peak strain $\sim 10^{-21}$, and is not yet a decisive test. A coherent H1-L1 network analysis with antenna-pattern weighting and time-delay alignment, expected to gain a factor $\sim \sqrt{2}$ in SNR, and coherent stacking across additional high-SNR BBH ringdowns, are required for a definitive test.} **The decisive single-event test is therefore a coherent network analysis on the highest-SNR BBH events in O3-O4, supplemented by stacking across events with similar remnant parameters.** If a coherent matched-filter analysis on a sufficiently high-SNR event or a stacked analysis returns no detection at the OI template's predicted amplitude, the prediction would be falsified. Joint confirmation of dark energy evolution ($\nu_{\text{OI}}$) *and* GW echoes would uniquely favor an information-theoretic origin.

**Independent horizon-physics signals in GW250114.** Two recent analyses of GW250114 detect non-trivial structure beyond the standard quasi-normal-mode spectrum. Lu et al. [57] report observational evidence for a "direct wave" component oscillating near $2\Omega_H$ (the horizon angular frequency) and decaying at the surface gravity $\kappa$, with matched-filter SNR $14.0^{+0.2}_{-0.1}$ in LIGO Hanford and $13.5^{+0.1}_{-0.2}$ in Livingston. The Lu et al. signal is a different proposed horizon effect with different morphology (smooth damped sinusoid rather than the OI comb), and its per-detector SNR is significantly higher than the predicted OI echo SNR ($\sim 4$–$7$ per detector); we make no quantitative comparison between the two signals. The relevance of the Lu et al. result is methodological: it establishes that horizon-scale structure beyond the standard QNM picture is detectable in current LIGO data, and that the LIGO-Virgo-KAGRA analysis infrastructure for non-standard horizon physics on GW250114 is operational. Wang et al. [58] independently detect six quadratic quasi-normal modes in the GW250114 ringdown at Bayes factor 74, demonstrating that the post-merger signal contains nonlinear structure requiring more than the linear $(\ell, m, n) = (2, 2, 0/1)$ modes — the standard ringdown analysis is incomplete. The standard area-law confirmation from Abac et al. [59] independently verifies the Bekenstein-Hawking entropy formula derived in §6, including the factor of $1/4$. None of these results is the OI echo, but each establishes that horizon physics beyond standard GR is now a tractable observational target. The dedicated OI matched-filter search on GW250114, supplemented by coherent network analysis and stacking across additional events, remains the highest-leverage near-term test.
...
This prediction is borne out by the data. Genzel et al. [42, 43] report stacked rotation curves for massive star-forming galaxies at $z = 0.9\text{–}2.4$ showing a declining outer velocity at $> 3\sigma$ significance relative to local spirals — consistent with the OI prediction. In ΛCDM, flat rotation curves from NFW halos are expected at all redshifts; explaining the Genzel data requires anomalously low halo concentrations or baryon-dominated inner regions. The cleanest local test is Jiao et al. [61], who detect for the first time a Keplerian decline in the Milky Way's own rotation curve from $\sim 19$ to $\sim 26.5$ kpc using Gaia DR3 kinematics, with the flat rotation curve hypothesis rejected at $3\sigma$. Our own galaxy is the strongest single piece of evidence for the $H(z)$-dependent crossover prediction at $z = 0$, where the OI framework predicts the crossover at $r_M \sim 17$ kpc for the Milky Way's baryonic mass — essentially where Jiao et al. observe the transition.
...
[47] K. Brandner, "Dynamics of microscale and nanoscale systems in the weak-memory regime," *Phys. Rev. Lett.* **134**, 037101 (2025).

[48] K. Brandner, "Dynamics of microscale and nanoscale systems in the weak-memory regime: A mathematical framework beyond the Markov approximation," *Phys. Rev. E* **111**, 014137 (2025).

[49] S. W. P. Kim, "Measurement-induced phase transitions in quantum inference problems and quantum hidden Markov models," arXiv:2504.08888 (2025).

[50] J. Mehl, B. Lander, C. Bechinger, V. Blickle, and U. Seifert, "Role of hidden slow degrees of freedom in the fluctuation theorem," *Phys. Rev. Lett.* **108**, 220601 (2012).

[51] S. Gröblacher, A. Trubarov, N. Prigge, G. D. Cole, M. Aspelmeyer, and J. Eisert, "Observation of non-Markovian micromechanical Brownian motion," *Nature Communications* **6**, 7606 (2015).

[52] H. C. Bäcker, S. Milz, and K. Modi, "Operational discrimination of classical and quantum memory in non-Markovian processes," (2023).

[53] B. Yosifov et al., "Distinguishing classical and quantum memory in collision models with controllable correlations," (2025).

[54] DESI Collaboration: W. Elbers et al., "Constraints on Neutrino Physics from DESI DR2 BAO and DR1 Full Shape," arXiv:2503.14744 (2025).

[55] JUNO Collaboration: A. Abusleme et al., "First measurement of reactor neutrino oscillations at JUNO," arXiv:2511.14593 (2025).

[56] F. Capozzi et al., "Updated bounds on the (1,2) neutrino oscillation parameters after first JUNO results," arXiv:2511.21650 (2025).

[57] N. Lu, S. Ma, O. J. Piccinni, L. Sun, and E. Finch, "GW250114 reveals black hole horizon signatures," arXiv:2510.01001 (2025).

[58] Y.-F. Wang, S. Ma, et al., "A nonlinear voice from GW250114 ringdown," arXiv:2601.05734 (2026).

[59] LIGO Scientific Collaboration, Virgo Collaboration, and KAGRA Collaboration: A. G. Abac et al., "GW250114: Testing Hawking's Area Law and the Kerr Nature of Black Holes," *Phys. Rev. Lett.* **135**, 111403 (2025).

[60] LZ Collaboration: J. Aalbers et al., "Dark matter search results from 4.2 tonne-years of exposure of the LUX-ZEPLIN (LZ) experiment," (2025); 417-day analysis presented at SURF, December 2025.

[61] Y. Jiao et al., "Detection of the Keplerian decline in the Milky Way rotation curve," *Astronomy & Astrophysics* **678**, A208 (2023).
