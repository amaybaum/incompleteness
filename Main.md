...
2. **Finiteness.** The effective configuration space of the visible sector is finite-dimensional, with a discreteness scale $\epsilon$ providing a finite minimal cell volume. Any observer bounded by a finite-area surface can couple to only finitely many modes across that boundary; independent support comes from holographic entropy bounds [27]. In Part I, $\epsilon$ is free; self-consistency (§6) pins $\epsilon = 2\,l_p$. The finiteness requirement applies to the visible sector and its boundary layer; the deep hidden sector may be infinite (§9.7).
...
**Lemma (parameter independence from spatial locality).** *Let $V_A$ and $V_B$ be space-like separated visible-sector subsystems with disjoint coupling neighborhoods on the coupling graph $G_\varphi$. Then for any measurement setting $a$ chosen at $V_A$ and any measurement setting $b$ chosen at $V_B$:*

$$P(x_B \mid a, b) = P(x_B \mid b)$$

*Proof.* The measurement setting $a$ determines which local observable Alice measures — that is, which function of her visible-sector variables she evaluates. By the spatial Markov property of range-1 dynamics on $G_\varphi$ (Fundamental, §5, area-law lemma), $V_A$ and $V_B$ are conditionally independent given the boundary data between them. Alice's choice of $a$ affects only the function she computes on $V_A$; it does not alter the transition probabilities $T_{ij}(t)$ for Bob's sector $V_B$, which depend only on $V_B$'s local coupling graph neighborhood. Bob's marginal statistics $P(x_B \mid b)$ are therefore independent of $a$. The no-signaling condition is enforced structurally by the locality of $G_\varphi$ — no fine-tuning of the hidden-variable distribution is required. $\square$

The lemma applies to all bounded-degree graph topologies satisfying range-1 coupling, not merely to the cubic lattice. Combined with the outcome-independence violation from stochastic entanglement, this gives the precise Jarrett decomposition: parameter independence holds structurally; outcome independence fails generically.
...
**Remark (phase recovery under discrete sampling).** In the lattice realization, the observer accesses $T_{ij}(t)$ at multiples of the dynamical timescale $\tau_S$, not at arbitrary continuous times. The Fourier analysis in the phase-locking lemma requires resolving frequencies $\omega_{kl} = E_k - E_l$. Discrete sampling at intervals $\tau_S$ introduces an aliasing bound: frequencies are determined modulo $2\pi/\tau_S$. Since the emergent energy eigenvalues satisfy $E_k \leq \hbar/\tau_S$ (visible-sector energies are bounded by the cutoff), the maximum frequency difference is $\omega_{\max} \leq 2\hbar/\tau_S$. For the cosmological realization, $\tau_S \sim 10^{-15}$ s for laboratory processes, giving $\omega_{\max} \sim 10^{15}$ rad/s — well below the Nyquist frequency $\pi/\tau_S \sim 10^{15}$ rad/s. Aliasing occurs only for trans-Planckian frequencies, which lie outside the emergent QFT's domain. More precisely, the continuous-time extension (§2.3) provides $T_{ij}(t)$ as a genuinely continuous function of $t$ via the Liouville marginalization of the Hamiltonian flow (Axiom 1): the spatial lattice discreteness does not discretize time. The D-gauge theorem therefore applies to the continuous object without aliasing corrections, and the phase-recovery error is controlled by the same $\mathcal{O}(\tau_S/\tau_B) \sim 10^{-32}$ suppression that governs the boundary-only dependence of §5.2.
...
**Cumulative evidence for the ordering.** The geometry-first ordering produces six independent consequences that match observation, none of which were designed into the axioms: (1) the CC dissolution — the observed vacuum energy sits at $\rho \sim H^2/G$, the classical baseline, without fine-tuning (§7.3); (2) the Bekenstein-Hawking formula including the $1/4$ factor, derived from $\epsilon = 2\,l_p$ (§6); (3) the value of $\hbar$, determined by partition geometry rather than treated as a free parameter (§5.2); (4) the dark-sector concordance — ~95% of $\rho_{\text{crit}}$ is invisible to QFT, matching the observed dark sector (§8.3); (5) dark energy evolution with $\nu_{\text{OI}}$ in the range consistent with DESI data (§8.1); (6) a testable GW echo prediction at the discreteness scale (§8.2). Quantum-first orderings produce the $10^{122}$ discrepancy, have no natural account of the dark sector, and treat $\hbar$ and the $1/4$ factor as unexplained inputs. The cumulative case is not a single tiebreaker but a pattern: every quantitative output of the geometry-first ordering is consistent with observation; the central quantitative output of the quantum-first ordering is the worst prediction in physics.
...
The corollary is a consistency check, not a derivation of the dark sector's internal structure. The uniform component of the boundary entropy corresponds to dark energy (§7); whether matter-induced redistribution of the boundary entropy produces observable local effects — and if so, whether the redistribution profile accounts for the structured component (dark matter) — requires a quantitative force law beyond Axioms 1–4 [37] and is deferred to future work. The ratio $\rho_{\text{QFT}} / \rho_s \sim S_{\text{dS}} \sim 10^{122}$ confirms the parallel with §7.3: the information compression ratio and the gravitational occlusion fraction are two aspects of a single phenomenon.
...
**Effective finiteness.** Axiom 2 as stated requires total finiteness of $S = \mathcal{C}_V \times \mathcal{C}_H$. A strictly weaker condition suffices: finiteness of $\mathcal{C}_V$ and of the boundary layer $\mathcal{C}_B$ (the hidden-sector modes within the interaction range of $H_{\text{int}}$), with no constraint on the deep hidden sector $\mathcal{C}_D$. Both $|\mathcal{C}_V|$ and $|\mathcal{C}_B|$ are bounded by the holographic entropy bound [27] applied to the cosmological horizon (finite area $\Rightarrow$ finite entropy), which is a well-established result independent of the de Sitter finite-dimensionality conjecture. The deep sector may be infinite — as in a spatially open or flat cosmology. The framework's results survive for two reasons. First, the boundary-only dependence lemma (§5.2) establishes that $T_{ij}(t) = T^{(B)}_{ij}(t) + \mathcal{O}(t/\tau_B)$, where $T^{(B)}$ depends only on the dynamics restricted to the finite space $\mathcal{C}_V \times \mathcal{C}_B$: the deep sector's contribution is suppressed by $\tau_S/\tau_B \sim 10^{-32}$. Second, the accessible-timescale backflow lemma (§2.3) establishes P-indivisibility on all timescales $t \ll \tau_B$ without invoking Poincaré recurrence of the total system — its proof uses only (C1)–(C2) and is independent of $|\mathcal{C}_H|$. Alternatively, Poincaré recurrence applies directly to the effective dynamics on the finite space $\mathcal{C}_V \times \mathcal{C}_B$. The gap equation (§5.2), Bekenstein-Hawking entropy (§6), CC dissolution (§7), and the Standard Model derivation (Fundamental, §§7–8) reference no property of $\mathcal{C}_D$. The one structural cost: the characterization theorem becomes an equivalence on observable timescales ($T_{\text{obs}} \ll \tau_B \sim 10^{17}$ s) rather than a timeless identity — physically indistinguishable for all experiments within the age of the universe.
...
**Open problems: nested and time-dependent partitions.** The derivation chain assumes a single partition into visible and hidden sectors. In reality, multiple causal boundaries coexist — most naturally, a black hole horizon inside the cosmological horizon — creating nested partitions with distinct geometries. Three layers of the problem can be identified.

*(i) Consistency of nested trace-outs.* Let $V \subset W \subset S$ be nested visible sectors (e.g., $V$ is the region between the black hole and cosmological horizons, $W$ is the cosmological interior). The cosmological trace-out $\text{Tr}_{S \setminus W}$ produces QM on $W$; a subsequent trace-out $\text{Tr}_{W \setminus V}$ over the black hole interior produces a further-reduced description on $V$. The question is whether $\text{Tr}_{S \setminus W} \circ \text{Tr}_{W \setminus V} = \text{Tr}_{S \setminus V}$. For standard partial traces on tensor products this is trivially true, and the spatial product structure of $\mathcal{C}_V$ (§7.1) together with the locality of the classical Hamiltonian ensures that the emergent Hilbert space inherits a compatible tensor product factorization at each level. The associativity of nested trace-outs is therefore expected to hold, but a rigorous proof for the specific class of P-indivisible processes generated here — where the product structure is emergent rather than postulated — remains open. What the framework does predict: both trace-outs yield the same $\hbar$, since $\hbar = c^3 \epsilon^2 / (4G)$ depends only on local boundary quantities ($c$, $G$, $\epsilon$), not on which horizon defines the partition (§5.2, Steps 2–3).

*(ii) The entropy budget and the generalized second law.* The Bekenstein-Hawking entropy $S_{\text{dS}} = A_{\text{dS}} / (4\,l_p^2)$ is the information compression ratio of the cosmological trace-out. For a black hole with entropy $S_{\text{BH}} = A_{\text{BH}} / (4\,l_p^2)$, the framework interprets $S_{\text{BH}}$ as the compression ratio of the second (black hole) trace-out. The generalized second law — that $S_{\text{dS}} + S_{\text{BH}} + S_{\text{matter}}$ does not decrease — should emerge as a consistency condition on nested partitions: any process that decreases one boundary's area while increasing accessible information must compensate via the other boundary or the matter sector. Deriving this from the bijection structure of $\varphi$ would provide a strong independent check on the framework.

*(iii) Time-dependent partitions and the black hole information paradox.* An evaporating black hole has a shrinking partition boundary: degrees of freedom transition from hidden to visible as the horizon area decreases. In the fundamental description $(S, \varphi)$, the evolution is always bijective — information is never lost. The apparent information loss in Hawking's calculation is an artifact of treating the emergent QFT's trace-out as if the partition were static. As the black hole evaporates, the partition moves through the state space, and the emergent QM description must be continuously updated. The Page curve — the entropy of Hawking radiation as a function of time — should be derivable from the time-dependent partition geometry: as $A_{\text{BH}}$ shrinks, $S_{\text{BH}}$ decreases, and the mutual information between the radiation and the remaining black hole interior increases. The framework's prediction is definite: the Page curve turns over (unitarity is preserved), because the total evolution is bijective. The quantitative derivation — the Page curve's functional form from the combinatorics of time-dependent trace-outs on a finite coupling graph — is the central open problem.

These three layers define a research program: associativity of nested trace-outs, the generalized second law as a partition consistency condition, and the emergent Page curve from time-dependent partitions on $(S, \varphi)$. §8.2 treats the specific case of GW echoes from the black hole partition boundary; the general theory of nested and evolving partitions requires further development. Laboratory tests of the characterization theorem may be possible in analogue gravity systems where the hidden sector capacity is tunable.
...
**Convergence and complementarity.** The two routes agree on the physical observable: both produce the same transition probabilities $T_{ij}(t)$, and hence the same Born-rule statistics for all visible-sector measurements. Their agreement is a non-trivial consistency check — one route starts from a property of the stochastic process (P-indivisibility) with no reference to the hidden sector, while the other starts from the explicit dilation (bijection + partial trace) with full knowledge of the hidden sector's structure.

The Stinespring route is the more constructive of the two and delivers additional structure that the Barandes route does not require: (a) the tensor product $\mathcal{H}_V \otimes \mathcal{H}_H$ and the CPTP channel $\Phi$ with its Kraus decomposition (§A.3); (b) the emergence of genuine quantum coherence from non-trivial coupling (Theorem A.3); (c) CP-indivisibility as a consequence of P-indivisibility (Theorem A.4); and (d) the approximately unitary visible-sector dynamics on observable timescales $t \ll \tau_B$ (§A.7), with the dissipator $\mathcal{D} \sim \mathcal{O}(\tau_S / \tau_B) \sim 10^{-32}$ for the cosmological realization.

The Barandes route is more general: it establishes the existence of unitary QM for *any* P-indivisible process, whether or not it arises from a known deterministic dilation. This generality is what powers the (ii) $\implies$ (i) direction of the characterization theorem. The Stinespring route supplies the (iii) $\implies$ (i) direction independently, using only the dilation structure. The agreement of two logically independent derivations — one from the process, one from the construction — establishes that the emergence of QM is robust: it does not depend on the details of either proof strategy.
