...
where $\pi_V$ is the projection onto visible-sector coordinates, $\phi_t$ is the Hamiltonian flow, and $\mu$ is the Liouville measure on $\Gamma_H$. As established in §1.4, the Liouville measure is the unique choice preserved by the Hamiltonian flow, ensuring that the transition probabilities — and hence the emergent description — are uniquely determined by the partition.
...
**Dependence on Barandes' correspondence.** The theorem inherits the status of Barandes' stochastic-quantum correspondence, which is its mathematical engine. A candid assessment of this dependence is warranted.

*What the correspondence establishes.* Barandes [10, 11] proves that any indivisible stochastic process on a finite configuration space of size $n$ can be embedded into a unitarily evolving quantum system on a Hilbert space of dimension $\leq n^3$, and conversely that every quantum system can be understood as such a process. The correspondence is bidirectional and exact: it reproduces all first-order conditional probabilities — and hence all empirical predictions of quantum mechanics, including interference, entanglement, and decoherence — from indivisible stochastic dynamics on a classical configuration space. The Born rule emerges as the equilibrium distribution rather than an independent postulate. The correspondence has been published in a peer-reviewed venue [11] and has generated a growing body of independent follow-up work: Calvo [12] extended it to infinite-dimensional systems, Pimenta [30] provided a complete geometric characterization of divisible and indivisible dynamics within the correspondence's framework, and Doukas [31] gave a rigorous mathematical analysis that proved several key structural properties while clarifying the conditions under which effective Markovian behavior can emerge from non-Markovian foundations. Barandes, Hasan, and Kagan [14] derived the Tsirelson bound from indivisibility plus causal locality.

*What the correspondence does not establish.* The higher-order conditional probabilities of the indivisible stochastic process — the multi-time statistics beyond two-point transition matrices — are not uniquely fixed by the correspondence. Different choices yield the same first-order predictions and hence the same empirical content, but may differ at the multi-time level. The present theorem is not affected by this ambiguity: partition-relativity (§1.4) uniquely determines the stochastic process, including its multi-time structure, from the physical content of the partition (§5.3). However, if future work were to reveal that the correspondence fails for some subclass of indivisible processes — for instance, if certain processes generated by the slow-bath mechanism do not admit the unitary embedding — the theorem's scope would narrow accordingly.

*How the theorem's conclusions would change.* The dependence is load-bearing but circumscribed. Partition-relativity, emergent stochasticity, and P-indivisibility (§§1.4, 2) do not invoke Barandes at all — they follow from the axioms and conditions alone. The correspondence enters at a single step: the map from P-indivisible stochastic process to quantum mechanics (§3.1). If the correspondence were restricted, the theorem would still establish that the embedded observer's reduced description is a P-indivisible stochastic process with persistent memory and information backflow; what would be lost is the identification of that process with unitarily evolving quantum mechanics specifically. The framework's qualitative conclusions — that the embedded observer cannot see classical physics, that Bell violations arise without superdeterminism — would survive in weaker form as properties of the stochastic process itself, even without the quantum identification.
...
**Step 2: The action scale is structural, not volumetric.** Among the quantities intrinsic to the cosmological partition, some characterize the *structure* of the coupling across the boundary — the local geometry, the surface gravity, the discreteness scale — while others characterize the *volume* of the hidden sector — the horizon area $A$, the number of modes $N_{\text{modes}} = A/\epsilon^2$, or equivalently the Hubble parameter $H$ through $A = 4\pi c^2/H^2$. The action scale $\hbar$ governs the conversion between the stochastic and unitary descriptions: it sets the rate at which the unitary $U(t)$ accumulates phase. This conversion is determined by the *character* of the P-indivisibility — how the hidden sector couples to the visible sector and imprints correlations — not by the total capacity of the hidden sector.
...
where $\alpha$ is a dimensionless constant of $\mathcal{O}(1)$ and the factor $8\pi$ matches Jacobson's normalization. As a consistency check, the classical horizon temperature from §5.1 confirms the result: $k_B T_{\text{cl}} / H = c^3 \epsilon^2 / 8\pi G$ is independent of $H$, confirming that the $H$-dependence of $T_{\text{cl}}$ is purely volumetric and cancels when converted to an action scale.
...
The scaling $\hbar \propto c^3 \epsilon^2 / G$ is established by partition-relativity, the structural/volumetric distinction, and dimensional analysis (§5.2). The exact prefactor is an open problem (§5.3), but the self-consistency argument depends only on the scaling. Writing $\hbar = (\alpha / 8\pi) c^3 \epsilon^2 / G$ and rearranging:
...
**Consistency check.** After the emergence of quantum mechanics and the identification $\epsilon \sim l_p$, the classical horizon temperature becomes $T_{\text{cl}} \sim \hbar \kappa / (k_B c)$ — the standard Gibbons-Hawking temperature [17] up to order-unity factors. This recovery is a consequence, not an input.
...
**The cosmological application (Part II).** The cosmological horizon provides a physical realization where the sharp-partition approximation is exact and all three conditions are satisfied. The theorem yields: (a) the scaling $\hbar \propto c^3 \epsilon^2 / G$ from partition-relativity, the structural/volumetric distinction, and dimensional analysis, with self-consistency fixing $\epsilon \sim l_p$; (b) a dissolution of the cosmological constant problem, the $10^{122}$ discrepancy identified as $S_{\text{dS}}$; and (c) falsifiable predictions including dark energy evolution in RVM form and gravitational wave echoes.
...
*Barandes' stochastic-quantum correspondence* is the mathematical engine of the theorem. Its status, scope, and the consequences of potential refinements are discussed in §3.4.

**Assumptions and limitations of the application:**
*The scaling $\hbar \propto c^3 \epsilon^2 / G$* follows from partition-relativity, the structural/volumetric distinction, and dimensional analysis. The exact numerical value of $\hbar$ is in principle determined by the process tensor of the trace-out (§5.3), but proving this requires new mathematics; the key open step is showing that multi-time statistics fully resolve the Schur-Hadamard gauge freedom.
...
[30] L. S. Pimenta, "Divisible and indivisible Stochastic-Quantum dynamics," arXiv:2505.08785 (2025).
[31] J. Doukas, "On the emergence of quantum mechanics from stochastic processes," arXiv:2602.22095 (2026).
[32] N. Bao, S. M. Carroll, and A. Singh, "The Hilbert space of quantum gravity is locally finite-dimensional," *Int. J. Mod. Phys. D* **26**, 1743013 (2017).
