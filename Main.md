...
**Projection 1: Fluctuation statistics (QM).** Quantum mechanics characterizes the hidden sector through its fluctuation structure. The QFT vacuum energy calculation sums the zero-point energies of all field modes up to a cutoff. Strictly, the result is an expectation value — $\langle 0 | H | 0 \rangle$ — which is formally a first moment. However, it has the mathematical character of a variance: each mode contributes $+\frac{1}{2}\hbar\omega$, a quantity proportional to the position and momentum variances of the quantum ground state. (In the classical ground state both variances vanish and the zero-point energy is zero.) No cancellation between modes is possible — every term is positive, and the sum grows linearly with the number of modes. The entire QFT vacuum energy is therefore a measure of how much the quantum fields fluctuate — a positive-definite, variance-type quantity whose total scales proportionally to the number of contributing degrees of freedom.

**Projection 2: Mean-field pressure (gravity).** Gravity characterizes the hidden sector through its net mechanical effect — the aggregate pressure that the hidden degrees of freedom exert on spacetime. The observed vacuum energy density is obtained by coupling to the expectation value of the stress-energy tensor — also formally a first-moment quantity. But unlike the QFT sum, the stress-energy tensor is not positive-definite: different field sectors (bosonic vs. fermionic, different spin types, vacuum condensates) contribute to the effective cosmological constant with different signs, and the Einstein equations couple to the net signed aggregate. Cancellation between sectors is not only possible but generically expected. Gravity couples to the net, signed, aggregate energy-momentum content — not to individual mode amplitudes or their variance — making this a mean-type quantity.
...
**The key identification.** Both projections are formally expectation values, but they have structurally different aggregation properties. The QFT calculation adds positive-definite contributions (every mode adds); the gravitational measurement couples to the net signed result (where different sectors cancel). These correspond to the variance and mean of the hidden-sector distribution respectively: the variance is a positive-definite sum that grows proportionally to $N$, while the mean is a signed sum that grows by the central limit theorem only as $\sqrt{N}$.

For a distribution with many degrees of freedom, the variance can be enormously larger than the mean. The analogy is a collection of $N$ random debts and credits: the total absolute amount owed (variance-like) grows proportionally to $N$, but the net balance (mean-like) grows only as $\sqrt{N}$. The ratio therefore grows as $\sqrt{N}$, diverging without bound. For the vacuum, $N$ is the number of hidden-sector degrees of freedom; §3 shows that the observed ratio of $10^{120}$ implies $N$ of order $10^{240}$.
...
Consider a hidden sector with $N$ independent degrees of freedom, each contributing energy with a characteristic magnitude of order one in Planck units (as set by dimensional analysis at the projection boundary). Different sectors contribute to the effective cosmological constant with different signs — bosonic fields contribute positively, fermionic fields contribute negatively, and vacuum condensates shift the balance further. The QFT mode-sum does not see these cancellations: it sums positive-definite zero-point energies mode by mode, so every contribution adds. The gravitational coupling sees the net result after all inter-sector cancellations.

The **total fluctuation content** (quantum projection) sums the absolute magnitude of all contributions, scaling proportionally to $N$. The **net mechanical effect** (gravitational projection) sums the signed contributions; by the central limit theorem, this scales as the square root of $N$. The variance-to-mean ratio therefore scales as the square root of $N$.
