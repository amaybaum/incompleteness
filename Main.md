...
**Baryogenesis.** The framework's neutrino predictions (Majorana masses from the Weinberg operator $c_{ij} L_i L_j H H / \Lambda$, large PMNS CP phases from the Dirac/Majorana matrix mismatch, hierarchical spectrum) feed into a leptogenesis calculation. The Weinberg operator mediates $\Delta L = 2$ scattering ($LL \leftrightarrow HH$) with rate $\Gamma \sim (m_\nu/v^2)^2 T^3$. This rate exceeds the Hubble rate $H \sim T^2/M_{\text{Pl}}$ at $T \gtrsim 10^{14}$ GeV: the operator is in thermal equilibrium in the early universe and decouples at $T_{\text{dec}} \sim 10^{14}$ GeV. At decoupling, CP violation in the operator coefficients (parametrized by the PMNS Jarlskog invariant $J_{\text{PMNS}} \approx 0.033$ for maximal phase) generates a lepton asymmetry through interference of tree-level and gauge-loop diagrams. Sphalerons, in equilibrium at $T_{\text{dec}} \gg T_{\text{EW}}$, convert this to a baryon asymmetry: $\eta_B = (28/79) \times \epsilon_{\text{CP}} \times \kappa / g_* \sim 3 \times 10^{-10}$ for CP asymmetry $\epsilon_{\text{CP}} \sim (\alpha_W/4\pi) J_{\text{PMNS}} \sim 10^{-5}$ and washout efficiency $\kappa \sim 10^{-3}$. This is within a factor of 2 of the observed $\eta_B = (6.1 \pm 0.2) \times 10^{-10}$. The mechanism is not unique to OI (any framework with Majorana neutrinos and large PMNS phases gives similar results), but the framework's structural predictions — Majorana guaranteed (singlet taste = Higgs), large PMNS phases guaranteed (cubic group mismatch), hierarchical spectrum guaranteed (taste-breaking) — ensure that the ingredients are present without additional assumptions. A precise prediction requires the solution-specific Majorana mass matrix $c_{ij}$ (the same information needed for the flavor program). *Epistemic status: consistent at order-of-magnitude level; precise prediction requires solution-specific input.*
