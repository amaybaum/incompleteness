...
**Statement of the theorem.** Under Axioms 1–3, the finiteness condition, and conditions (C1)–(C3), the embedded observer's reduced description of the visible sector is mathematically equivalent to unitarily evolving quantum mechanics. The remainder of Part I constitutes the proof. The conditions are shown to be not merely sufficient but *necessary* for P-indivisibility in any deterministic dilation (§3.3), establishing a full equivalence between quantum mechanics and embedded observation.
...
*Proof.* The proof uses the total variation distance $d(p, q) = \frac{1}{2}\sum_k |p_k - q_k|$ between probability distributions. A stochastic process is P-divisible if and only if $d(p(t), q(t))$ is non-increasing in $t$ for all initial distributions $p, q$ [6, 7]. P-indivisibility is established by exhibiting a non-monotonicity.

*Step 1 (Recurrence).* Since $\varphi$ is a bijection on a finite set, it has finite order: there exists $N \geq 1$ such that $\varphi^N = \text{id}$. Every state $(x_i, h)$ returns to itself after $N$ steps, so $T^{(N)} = I$. In particular, $d(\delta_i T^{(N)}, \delta_j T^{(N)}) = d(\delta_i, \delta_j) = 1$ for all $i \neq j$.

*Step 2 (Strict contraction at step 1).* Since $T$ is row-stochastic and not a permutation matrix, there exist distinct indices $i, j$ whose rows have overlapping support: some column $l$ satisfies $T_{il} > 0$ and $T_{jl} > 0$. (If no two rows had overlapping support, their supports would be pairwise disjoint, forcing each row to concentrate on a single column — a permutation matrix, contradicting the hypothesis.) For such $i, j$:
...
*Step 3 (Non-monotonicity).* Steps 1 and 2 together give:
...
The theorem of §1.3 establishes sufficiency: the axioms, finiteness, and conditions (C1)–(C3) guarantee quantum mechanics. This section proves the converse — that quantum mechanics *requires* these conditions — establishing a full equivalence between quantum mechanics and embedded observation.

**The logical structure.** Barandes' stochastic-quantum correspondence is already bidirectional: a stochastic process is P-indivisible if and only if it is mathematically equivalent to unitarily evolving quantum mechanics [10, 11]. The equivalence
...
**Dilation existence (necessity of the axioms).** Any stochastic process on a finite configuration space can be realized as the marginalization of a deterministic process on a larger state space: one enlarges the state space to encode the randomness as ignorance of auxiliary variables and recovers the original process by tracing them out. This is a standard construction in classical probability theory. The dilation furnishes a deterministic total system (Axiom 1), a partition into visible and hidden sectors (Axiom 2), and classical marginalization (Axiom 3). The three axioms are therefore necessary in the sense that any stochastic process — and hence any P-indivisible process — admits a deterministic embedding with a causal partition. The finiteness of the configuration space is inherited from the original process.

**Theorem (C1 equivalence).** *Let $\mathcal{C}_V$ and $\mathcal{C}_H$ be finite sets with $|\mathcal{C}_V| \geq 2$, and let $\varphi: \mathcal{C}_V \times \mathcal{C}_H \to \mathcal{C}_V \times \mathcal{C}_H$ be a bijection. Define the stochastic process on $\mathcal{C}_V$ by marginalizing over $\mathcal{C}_H$ with respect to the uniform measure. The process is P-indivisible if and only if (C1) holds — that is, if and only if the marginalized one-step transition matrix $T$ is not a permutation matrix.*

*Proof.* The forward direction (C1 $\implies$ P-indivisibility) is the theorem of §2.3. The reverse is proved by contraposition. If $T$ is a permutation matrix, then $T$ represents a deterministic map $\sigma: \mathcal{C}_V \to \mathcal{C}_V$ with $T_{ij} = \delta_{j, \sigma(i)}$. The $k$-step transition matrix is $T^{(k)}_{ij} = \delta_{j, \sigma^k(i)}$, also a permutation matrix for every $k$. The intermediate propagator $\Lambda(k_2, k_1) = T^{(k_2)} \cdot [T^{(k_1)}]^{-1} = T^{(k_2 - k_1)}$ is a permutation matrix and hence a valid stochastic matrix (non-negative entries, rows summing to 1) for all $k_2 > k_1$. The process is P-divisible. Contrapositively, P-indivisibility requires $T$ not to be a permutation matrix, which requires non-trivial coupling: $H_{\text{int}} \neq 0$. $\square$

**Theorem (C2 necessity: Markovian limit).** *Let $\varphi_{\text{coup}}: \mathcal{C}_V \times \mathcal{C}_H \to \mathcal{C}_V \times \mathcal{C}_H$ be a bijection implementing the visible-hidden coupling, and let $\varphi_H: \mathcal{C}_H \to \mathcal{C}_H$ be an ergodic permutation (a single cycle of length $|\mathcal{C}_H|$). Define the interleaved dynamics $\Phi_m = (\text{id}_V \times \varphi_H^m) \circ \varphi_{\text{coup}}$, where $m$ represents the number of hidden-sector mixing steps between coupling events ($m$ is the ratio $\tau_S / \tau_B$, so large $m$ represents the fast-bath regime $\tau_B \ll \tau_S$). Let $T^{(k)}_m$ denote the $k$-step visible-sector transition matrix of $\Phi_m$. Then for $m$ coprime to $|\mathcal{C}_H|$, the process $\{T^{(k)}_m\}$ converges to a Markov chain as $|\mathcal{C}_H| \to \infty$ with the coupling structure held fixed. In the Markovian limit — when the hidden sector's state is independent of the visible sector's history before each coupling event — the reduced process is exactly P-divisible.*

*Proof.* The proof proceeds in two parts: first, that the idealized Markovian limit is P-divisible; second, that fast mixing drives the system to this limit.

*Part 1 (Markovian limit is P-divisible).* Consider the idealized process in which the hidden-sector state is drawn independently from the uniform measure on $\mathcal{C}_H$ before each coupling event, regardless of the visible sector's history. The one-step transition matrix is:
...
*Part 2 (Fast mixing approaches the Markovian limit).* Since $\varphi_H$ is an ergodic permutation (a single cycle of length $|\mathcal{C}_H|$), the map $\varphi_H^m$ with $m$ coprime to $|\mathcal{C}_H|$ is also a single cycle of length $|\mathcal{C}_H|$. For any initial distribution $\mu$ on $\mathcal{C}_H$, repeated application of an ergodic permutation drives $\mu$ toward the uniform distribution in the Cesàro sense: the time-averaged distribution $\bar{\mu}_N = \frac{1}{N}\sum_{k=0}^{N-1} (\varphi_H^m)^k_* \mu$ converges to the uniform distribution $\mu_{\text{unif}}$ as $N \to \infty$.
...
**Corollary (necessity of C3).** *P-indivisibility across all $n$ visible-sector configurations requires non-Markovian correlations spanning the full configuration space. The non-Markovian information required to sustain P-indivisibility across $n$ modes scales as $\Omega(\log n)$: distinguishing the non-Markovian evolution of $n$ visible-sector states requires at least $\log_2 n$ bits of hidden-sector memory. By the capacity bound, this requires $\log_2 m \geq \Omega(\log n)$, i.e., $m \geq n^\alpha$ for some $\alpha > 0$. The hidden sector must scale with the visible sector — precisely condition (C3).*

*Proof.* Suppose the visible sector has $n$ configurations and the reduced process is P-indivisible with information backflow affecting all $n$ configurations. The non-monotonicity of the trace distance (§2.3) requires that the hidden sector stores which of the $n$ visible-sector states the system previously occupied — or more generally, stores a trajectory-dependent record that affects the future transition probabilities of each visible-sector state differently. Storing a record that distinguishes $n$ possible states requires at least $\log_2 n$ bits. By the capacity bound theorem, the total non-Markovian information is at most $\log_2 m$. Therefore:

$$\log_2 m \geq \log_2 n \implies m \geq n$$

If the P-indivisibility is to persist over $K$ successive transitions (i.e., the hidden sector must simultaneously store information about $K$ distinct past events affecting $n$ configurations), the bound tightens to $\log_2 m \geq K \cdot f(n)$ for some function $f$ depending on the strength of the correlations, requiring $m/n$ to grow with the observation window. In particular, if the observation window includes $K \sim n$ transitions (as in any physically realistic scenario where the observer probes the visible sector repeatedly), the hidden sector cannot absorb these imprints without saturation unless $m/n$ is large. This is condition (C3).

*Contrapositive:* if $m$ is not sufficiently large relative to $n$, the $\log_2 m$ capacity bound chokes the non-Markovian information flow before it can sustain P-indivisibility across the full visible sector. The hidden sector saturates — successive imprints overwrite earlier ones — and the reduced dynamics on the affected configurations cross over to effective P-divisibility. $\square$

*Remark.* The capacity bound is *information-theoretic* — it does not depend on the details of the bijection $\varphi$, only on the cardinalities of the two sectors. This universality is what makes C3 a necessary condition rather than a model-dependent feature: *any* deterministic dilation of a P-indivisible process on $n$ configurations must have a hidden sector of at least $n$ states, and a hidden sector much larger than $n$ for the P-indivisibility to persist under repeated observation. The bound is tight in the sense that the minimal model of §2.4 achieves P-indivisibility with $m = 6$ for $n = 2$, consistent with $m \geq n$.

**Characterization theorem (equivalence of quantum mechanics and embedded observation).** *Let $\mathcal{C}_V$ be a finite configuration space with $|\mathcal{C}_V| \geq 2$, and let $\mathcal{S}$ be a stochastic process on $\mathcal{C}_V$. The following are equivalent:*

*(i) $\mathcal{S}$ is mathematically equivalent to unitarily evolving quantum mechanics (via Barandes' stochastic-quantum correspondence [10, 11]).*

*(ii) $\mathcal{S}$ is P-indivisible.*

*(iii) $\mathcal{S}$ arises from the marginalization of a deterministic bijection $\varphi$ on $\mathcal{C}_V \times \mathcal{C}_H$ over a hidden sector $\mathcal{C}_H$, with:*
- *(C1) non-trivial coupling: the marginalized transition matrix $T$ is not a permutation matrix;*
- *(C2) slow-bath memory: the hidden sector retains correlations between successive coupling events (its mixing timescale exceeds the visible sector's dynamical timescale);*
- *(C3) sufficient capacity: $|\mathcal{C}_H| \geq |\mathcal{C}_V|$, with $|\mathcal{C}_H| / |\mathcal{C}_V|$ large enough that the hidden sector is not saturated by visible-sector interactions over the relevant observation window.*

*Proof.* (i) $\iff$ (ii) is Barandes' stochastic-quantum correspondence [10, 11]. (iii) $\implies$ (ii) is the theorem of §2.3. (ii) $\implies$ (iii) is proved by the three necessity theorems above: dilation existence provides the deterministic embedding and the axioms; the C1 equivalence theorem establishes that the coupling must be non-trivial; the C2 necessity theorem establishes that the hidden sector must retain memory; and the C3 capacity bound establishes that the hidden sector must have sufficient dimensionality. $\square$
...
**Process tensor uniqueness.** The P-indivisible stochastic process is characterized by the complete hierarchy of multi-time correlation functions — the *process tensor* $\mathcal{T}_{n:0}$ of Pollock et al. [8]. Every process tensor admits a unitary dilation. In the present framework, the dilation is not a free choice: the "environment" is the hidden sector in the thermal state $T_{\text{cl}}$ (§5.1), and the total dynamics are the unique Hamiltonian flow (Axiom 1). The process tensor is therefore uniquely determined by the partition.

**The phase ambiguity resolution (Conceptual Bridge).** The central mathematical challenge in deriving a unique quantum description from classical probabilities is the Schur-Hadamard gauge freedom: transition probabilities $T_{ij} = |U_{ij}|^2$ strictly define the moduli of the unitary matrix elements but leave their quantum phases completely ambiguous. Left unresolved, this ambiguity would imply that a single stochastic process could map to a vast family of distinct quantum realities with entirely different interference patterns and energy eigenvalues. However, because the observer has access to continuous-time transition data—not just a single static snapshot—the dynamics over time rigidly lock the phases together. The theorem below demonstrates that for any generic Hamiltonian, matching the continuous time-evolution of the transition probabilities forces the quantum phases into a uniquely determined configuration. The only remaining freedom is a time-independent basis rephasing (the D-gauge) that has absolutely no observable physical consequences.

**Theorem (D-gauge completeness).** *Let $U(t) = e^{-iHt}$ be a unitary propagator on an $n$-dimensional Hilbert space with a fixed configuration basis $\{|i\rangle\}$, where $H$ has non-degenerate eigenvalues $\{E_a\}$ and non-vanishing overlaps $|\langle i | a \rangle| > 0$ for all $i, a$. If $U'(t) = e^{-iH't}$ satisfies $|U'_{ij}(t)|^2 = |U_{ij}(t)|^2$ for all $i, j$ and all $t$, then $H' = DHD^\dagger$ for a time-independent diagonal unitary $D = \text{diag}(e^{i\alpha_1}, \ldots, e^{i\alpha_n})$.*

*Proof.* Write $c_{ia} = \langle i | a \rangle$. Expanding in the energy eigenbasis:
...
**Corollary (time-dependent D-gauge completeness).** Under condition (C2), $H_{\text{eff}}(t)$ is approximately time-independent on intervals $\tau_S \ll \Delta t \ll \tau_B$. The D-gauge theorem applies within each interval; the cross-interval matching lemma — which is exact and algebraic — fixes the relative gauge at each junction up to a global phase. In the continuum limit $\Delta t \to 0$, the within-interval approximation error vanishes ($\mathcal{O}(\Delta t / \tau_B) \to 0$), and the chain of global phases becomes a smooth function $e^{i\theta(t)}$ — a time-dependent global phase that changes no transition probability, density matrix, or expectation value.

**Consequence.** The continuous-time transition probabilities $\{T_{ij}(t)\}_{i,j,t}$ determine the unitary propagator — for both time-independent and time-dependent effective generators — up to physically irrelevant rephasing. All gauge-invariant predictions of the emergent quantum mechanics are fixed by the process tensor alone.

**The dimensional obstruction and the role of Step 4.** The preceding results resolve all *dimensionless* ambiguity but leave $\hbar$ untouched — and this is a structural feature, not a gap. The unitary $U(t)$ is dimensionless; $\hbar$ enters only when defining $\hat{H} = i\hbar \, \partial_t U \cdot U^\dagger$, converting dimensionless rates into energy units. Different values of $\hbar$ yield different energies but the *same* $U(t)$ and transition probabilities. No quantity of dimensionless probability data can fix a dimensionful constant.
...
*Status: Phase uniqueness established (D-gauge theorem + cross-interval matching + (C2)). Volume-independence established (volume-independence lemma + (C3)). Scale determination via thermodynamic equilibrium (finite-dimensional, no continuum input), with lattice corrections bounded by $\mathcal{O}(10^{-122})$. No remaining gap in the derivation of $\hbar$.*

---

## 6. SELF-CONSISTENCY AND THE DISCRETENESS SCALE

Before proceeding, it is crucial to reiterate that the classical temperature $T_{\text{cl}}$ and the emergent quantum temperature $T_{\text{GH}}$ were derived from completely independent starting points before being equated. The value $\hbar = c^3 \epsilon^2 / (4G)$ is established by the chain of constraints in §5.2: partition-relativity, the structural/volumetric distinction, dimensional analysis, and thermal self-consistency. Rearranging:
...
**The observed baseline as existing evidence.** The dissolution is not a prediction awaiting future confirmation. The observed vacuum energy density has been measured since the Type Ia supernova surveys of 1998, and it sits at the classical geometric scale $\rho \sim H^2/G$ — exactly the value the framework expects. Any framework in which the quantum description is fundamental must explain why the vacuum energy is $10^{122}$ times smaller than the zero-point sum; no such explanation has been found in over five decades of searching, despite sustained effort including supersymmetric cancellation mechanisms (increasingly constrained by LHC null results), anthropic landscape arguments (which constrain the range of $\Lambda$ but do not predict the specific observed value), and unimodular gravity (which removes the absolute zero-point energy from the field equations by fiat rather than from a structural principle). The persistent absence of a cancellation mechanism is itself consistent with the framework's prediction that no such mechanism exists: the two numbers were never supposed to agree, because they describe different levels of the same reality. The observed baseline value, the precise identification of the discrepancy as $S_{\text{dS}}$, and the failure of all cancellation programs collectively constitute existing empirical evidence favoring an emergent quantum description over a fundamental one.
...
where $\nu_{\text{OI}}$ is a dimensionless coefficient encoding the rate at which the hidden-sector dimensionality responds to changes in $H$. This is precisely the form of the Running Vacuum Model (RVM) [20, 21].
...
*Magnitude.* Condition (C3) requires $\nu_{\text{OI}} \ll 1$: the fractional change in the emergent description per unit change in $N_{\text{modes}}$ must be small, or the hidden sector would be effectively saturated.

*Value.* The coefficient can be computed from the spectral structure of the partition boundary. The $S_{\text{dS}}$ horizon modes span frequencies from $\omega_{\text{IR}} = H$ (the horizon scale) to $\omega_{\text{UV}} = c/\epsilon$ (the discreteness cutoff). Massless field modes on the horizon 2-sphere have a conformal mode density: equal numbers of independent modes per logarithmic frequency interval. The number of spectral decades is:

$$\mathcal{N}(H) = \ln\frac{c}{\epsilon H}$$

The emergent vacuum energy — the gravitating residual of the trace-out compression — is distributed across these $\mathcal{N}$ independent spectral channels. As $H$ changes, $\mathcal{N}$ changes, and the noise per channel changes inversely. The effective vacuum energy therefore scales as:

$$\rho_\Lambda(H) \propto \frac{1}{\mathcal{N}(H)} = \frac{1}{\ln(c/\epsilon H)}$$

This is a non-perturbative result: the dependence on $H$ enters through the logarithmic spectral range of the trace-out, not through any power of $H/M_{\text{Pl}}$. Standard perturbative QFT gives corrections suppressed by $(H/M_{\text{Pl}})^2 \sim 10^{-122}$; the trace-out's spectral response gives $1/\ln(c/\epsilon H) \sim 1/140$, which is $10^{119}$ times larger.

Normalizing $\rho_\Lambda(H_0) = \Omega_\Lambda \rho_c$ and Taylor-expanding around $H_0$:

$$\frac{d\rho_\Lambda}{dH^2}\bigg|_{H_0} = \frac{\Omega_\Lambda \rho_c}{2\,\mathcal{N}(H_0)\,H_0^2}$$

Matching to the RVM form $\rho_\Lambda = \rho_\Lambda^0 + \frac{3\nu_{\text{OI}} c^2}{8\pi G}(H^2 - H_0^2)$ and using $\rho_c = 3H_0^2 c^2/(8\pi G)$:

$$\boxed{\nu_{\text{OI}} = \frac{\Omega_\Lambda}{2\,\ln(c/\epsilon H_0)}}$$

Evaluating with $\epsilon = 2\,l_p$, $H_0 = 67.4$ km/s/Mpc, and $\Omega_\Lambda = 0.685$:

$$\mathcal{N}(H_0) = \ln\frac{c}{2\,l_p\,H_0} \approx 139.6$$

$$\nu_{\text{OI}} = \frac{0.685}{2 \times 139.6} = (2.45 \pm 0.03) \times 10^{-3}$$

The result is insensitive to $H_0$: varying $H_0$ from 66 to 73 km/s/Mpc changes $\nu_{\text{OI}}$ by less than 0.1%, because $H_0$ enters only inside the logarithm. The uncertainty is dominated by $\Omega_\Lambda$.

For comparison, Solà Peracaula, Gómez-Valent, and de Cruz Pérez [21] fit the RVM to CMB, BAO, and SNIa data and consistently obtain $\nu_{\text{eff}} \sim \mathcal{O}(10^{-3})$, with the ΛCDM value $\nu = 0$ disfavored at $3$–$4\sigma$ in several dataset combinations. The framework's prediction $\nu_{\text{OI}} = 2.45 \times 10^{-3}$ lies within the phenomenological range from all major dataset combinations.

The formula yields an independently testable relationship: $\nu_{\text{OI}} / \Omega_\Lambda = 1/(2\,\ln(c/\epsilon H_0)) \approx 0.00358$. If future surveys measure both $\nu$ and $\Omega_\Lambda$ with sufficient precision, this ratio is falsifiable.

DESI's 2024–2025 data releases [22, 30] report evidence for evolving dark energy at $2.8\sigma$–$4.2\sigma$, with $w_0 > -1$ and $w_a < 0$ — the qualitative signature the RVM predicts. Direct RVM fits to the latest data have now been performed: de Cruz Pérez, Gómez-Valent, and Solà Peracaula [31] fit the RVM and several of its variants to DESI DR2 BAO combined with Planck PR4 CMB and both Pantheon+ and DES-Y5 supernovae, finding significant evidence for dynamical vacuum energy at $2.7\sigma$–$3.1\sigma$ depending on the model variant and dataset combination, with the $\Lambda$CDM value $\nu = 0$ disfavored at levels comparable to the generic $w_0 w_a$CDM parameterization. The framework's prediction $\nu_{\text{OI}} = 2.45 \times 10^{-3}$ lies within the phenomenological range from all major dataset combinations. The outstanding discriminating test is whether the one-parameter RVM form ($\Lambda_0 + \nu H^2$) fits the data as well as or better than the two-parameter $w_0 w_a$ model — a result that would favor the specific functional form this framework predicts over generic dark energy evolution — and whether the measured $\nu / \Omega_\Lambda$ ratio converges to the predicted value $1/(2\,\ln(c/\epsilon H_0)) \approx 0.00358$.
...
**What the theorem says.** The characterization theorem (§3.3) establishes a full equivalence: a stochastic process on a finite configuration space is quantum mechanics if and only if it arises from embedded observation under conditions (C1)–(C3). The sufficiency direction — that the axioms, finiteness, and (C1)–(C3) produce quantum mechanics — is the constructive result. The necessity direction — that any quantum system *requires* non-trivial coupling, slow-bath memory, and sufficient hidden-sector capacity when realized as a deterministic dilation — is what distinguishes the framework from programs that merely *permit* an epistemic interpretation of quantum mechanics. The Schrödinger equation, Born rule, and Bell inequality violations are structural consequences of the equivalence, not independent postulates.
...
The result $\epsilon = 2\,l_p$ is a *consequence* of the framework, not an input. The Planck length itself is a combination of $c$, $G$, and $\hbar$ — but here $\hbar$ is derived (§5.2), so $l_p$ is determined by the same self-consistency loop. The apparent circularity is broken because the classical ingredients ($c$, $G$, and $\epsilon$) enter before $\hbar$ is defined, and the relationship $\epsilon = 2\,l_p$ emerges after $\hbar$ has been fixed. No quantum concept is presupposed.

The deeper question is whether any framework claiming to derive quantum mechanics must posit *some* form of discreteness to avoid an infinite-dimensional configuration space — in which case the finiteness condition would be a necessary structural ingredient rather than an ad hoc addition. Holographic entropy bounds [27] provide independent, non-quantum motivation: the maximum entropy of a region bounded by area $A$ is $A/(4\,l_p^2)$, suggesting that finite-area boundaries enforce finite-dimensionality regardless of the theory used to describe the interior.

### 9.5 Observable Vacuum Energy Effects
