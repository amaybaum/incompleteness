...
**Lemma 1** (Finiteness). *The observer has finitely many distinguishable internal states, so the visible configuration space $\mathcal{C}_V$ is finite, with a discreteness scale $\epsilon$ providing a finite minimal cell volume.* This is taken as a structural postulate about the observer, not derived. The postulate is motivated empirically from two directions: an observer with finite memory can register only finitely many outcomes, and holographic entropy bounds [2] independently predict a finite Hilbert-space dimension for any bounded region. Strict classical field theory would permit infinitely many modes on a finite boundary, so the finiteness assumed here is at least as strong as a UV cutoff; the framework takes the cutoff as part of the definition of "observation" rather than as a consequence of quantum or gravitational reasoning imported from outside.
...
**Lemma 3** (Determinism and unique measure). *$\varphi$ is a bijection: distinct states have distinct successors (determinism) and distinct predecessors (reversibility). The counting measure on $S$ is the unique $\varphi$-invariant measure.* Determinism (injectivity) is the requirement that the dynamics preserves the observer's records — non-injective evolution would merge distinct total states, erasing the distinction between outcomes the observer has already recorded. On a finite set, injectivity implies surjectivity, so $\varphi$ is a bijection.

The paper uses two equivalent pictures of this setup and moves freely between them: the *discrete-finite picture*, in which $\varphi$ is a bijection on a finite set $S$ and the counting measure is the unique invariant measure; and the *continuum picture*, in which the finite set is viewed as the discretisation of a compact phase-space region $\Gamma$ with symplectic structure, the dynamics is a Hamiltonian flow $\phi_t$ on $\Gamma$, and the invariant measure is the Liouville measure $\mu$. The two agree in the standard sense: under coarse-graining by cells of size $\epsilon$ (Lemma 1), Liouville measure on $\Gamma$ reduces to counting measure on $S$, and a Hamiltonian flow $\phi_t$ on $\Gamma$ restricts to a bijection $\varphi$ on $S$ up to corrections of order $\epsilon$ per cell crossing. In the continuum picture:
...
**Convergence with prior work.** The theorem converges with independent results: Pechukas [6] (reduced dynamics need not preserve positivity), Rivas et al. [4, 5] (divisibility failure ↔ information backflow), Pollock et al. [7] (process-tensor Markovianity conditions), and Strasberg and Esposito [8] (non-Markovian reduced dynamics in the slow-bath regime of C2). Bylicka, Johansson, and Acín [9] prove that for *invertible* dynamical maps $\Lambda_t$ (those with $\det\Lambda_t \neq 0$), P-divisibility implies CP-divisibility. The substratum dynamics $\varphi$ is bijective on $\mathcal{C}_V \times \mathcal{C}_H$, but the marginal stochastic matrix $T(t)$ on $\mathcal{C}_V$ is generally not invertible — projecting onto the visible sector loses information. The Bylicka equivalence therefore applies only when $T(t)$ happens to have full rank (which includes the minimal model of §2.4 below, where $\det T(1) = 1/3 \neq 0$); for the general derivation here, the load-bearing property is P-indivisibility, which is exactly what §3.1's stochastic-quantum correspondence requires. Milz et al. [10] further show that even CP-divisibility does not guarantee Markovianity in the process-tensor sense, confirming that the non-Markovian character identified here is robust under all current definitions.

**Continuous-time extension.** The Hamiltonian flow on finite-dimensional phase space preserves Liouville measure on compact energy surfaces. $T_{ij}(t)$ is continuous with $T(0) = I$. By (C1), $T(t)$ departs from the permutation class for $t > 0$. Two routes establish P-indivisibility in continuous time. *Formal route:* Poincaré recurrence gives almost-everywhere return: for almost every $(x_i, h)$, the orbit returns arbitrarily close to its initial condition in finite time. In the strictly finite-discrete setting this strengthens to $\varphi^N = \mathrm{id}$ for some $N$, yielding exact return of every state and non-monotonic trace distance; in the continuous-time setting this delivers recurrence-in-measure that is adequate to exclude strict P-divisibility but not to establish observable backflow on laboratory timescales, because the relevant return time grows rapidly with the hidden-sector size. *Physical route:* the accessible-timescale lemma below, which uses (C2) and (C3) to show that information backflow occurs on timescales $k\tau_S \ll \tau_B$, many orders of magnitude shorter than any recurrence time. This is the load-bearing route for everything that follows; the formal route is retained only to rule out the degenerate limit in which $T$ is a permutation matrix.
...
*Proof.* The coupling $H_{\text{int}}$ transfers visible-sector information to the hidden sector at each interaction, at rate $\sim 1/\tau_S$. Between interactions, the hidden sector evolves under its own Hamiltonian $H_H$. In a closed finite-dim Hamiltonian system no true relaxation occurs (the spectrum is purely imaginary, recurrence is quasi-periodic); what occurs under the ETH assumption on $H_H$ is *dephasing* of the conditional distribution $\mu_H(\cdot|x_i)$ toward its diagonal ensemble on the timescale $\tau_B$ (the dephasing / Thouless time of the hidden sector, set by $\hbar/\Delta E$ where $\Delta E$ is the typical energy spread). The effective decay of visible-sector-accessible correlations between coupling events is therefore $e^{-\tau_S/\tau_B}$. When $\tau_S \ll \tau_B$, $\tau_S/\tau_B \ll 1$, so the decay per visible-sector transition is $1 - e^{-\tau_S/\tau_B} \approx \tau_S/\tau_B \ll 1$: correlations survive each step essentially intact. After $k$ transitions spanning a time $k\tau_S \ll \tau_B$, the cumulative decay is $e^{-k\tau_S/\tau_B} \approx 1 - k\tau_S/\tau_B$, which remains close to unity. The hidden sector therefore retains $\sim k$ bits of visible-sector history over this window, and the $(k+1)$-th transition reads back stored correlations through $H_{\text{int}}$, producing history-dependent transition probabilities — i.e., information backflow. More precisely, the mutual information satisfies:
...
**Role of (C2) and (C3).** The formal-route (recurrence-in-measure) and accessible-timescale arguments are independent: the former establishes P-indivisibility in principle from (C1) and finiteness alone; the latter, using (C2) and (C3), promotes it from a formal property to an observationally dominant one on laboratory timescales. Everything downstream in the paper uses the accessible-timescale version.
...
Conditions (G1)–(G3) fail only on a measure-zero set of Hamiltonians. A stronger argument applies whenever the partition itself breaks symmetries of $H_{\text{tot}}$. The genericity conditions concern the *effective* visible-sector Hamiltonian $\hat{H}_{\text{eff}}$ — the operator emerging after the trace-out — not the total Hamiltonian $H_{\text{tot}}$. The trace-out generically breaks the symmetries of $H_{\text{tot}}$ because the partition boundary does not respect them: a partition centered on a specific observer breaks translational invariance, rotational symmetry about distant points, and boost invariance. Any degeneracy in $H_{\text{tot}}$ that relies on correlations between visible and hidden sectors — which includes all degeneracies associated with symmetries the partition breaks — is lifted in $\hat{H}_{\text{eff}}$. The residual degeneracies of $\hat{H}_{\text{eff}}$ fall into two classes: (a) those associated with symmetries acting entirely within the visible sector that commute with the trace-out — these are precisely the gauge symmetries of the emergent description, correspond to physically equivalent states, and do not affect the phase-locking argument which operates on gauge-inequivalent transition data; and (b) accidental or discrete residual symmetries (e.g. reflections internal to $V$ that survive the trace-out) which would produce finite-order degeneracies. These are lifted generically by any small perturbation of the partition boundary, and in the physical realisations considered in [SM] and [GR] the partition is never exactly symmetric, so such residual symmetries are absent. Where they do survive, the phase-locking argument yields $H$ up to the corresponding finite permutation of eigenlabels, which is physically inert. $\hat{H}_{\text{eff}}$ therefore satisfies G1–G3 not by typicality in an abstract ensemble, but because the partition itself acts as a symmetry-breaking mechanism, with any remaining discrete ambiguity absorbed into gauge equivalence.
...
*Remark (scope).* The forward direction (Permutation Unitarity Lemma) and this reverse direction establish bidirectional correspondence at the **stochastic-process level**: a bijection on $\mathcal{C}_V \times \mathcal{C}_H$ with uniform prior yields a stochastic process on $\mathcal{C}_V$, and every stochastic process on $\mathcal{C}_V$ arises this way. The correspondence is not at the full CPTP-channel level — a permutation unitary with uniform ancilla maps basis-state inputs to diagonal outputs, so channels that create coherences between basis states from diagonal inputs have no direct realisation as $\mathrm{Tr}_H[U_\varphi(\rho_V \otimes I/m)U_\varphi^\dagger]$ with $U_\varphi$ a permutation. This is the correct scope for the characterization theorem (§3.4), which is formulated in terms of transition probabilities; full unitary QM on $\mathcal{H}_V$ is recovered as the effective description of the visible sector via the approximate-unitarity result later in this section, not by matching arbitrary abstract CPTP channels. The holographic bound $\dim \mathcal{H} \leq e^{A/4}$ bounds the size of the emergent $\mathcal{H}_V$ for any bounded region, giving finiteness of the visible sector. This is the reverse direction invoked by [Substratum, Theorem 23, Stage 1].
...
**Definition (unitarily evolving QM at the transition-probability level).** A stochastic process $\mathcal{S}$ on a finite configuration space $\mathcal{C}_V = \{x_1, \ldots, x_n\}$ is *mathematically equivalent to unitarily evolving quantum mechanics at the transition-probability level* if there exists a Hilbert space $\mathcal{H}_V$ of dimension $\leq n^3$, a Hermitian operator $\hat{H}$ on $\mathcal{H}_V$, and a unitary family $U(t) = e^{-i\hat{H}t}$ such that $T_{ij}(t) = |U_{ij}(t)|^2$ for all $i, j, t$ — i.e., computational-basis transition probabilities are exactly Born-rule probabilities. This definition captures the Hilbert space, unitary dynamics, and the Born rule for transitions between the classical-basis states $\mathcal{C}_V$ inherits from the partition. It does not, by itself, capture superposition inputs, entangled initial states, sequential-measurement state update, or multi-particle structure; these are additional quantum-mechanical structures discussed in the remarks following the theorem, where they are shown to emerge naturally from the construction (Stinespring route, §3.2) rather than requiring separate postulates, but their derivation lies outside the scope of the equivalence proved below.
...
In the double-slit experiment, the substratum dynamics is deterministic, and one may consistently assign the particle a definite trajectory through a single slit at the substratum level (this is an interpretive choice, not a theorem of the framework). The interference pattern arises because opening or closing the second slit changes the boundary conditions of the transition matrix, altering the distribution of detection events. A which-path detector at one slit couples the trajectory to additional visible-sector degrees of freedom, changing the transition matrix and eliminating the interference terms. In Wigner's friend, the Friend has a definite outcome; Wigner's superposition reflects his epistemic deficit. The Everettian measure problem dissolves: the system evolves as a single reality; "branches" are features of the compressed description. The Born rule, often treated as an independent postulate, is derived: it is the equilibrium distribution of the indivisible stochastic process (§3.1), not an additional assumption.

### 4.2 Scope

The characterization theorem is an equivalence at the transition-probability level: QM (in the sense of §3.4's definition) $\iff$ embedded observation under (C1)–(C3), modulo the ETH conditionality on (C2). The theorem does not identify which physical systems satisfy the conditions; this is an empirical question. Universality in our universe follows from the shared causal partition defined by null geodesics: the cosmological horizon provides a partition satisfying (C1)–(C3) for every embedded observer, with the quantitative estimates ($\tau_S/\tau_B \sim 10^{-32}$ laboratory-to-Hubble, $N_H \sim 10^{122}$) derived in [GR §§2–3].

### 4.3 The Status of the Discreteness Scale

$\epsilon$ is a structural postulate of the framework, not a derived consequence. It does not smuggle in *quantum-mechanical* content specifically — the framework makes no use of $\hbar$, wavefunctions, or amplitudes at the level of Lemma 1 — but it does assert a UV cutoff that is stronger than classical field theory would provide. The cutoff is motivated independently by the empirical observability of finite outcomes and by holographic entropy bounds [2], and it is the only way to instantiate the definition of observation on a world with finite-area boundaries. The framework's claim is that this cutoff is the minimum structural content needed to formalise embedded observation, not that finiteness is derivable without additional input.
