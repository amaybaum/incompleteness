...
**Step 4: Thermal self-consistency.** The classical substratum assigns the partition boundary a temperature $k_B T_{\text{cl}} = c^3 \epsilon^2 \kappa / (8\pi G)$ (§5.1), containing no $\hbar$. The emergent QFT of Part I lives on this classical background, which has a bifurcate Killing horizon with surface gravity $\kappa$. Regularity of the Wick-rotated metric at the horizon requires Euclidean period $\beta = 2\pi c / \kappa$; any QFT on this background — including a lattice-regularized one — must therefore be periodic in imaginary time with the same period, giving a KMS state at temperature $T_Q = \hbar \kappa / (2\pi c k_B)$, with $\hbar$ unknown. This is a theorem *within* the derived QFT, not an external import. A thermometer at the boundary is a visible-sector object whose click rate is computable in either description; the classical and emergent accounts of one boundary cannot disagree, so $T_{\text{cl}} = T_Q$:
...
### 8.3 The Dark Sector as Corroboration

The trace-out that produces QM has a gravitational consequence: the boundary entropy is invisible to the emergent QFT but gravitationally active. Part III shows that this accounts for the observed fact that ~95% of the universe's gravitational content has no source in particle physics — providing independent corroboration of the framework's structural claims.

---

# PART III: THE DARK SECTOR AS CORROBORATION

## 9. THE INVISIBLE GRAVITATIONAL BUDGET

Observationally, ~95% of the universe's gravitational content — the dark sector comprising dark energy (~68%) and dark matter (~27%) — has no source in quantum field theory. The baryonic sector accounts for only ~5% of $\rho_{\text{crit}}$. This section shows that the trace-out producing QM simultaneously and unavoidably renders the boundary entropy gravitationally active but invisible to the emergent description, with total energy density equal to $\rho_{\text{crit}}$. The framework does not set out to explain the dark sector; it produces the dark sector as an automatic consequence of the same mechanism that produces quantum mechanics.

Part II identified the $10^{122}$ cosmological constant discrepancy as the *information compression ratio* of the trace-out — the cost of describing the visible sector in the emergent quantum language. This section identifies a complementary quantity: the *gravitational occlusion fraction* — the share of the universe's gravitational budget that the trace-out renders invisible to the emergent description. The compression ratio measures how much the emergent description distorts what the observer can see; the occlusion fraction measures how much of what gravitates the observer cannot see at all. Together, they account for the two largest anomalies in modern cosmology as two aspects of a single phenomenon: the cost of observing the universe from within.
...
*Proof.* The horizon carries $S_{\text{dS}} = A/(4\,l_p^2) = A/\epsilon^2$ modes (§6). The thermal self-consistency of §5.2 establishes $T_{\text{GH}} = T_{\text{cl}}$; critically, the classical temperature $T_{\text{cl}} = c^3 \epsilon^2 H / (8\pi G k_B)$ is computed entirely within the classical substratum (§5.1) — the Jacobson identity, the entropy density $\eta = 1/\epsilon^2$, and the surface gravity $\kappa = cH$ — with no reference to $\hbar$ or the emergent quantum description. The total thermal energy is $E_s = S_{\text{dS}} \cdot k_B T_{\text{cl}}$. Distributing this over the enclosed volume $V_H = 4\pi c^3/(3H^3)$ gives the effective density:
...
In the regime where baryonic gravity dominates ($g_b \gg cH$), the boundary entropy's influence is a small perturbation. The partition boundary's surface gravity $cH$ is the only acceleration scale intrinsic to the partition geometry, suggesting a transition at $g_b \sim cH$; but whether the redistribution produces significant local effects — and if so, at what precise scale — requires a quantitative force law beyond the axioms (§9.4).
...
**Theorem (Invisible gravitational budget).** *Under Axioms 1–4 and conditions (C1)–(C3), the cosmological trace-out that produces quantum mechanics simultaneously produces a gravitational budget with the following properties: (i) total effective energy density $\rho_{\text{crit}}$ (§9.1); (ii) no source in the emergent QFT (§9.2); (iii) persistent entropy displacements by matter, producing history-dependent gravitational structure (§9.3).*

*Proof.* Each property is proved in the referenced subsection. $\square$

The framework accounts for the baryonic sector (~5% of $\rho_{\text{crit}}$) through the emergent QFT. The remaining ~95% is the boundary entropy — gravitationally active but invisible to the emergent description. Crucially, this ~95% gravitates because the boundary entropy's thermal energy is a classical quantity computed before the trace-out (§9.1, first clarification), unlike the QFT zero-point energy which is an artifact of the emergent description and does not gravitate (§7.2). This matches the observed composition of the universe, in which ~95% of the gravitational content (dark energy + dark matter) has no source in particle physics. The uniform component of the boundary entropy corresponds to dark energy (addressed in §7); the matter-induced redistribution (property (iii)) is a candidate mechanism for the structured component (dark matter), though the redistribution profile — and hence the dark energy/dark matter split — requires a quantitative force law beyond Axioms 1–4 [37], deferred to future work.

The concordance is non-trivial. The axioms were formulated to address embedded observation, not cosmological composition. That they produce a gravitational budget matching the observed dark sector is independent corroboration that observational incompleteness is capturing real physical structure. The parallel with the CC dissolution is exact: the $10^{122}$ compression ratio (§7.3) and the ~95% occlusion fraction are both mandatory consequences of the same trace-out, addressing the two largest anomalies in cosmology through a single mechanism.

**Falsifiability.** The invisible-budget prediction would be falsified by detection of dark matter particles accounting for the full ~27% (contradicting (ii)), or by discovery that the dark sector's gravitational effects are unrelated to the boundary geometry.
...
**The dark sector as corroboration (Part III).** The trace-out that produces quantum mechanics simultaneously renders ~95% of the universe's gravitational budget invisible to the emergent QFT: the boundary entropy has effective energy density $\rho_{\text{crit}}$, no QFT operator, and matter-induced displacements that persist through P-indivisibility. This gravitational occlusion fraction parallels the $10^{122}$ information compression ratio of Part II: one measures how much the trace-out distorts the emergent vacuum energy, the other how much of the gravitational universe the trace-out hides. Together they account for the cosmological constant problem and the dark sector — the two largest anomalies in modern cosmology — as two aspects of a single phenomenon.

The apparent conflict between quantum mechanics and general relativity, the $10^{122}$ vacuum energy discrepancy, and the dark sector are three faces of the same fact: the information-theoretic cost of observing the universe from within.
