...
Physical observers are subsystems of the universes they measure, permanently barred from accessing degrees of freedom beyond their causal reach. We prove that the structural constraints of this epistemic reduction necessarily give rise to quantum mechanics. An observer coupled to a slow, high-capacity hidden sector on a finite-dimensional configuration space must describe the visible sector using P-indivisible stochastic dynamics. By the stochastic-quantum correspondence, this is mathematically equivalent to unitary quantum mechanics.
...
**Weak-coupling and fast-bath regimes.** Known P-divisible systems with non-zero coupling illustrate the necessity of (C2), not a failure of the theorem. The Jaynes-Cummings model with Lorentzian spectral density is P-divisible when dissipation dominates coupling — the fast-bath regime where $\tau_B \ll \tau_S$, violating (C2). Similarly, the Davies-Merkli theorem proves reduced dynamics converge to a Markovian semigroup in the weak-coupling limit; this is the Born-Markov regime, which is again a fast-bath limit where bath correlations decay before the system evolves appreciably. The §3.3 necessity proof for (C2) establishes this formally: fast mixing drives conditioned hidden-sector distributions to uniformity, yielding P-divisible Markov chains. These cases confirm the theorem's structure rather than contradicting it — (C1) alone guarantees P-indivisibility on finite discrete systems, but (C2) is required for it to survive in physically relevant regimes.
...
**Finite-dimensionality.** Barandes' proof requires a finite-dimensional configuration space: Axiom 2 in Part I, the horizon's finite area ($A/\epsilon^2$ modes) in Part II. Calvo [12] extends the correspondence to infinite dimensions, but this is not required here.
...
**Fine-tuning and causal structure.** The framework does not claim to restore Bell locality: outcome independence is genuinely violated, and this constitutes nonlocality in Shimony's sense. What the framework provides is a *derivation* of this nonlocality from the causal partition structure, rather than treating it as axiomatic. The fine-tuning objection — that any classical causal model reproducing Bell violations while preserving no-signaling requires precise parameter adjustment (Wood and Spekkens, 2015) — assumes the causal structure admits a directed acyclic graph (DAG) with Markov factorization. P-indivisible processes carry irreducible multi-time correlations that violate the Markov condition on any DAG; the appropriate causal framework is the process tensor [8], not the classical DAG. Within the process tensor framework, no-signaling is an automatic consequence of the local marginalization structure, not a fine-tuned coincidence. The OI violation is not partial but saturates Tsirelson's bound — consistent with recent results showing that partial OI relaxation is experimentally excludable.
...
**Step 2: $\hbar$ is structural, not volumetric.** The action scale governs the stochastic-to-unitary conversion at the boundary, not total hidden-sector capacity. This is made precise by:

**Lemma (volume-independence).** *Decompose $\mathcal{C}_H = \mathcal{C}_B \times \mathcal{C}_D$ into boundary and deep modes. If $\pi_V(\varphi_t(x, h_B, h_D)) = \pi_V(\varphi_t(x, h_B, h'_D))$ for all $h_D, h'_D$ and $t \leq t_*$, then $T_{ij}(t) = T^{(B)}_{ij}(t)$ for $t \leq t_*$.*

*Proof.* The indicator in $T_{ij}(t) = \frac{1}{|\mathcal{C}_B||\mathcal{C}_D|} \sum_{h_B, h_D} \mathbf{1}[\pi_V(\varphi_t(x_i, h_B, h_D)) = x_j]$ is independent of $h_D$; the $\mathcal{C}_D$ sum and prefactor cancel. $\square$
...
**Why not circular.** The derivation is a gap equation with $\epsilon$ as the free geometric input and $\hbar$ as the output. Two potential circularity charges should be addressed explicitly.

*(i) "The Gibbons-Hawking temperature contains $\hbar$."* It does — and that is the point: $T_{\text{GH}}(\hbar)$ is a prediction of the emergent QFT with $\hbar$ as an unknown parameter. $T_{\text{cl}}(\epsilon)$ is derived independently from the classical substratum with no reference to quantum mechanics. The matching condition $T_{\text{cl}} = T_{\text{GH}}$ then *solves for* $\hbar$ in terms of $\epsilon$, $c$, and $G$. This is the same logical structure as a BCS gap equation: one side is the microscopic input, the other the emergent prediction, and the self-consistency condition fixes the emergent parameter.

*(ii) "The result $\epsilon = 2\,l_p$ contains $\hbar$."* The Planck length $l_p = \sqrt{\hbar G/c^3}$ is not an input; it is the *statement of the result* in conventional units. The derivation's input is $\epsilon$ (Axiom 2); its output is $\hbar = c^3 \epsilon^2/(4G)$. Rewriting the output as $\epsilon = 2\,l_p$ merely translates the result into familiar language. Substituting back and obtaining $\hbar = \hbar$ confirms algebraic consistency, not circularity — just as deriving $E = mc^2$ and then verifying $m = E/c^2$ is not circular.

*(iii) Jacobson's inputs.* Jacobson's original derivation [16] uses $T_{\text{Unruh}} = \hbar a/(2\pi c)$ and $S_{\text{BH}} = A/(4 l_p^2)$. This paper does not use those forms. It uses the classical thermodynamic identity $dE = (c^2 \kappa / 8\pi G)\,dA$ (which follows from Einstein's equations alone) and the classical entropy density $\eta = 1/\epsilon^2$ (Axiom 2). The resulting $T_{\text{cl}}$ is $\hbar$-free. The $\hbar$-containing Unruh and Bekenstein-Hawking formulas are *recovered* after the matching, not assumed before it.
...
Joint confirmation of dark energy evolution *and* GW echoes would uniquely favor an information-theoretic origin: standard RVMs predict $\Lambda(H)$ running but have no mechanism for discreteness-scale echoes.
...
**The Higgs potential.** The strongest form of the cosmological constant objection is not the zero-point sum but the electroweak Higgs potential: symmetry breaking shifts $V(\phi)$ by $\sim (200\;\text{GeV})^4$, which exceeds the observed $\Lambda$ by $\sim 55$ orders of magnitude. This is not speculative vacuum fluctuation energy — it is the classical potential energy of a measured scalar field. The framework's response follows from its ontological ordering: the Higgs field, like all quantum fields, is part of the emergent QFT description (§7.1). The classical substratum has deterministic dynamics on a finite configuration space; the degrees of freedom that the emergent theory organizes as a scalar field with spontaneous symmetry breaking have a different, non-field-theoretic character in the substratum. The vacuum energy shift $\Delta V \sim (200\;\text{GeV})^4$ is therefore a property of the emergent ground state — the same category as the zero-point sum, though arising from a different mechanism within the emergent theory. This is a strong ontological commitment that the framework demands: if QM is emergent, then so is the Higgs mechanism and its vacuum energy. An empirical test distinguishing the Higgs sector's gravitational coupling from its gauge coupling would provide direct evidence for or against this commitment.
...
The framework offers an alternative reading. Tracing out across a geometric boundary produces entanglement entropy proportional to boundary area ($A/\epsilon^2$ modes), so the Ryu-Takayanagi formula *can* be read as geometry assembling entanglement. But the stronger response is a scope distinction. In AdS/CFT, the boundary observer is *external* — the CFT is complete, with no hidden sector and no causal partition. By the present theorem's own logic, an observer with no hidden sector has no emergent quantum description; the boundary theory's quantum mechanics is fundamental precisely because the observer is external and immune to the inferential limits of embedded observers [19]. The "it from qubit" program's results — Ryu-Takayanagi, Van Raamsdonk's disconnection argument, tensor networks, quantum extremal surfaces — characterize this external-observer regime. The present framework applies to the complementary regime: observers *inside* the bulk, bounded by horizons, where AdS/CFT's own reconstruction tools are least well-developed. The two orderings (entanglement-first for external observers, geometry-first for embedded observers) are not contradictory but describe different epistemic situations. Witten's Type II algebra program achieves finite entropy without finite-dimensional Hilbert spaces; this is compatible with the present framework, since its predictions depend on finite entropy (the physical content of Axiom 2), not on the specific mathematical realization.
