...
**Finiteness condition.** Any observer bounded by a finite-area surface can couple to only finitely many modes across that boundary. Holographic arguments independently require the Hilbert space of quantum gravity to be locally finite-dimensional [27]. These physical constraints motivate a single mathematical requirement: the effective configuration space of the visible sector is finite-dimensional, with a discreteness scale $\epsilon$ providing a finite minimal cell volume. This condition enters the proof at a single point — the stochastic-quantum correspondence (§3.1), which requires a finite configuration space. In Part I, $\epsilon$ is a free parameter; its value is determined *a posteriori* by self-consistency (§6), which pins $\epsilon = 2\,l_p$ exactly. In Part II, the cosmological horizon provides the finite-area boundary that makes the condition a consequence of the partition geometry rather than an independent assumption.
...
**Finite-dimensionality.** Barandes' proof requires a finite-dimensional configuration space. In Part I, the finiteness condition guarantees this directly. In Part II, the horizon's finite area bounds the coupled modes to $A/\epsilon^2$. Calvo [12] has extended the correspondence to infinite-dimensional systems, but the extension is not required here.
