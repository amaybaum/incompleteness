...
The D-gauge is physically trivial: $U \to DUD^\dagger$ corresponds to a time-independent basis rephasing that leaves all transition probabilities, energy-level spacings, and interference patterns invariant. The theorem requires non-degenerate eigenvalues and non-vanishing overlaps $|\langle i | a \rangle| > 0$; both hold generically (the degenerate set has measure zero).
...
**AdS/CFT.** The boundary CFT's completeness follows because the boundary observer is *external* to the system, immune to the inferential limits constraining embedded observers [19]. The holographic description works precisely because no causal partition restricts the boundary observer's access.

### 9.7 Assumptions, Limitations, and Falsifiability

**The theorem** requires the finiteness condition (independent assumption in Part I; consequence of the partition geometry in Part II) and Barandes' correspondence (entering at a single step, §3.1). The genericity assumptions — non-degenerate spectrum and non-vanishing configuration-basis overlaps — hold for all but a measure-zero set of systems. The tensor product structure of the emergent QFT follows from spatial locality and is standard for lattice systems; a rigorous proof that Barandes' correspondence preserves spatial locality is expected but not provided here (required for the CC dissolution but not for the derivation of $\hbar$).

**The Planck-scale ordering.** The framework's ontological commitment — classical spacetime prior to quantum mechanics — may break down at the Planck scale. The framework does not address the deep Planck regime; its domain is the macroscopic causal structure of the cosmological horizon. The self-consistency loop fixes $\epsilon$ at $2\,l_p$, above the scale where the ordering might break down.
