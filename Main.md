...
**Tier 3 (consistency checks and solution-specific elements).** The dark-sector concordance (~95% of $\rho_{\text{crit}}$, §8.3), the dark energy parameter $\nu_{\text{OI}}$ (§8.1), the GW echo timescale (§8.2), and the dark matter interpolation steepness (§8.4, Step 4). These are derived consequences or solution-specific elements; they would be falsified by observation but do not carry the same deductive weight as Tiers 1–2.
...
These constraints narrow the interpolation to a one-parameter family indexed by the transition steepness — a property of the particular bijection $\varphi$, in the same category as fermion masses (Fundamental, §9.7). A representative member:
