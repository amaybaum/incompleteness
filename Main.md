...
**Step 1: Uniqueness from partition-relativity.** By the lemma of §1.4, the three inputs — dynamics $\phi_t$, Liouville measure $\mu$, and partition $\Gamma_V \times \Gamma_H$ — yield a single stochastic process, which maps to a single quantum description. The action scale $\hbar$ is therefore a derived quantity, not a free parameter.
...
**The theorem** requires Axiom 2 (independent assumption in Part I; consequence of the partition geometry in Part II) and Barandes' correspondence (entering at a single step, §3.1). The genericity assumptions — non-degenerate spectrum and non-vanishing configuration-basis overlaps — hold for all but a measure-zero set of systems. The tensor product structure of the emergent QFT follows from the locality preservation theorem (§7.1), which shows that spatial locality of the classical substratum is inherited by the emergent quantum Hamiltonian.

**The Planck-scale ordering.** The framework's ontological commitment — classical spacetime prior to quantum mechanics — may break down at the Planck scale, where the distinction between "geometric" and "quantum" degrees of freedom loses operational meaning. The framework does not address the deep Planck regime; its domain is the macroscopic causal structure of the cosmological horizon. The self-consistency loop fixes $\epsilon$ at $2\,l_p$, at the boundary of the regime where the ordering might break down. If a deeper theory unifies geometry and quantum mechanics at sub-Planckian scales, the present framework would describe the effective macroscopic regime in which the geometric description is valid and the quantum description emerges from it — analogous to how thermodynamics describes the macroscopic regime of statistical mechanics without being invalidated by it.
