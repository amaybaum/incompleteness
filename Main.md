...
**Cumulative evidence for the ordering.** The geometry-first ordering produces five independent consequences that match observation, none of which were designed into the axioms: (1) the CC dissolution — the observed vacuum energy sits at $\rho \sim H^2/G$, the classical baseline, without fine-tuning (§7.3); (2) the Bekenstein-Hawking formula including the $1/4$ factor, derived from $\epsilon = 2\,l_p$ (§6); (3) the value of $\hbar$, determined by partition geometry rather than treated as a free parameter (§5.2); (4) dark energy evolution with $\nu_{\text{OI}}$ in the range consistent with DESI data (§8.1); (5) a testable GW echo prediction at the discreteness scale (§8.2). A further consequence — that ~95% of $\rho_{\text{crit}}$ is invisible to the emergent QFT, matching the observed dark sector fraction — is noted in §9 without full development. Quantum-first orderings produce the $10^{122}$ discrepancy, have no natural account of the dark sector, and treat $\hbar$ and the $1/4$ factor as unexplained inputs. The cumulative case is not a single tiebreaker but a pattern: every quantitative output of the geometry-first ordering is consistent with observation; the central quantitative output of the quantum-first ordering is the worst prediction in physics.
...
### 8.3 A Gravitational Consequence

The trace-out that produces QM has an additional consequence for the gravitational budget, noted without full development in §9.

---

## 9. COROLLARY: THE INVISIBLE GRAVITATIONAL BUDGET

The trace-out that produces quantum mechanics has a gravitational consequence that we note here without full development.

The horizon carries $S_{\text{dS}} = A/\epsilon^2$ modes at the classical temperature $T_{\text{cl}} = c^3 \epsilon^2 H / (8\pi G k_B)$ (§5.1). The total thermal energy, distributed over the Hubble volume, gives an effective density:

$$\rho_s = \frac{A}{\epsilon^2} \cdot \frac{c^3 \epsilon^2 H}{8\pi G} \cdot \frac{3H^3}{4\pi c^3} = \frac{3c^2 H^2}{8\pi G} = \rho_{\text{crit}}$$

This calculation uses only pre-trace-out quantities; it does not invoke the emergent quantum description. The boundary modes are hidden-sector degrees of freedom — the variables traced out to produce QM — so by construction, no operator on $\mathcal{H}_V$ corresponds to their gravitational influence. Under the geometry-first ordering of §7.2, the boundary entropy's thermal energy gravitates at the classical level but contributes no term to the emergent $\langle \hat{T}_{\mu\nu} \rangle$.

The result is that the trace-out renders a fraction of the gravitational budget invisible to the emergent QFT. The baryonic sector accounts for ~5% of $\rho_{\text{crit}}$ through the emergent description; the remaining ~95% is boundary entropy — gravitationally active but invisible to the emergent theory. This matches the observed composition of the universe, in which ~95% of the gravitational content has no source in particle physics.

Two caveats are essential. First, $\rho_s = \rho_{\text{crit}}$ is an algebraic consequence of the Friedmann equation and the gap equation $\hbar = c^3\epsilon^2/(4G)$; it is a consistency check, not an independent prediction. Second, the framework identifies the total invisible budget but does not provide a quantitative account of its decomposition into dark energy (~68%) and dark matter (~27%). The uniform component maps onto dark energy (§7); whether matter-induced redistribution of the boundary entropy [37] produces local gravitational effects resembling dark matter is an open question requiring a force law beyond Axioms 1–4, deferred to future work.
...
**A gravitational corollary (§9).** The trace-out that produces quantum mechanics simultaneously renders ~95% of the universe's gravitational budget invisible to the emergent QFT. The boundary entropy has effective energy density $\rho_{\text{crit}}$ and no QFT operator. This matches the observed dark sector fraction, though the decomposition into dark energy and dark matter remains an open problem.

The apparent conflict between quantum mechanics and general relativity and the $10^{122}$ vacuum energy discrepancy are two faces of the same fact: the information-theoretic cost of observing the universe from within. The dark sector fraction may be a third.
