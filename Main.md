...
**Resolution of the tension.** The apparent tension is resolved quantitatively by the morphology mismatch between the search and the signal. The Wu et al. [45] search targets long-lived quasi-monochromatic oscillations persisting after the ringdown — effectively a damped sinusoid with $\tau_{\text{LL}} \gg \tau_{\text{QNM}}$. The OI echo is a *train of short bursts* — each lasting only $\tau \sim 3.8$ ms (the physical QNM damping time), separated by $\sim 45$ ms of silence, with alternating phase $(-1)^n$. The fitting factor between the OI template and a single damped sinusoid depends strongly on the template's assumed damping time: for $\tau_{\text{LL}} \sim \tau_{\text{QNM}}$, the best overlap is $\text{FF} \approx 0.83$; for $\tau_{\text{LL}} \sim 10\,\tau_{\text{QNM}}$, $\text{FF} \approx 0.50$; and for $\tau_{\text{LL}} \sim 10^2\,\tau_{\text{QNM}}$ (the regime targeted by model-independent searches for long-lived echo-type signals), $\text{FF} \approx 0.19$. The long-lived QNM search therefore recovers only $\sim 19\%$ of the optimal SNR against the OI template, a $5.3\times$ loss. For GW250114: the matched-filter echo SNR is $\sim 15$ (detectable), but the long-lived QNM search SNR is $\sim 15 \times 0.19 \approx 2.8$ — well below any detection threshold. The non-detection is **fully consistent** with the OI prediction *for the specific class of long-lived templates used by existing model-independent searches*; a dedicated search using the OI template directly remains the definitive test.

| Parameter | Formula | $M = 62.7 M_\odot$, $a/M = 0.68$ |
|---|---|---|
| $\Delta t$ | $(r_+/c)\ln(r_+/2l_p)$ | 48.9 ms |
| $f_{\text{QNM}}$ | Berti+2006 ($l=2$, $m=2$) | 271 Hz |
| $\tau$ | Berti+2006 | 3.76 ms |
...
**Observational status.** Model-independent searches for post-merger echoes in GW150914, GW231226, and GW250114 find no statistically significant evidence [45]. This non-detection is consistent with the OI prediction because the searches target long-lived QNMs (single spectral line), while the OI echo is a comb of $\sim 13$ spectral teeth — the $5.3\times$ SNR loss (fitting factor $\approx 0.19$ against long-lived templates, as discussed above) pushes the predicted signal below the search sensitivity. A dedicated matched-filter search using the OI template on GW250114 data would recover the full echo power at predicted SNR $\sim 15$ and provide the definitive test. If matched-filter SNR $< 5$ with the OI template, the prediction is falsified. Joint confirmation of dark energy evolution ($\nu_{\text{OI}}$) *and* GW echoes would uniquely favor an information-theoretic origin.

**Independent horizon-physics signals in GW250114.** Two recent analyses of GW250114 detect non-trivial structure beyond the standard quasi-normal-mode spectrum at amplitudes consistent with the OI predicted scale. Lu et al. [57] report observational evidence for a "direct wave" component oscillating near $2\Omega_H$ (the horizon angular frequency) and decaying at the surface gravity $\kappa$, with matched-filter SNR $14.0^{+0.2}_{-0.1}$ in LIGO Hanford and $13.5^{+0.1}_{-0.2}$ in Livingston — the same SNR range the OI echo is predicted to occupy ($\sim 15$ for GW250114). The Lu et al. signal is a different proposed horizon effect with different morphology (smooth damped sinusoid rather than the OI comb), but its detection establishes that horizon-scale structure beyond the standard QNM picture is detectable in current data at the predicted scale, and that the LIGO-Virgo-KAGRA analysis infrastructure for non-standard horizon physics on GW250114 is operational. Wang et al. [58] independently detect six quadratic quasi-normal modes in the GW250114 ringdown at Bayes factor 74, demonstrating that the post-merger signal contains nonlinear structure requiring more than the linear $(\ell, m, n) = (2, 2, 0/1)$ modes — the standard ringdown analysis is incomplete. The standard area-law confirmation from Abac et al. [59] independently verifies the Bekenstein-Hawking entropy formula derived in §6, including the factor of $1/4$. None of these results is the OI echo, but each tightens the empirical context: horizon physics beyond standard GR is now a tractable observational target, GW250114 has the SNR to detect it, and the predicted OI echo scale is comparable to what is being seen in adjacent analyses. The dedicated OI matched-filter search on GW250114 remains the highest-leverage near-term test.
