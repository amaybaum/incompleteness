...
\textbf{Cosmological commitment, with a tightened magnitude bracket.}
The dark energy sector is subtler than Classes A--C. The framework
commits to the Type II running-vacuum functional form (GR §7.1), with
magnitude given at theorem level by the emergent-QFT calculation
\(|\nu_{\rm QFT}| \sim (M_{\rm SM}/M_{\rm Pl})^2 \sim 10^{-32}\),
observationally indistinguishable from zero. Earlier framework-internal
dimensional arguments suggesting \(|\nu| \sim 10^{-3}\) (v1.6.4) and
\(|\nu| \sim 10^{-4}\) (v1.6.5--1.8.x) have been retracted; both relied
on a \(1/\mathcal{N}_0\) suppression that imports the bulk emergent-QFT
decade structure as a property of the boundary mode count, which §3-§5's
spatial mode counting (one mode per \(\epsilon^2\) cell, no inherent
frequency assignment) does not support (see GR §7.1 Retraction notes).
The framework's theorem-level magnitude prediction is therefore
...
measurement \(|\nu| > 10^{-3}\) at high significance would also exceed
the retracted dimensional-argument range; a measurement of \(\nu\) with
high-significance positive sign would falsify the Type II functional
form itself, which is a structural commitment rather than a magnitude
one. The first two outcomes are refinements within the current bracket;
the third is a structural falsification.
