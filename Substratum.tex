...
mechanics with Bell violations, finite boundary entropy, spatial
isotropy and homogeneity, and the observed Standard Model structure
--- uniquely determines the equivalence class
\([(S, \varphi)]/\mathcal{G}_{\text{sub}}\) at the lattice level, with
the structural conditions (finiteness, determinism, bounded coupling,
center independence, linearity, background independence) following
from the empirical inputs combined with the framework's gauge
structure rather than being independent commitments; the Standard
Model gauge group
\(\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)\) and
\(\bar\theta = 0\) are forced retrodictions, and the anomaly-free
hypercharge assignment is forced once the observed family pattern is
given (fermion embedding closed via the link-carrier construction,
with the absolute generation count locked at exactly three by
coupling-degree minimality); and (ii) the \emph{substratum gauge
group} \(\mathcal{G}_{\text{sub}}\) (Theorem 24), with four explicit
families of generators proved to exhaust all observables-preserving
...
violations, finite boundary entropy, spatial isotropy and homogeneity,
and the observed Standard Model structure uniquely determine the
equivalence class of bijections on a lattice, with the structural
conditions of the framework (finiteness, determinism, bounded coupling,
center independence, linearity, background independence) following
from the empirical inputs combined with the framework's gauge
structure (per §3.1). The substratum gauge group identifies the kernel
of this inverse map --- four families of transformations of
\((S, \varphi)\) that exhaust all observables-preserving operations
...
is not chosen from a landscape, the gravitational thermodynamics are
not appended to a quantum description, and the emergent QM is not
...
\([(S, \varphi)] / \mathcal{G}_{\text{sub}}\). What is real is the
equivalence class, not any specific substratum representative. This is
the strongest claim the framework's own theorems license, and it is
forced by them: the reconstruction theorem (§3) shows that the
equivalence class is uniquely determined by the empirical inputs
(with the structural conditions A1--A6 following from those inputs
combined with the framework's gauge structure, per §3.1), and the
substratum gauge group (§4) shows that different representatives of
the class are observably indistinguishable. The class is what the
data pin down; individual substrates are gauge representations of it.
A stronger claim --- realism about a specific bijection --- commits
to features the framework's own gauge group identifies as
unobservable. A weaker claim --- pure explanatory economy without
ontic commitment --- undersells what the uniqueness theorem actually
establishes.
...
$\phi$)}\label{the-physical-interpretation-of-s-ux3c6}}

\textbf{Storage and memory.} S is the set of all distinguishable states:
\emph{finite capacity}. $\phi$ is a bijection: \emph{perfect memory} ---
information is never created or destroyed. Together, (S, $\phi$) is a finite
...
storage capacity of the partition boundary. The action scale $\hbar$ is the
...
\textbf{The substrate objection.} ``What is the memory made of?'' (S, $\phi$)
is a \emph{complete description} of reality --- it determines all
observables. Space, time, matter, and energy are derived from (S, $\phi$), so
they cannot appear in its definition without circularity. Whether (S, $\phi$)
...
access window, $\phi$ is the transition function. But three differences are
...
irreversible (it can erase, overwrite, and halt); $\phi$ is a bijection ---
nothing is erased, nothing is created, there is no halting. Third, a
Turing machine computes an \emph{extrinsic function} (input $\to$ output for
an external user); (S, $\phi$) computes no extrinsic function --- it is a
...
tape, $\phi$ is a reversible transition function, and C1--C3 characterize the
...
\textbf{The arrow of time.} The substratum has no arrow of time --- $\phi$
and $\phi^{-1}$ are equally valid. Observers satisfying C1--C3 are nonetheless
...
(E5) \textbf{Spatial homogeneity.} Observed physics is
translation-invariant; no spatial location is preferred. Together with
E4, this is the standard ``no preferred frame'' content of observed
physics; we separate E4 (rotations) from E5 (translations) because they
enter the reconstruction at different stages.

(E6) \textbf{Standard Model structure observed.} The observed gauge
group is \(\text{SU}(3) \times \text{SU}(2) \times \text{U}(1)\) with
three chiral generations of matter and one Higgs doublet. This is a
specific empirical fact about our universe (rather than a logical
consequence of E1-E5); it serves as the framework's empirical target
...
\emph{Forced by E3} under the standard holographic interpretation of
finite boundary entropy as a Hilbert-space dimension cutoff
\(\dim \mathcal{H} \leq e^{A/4}\).

(A2) \textbf{Determinism.} \(\varphi: S \to S\) is a bijection
(deterministic, reversible dynamics). \emph{Forced by E1 and the
classical-substratum descent argument} (parallel to §4 Theorem 24's
completeness proof). Stinespring dilation lifts the visible-sector CPTP
channel to a unitary \(U\) on \(\ell^2(S)\); for \(U\) to descend to
an \(S\)-level transformation preserving classical product structure
(rather than producing superpositions of classical configurations),
\(U\) must be a permutation matrix --- the only unitaries on
\(\ell^2(S)\) that map basis states to basis states. Permutation
matrices correspond bijectively to bijections \(\varphi: S \to S\).
Probabilistic kernels lift to non-permutational unitaries (or to
non-unitary CPTP channels) and do not descend to classical-substratum
transformations.

(A3) \textbf{Bounded coupling degree.} Each site is coupled to a bounded
number of neighbors in \(\varphi\)'s action. \emph{Forced by E1 and E2
with the finite light-cone speed implicit in standard QM}: infinite
coupling at substratum level produces instantaneous correlations
everywhere, inconsistent with finite signal propagation.

(A4) \textbf{Center independence.} The dynamics \(\varphi\) commutes
with lattice translations up to gauge. \emph{Forced by E5}: a substratum
dynamics with a preferred center site would, under the trace-out,
produce a preferred-location effect inconsistent with E5.

(A5) \textbf{Linearity.} The wave equation for \(\varphi\) is linear.
\emph{Forced by the \(q\)-gauge invariance theorem of [SM §2.7]}: the
alphabet size \(q\) is gauge --- no observable depends on \(q\).
Non-linear dynamics over \(\mathbb{Z}/q\mathbb{Z}\) generically have
\(q\)-dependent emergent physics (\(x^2 \bmod 3\) and \(x^2 \bmod 5\)
have different algebraic structure), so non-linear emergent physics
would violate the gauge equivalence. [SM §4.1] gives the necessity
argument and three additional independent criteria (maximum propagation
speed, maximum P-indivisibility, KMS condition for Unruh thermality)
all converging on the same dynamics.
...
\emph{Forced by E1 with the observation that local gauge invariance
holds in the emergent QFT}: a local-gauge-invariant emergent QFT
requires local-symmetry-invariant substratum dynamics.

A1, A3, A4, A6 follow from the empirical inputs E1-E5 (as established
by the parentheticals above). A2 follows from E1 and the
classical-substratum descent argument. A5 follows from the
\(q\)-gauge invariance theorem of [SM §2.7]. The reconstruction's
substantive content is therefore the foundational ontology --- the
world is a classical-substratum-with-embedded-observer system ---
together with the empirical inputs E1-E6.

The reconstruction theorem's claim is: the
classical-substratum-with-embedded-observer ontology together with the
observed Standard Model uniquely determines the substratum modulo
gauge \(\mathcal{G}_{\text{sub}}\).
...
\item
  ...
  process (Bell violations $\to$ P-indivisibility)
\item
  {[}Main §3.2{]} Forward Stinespring dilation (bijection $\to$ CPTP
  channel) and reverse-direction lemma (stochastic process $\to$ bijection)
\item
  {[}Main §3.4{]} Characterization theorem (Bell violation $\Rightarrow$ C1, QM $\Rightarrow$ C2
  + C3, conditional on ETH for C2)
\item
  {[}SM §3.2{]} Coupling-graph dimension $\to$ d = 3
\item
  {[}SM §4.1{]} Center independence + isotropy + linearity $\to$ wave
  equation
\item
  {[}SM Theorems 5--15{]} Gauge group, generations, hypercharges
  derivation chain
\item
  {[}SM Theorems 17--21{]} T-invariance $\to$ \(\bar\theta = 0\)
\item
  {[}GR §3{]} Gap equation from thermal self-consistency $\to$
...
\subsubsection{3.2 Stage 1: Observed QM $\to$ deterministic embedding with
...
\subsubsection{3.3 Stage 2: Embedding + isotropy $\to$ specific lattice +
gauge
...
\textbf{Remark} (Across-class uniqueness). Each step (a)--(f)'s
``iff'' is over the class of substrata satisfying the framework's
foundational commitments (E1--E6), not over a smaller subclass. Step
(a)'s coupling graph is structurally forced by Stage 1 (Stinespring
lift + classical-substratum descent of §3.2), not a substratum-type
restriction; alternative substratum descriptions (tensor networks,
spin networks, causal sets) reduce to graph descriptions through
their site-coupling or order-relation structure. Step (b)'s wave
equation is uniquely selected by linearity (forced by \(q\)-gauge
invariance, [SM §2.7]), isotropy (E4), and homogeneity (E5);
alternative dynamics violate one of these constraints. Steps (c)--(f)
compose unique selections at the algebraic and representation-theoretic
level --- coupling-degree minimization for \(K = 2d\), cubic-group
commutant for the gauge group, anomaly cancellation for hypercharges,
T-invariance (from A2 bijection structure) for \(\bar\theta = 0\).
None of these is a framework-specific choice. The chain establishes
uniqueness across the class of all substrata consistent with E1--E6.

\textbf{Status:} Theorem at the lattice level.

\hypertarget{stage-3-dynamics-emergent-fundamental-constants}{%
\subsubsection{3.4 Stage 3: Dynamics $\to$ emergent fundamental
...
the same observed physics (E1--E6) satisfying the structural assumptions
...
\textbf{Theorem 23} (Layered reconstruction). \emph{Let E1--E6
(empirical inputs: QM with Bell violations, finite boundary entropy,
spatial isotropy and homogeneity, observed Standard Model structure)
hold. Then the structural conditions A1--A6 are forced (per §3.1's
analysis) and the equivalence class
...
The reconstruction's claim is that within the class of finite
deterministic bounded-coupling systems with the structural assumptions,
the substratum is unique. The structural conditions A1-A6 are all
consequences of the empirical inputs E1-E5 combined with the
framework's gauge structure ([SM §2.7] \(q\)-gauge invariance, §4
Theorem 24's \(\mathcal{G}_{\text{sub}}\) classical-substratum descent
restriction), per the analysis of §3.1. Equivalently, the
reconstruction's content is ``the classical-substratum-with-embedded-
observer ontology together with the observed Standard Model (E6)
determines the substratum modulo gauge \(\mathcal{G}_{\text{sub}}\).''
Challenging this claim requires challenging either the empirical
inputs E1-E6 or the framework's gauge-structure theorems on which the
structural-condition derivations rest.
...
with observation. The framework's claim is that E1--E6 is a consistent
set of empirical inputs producing our observed physics, with the
structural conditions A1--A6 forced by the inputs (per §3.1's analysis);
alternative input sets are not investigated.
...
inputs). The empirical inputs E1--E6 are stated as structural facts
...
rotational invariance and translational homogeneity, and the observed
Standard Model structure. Operationally, no finite observation set
fully establishes any of these; finite measurements support them only
in the sense of being consistent with the data so far. The
reconstruction theorem holds for the empirical inputs \emph{as stated}
--- the idealized infinite-data regime --- and proves uniqueness of the
...
\[\text{Observed physics (E1--E6)} \quad \longleftrightarrow \quad [(S, \varphi)] / \mathcal{G}_{\text{sub}}\]
...
\textbf{Remark} (Structure of the four families). The four generators
are heterogeneous in character --- a permutation of states, a recoding
of the alphabet, a change of cardinality of the deep sector, an
isomorphism class of graphs. This heterogeneity reflects the layered
structure of the substratum description, with each generator acting on
the layer where its corresponding redundancy lives: (i) on the
configuration-space level, (ii) on the alphabet level, (iii) on the
cardinality level of inaccessible degrees of freedom, (iv) on the
coupling-graph level. \(\mathcal{G}_{\text{sub}}\) is naturally a
discrete equivalence classification rather than a Lie group because it
operates on the substratum's combinatorial structure; the continuous
Lie-group character of the Standard Model gauge group
\(\text{SU}(3) \times \text{SU}(2) \times \text{U}(1)\) arises at the
\emph{emergent} level under the trace-out, as the commutant of the
cubic-group action on the \(K = 6\) internal components ([SM §3.1]).
The four-generator structure of \(\mathcal{G}_{\text{sub}}\) and the
continuous structure of the SM gauge group are not redundant:
\(\mathcal{G}_{\text{sub}}\) classifies which substratum descriptions
are observationally equivalent, while
\(\text{SU}(3) \times \text{SU}(2) \times \text{U}(1)\) is a feature of
any specific equivalence-class representative.

\textbf{Completeness of the generators.} The four families exhaust
\(\mathcal{G}_{\text{sub}}\). The proof proceeds in two steps: first
characterizing the class of observationally-equivalent transformations
using the framework's foundational commitments, then showing that any
transformation in this class decomposes into (i)--(iv).

\textbf{Step 1: Characterization of observationally-equivalent
transformations.} The equivalence relation \(\sim\) defined above is
observational equivalence --- preservation of the embedded observer's
marginal statistics. Two structural restrictions follow from the
classical-substratum ontology and Stinespring uniqueness.

(a) \emph{Bijection structure.} Non-bijection dynamics (probabilistic
kernels) lift via Stinespring to non-permutational unitaries on
\(\ell^2(S)\), which do not descend to \(S\)-level transformations
preserving classical product structure (cf. §3.1's analysis of A2 and
the descent argument used in Step 2 below). Probabilistic-kernel
candidates therefore violate the classical-substratum ontology rather
than producing a distinct gauge equivalence class.

(b) \emph{Tensor factorization.} Non-tensor enlargements of the hidden
Hilbert space (e.g., \(\mathcal{H}_H \oplus \mathcal{H}_{\rm extra}\)
rather than \(\mathcal{H}_H \otimes \mathcal{H}_{\rm extra}\)) destroy
the visible-hidden factorization
\(\mathcal{H} = \mathcal{H}_V \otimes \mathcal{H}_H\), leaving the
partial trace undefined and the visible-sector CPTP channel
unconstructible. Such transformations do not produce a well-defined
observable structure.

Three additional candidate sources of gauge freedom beyond (i)-(iv)
are subsumed in the explicit fifth-family analysis below: time
reversal, hidden-sector dynamical reparametrization, and visible-sector
global phase. Observer-choice redefinitions (changing the visible-
hidden partition) are not in \(\mathcal{G}_{\rm sub}\): different
observer choices produce different observed physics, not the same
physics in different gauge representatives. The gauge group is
parameterized given a fixed observer/visible-sector identification.

The bijection-preserving and tensor-factorized character of
\(\mathcal{G}_{\rm sub}\) is therefore derived from observational
equivalence together with the classical-substratum ontology and
Stinespring uniqueness.

\textbf{Step 2: Decomposition into (i)--(iv).} Let
\(g: (S, \varphi) \to (S', \varphi')\) preserve all observables
...
{[}Main, §3.2{]} constrains the freedom at the \emph{Hilbert-space}
level: any two dilations of the same channel on the same visible
Hilbert space differ by a partial isometry on the hidden-sector
Hilbert space \(\mathcal{H}_H = \ell^2(\mathcal{C}_H)\). The
substratum-level transformation \(g\) acts on the classical
configuration space \(\mathcal{C}_H\), not on the full
\(\mathcal{H}_H\), and the only partial isometries on
\(\mathcal{H}_H\) that descend to \(\mathcal{C}_H\)-level bijections
--- i.e., that preserve the classical product structure of
\(S = \mathcal{C}_V \times \mathcal{C}_H\) rather than producing
superpositions of classical configurations --- are permutations of
\(\mathcal{C}_H\). Non-permutational unitaries on \(\mathcal{H}_H\)
are valid Stinespring freedoms at the channel level but do not lift
to substratum-level bijections; they are excluded from
\(\mathcal{G}_{\text{sub}}\) by the descent restriction of Step 1.
The Stinespring partial isometry therefore reduces, at the substratum
level, to a relabeling of hidden-sector states (generator (i)
restricted to \(\mathcal{C}_H\)) when the hidden sectors have equal
cardinality, or to a composition of relabeling and deep-sector
enlargement/reduction (generators (i) + (iii)) when they differ in
size. The alphabet may change freely (generator (ii), by {[}SM,
§2.7{]}). The coupling graph \(G_{\varphi'}\) must have the same
statistical properties as \(G_\varphi\) --- dimension, spectral
dimension, isotropy --- since these determine the derivation chain's
outputs; two such graphs are related by generator (iv). These four
degrees of freedom --- hidden-sector relabeling, deep-sector size,
alphabet, and graph statistical class --- exhaust the free parameters
in the reconstruction, so the four generators span
\(\mathcal{G}_{\text{sub}}\). \(\square\)
...
\item
  ...
  hidden-sector dimension differ by a hidden-sector unitary; the
  substratum-level descent restricts this to permutations of
  \(\mathcal{C}_H\) (the classical-structure-preserving subset),
  which is generator (i) restricted to \(\mathcal{C}_H\).
\item
...
$\phi$)}\label{synthesis-three-projections-of-s-ux3c6}}
...
consistent with the $\Lambda CDM$ limit); a \textasciitilde{}\(95\%\) dark sector
...
\([(S, \varphi)] / \mathcal{G}_{\text{sub}}\): what exists is the
equivalence class of bijections, uniquely determined by observations
plus structural assumptions (Theorem 23), with individual substratum
representatives related by the substratum gauge group (Theorem 24). The
class is real; specific substrates are representations.

This is the strongest claim the framework's theorems license without
overclaiming. Two alternative framings merit explicit comparison.
...
The triple (S, $\phi$, V) generates every concept in fundamental physics, not
as independent substances but as different aspects of the same
structure. \textbf{Space} is the coupling structure of $\phi$ --- the graph
G\_$\phi$ determined by which degrees of freedom affect which others ({[}SM,
...
these are independent entities; they are descriptions of (S, $\phi$, V) at
different scales.
...
wave function is not a component of (S, $\phi$, V) --- it is a derived
...
Branching is forbidden by the rigidity of $\phi$. A fixed bijection on a
...
G\_$\phi$. Two visible sites prepared in a joint state (entangled) have
...
\hypertarget{relation-to-quantum-no-go-theorems}{%
\subsubsection{6.4 Relation to quantum no-go
theorems}\label{relation-to-quantum-no-go-theorems}}

The framework's classical-substratum-with-emergent-QM ontology raises
the question of how a local-classical substratum is consistent with
experimentally verified Bell violations and with the standard no-go
theorems against hidden-variable theories: Kochen-Specker, PBR, and
Wigner-friend paradoxes such as Frauchiger-Renner. The general
structure of the framework's relation to these theorems is that each
targets hidden-variable theories where the hidden variables exist at
the same descriptive level as the observable quantities; the
framework's hidden variables --- substratum configurations
\(x \in S\) --- live at a different descriptive level, related to
observables only through the trace-out over the hidden sector. Each
theorem assumes a property at the hidden-variable level that, in the
framework, holds only at the observable level. The level distinction
is what determines how the framework relates to each theorem.

\emph{Bell.} The Bell-CHSH inequality is derived under the assumptions
of realism, locality, measurement independence, and factorizability of
the joint distribution given the hidden variable. The substratum is
locally classical --- deterministic bijection with bounded coupling
--- and at the substratum level all four assumptions hold. At the
observer level, however, the marginal stochastic process is
P-indivisible ({[}Main, §2.3{]}): the embedded observer's marginal
joint distribution does not admit a hidden-variable factorization
\(\int P(a|x,\lambda) P(b|y,\lambda) \rho(\lambda)\, d\lambda\).
P-indivisibility is the statement that no such \(\lambda\) exists for
the observer's marginal, even though the substratum's hidden variable
(the configuration \(x\)) does exist at its own level. The Bell
violations the framework predicts are thus expected consequences of
P-indivisibility, not signatures of fundamental nonlocality. The
substratum violates Bell-factorizability not because of any
superluminal influence --- locality holds at the substratum level ---
but because the trace-out destroys the factorizable structure that
Bell's argument requires.

\emph{Kochen-Specker.} The Kochen-Specker theorem rules out
noncontextual hidden-variable assignments to quantum observables of
dimension \(\geq 3\). The substratum's determinate states are not
assignments to quantum observables: a configuration \(x \in S\) is a
classical configuration, not an eigenvalue of an emergent observable
\(\hat{O}\). The mapping from \(x\) to the observer's outcome
distribution for \(\hat{O}\) goes through the trace-out conditioned on
the observer's measurement context, which depends on which substratum
subspaces the measurement effectively projects onto. Different
measurement contexts --- different orientations of a Stern-Gerlach
apparatus, different choices of polarizer --- produce different
effective conditionings, hence different outcome distributions. This
is observer-level contextuality of a familiar kind: the framework
belongs to the class of contextual hidden-variable theories (alongside
Bohmian mechanics) that Kochen-Specker does not exclude.

\emph{PBR.} The Pusey-Barrett-Rudolph theorem rules out psi-epistemic
hidden-variable theories under the assumption of preparation
independence --- that the hidden-variable distributions for
independently prepared systems factor as products
\(\rho_{12}(\lambda_1, \lambda_2) = \rho_1(\lambda_1) \rho_2(\lambda_2)\).
The framework's hidden variable is the global substratum configuration;
there is no factorization of \(\lambda\) into independent factors per
prepared system. The substratum is a single global object, and
``independent preparation'' at the observable level corresponds to
preparation of distinct visible-sector marginals against a shared
substratum background, not to independent hidden-variable
distributions. The framework therefore violates PBR's
preparation-independence assumption at the substratum level, while
preserving the observable consequences of independent preparation at
the marginal level. The wave function remains psi-epistemic in the
framework --- observer bookkeeping rather than ontic state --- because
PBR's argument does not reach a theory with a global hidden variable.

\emph{Frauchiger-Renner.} The Frauchiger-Renner setup derives
contradictions from the assumption that all observers in a Wigner-friend
chain treat QM as their fundamental description and assume measurement
outcomes are absolute. The framework adopts QM only as the embedded
observer's marginal-statistics description, not as a fundamental
theory: the substratum has definite states evolving deterministically,
and measurement outcomes are absolute at the substratum level. When a
Wigner-friend chain unfolds, the friend inside the lab has a
marginal-statistics description in which their measurement is a
collapse, while Wigner outside has a marginal-statistics description
in which the lab + friend system evolves unitarily --- because the
friend is part of Wigner's hidden sector. These two descriptions are
not contradictory; they describe the same substratum-level events
from different visible-sector partitions. The contradictions
Frauchiger-Renner derive depend on conflating these two descriptions,
which the framework keeps separated at the substratum level. The
framework's substratum-absolute / observer-marginal two-level
structure is what avoids the paradox.

The four cases share a common structure. Each no-go theorem assumes a
property of hidden variables that is built in at the level where the
standard hidden-variable theory operates. The framework's hidden
variables --- substratum configurations --- operate at a different
level, with the trace-out structure of [Main] mediating between
substratum and observer. Each theorem's assumption that fails in the
framework (Bell-factorizability, Kochen-Specker noncontextuality, PBR
preparation independence, Frauchiger-Renner universal-QM
applicability) fails because the trace-out destroys the structure the
assumption requires; this is a feature of the construction in [Main],
not a separate commitment. The experimental content of each theorem
(Bell violations, Kochen-Specker contextuality, PBR's psi-ontic
theorem under preparation independence, Frauchiger-Renner's
single-world inconsistency under universal QM application) is
preserved as a consequence of the framework's emergent-QM construction.

\hypertarget{pre-registered-falsification-conditions}{%
\subsubsection{6.5 Pre-registered falsification
...
\item
  \emph{Gauge group SU(3) $\times$ SU(2) $\times$ U(1) exactly.} Any observation of a
...
\item
  ...
  with
  \(\delta(\hat k) = \frac{1}{24}\left(\sum_i \hat k_i^4 - \frac{1}{3}\right)\)
  evaluated on the rigid \(\mathbb{Z}^3\) representative. The
  rigid-lattice anisotropy (values \(\delta_{[100]} = 1/36\),
  \(\delta_{[111]} = 0\) --- the body diagonals achieve the
  Cauchy-Schwarz lower bound \(\sum_i \hat k_i^4 \geq 1/3\) on the unit
  sphere, giving no leading-order correction along these directions) is
  \emph{not} a gauge-invariant prediction --- \(\mathcal{G}_{\text{sub}}\)
  generator (iv) of §4 identifies \(\mathbb{Z}^3\) with any bounded-degree
  random graph of the same polynomial growth exponent and statistical
  isotropy (E4), which has no preferred axes. The
  \emph{orientation-averaged} coefficient
  ...
  sphere (with equality at the body diagonals
  \(\hat k = \pm(1,1,1)/\sqrt 3\) and equivalent rotations, which are
  the unique directions saturating the Cauchy-Schwarz lower bound); the
  orientation-averaged \(\langle\delta\rangle > 0\) is invariant under
...
\item
  ...
  with four bridge gaps in the $S\chi PT$ inheritance ({[}SM §7.5{]}): the
  substratum measure reduction under Theorem 2, the 3D MC vs 4D $S\chi PT$
  ...
  that share the $S\chi PT$ machinery. The closure path is the two-loop $S\chi PT$
...
Three sets of open problems remain. Neither these results nor the
four-paper sequence require the framework to be the final theory of
