...
equivalence class is uniquely determined by the observations together
with the structural assumptions, and the substratum gauge group (§4)
shows that different representatives of the class are observably
indistinguishable. The class is what the data pin down; individual
...
\(\dim \mathcal{H} \leq e^{A/4}\) --- A1 is not an independent
assumption beyond the empirical inputs.

(A2) \textbf{Determinism.} \(\varphi: S \to S\) is a bijection
(deterministic, reversible dynamics). \emph{Forced by E1 +
...
\(U\) must be a permutation matrix --- those are the only unitaries on
...
non-unitary CPTP channels) and therefore do not descend to
classical-substratum transformations. A2 is forced by the
classical-substratum commitment, not parsimony.

(A3) \textbf{Bounded coupling degree.} Each site is coupled to a bounded
number of neighbors in \(\varphi\)'s action. \emph{Forced by E1+E2 +
finite light-cone speed}: infinite coupling at substratum level produces
instantaneous correlations everywhere, inconsistent with the finite
signal propagation speed implicit in E1's standard QM and E2's
Bell-violation structure. A3 is not an independent assumption beyond
the empirical inputs.

(A4) \textbf{Center independence.} The dynamics \(\varphi\) commutes
with lattice translations up to gauge. \emph{Forced by E5 (spatial
homogeneity)}: a substratum dynamics with a preferred center site would,
under the trace-out, produce a preferred-location effect at the
observable level inconsistent with E5. A4 is not an independent
assumption beyond the empirical inputs.

(A5) \textbf{Linearity.} The wave equation for \(\varphi\) is linear.
\emph{Forced by E1 + the \(q\)-gauge invariance theorem of [SM §2.7]}:
the alphabet size \(q\) is gauge ([SM §2.7] proves no observable depends
on \(q\)); non-linear dynamics over \(\mathbb{Z}/q\mathbb{Z}\) generically
have \(q\)-dependent emergent physics (\(x^2 \bmod 3\) and \(x^2 \bmod 5\)
have different algebraic structure), so the emergent physics from
non-linear dynamics would be \(q\)-dependent, contradicting the gauge
equivalence. Linearity is necessary, not parsimony --- see [SM §4.1] for
the necessity argument and three additional independent criteria
(maximum propagation speed, maximum P-indivisibility, KMS condition for
Unruh thermality) all converging on the same dynamics. A5 is forced by
the framework's gauge structure.
...
\emph{Forced by E1}: the trace-out's emergent QFT must have local gauge
invariance (the standard model gauge group is observed); local-gauge-
invariant emergent QFT requires local-symmetry-invariant substratum
dynamics. A6 is not an independent assumption beyond the empirical
inputs.

After this analysis, \textbf{all six structural assumptions A1-A6 are
forced} by the empirical inputs E1-E5 (E1 via Main paper machinery, E2
via Bell-violation structure, E3 via holographic interpretation, E4-E5
via parsimony for substratum locality + symmetry) combined with the
framework's gauge structure ([SM §2.7] \(q\)-gauge invariance, §4
Theorem 24's \(\mathcal{G}_{\rm sub}\) classical-substratum descent
restriction). \textbf{No structural assumption is genuinely independent
of the empirical inputs and the framework's existing theorems.} The
reconstruction's foundation is therefore a single ontological
commitment: the world is a classical-substratum-with-embedded-observer
system, with the observed physics being the Standard Model (E6).
Theorem 23 then determines the substratum modulo gauge
\(\mathcal{G}_{\rm sub}\).

The framework's claim therefore reads: \textbf{``The
classical-substratum-with-embedded-observer ontology together with the
observed Standard Model uniquely determines the substratum modulo gauge
\(\mathcal{G}_{\rm sub}\).''} No parsimony assumptions remain.
...
is unique. Per §3.1's analysis above, \textbf{all six structural
assumptions A1-A6 are forced} by the empirical inputs E1-E5 combined
with the framework's gauge structure ([SM §2.7] \(q\)-gauge invariance,
§4 Theorem 24's \(\mathcal{G}_{\rm sub}\) classical-substratum descent
restriction). The framework's claim therefore reduces to ``the
classical-substratum-with-embedded-observer ontology + observed
Standard Model (E6) uniquely determines substratum modulo gauge
\(\mathcal{G}_{\rm sub}\)'' --- with no residual parsimony assumptions.
A separate argument would be required to challenge any of the empirical
inputs E1-E6 themselves, or to challenge the framework's existing
theorems ([SM §2.7] \(q\)-gauge invariance, §4 Theorem 24's
completeness) on which the structural-assumption derivations depend.
...
reducing observational equivalence to a definite class of
transformations using the framework's foundational commitments, then
showing that any transformation in this class decomposes into (i)--(iv).

\textbf{Step 1: Reduction to bijection-preserving, tensor-factorized
...
marginal statistics. The classical-substratum ontology + Stinespring
uniqueness force any observationally-equivalent transformation to (a)
preserve bijection structure on \(S\) and (b) factor through
visible-hidden tensor decomposition.

For (a): non-bijection dynamics (probabilistic kernels) lift via
Stinespring to non-permutational unitaries on \(\ell^2(S)\), which do
not descend to \(S\)-level transformations preserving classical product
structure (per §3.1's analysis of A2 --- equivalently, the descent
argument used in this proof's main body below). Non-bijection
candidates are therefore excluded \emph{before} the gauge group is
defined; they violate the classical-substratum ontology rather than
producing a different gauge equivalence class.

For (b): non-tensor enlargements of the hidden Hilbert space (e.g.,
\(\mathcal{H}_H \oplus \mathcal{H}_{\rm extra}\) rather than
\(\mathcal{H}_H \otimes \mathcal{H}_{\rm extra}\)) destroy the
visible-hidden factorization
\(\mathcal{H} = \mathcal{H}_V \otimes \mathcal{H}_H\), leaving the
partial trace undefined and the visible-sector CPTP channel
unconstructible. Such transformations don't produce a well-defined
observable structure and are excluded automatically.

Three further candidate sources of ``extra'' gauge freedom beyond
(i)-(iv) are subsumed in the explicit fifth-family analysis below
(time reversal, hidden-sector dynamical reparametrization,
visible-sector global phase). A fourth --- observer-choice
redefinitions (changing the visible-hidden partition) --- is not in
\(\mathcal{G}_{\rm sub}\) at all: different observer choices in the
same substratum produce different observed physics, not the same
...
The gauge group \(\mathcal{G}_{\rm sub}\) is therefore not stipulated
to consist of bijection-preserving transformations; the
bijection-preserving + tensor-factorized character is \emph{derived}
from observational equivalence + the classical-substratum ontology +
...
are valid Stinespring freedoms at the channel level, but they do not
lift to substratum-level bijections because the substratum is
classical; they are not in \(\mathcal{G}_{\text{sub}}\) by definition
of the equivalence relation, which acts on bijection-preserving
transformations between substrata.) At the substratum level, the
Stinespring partial isometry therefore reduces to a relabeling of
hidden-sector states (generator (i) restricted to \(\mathcal{C}_H\))
when the hidden sectors have equal cardinality, or a composition of
relabeling and deep-sector enlargement/reduction (generators (i) +
(iii)) when they differ in size. The alphabet may change freely
(generator (ii), by {[}SM, §2.7{]}). The coupling graph
