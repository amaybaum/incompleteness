...
concordance) replace the chain-self-consistency framing of an earlier
version with explicit forward filters for \(d = 3\) --- see Stage 2(a)
below.
...
failures of ETH would require a separate argument. The conditional is
honest, not hidden: the framework's gauge-group derivation through Stage
2 inherits this conditional.
...
\item
  ...
  speed \(v = \alpha\). The constant \(\alpha = 1\) is then fixed by the
  requirement of maximum lattice-scale propagation speed, consistent
  with the empirical relativistic speed limit being realized at the
  lattice cutoff. (This is an additional empirical input --- call it E8
  ``relativistic causality with maximum signal speed \(c\)'' --- that
  the framework treats implicitly; surfacing it explicitly closes a
  minor presentation gap. The structural conclusion is unchanged.)
\item
