...
\item
  \emph{Leading Lorentz-invariance-violation coefficient
  \(\lesssim (E/M_{\text{Pl}})^2\).} The cubic-lattice dispersion
  relation ({[}SM, §4.4{]} Theorem 4: \(\cos\omega_a = \mu_a \cos k\))
  yields standard emergent dispersion \(\omega = |k|\) at leading order,
  with the first correction suppressed by
  \((k\epsilon)^2 = 4(E/M_{\text{Pl}})^2\).
  Linear-in-\(E/M_{\text{Pl}}\) LIV is forbidden at the substrate level
  --- the dispersion relation has no \(\mathcal{O}(k^3)\) term because
  cubic-lattice symmetry does not admit one --- so any demonstration of
  linear LIV at any scale falsifies the framework's substrate-level
  commitment to the cubic-lattice wave equation. Quadratic LIV at the
  \((E/M_{\text{Pl}})^2\) level is a structural commitment; current
  bounds from GRB time-of-flight probe only
  \(E_{\text{QG}} \gtrsim 10^6\) GeV (roughly thirteen orders of
  magnitude below \(M_{\text{Pl}}\)), so this is a consistency
  commitment rather than a near-term empirical test. Any future bound
  that puts \(E_{\text{QG}}\) above \(M_{\text{Pl}}\) in the quadratic
  sector, or confirms linear LIV at any scale, tests or falsifies this
  commitment.
\end{itemize}
