...
isotropy --- uniquely determines the equivalence class
...
of generators that exhaust all transformations preserving the
framework's structural inputs. The Standard Model gauge group is the
...
\((S, \varphi)\) to observed physics is invertible: observed quantum
mechanics with Bell violations, together with finite boundary entropy
and spatial isotropy, uniquely determine the equivalence class of
...
inverse map --- the transformations of \((S, \varphi)\) that preserve
all observables --- and shows that the Standard Model gauge group is its
...
$\varphi$)}\label{the-physical-interpretation-of-s-ux3c6}}

\textbf{Storage and memory.} S is the set of all distinguishable states:
\emph{finite capacity}. $\varphi$ is a bijection: \emph{perfect memory} ---
information is never created or destroyed. Together, (S, $\varphi$) is a finite
...
\textbf{The substrate objection.} ``What is the memory made of?'' (S, $\varphi$)
is a \emph{complete description} of reality --- it determines all
observables. Space, time, matter, and energy are derived from (S, $\varphi$), so
they cannot appear in its definition without circularity. Whether (S, $\varphi$)
...
access window, $\varphi$ is the transition function. But three differences are
...
irreversible (it can erase, overwrite, and halt); $\varphi$ is a bijection ---
nothing is erased, nothing is created, there is no halting. Third, a
Turing machine computes an \emph{extrinsic function} (input $\rightarrow$ output for
an external user); (S, $\varphi$) computes no extrinsic function --- it is a
...
tape, $\varphi$ is a reversible transition function, and C1--C3 characterize the
...
\textbf{The arrow of time.} The substratum has no arrow of time --- $\varphi$
and $\varphi$$^-$$^1$ are equally valid. Entropy increase is a property of the
...
The forward direction --- from (S, $\varphi$) to observed physics --- is
established by {[}SM, §§3.1, 4--5{]} and the companion paper {[}Main{]}.
The converse question is whether the observed physics uniquely
determines (S, $\varphi$). The reconstruction proceeds in three stages.
...
set {[}Main, §3.2{]}. Bell violations require non-trivial coupling (C1).
The characterization theorem {[}Main, §3.4{]} requires slow-bath memory
(C2) and sufficient capacity (C3).

\emph{Output:} The observed QM arises from some (S, $\varphi$, V) satisfying
...
\emph{Derivation.} The coupling graph gives d = 3 ({[}SM, §3.2{]}).
Center independence + isotropy + linearity uniquely select the wave
equation ({[}SM, §4.1{]}). The wave equation determines the SM gauge
structure: K = 2d = 6 (Theorem 6), cubic decomposition (3,2,1) (Theorem
7), local gauge invariance ({[}SM, §3.1{]}), staggered species $\rightarrow$ three
degenerate triplet-taste sectors, assigned to three generations, with
the Higgs sector per Theorem 11 (Theorems 8--11; fermion embedding
closed via the link-carrier construction, with the absolute generation
count locked at exactly three by coupling-degree minimality),
partition-spinor identification (Theorem 12), chirality (Theorem 13),
anomaly cancellation (Theorems 14--15), T-invariance $\rightarrow$ $\theta$̄ = 0 (Theorems
17--21).
...
self-consistency {[}GR, §3{]} gives \(\hbar = c^3 \epsilon^2 / (4G)\)
with $\varepsilon$ = 2 l\_p.~\emph{Status:} Theorem.
...
\emph{Proof.} Stage 1 $\Rightarrow$ Stage 2 $\Rightarrow$ Stage 3, as detailed above.
\(\square\)

The theorem starts from empirical inputs (QM, Bell violations, finite
entropy, isotropy) and derives the framework. The SM gauge structure, $\theta$̄
= 0, and the Bekenstein-Hawking formula are retrodictions --- derived,
not assumed. The match to three SM generations follows from cubic
symmetry (three degenerate \(T_1\)-taste sectors); the fermion embedding
into SM gauge representations is closed by the link-carrier construction
(SM §4.7.1.2), with anomaly cancellation fixing the hypercharges
uniquely. The absolute generation count is locked at exactly three by
coupling-degree minimality: the uniqueness of the \(K = 2d = 6\)
minimizer (Theorem 6) forces exactly three spin-1/2 staggered tastes as
a derived consequence (SM §4.7.1.2, Step 3).
...
$\varphi$)}\label{synthesis-qm-and-gr-as-projections-of-s-ux3c6}}
...
The triple (S, $\varphi$, V) generates every concept in fundamental physics, not
as independent substances but as different aspects of the same
structure. \textbf{Space} is the coupling structure of $\varphi$ --- the graph
G\_$\varphi$ determined by which degrees of freedom affect which others ({[}SM,
...
these are independent entities; they are descriptions of (S, $\varphi$, V) at
different scales.
...
wave function is not a component of (S, $\varphi$, V) --- it is a derived
...
Branching is forbidden by the rigidity of $\varphi$. A fixed bijection on a
...
G\_$\varphi$. Two visible sites prepared in a joint state (entangled) have
...
quantum mechanics with Bell violations, finite boundary entropy, and
spatial isotropy uniquely determine the equivalence class
\([(S, \varphi)]/\mathcal{G}_{\text{sub}}\) at the lattice level, with
