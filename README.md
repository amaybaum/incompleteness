...
6. **Standard Model structure.** The wave equation factors into staggered Dirac fermions (theorem). Center independence enforces chiral symmetry, mandating the Higgs (theorem). K = 2d = 6 from coupling-degree minimization — the canonical factorization of the bijection, proved unique for any bounded-degree graph with statistical isotropy (theorem). The cubic group decomposition gives multiplicities (3, 2, 1) → SU(3) × SU(2) × U(1) (theorem). Chirality: the trace-out makes SU(2) chiral while SU(3) remains vector-like (theorem). Three SM generations from cubic symmetry + anomaly cancellation uniqueness (theorem). Higgs quantum numbers (1, 2, +1/2) from representation theory (theorem). The primary derivation chain is proved end-to-end; remaining propositions are redundant second-route confirmations.
...
8. **Dark-sector concordance and dark matter.** The trace-out producing QM renders ~95% of ρ_crit invisible to the emergent QFT (theorem — total budget). The structured component is derived from entropy displacement: baryonic matter displaces boundary entropy via the Clausius relation, the Jacobson mechanism converts the entropy gradient into curvature, yielding the MOND acceleration scale a₀ = cH/6 ≈ 1.2 × 10⁻¹⁰ m/s² and the baryonic Tully-Fisher relation v⁴ = GM_B · cH/6 — both parameter-free. The simple interpolation function ν(y) = (1+√(1+4/y))/2 matches galaxy rotation curves, the radial acceleration relation, AND cluster masses (Coma to <1%). The Bullet Cluster lensing offset is explained by frozen boundary entropy: the relaxation time ~H⁻¹ ≈ 14 Gyr vastly exceeds the collision crossing time ~0.15 Gyr. The CMB acoustic peak pattern is reproduced because the Clausius relation involves net heat transfer — oscillating perturbations have zero net entropy displacement per cycle, so only the growing mode is tracked, providing non-oscillating potential wells identical to CDM in the linear regime. At high redshift, a₀(z) = cH(z)/6 predicts declining rotation curves (confirmed by Genzel+2017 at >3σ) and no evolution in the stellar Tully-Fisher relation (confirmed by McGaugh+2024; the gas fraction compensates the dynamical shift). *Status: theorem (total budget and acceleration scale); interpolation steepness is solution-specific; CMB prediction pending numerical verification.*
...
11. **Falsifiable predictions.** Dark energy evolution with ν_OI = (2.45 ± 0.04) × 10⁻³ (±1.8%, independently testable ratio ν/Ω_Λ = 0.00358); GW echoes with first-echo amplitude ~45% of ringdown, echo delay Δt = (r₊/c)ln(r₊/2l_p), and fitting factor FF = 0.19 against long-lived QNM searches (non-detection by Wu+2025 is consistent); gauge coupling prediction 1/α₀ = 23.25 reproducing all three SM couplings at M_Z to <0.1%; θ̄ = 0 exactly (no axion, neutron EDM = 0); no SUSY partners; no fourth generation; Majorana neutrinos with normal ordering, large PMNS mixing, and hierarchical spectrum (Σm_ν near 0.059 eV minimum; DESI+CMB gives <0.064 eV); declining rotation curves at high z (confirmed); no stellar TF evolution (confirmed — gas fraction compensates); baryonic TF evolves as v⁴ ∝ H(z) (untested — needs ALMA gas masses at z > 1). The conjunction is distinctive to this framework.
