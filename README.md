...
- **[`Main`](papers/Main.md)** — proves the per-horizon equivalence — accessible non-Markovianity ⟺ deterministic (C1)/(C3)/(C4) realization, where "accessible non-Markovianity" is the framework's term for finite-horizon memory / information backflow in the visible law and is not generic quantum non-Markovianity — and the finite-horizon representation equivalence $S \iff D \iff Q_{\mathrm{fb}}$ (the quantum representation internal and universal, the compressed form via the imported correspondence under the translation hypothesis (T)); the universal hidden-memory theorem constrains every deterministic completion, and (C2)'s necessity is the conditional physical-memory theorem. Minimal explicit model, stated falsification conditions. ([`.tex`](papers/Main.tex), [`.pdf`](papers/Main.pdf))
