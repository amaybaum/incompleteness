...
```
Runs in a few minutes, outputting δ₀(SE+VC via Ward 3×) grid convergence, the computed sails contribution, and the Π_s(p) correction estimate. The original "$8.0 \pm 2$" result produced by this script was audited in April 2026 and found to use a scheme-locked $\Pi_s$ convention (see `oi_lattice_code/gauge/AB_derivation/PI_S_CONVENTION_NOTE.md`); it is held in reserve pending a rebuilt calculation. In the meantime, $\delta_0 = 10.02$ is empirically fixed by the U(1) row (SM §6.2).
