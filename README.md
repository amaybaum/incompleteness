...
- **[`Main`](papers/Main.md)** — proves the per-horizon equivalence — accessible non-Markovianity ⟺ deterministic (C1)–(C4) realization — with a quantum representation for the P-indivisible subclass via the imported correspondence under the translation hypothesis (T); (C2)'s necessity is the conditional physical-memory theorem. Minimal explicit model, stated falsification conditions. ([`.tex`](papers/Main.tex), [`.pdf`](papers/Main.pdf))
