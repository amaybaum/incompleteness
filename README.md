...
```
Runs in a few minutes, outputting δ₀(SE+VC via Ward 3×) grid convergence, the computed sails contribution, and the Π_s(p) correction estimate. An audit of this implementation (see `oi_lattice_code/gauge/AB_derivation/PI_S_CONVENTION_AUDIT.md`) found it uses a scheme-locked $\Pi_s$ convention; the "$8.0 \pm 2$" result is held in reserve pending a rebuilt calculation. $\delta_0 = 10.02$ is empirically fixed by the U(1) row (SM §6.2).
