...
6. **Standard Model structure.** The wave equation factors into staggered Dirac fermions; center independence mandates the Higgs; K = 2d = 6 from coupling-degree minimization; the cubic group gives multiplicities (3, 2, 1) → SU(3) × SU(2) × U(1); the trace-out makes SU(2) chiral; anomaly cancellation uniquely determines the hypercharges given the observed family pattern (fermion embedding closed by link-carrier construction, SM §4.7.X.2; absolute generation count remains open); Higgs quantum numbers (1, 2, +1/2). Primary derivation chain proved end-to-end.
