...
- **[`Main`](papers/Main.md)** — proves that embedded observation of a deterministic substrate generically yields P-indivisible dynamics, identified with quantum mechanics via the Barandes correspondence; converse characterization theorem (one necessity step conditional on ETH). Minimal explicit model, stated falsification conditions. ([`.tex`](papers/Main.tex), [`.pdf`](papers/Main.pdf))
- **[`SM`](papers/SM.md)** — derives the Standard Model from a $d=3$ cubic lattice. SU(3)×SU(2)×U(1); threefold taste multiplicity (candidate generations under H-spin' — free kernel refuted, sharpened form open; chirality under H-χ' — sharpened, the minimal embedding certified vector-like); $\bar\theta=0$ within the construction; twenty-two quantitative observables. ([`.tex`](papers/SM.tex), [`.pdf`](papers/SM.pdf))
...
| 6 | SM gauge group SU(3)×SU(2)×U(1); hypercharges; threefold taste multiplicity (generations under H-spin', chirality under H-χ', sharpened: the minimal embedding is certified vector-like); $\bar\theta = 0$ within-construction (strong CP itself unresolved) | theorem + named hypotheses | SM §§4, 5 |
