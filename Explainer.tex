...
% Header info
\title{The Incompleteness of Observation: Why Physics' Biggest Contradiction Might Not Be a Contradiction at All\\[0.5em]\large With Complete Mathematical Detail}
\author{Alex Maybaum}
\date{March 2026}
...
\textbf{Step 3 (Dimensional analysis).} There's a unique combination of c, G, and ε with dimensions of action:
...
A thermometer at the horizon must read the same temperature either way. Setting T\_cl = T\_Q:
...
\textbf{Why this is not circular.} The KMS temperature T\_Q contains ℏ as an \emph{unknown}. The classical temperature T\_cl contains no ℏ at all. The logic is sequential: Part I proves quantum mechanics emerges with some ℏ → the emergent QM predicts T\_GH(ℏ) → the classical temperature T\_cl was calculated independently → matching them fixes ℏ. This is a ``gap equation,'' the same structure used routinely in condensed matter physics. If T\_cl had depended on the deep hidden-sector volume (it doesn't), or if T\_Q had been state-dependent (it isn't), the matching wouldn't work. The fact that it works is a non-trivial self-consistency check.
...
The conjunction is distinctive: dark energy evolution in RVM form, gravitational wave echoes, and a natural account of the dark sector's total gravitational budget --- all from four axioms about embedded observation. No competing framework produces all three. The parallel with the cosmological constant dissolution is exact. Part II identifies the \(10^{122}\) discrepancy as the \emph{information compression ratio} of the trace-out --- how much the emergent description distorts the vacuum energy. Part III identifies the \textasciitilde95\% dark sector as the \emph{gravitational occlusion fraction} --- how much of the gravitational universe the trace-out hides from the emergent description. Together, they account for the two largest anomalies in modern cosmology as two aspects of a single phenomenon: the cost of observing the universe from within.
...
\textbf{``What about dark matter?''} The framework doesn't set out to explain dark matter, but it produces a natural account of why \textasciitilde95\% of the universe's gravitational budget has no source in particle physics. The boundary entropy --- the modes traced out to produce QM --- has thermal energy equal to the critical density. Crucially, this thermal energy is computed from purely classical quantities (the number of boundary modes and the classical horizon temperature) --- it exists before the trace-out, unlike the QFT zero-point energy, which is an artifact of the emergent description. The boundary entropy is therefore gravitationally active (it curves spacetime at the classical level) but invisible to the emergent QFT (the boundary modes were traced out to produce it). The baryonic sector accounts for \textasciitilde5\% of \(\rho_{\text{crit}}\); the rest is boundary entropy. The uniform component is dark energy (Part II). Whether the matter-induced \emph{redistribution} of this entropy produces local effects resembling dark matter at galaxy scales is an open question requiring a quantitative force law beyond the axioms. What the framework proves is that the total dark sector budget is an automatic consequence of the trace-out --- not an additional input.
...
S is the set of all distinguishable states. Its physical meaning is \emph{finite capacity}: there are \textbar S\textbar{} configurations that can be told apart, carrying log₂\textbar S\textbar{} bits of information. The finiteness is not a regularization --- it's load-bearing. Finite sets recur (φ\^{}N = id), recurrence produces P-indivisibility, and P-indivisibility produces quantum mechanics. An infinite S wouldn't recur, and the whole mechanism breaks.
...
Together, (S, φ) is a \emph{finite lossless memory}: a register with finite capacity whose contents are updated without loss at each step. The partition V defines the observer's \emph{read access} --- which portion of the register they can see. The hidden sector H is the rest: storage that the dynamics writes and reads but the observer cannot.
...
Most fundamentally: a Turing machine computes an \emph{extrinsic function} --- it maps input to output for an external user who feeds in the input and reads the result. (S, φ) has no external user. There is no input, no output, no halting, no answer. It's a closed permutation that cycles through states and returns.

An information theorist might object: any reversible finite automaton \emph{is} a computer --- the initial state is the input, the final state is the output, and the cycle is the computation. Formally, that's correct. But the distinction isn't formal --- it's about \emph{purpose}. A Turing machine computes something \emph{for someone}. (S, φ) computes nothing for nobody. It's a closed system; there's no recipient of the ``answer.'' What the framework shows is that the appearance of purposeful physical law --- evolution equations, conserved quantities, gauge symmetries --- arises entirely from the \emph{internal} perspective of the partition. The complexity is in the observer, not in the map.

People ask: ``Is the universe a computer?'' The framework's answer: the universe is something \emph{less} than a computer. It's a finite lossless cycle. And physics is the \emph{appearance of complexity} created by observing a simple thing through a keyhole.
...
The common thread: a system with finite resources tries to completely model something it's part of, and the structural impossibility of doing so determines the \emph{form} of what it produces instead. The limitation and the law are the same object viewed from two sides.
...
The framework has one free parameter (ε). The gap equation relates ε to ℏ. You need to know one to determine the other. The predictive content is: - The \emph{relationship} ℏ = c³ε²/(4G) --- which could have been any function of c, G, ε - The \emph{consequences} --- Bekenstein-Hawking entropy, CC dissolution, dark-sector fraction --- which follow from this specific relationship
