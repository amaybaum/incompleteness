...
That measurement gives about 6 × 10⁻¹⁰ joules per cubic meter, which is
a very tiny number.
...
You get an enormous number: roughly 10¹¹⁰ joules per cubic meter.
...
the vacuum must contain. You get a tiny number: roughly 6 × 10⁻¹⁰ joules
per cubic meter.
...
the vacuum exerts on spacetime. The 10¹²⁰ ratio between them is not an
...
outside our bubble than inside it. Their collective random fluctuations
press against our bubble, creating the ``pressure'' we measure as dark
energy.
...
The roughly 10¹²⁰ degrees of freedom accessible to us --- the
Bekenstein-Hawking entropy of the cosmological horizon --- is determined
by the \emph{area} of this light-speed boundary. The 10²⁴⁰ degrees of
...
field mode. Why this is a variance-type quantity: for each mode, the
expectation values of position and momentum are both zero. So the
zero-point energy is \emph{entirely} due to the spread (variance) of
position and momentum.

In a classical vacuum, both variances would be zero and the vacuum
energy would vanish. The entire zero-point energy per mode is pure
fluctuation content. You're standing next to a calm lake --- quantum
mechanics measures how choppy the water is, the total wave energy from
all the ripples, regardless of their direction. Every ripple contributes
positively, so the total can be enormous.
...
\emph{expectation value} (average) of the stress-energy tensor. The left
side (curvature) is smooth. The right side is an average over all
quantum configurations. Gravity doesn't care about individual ripples
--- it only feels the net, aggregate energy content.
...
change one without changing the other. In statistics, you can construct
distributions with the same mean but different variances (compare a
narrow bell curve with a wide bell curve, both centered at zero), and
you can construct distributions with the same variance but different
means (shift a bell curve left or right without changing its width). So
mean and variance are indeed independently configurable.
...
number could accidentally be divisible by 10⁹⁹⁹ --- mathematically
...
of quantum mechanics via Barandes, the 10²⁴⁰ degree-of-freedom count,
...
of a distribution whose second moment is 10¹²⁰ times larger. Remember
...
10²⁴⁰ hidden-sector degrees of freedom pushing in random directions,
basic statistics (the central limit theorem) predicts that the leftover
net push should be roughly 10¹²⁰ times smaller than the total activity
...
sector. It averages over the 10²⁴⁰ hidden degrees of freedom and reports
...
one contributes an energy with a random sign (positive or negative
contribution), a characteristic energy per mode (like a typical salary),
and a random fluctuation around the typical value.
...
This ``coincidence'' --- that the hidden sector has (10¹²⁰)² degrees of
...
a coincidence: the 10¹²⁰ is the one number where both projections make
...
hidden sector with roughly 10²⁴⁰ degrees of freedom.
