...
The structural reading of the OI framework aligns with ontic structural realism but does not require it. What the framework proves is that (S, $\varphi$) is a *complete description* of reality up to gauge equivalence (the reconstruction theorem, §8.6). Whether the structure described *is* reality (the OSR commitment) or *describes* a reality that exists independently (standard realism) is a question the framework identifies as gauge — provably undecidable by any measurement. The "stuff" of the universe, in any reading, is fully characterized by the abstract structure of a bijection on a finite set with bounded coupling.
...
Items 1–6 come from the general theorem [1]. Items 7–8 come from the rigidity test [2]. Items 9–11 come from this paper. Each item is independently verifiable. No item was built into the axioms. The framework further constrains the emergent QFT: the wave equation on a $d = 3$ lattice, combined with coupling-degree minimization and cubic symmetry, determines the gauge group, matter content, and discrete symmetries — extending the derivation chain from the axioms to particle physics (see §8.6, Stage 2).
...
**The substrate objection.** The word "memory" invites the question: what is it made of? In everyday experience, memory requires a physical substrate — silicon, magnetic domains, neurons. The answer is that (S, $\varphi$) is a *complete description* of reality — it determines all observables, with nothing left over that a measurement could detect. The finite set S is the totality of distinguishable configurations — the state space itself. The bijection $\varphi$ is the structure of dynamical dependence. Space, time, matter, and energy are all derived from (S, $\varphi$), so they cannot appear in its definition without circularity. Whether (S, $\varphi$) *is* reality or *describes* reality is a question the framework proves to be gauge — undecidable by any measurement (§8.6, reconstruction theorem). The "memory" is a complete mathematical description from which everything observable is derived. Asking what it is "made of" presupposes a deeper level that no experiment can access — like asking what the number 7 is made of.
...
The OI framework provides a mechanism. The mathematical description is (S, $\varphi$) — a finite set and a bijection — and it is complete: everything observable is determined by it, though it also contains structure that no observation can access (the nilpotent monodromy, the specific bijection within the equivalence class, the value of $q$). An observer with partial access (the partition V) necessarily describes the visible sector using quantum mechanics. The reconstruction theorem (§8.6 below) proves that the mathematical description and the physical description determine each other up to gauge. Physics is what the complete mathematical description looks like from the inside — the observable part, what we have called *observable mathematics*: the unique, rigid projection that partial access produces when the complete description cannot be seen. Whether the complete description *is* reality or merely *describes* reality is a separate question that the framework identifies as undecidable (§8.6).
...
The answer depends on the level of description. At the lattice level — the natural resolution of the framework — the converse holds as a theorem with no gaps. At the continuum level, it requires the standard (unproved) assumption of lattice QFT universality. The reconstruction proceeds in three stages, each adding one empirical input and deriving additional structure. Only the first stage's inputs go beyond what any physicist already accepts; the remaining stages are internal derivations.
...
The finite-entropy condition is not framework-specific: the holographic entropy bound [3] applied to any finite-area causal boundary (established within semiclassical GR by Bousso) gives $S \leq A / (4 l_p^2) < \infty$. Bao, Carroll, and Singh [20] argue on general grounds that the local Hilbert space of quantum gravity is finite-dimensional. The cosmological horizon has finite area $A = 4\pi c^2 / H^2$; the bound applies.

*Derivation.* Any CPTP quantum channel on a finite-dimensional Hilbert space admits a Stinespring dilation: a unitary on a larger space such that the channel is recovered by partial trace [1, Appendix A]. The finite permutation construction [1, Appendix A.2] shows the dilation can be chosen as a bijection on a finite set. Bell violations require the channel to be non-entanglement-breaking [1, Theorem A.3], which requires the bijection to have non-trivial coupling across the partition (C1). The characterization theorem [1, §3.3] establishes that P-indivisibility — equivalent to QM by the stochastic-quantum correspondence — requires slow-bath memory (C2) and sufficient hidden-sector capacity (C3).

*Output:* The observed quantum mechanics arises from embedded observation in some $(S, \varphi, V)$ satisfying C1–C3, where $S$ is a finite set and $\varphi: S \to S$ is a bijection with non-trivial coupling across the partition $V$.

*Status:* Theorem. The only input beyond established physics is the finite-entropy condition, which has independent justification from holographic bounds.

**Stage 2: From the embedding to the dynamics and gauge structure.**

*Additional empirical input:* Spatial isotropy of the emergent description.

*Derivation.* Stage 1 delivers a bijection on a finite set with bounded coupling. The coupling graph defines a product decomposition of $S$ into local degrees of freedom (§3.1), with spatial dimension $d$ extracted from the causal partial order (§7.3). Four independent self-consistency filters converge on $d = 3$: the dark-sector concordance $\rho_s / \rho_{\mathrm{crit}} = 2/(d-1) = 1$ (this paper, §7.3), propagating gravity requiring $d \geq 3$ (Weyl tensor), stable matter requiring $d \leq 3$ (Ehrenfest [13, 14]), and renormalizable gauge couplings requiring $d = 3$ (dimensional analysis).

On the $d = 3$ lattice, among all second-order reversible nearest-neighbor dynamics, center independence (required for QM emergence by Stage 1), spatial isotropy (the empirical input of this stage), and linearity (selected by three independent criteria: maximum propagation speed, maximum P-indivisibility, and horizon thermality [2, §11.2]) uniquely select the discrete wave equation [2, Theorem 8]. This is a uniqueness theorem, not an assumption: the wave equation is the only dynamics consistent with the QM requirement plus observed isotropy.

The wave equation on a $d = 3$ lattice further constrains the emergent QFT. The argument proceeds in three steps; detailed proofs are given in a companion paper.

*(i) Internal components.* Extending the dynamics selection to multi-component fields, each site carries a $K$-component vector coupled by a matrix $M$. The spatial coupling graph has $2d$ nearest neighbors; coupling-degree minimization — the same principle that determines the product decomposition of $S$ (§3.1) — uniquely gives $K = 2d = 6$ internal components. In $d = 3$, the cubic rotation group $O$ acts on the 6-dimensional internal space and decomposes it as $6 = T_1(3) \oplus E(2) \oplus A_1(1)$, fixing the eigenvalue multiplicities of $M$ to $(3, 2, 1)$. The commutant of $M$ — the group of transformations preserving the dynamics — is $\mathrm{U}(3) \times \mathrm{U}(2) \times \mathrm{U}(1)$. Background independence (§7.4) makes $M$ site-dependent, promoting the global commutant to a local gauge symmetry: the link variable $M(\mathbf{n}, \hat{e}_j)$ transforms as a gauge connection, and the plaquette product gives the Wilson action.

*(ii) Matter content.* The wave equation is the massless lattice Klein-Gordon equation, which factors into staggered Dirac operators via the standard Susskind construction. Center independence is equivalent to exact chiral symmetry (vanishing lattice mass), mandating the Higgs mechanism. In $d = 3$, the staggered species decompose under the cubic group as $\mathbf{1} \oplus \mathbf{3}$: one singlet (spin-0, the Higgs sector) and three triplet tastes (spin-1/2, three fermion generations). Anomaly cancellation uniquely determines the hypercharges and matter representations — the unique solution is the Standard Model assignment.

*(iii) Discrete symmetries.* T-invariance of the wave equation forces $\theta = 0$. The bijection structure of $\varphi^n$ (a bijection for every $n$, since it is a power of a bijection on a finite set) guarantees detailed balance at every time scale. Detailed balance implies a T-invariant effective Hamiltonian at every energy scale, forcing real Yukawa matrices at every scale, hence $\bar{\theta} = 0$ at all energy scales. No axion is needed; the proof bypasses the instanton question entirely.

*Output:* The dynamics, dimension, gauge group, matter content, and discrete symmetries are all derived — not assumed. The SM gauge structure, three generations, and $\bar{\theta} \approx 0$ are retrodictions: they are derived from the Stage 1 output and checked against observation. Each is a non-trivial constraint that could have failed.

*Status:* The dynamics selection and dimension are theorems (proved in [2] and this paper respectively). The gauge derivation chain (steps (i)–(iii)) is stated here with proof sketches; end-to-end proofs are given in a companion paper. The individual steps use standard results (Susskind's staggered fermion construction, the cubic group decomposition, anomaly cancellation uniqueness) combined with the framework's lattice structure.

**Stage 3: From the dynamics to the emergent constants.**

*No additional empirical input.*

*Derivation.* The wave equation from Stage 2 produces all inputs for Jacobson's thermodynamic derivation of Einstein's equations: area-law entropy [2, Theorem 4], Lorentz-invariant dispersion [2, Theorem 5], the Unruh temperature [2, Theorem 6], and the Raychaudhuri identity [2, Theorem 7]. The gap equation — derived from thermal self-consistency between the classical horizon temperature and the emergent quantum temperature [1, §5; 2, Theorem 3] — uniquely determines $\hbar = c^3 \epsilon^2 / (4G)$ with $\epsilon = 2\,l_p$.

*Output:* The relationship $\hbar = c^3 \epsilon^2 / (4G)$ and the Bekenstein-Hawking formula $S = A/(4\,l_p^2)$ are derived consequences, not inputs.

*Status:* Theorem.

**The full reconstruction.** Combining all three stages:

**Theorem** (Layered reconstruction). *Observed quantum mechanics with Bell violations and finite boundary entropy, together with spatial isotropy, uniquely determine — at the lattice level — the equivalence class $[(S, \varphi)]/\!\sim$: a finite set with a bijection of bounded coupling degree and statistical isotropy, with $d = 3$, $K = 6$, coupling matrix eigenvalue multiplicities $(3, 2, 1)$, $\bar{\theta} = 0$, and $\hbar = c^3 \epsilon^2/(4G)$. The equivalence class is unique up to $q$-gauge freedom, partition genericity, and choice of specific isotropic bounded-degree representative.*

*Proof.* Stage 1 $\Rightarrow$ Stage 2 $\Rightarrow$ Stage 3, as detailed above. Each stage uses proved results from the companion papers and standard mathematical results. $\square$

The logical structure is important. The theorem does not assume the framework's outputs and conclude the framework — it starts from what is empirically established (QM with Bell violations, finite boundary entropy, spatial isotropy) and derives the framework as the unique lattice-level explanation. The wave equation, the SM gauge structure, the gap equation, and the dimensional selection are all intermediate results within the reconstruction, not inputs to it. The SM gauge structure, three generations, one Higgs doublet, and $\bar{\theta} = 0$ are retrodictions — derived from the lattice structure and checked against observation — not assumptions fed into the theorem.

The reconstruction can also be read as a prediction generator. Any feature derived in Stages 2–3 that is not part of the empirical input is a retrodiction (if already observed) or a prediction (if not yet tested). The retrodictions include three generations, one Higgs doublet, $\theta = 0$, and the Bekenstein-Hawking formula. The predictions — dark energy evolution with $\nu_{\text{OI}} \approx 2.45 \times 10^{-3}$, GW echoes, no axion, no SUSY partners, no fourth generation, Majorana neutrinos — are falsifiable.

**Remark** (Scope). The reconstruction establishes uniqueness within the class of deterministic bijections on finite sets with bounded coupling. It does not exclude the logical possibility of explanations outside this class — e.g., intrinsically continuum theories, or theories with fundamentally stochastic dynamics. What it establishes is that within the class of finite deterministic systems, the observed physics pins down the structure completely.

**Remark** (Continuum extension). The lattice-level reconstruction extends to the full continuum Standard Model under the assumption of lattice QFT universality: that the lattice gauge theory with the algebraic structure $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ flows to the correct continuum QFT under the renormalization group. This assumption is supported by decades of numerical evidence and is universally used in lattice QCD, but is not rigorously proved — it is essentially the Yang-Mills mass gap problem (Clay Millennium Prize). This is the sole remaining structural gap, shared with all lattice formulations of the Standard Model — it is not specific to the OI framework.
...
The distinction between "mathematics describes reality" and "mathematics is reality" therefore has no empirical content within the framework — not because the question is unimportant, but because it is provably undecidable by any measurement. The reconstruction theorem establishes that the mathematical description $[(S, \varphi)]/\!\sim$ and the observed physics determine each other up to gauge. Any experiment that could distinguish "the universe is $[(S, \varphi)]$" from "the universe is described by $[(S, \varphi)]$" would have to detect structure outside the equivalence class — but the equivalence class already accounts for everything observable.
