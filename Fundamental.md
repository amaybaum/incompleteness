...
All three SM gauge couplings are reproduced. The model has two undetermined parameters ($A$, $B$) fitted to two non-trivial data points (SU(2) and SU(3)); U(1) fixes $\delta_0$. The two-loop VP computation gives $\delta_0 = 8.0 \pm 2$ from first principles, with all four diagrams (SE, VC, $\Pi_q$, sails) computed directly, consistent with the required 10.0 within the systematic uncertainty.
