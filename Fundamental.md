...
**Theorem (factorization uniqueness for translation-invariant dynamics).** *Let $\varphi$ be a bijection on $S = (\mathbb{Z}/q\mathbb{Z})^N$ satisfying: (a) translation invariance (the update rule is the same at every site), (b) range-1 coupling on the natural factorization, and (c) the coupling is genuinely bidirectional (each site's output depends non-trivially on both neighbors). Then the natural factorization is the unique minimizer of coupling degree among all product decompositions $S = S_1 \times \cdots \times S_M$ with $|S_k| = q$ for all $k$.*

*Proof sketch.* Any alternative factorization $S = S_1' \times \cdots \times S_M'$ with $|S_k'| = q$ corresponds to a different labeling of the $N \log_2 q$ bits of the state. In the natural factorization, each site couples to exactly $2d$ neighbors (coupling degree $2d$). For the coupling degree to be at most $2d$ in the alternative factorization, each new "site" $S_k'$ must depend on at most $2d$ other new "sites" under $\varphi$. By condition (c), each natural site $i$ has irreducible dependence on $2d$ natural neighbors — the dependence cannot be decomposed into independent sub-blocks without increasing the coupling degree, because the wave equation mixes all input bits (the addition $x_{i-1} + x_{i+1} - x_{i}^{\text{prev}}$ creates algebraic dependence among all input components when $q > 2$). Any relabeling that splits or merges natural sites therefore either preserves the natural factorization (up to relabeling) or increases the coupling degree. The natural factorization is unique. $\square$
...
**Remark (status of coupling-degree minimization).** The objection that "nature need not minimize coupling degree" mistakes the logical role of this principle. The framework does not postulate that nature prefers minimal coupling — it derives the product decomposition of $S$ as the canonical factorization of $\varphi$, and shows that this factorization is unique (§3.1, factorization uniqueness theorem for translation-invariant dynamics). The coupling-degree minimization is not an aesthetic choice imposed on nature but the mathematical definition of "site" given a bijection. A system with $K = 8$ or $K = 4$ would correspond to a *different bijection* $\varphi'$ with different coupling structure — not to the same $\varphi$ with a non-minimal factorization. Since the factorization determines K, and the factorization is unique for the dynamics selected by center independence, isotropy, and linearity (§7.1), $K = 2d = 6$ is a theorem about the wave equation on $\mathbb{Z}^3$, not a selection rule.
