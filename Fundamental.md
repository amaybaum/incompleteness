...
**Relation to computation.** A Turing machine has a tape (storage), a head (partial read/write access), and a transition function (update rule). Under effective finiteness (Main, §9.7), the correspondence is not merely suggestive — it is structural. The visible sector $V$ is the head: a finite-state subsystem with bounded local coupling. The hidden sector $H$ is the tape: a memory register (possibly infinite) that stores correlations written by past interactions. The bijection $\varphi$ is a reversible transition function: no erasure, no halting, no information loss. The conditions C1–C3 characterize this architecture: C1 (coupling) is the read/write mechanism; C2 (slow bath) says the tape retains its contents between head operations; C3 (capacity) says the tape is large enough to store the full interaction history. These are not analogies — they are the structural properties of a reversible Turing machine with a finite head coupled to a persistent memory.

The sole genuine difference is the *question asked*. Turing asked: what can a machine compute for an external observer? The framework asks: what does computation look like to a component of the machine? The answer is quantum mechanics. The head cannot access the tape's global state; it sees only local read statistics filtered through its finite window. The characterization theorem proves that these statistics are necessarily P-indivisible — hence quantum-mechanical. The appearance of dynamics, probability, particles, and forces is entirely the head's perspective: what a reversible computation looks like from within.

This sharpens the incompleteness family (below): Turing's halting problem says the head cannot determine the global behavior of the computation from its finite window; the reconstruction theorem says the observer cannot determine whether $(S, \varphi)$ *is* reality or *describes* reality from its finite window — and identifies this undecidability as gauge. Both are instances of self-reference under finite resources.
