...
**Theorem 6** ($K = 2d$). *For any bounded-degree graph with polynomial growth exponent $d$ and statistical isotropy, the wave equation (selected by center independence, isotropy, and linearity) has a unique minimal-coupling factorization with $K = 2d$ components, where minimal-coupling factorizations are restricted to those in which every component is spatially coupled (i.e., $\delta(\text{component}) \geq 1$, no fully inert components).*

*Remark on the no-inert-components restriction.* A component with $\delta = 0$ — no spatial input from any neighbor — receives no information from any other site, and its evolution is determined by its own initial data alone. Such a component carries no propagating physics: it cannot transmit, receive, or participate in any observable interaction with the rest of the lattice. The factorization $\boldsymbol{\phi} = (\boldsymbol{\phi}_{\text{coupled}}, \boldsymbol{\phi}_{\text{inert}})$ separates into independent subsystems, and $\boldsymbol{\phi}_{\text{inert}}$ contributes no observable degrees of freedom. Removing $\boldsymbol{\phi}_{\text{inert}}$ leaves all observable physics unchanged, so the "minimal" in minimal-coupling is naturally taken to range over factorizations whose components all participate in the dynamics. Without this restriction, $K = 2d + n$ for any $n \geq 0$ trivially achieves $\delta = 1$ on the coupled sector by padding with inert components, and the minimization is degenerate.
...
*Step 3 (K > 2d forces δ ≥ 2 under the no-inert-components restriction).* With K > 2d components, each required to have $\delta \geq 1$, but only $2d$ distinct spatial channels available, the pigeonhole principle in the reverse direction guarantees that at least two components must share a channel — i.e., both depend on the same neighboring site's value through $M$. Two components that depend on exactly the same single input channel and on no others would be linearly dependent dynamical variables, contradicting the assumption that the K components are independent degrees of freedom. To remain independent, at least one of the channel-sharing components must additionally couple to a second spatial channel through $M$: δ(K) ≥ 2.

*Uniqueness.* Steps 1–3 show δ = 1 iff K = 2d.

*Extension to general graphs.* For a bounded-degree graph $G$ with polynomial growth exponent $d$ and statistical isotropy: (i) $G$ is quasi-isometric to $\mathbb{Z}^d$ (§6, Gromov's theorem); (ii) the wave equation on $G$ has $2d$ independent low-energy propagation modes, determined by the spectral dimension $d_s = d$ (from heat kernel asymptotics $p_t(x,x) \sim t^{-d/2}$ on graphs with polynomial growth exponent $d$); (iii) the coupling-degree minimizer $K = 2d$ on $\mathbb{Z}^d$ is stable under quasi-isometric deformations, because $\delta(K)$ depends on the graph only through its spectral dimension, which quasi-isometries preserve — and $K$ is integer-valued, so it cannot change without a discontinuous jump in $d_s$. $\square$

This is the per-site analog of the global factorization principle (§3.1): at the global level, $S = S_1 \times \cdots \times S_N$ is the unique factorization minimizing coupling degree — defining "space"; at the per-site level, $K$ components minimize internal coupling degree — defining "gauge structure." The objection that "nature need not minimize coupling degree" mistakes the logical role. The framework does not postulate a preference for minimal coupling — coupling-degree minimization is the mathematical definition of "site" given a bijection, and the factorization is unique (§3.1). A system with $K = 8$ or $K = 4$ would correspond to a *different bijection* $\varphi'$, not to the same $\varphi$ with a non-minimal factorization. Since the dynamics is selected by center independence, isotropy, and linearity (§7.1), and its factorization is unique, $K = 2d = 6$ is a theorem about the wave equation on $\mathbb{Z}^3$, not a selection rule.
...
*Proof.* The singlet taste is spin-0 (Theorem 9a) and carries the K = 6 internal structure of the wave equation, which by Theorem 7 decomposes under the gauge group as $T_1(\mathbf{3}) \oplus E(\mathbf{2}) \oplus A_1(\mathbf{1})$. The physical Higgs is identified with the $E(\mathbf{2})$ component.

*(i) Gauge structure.* The $E(\mathbf{2})$ component is the unique projection of the singlet taste that transforms as an SU(2) doublet, and is therefore the only component that can form gauge-invariant Yukawa couplings to the SM quark and lepton doublets. Concretely, the up-quark Yukawa $\bar Q_L \cdot \Phi \cdot u_R$ with $Q_L \sim (\mathbf{3}, \mathbf{2}, +\tfrac{1}{6})$ and $u_R \sim (\mathbf{3}, \mathbf{1}, +\tfrac{2}{3})$ requires $\Phi$ to transform as $(\mathbf{1}, \mathbf{2}, +\tfrac{1}{2})$ — uniquely an SU(2) doublet with hypercharge $+\tfrac{1}{2}$. The $T_1(\mathbf{3})$ projection of the singlet taste is a color triplet and cannot enter such a Yukawa coupling without violating SU(3) gauge invariance; the $A_1(\mathbf{1})$ projection is a color and weak singlet and cannot couple a doublet to a doublet without violating SU(2) gauge invariance. Only the $E(\mathbf{2})$ projection has the gauge structure required to produce SM fermion masses through Yukawa couplings.

*(ii) Hypercharge.* The bilinear $\bar Q_L u_R$ carries hypercharge $-\tfrac{1}{6} + \tfrac{2}{3} = +\tfrac{1}{2}$, which the scalar field must compensate for the Yukawa coupling to be gauge-invariant. The Higgs hypercharge is therefore $+\tfrac{1}{2}$. Anomaly cancellation (Theorem 14) confirms that this hypercharge assignment is consistent with the rest of the SM matter content; equivalently, no other hypercharge for $\Phi$ admits gauge-invariant Yukawa couplings to the entire fermion sector.

*(iii) Status of the $T_1$ and $A_1$ components.* The $T_1$ and $A_1$ projections of the singlet taste do not admit Yukawa couplings to the fermion sector and contribute no fermion masses. They appear at the lattice cutoff as auxiliary scalar degrees of freedom and do not enter the low-energy SM as additional Higgs-like fields. Their fate under the staggered-to-Dirac reconstruction is part of the standard staggered identification of physical degrees of freedom [5, 12]; in the emergent description below the cutoff, the singlet taste produces exactly one SU(2)-doublet Higgs with hypercharge $+\tfrac{1}{2}$. $\square$
...
**Remark (the two non-gauge U(1) combinations).** Theorem 14 establishes that *exactly one* linear combination of $\mathrm{U}(1)_B \times \mathrm{U}(1)_L \times \mathrm{U}(1)_S$ is anomaly-free and therefore consistently gauged — this is the SM hypercharge $\mathrm{U}(1)_Y$. The remaining two-dimensional space of $(B, L, S)$ combinations cannot be gauged: gauging them would introduce non-cancelling chiral anomalies, rendering the theory quantum-mechanically inconsistent. These two combinations survive instead as **accidental global symmetries** of the emergent theory — not imposed by the framework, but unavoidably present because the renormalizable Lagrangian admits no operators that violate them. This matches the standard SM construction, in which baryon number $B$ and lepton number $L$ also appear as accidental global symmetries (not gauge symmetries) of the renormalizable Lagrangian, broken only by non-renormalizable operators (e.g., the Weinberg operator $LLHH$ which violates lepton number) or by non-perturbative effects (e.g., the electroweak sphaleron). The framework's two extra $\mathrm{U}(1)$ factors therefore do not introduce new gauge bosons or new gauge interactions; they are conserved currents of the emergent SM, with the same status as $B$ and $L$ in the standard treatment. No mechanism is needed to "remove" the extra factors — they are simply not gauged, and their associated charges are automatically conserved at the renormalizable level. The prediction "$B$ and $L$ are accidental global symmetries" is therefore a structural feature of the framework, in agreement with observation (proton stability, lepton number conservation up to neutrino oscillations and the small Majorana mass implied by the Weinberg operator).
...
### 9.6 Status of the calculation

The components of the prediction in §9.3 occupy distinct epistemic statuses:

- **Structural prediction (zero parameters)**: $1/\alpha_0 = 23.25$ from the one-loop staggered vacuum polarization. This is an analytic result determined by $N_f = 6$ (from $K = 2d$, Theorem 6) and $T(R) = 1/2$, with no adjustable inputs.
- **Doubly-constrained free parameter**: $\delta_0$, the universal $C_2$-independent threshold. Constraint 1 — the U(1) coupling at $M_Z$ (which is unaffected by $C_2$-dependent gauge self-interactions) requires $\delta_0 = 10.02$ exactly. Constraint 2 — the two-loop staggered VP computation (§9.2) gives $\delta_0 = 8.0 \pm 2$ from first principles, evaluating an 8-dimensional lattice integral on $N^4$ grids up to $N = 12$ with Richardson extrapolation. The two values are consistent at $\sim 1\sigma$. The framework's quantitative content here is that the same value of $\delta_0$ that the structural calculation produces (within its uncertainty band) is the value U(1) requires.
- **MC measurements as physical inputs**: $(A, B) = (8.3, 5.59)$ are determined by fitting the logarithmic resummation form to two pure-gauge Monte Carlo plaquette measurements: $\langle P \rangle_{SU(2)} = 0.783$ at $\beta_2 = 7.4$, and $\langle P \rangle_{SU(3)} = 0.806$ at $\beta_3 = 11.1$. These plaquettes are non-trivial lattice computations at the OI couplings, not fits to M_Z observations. The fit is two parameters to two data points, so it is algebraically tight; the substantive content is in the data points themselves and in the chain that uses them.
- **Predictions of the chain (no further free parameters)**: with $1/\alpha_0$ structural, $\delta_0$ constrained by U(1), and $(A, B)$ determined by the MC plaquettes, the SU(2) and SU(3) couplings at $M_Z$ are predictions of the chain MC plaquette $\to$ lattice-to-MS-bar matching $\to$ SM RG running. They match observation at $<0.1\%$ (SU(2)) and $0.3\%$ (SU(3)).

**What is in development:** The current procedure uses pure-gauge Monte Carlo (Wilson action) at the OI couplings as the lattice computation, with the scheme conversion from the lattice plaquette to the framework's preferred scheme absorbed into the $(A, B)$ parametrization. A more direct first-principles calculation would simulate the OI-induced theory directly — the gauge action obtained by integrating out the staggered fermions, with the OI matter measure described in §9.5 (the discrete pushforward of the uniform measure on $(\mathbb{Z}/q\mathbb{Z})^{K \times N}$, not the Haar measure). This would eliminate the scheme conversion step and provide an independent cross-check of $(A, B)$. The HMC code for this calculation has been developed but currently uses Haar measure rather than the OI matter measure; preliminary $L = 2$–$4$ runs give the universal threshold $\delta_0 \approx 10$–$12$, consistent with the two-loop value within statistical and finite-volume systematics. Replacing the Haar measure with the discrete matter measure and pushing to $L \geq 8$ would be the natural next step. This is a research project on a multi-week timescale; the gauge coupling result as currently presented does not depend on its completion.
...
C2: the wave equation is a Hamiltonian system — it conserves energy and preserves phase-space volume. The hidden sector has spectral gap Δ = 0: correlations persist indefinitely, not just until some relaxation time. The C2 necessity proof [1, §3.3] establishes that $\Delta \tau_S \gg 1$ drives the system to Markovianity; for any Hamiltonian hidden sector, $\Delta \tau_S = 0 \ll 1$ — maximally in the slow-bath regime ($\tau_B \to \infty$, the strongest possible satisfaction of $\tau_S \ll \tau_B$), regardless of partition geometry.
...
From the gauge coupling prediction (§9): all three SM gauge couplings at $M_Z$ are reproduced through a one-parameter chain. The structural input is $1/\alpha_0 = 23.25$. The universal threshold $\delta_0$ is doubly constrained (the U(1) row requires $10.02$ exactly; an independent two-loop staggered VP calculation gives $8.0 \pm 2$; consistent at $\sim 1\sigma$). The $C_2$-dependent threshold is determined by pure-gauge MC plaquette measurements at the OI couplings. The SU(2) and SU(3) M_Z couplings are then predictions of the chain at $<0.1\%$ and $0.3\%$.
...
### 10.8 Neutrino masses from taste-breaking

The staggered fermion construction gives 4 tastes decomposing as $1(A_1) + 3(T_1)$ under the cubic group $O$ (§7.7). The singlet taste is identified as the Higgs sector — not a right-handed neutrino. Neutrinos therefore acquire mass only through the dimension-5 Weinberg operator $(LLHH)/\Lambda$, making them Majorana particles.
...
The framework starts from a single definition — observation is a triple $(S, \varphi, V)$ consisting of a total system, a deterministic dynamics, and an embedded observer — from which three structural lemmas follow (finiteness, causal partition, unique measure), with no quantum mechanics, general relativity, or specific dynamics. The companion paper [1] proves that embedded observation under conditions (C1)–(C3) is equivalent to unitary QM, derives ℏ = c³ε²/(4G) and the Bekenstein-Hawking formula, dissolves the CC discrepancy, and predicts dark energy evolution ($\nu_{\text{OI}} = (2.45 \pm 0.04) \times 10^{-3}$, consistent with DESI DR2) and dark matter phenomenology from galaxies through clusters to the Bullet Cluster. This paper establishes that center independence, isotropy, and linearity uniquely select the wave equation (§7.1), which passes all seven GR tests with no free parameters (§5). The framework's results depend on six structural properties and nothing else; the wave equation on a d = 3 lattice determines the Standard Model with the primary derivation chain proved end-to-end (§7), resolves the strong CP problem (§8), and reproduces all three SM gauge couplings at $M_Z$ via a one-parameter chain combining the structural induced coupling, a doubly-constrained universal threshold (U(1) and 2-loop, consistent at $\sim 1\sigma$), and a $C_2$-dependent threshold extracted from pure-gauge MC plaquettes (§9).
