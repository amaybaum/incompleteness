...
This is the per-site analog of the global factorization principle that determines the coupling graph (§3.1).
...
**Open problems.** (i) *Continuum-limit universality* — the proof that the lattice gauge theory flows to the correct continuum QFT. This is shared with all lattice QFT (essentially the Yang-Mills mass gap problem). (ii) *Generation identification* — the fermion count (three) is proved; the assignment of specific representations to specific tastes is a uniqueness argument from anomaly cancellation, not a geometric derivation. (iii) *Taste-breaking phenomenology* — whether specific patterns in φ reproduce the observed fermion mass ratios and mixing angles.
...
**Remark** (Continuum extension). The lattice-level reconstruction extends to the full continuum SM under lattice QFT universality — supported by decades of numerical evidence but not rigorously proved (the Yang-Mills mass gap problem). This gap is shared with all lattice QFT.
