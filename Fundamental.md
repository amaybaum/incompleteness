...
**Deterministic bijectivity.** The P-indivisibility proof [1, §2.3] requires a bijection on a finite set: bijectivity guarantees recurrence (φ^N = id), and recurrence produces the non-monotonic distinguishability that defines P-indivisibility. The entire framework rests on QM emerging from marginalizing deterministic dynamics — if the dynamics were already stochastic, there would be nothing to derive. Determinism cannot be weakened, but does not require a lattice.
...
The full proof for general bounded-degree graphs with statistical isotropy (without exact translation invariance) is an open problem; the theorem above covers the case relevant to the Standard Model derivation. The robustness of integer multiplicities under small perturbations (Theorem 7, remark on statistical isotropy) provides additional protection: even if an alternative factorization existed with marginally different coupling structure, the eigenvalue multiplicities of $M$ are integers and cannot change continuously.
...
The coupling graph is not embedded in a pre-existing space — there is no ambient manifold. Vertices are labels for degrees of freedom; edges record dynamical coupling. Asking "what is site i made of?" is a category error. The common objection to discrete models — if space is a lattice, what does the lattice sit in? — does not arise on the structural reading. Two sites are neighbors because φ couples them; locality is defined by the dynamics, and space emerges from locality.
...
**Explicit construction.** On a ring of L ≥ 4 sites with alphabet ℤ/qℤ, a state-dependent right-neighbor assignment $R(i, x)$ that depends only on $x_{i+1}$ (not $x_i$) produces dynamics satisfying all three constraints with range 1. Bijectivity follows from explicit inverse construction. Center independence, P-indivisibility, and bounded graph degree all hold for any L and q.
...
*Exponential growth* fails (no Lorentz-invariant dispersion; incompatible with stable matter and renormalizability). *Fractal growth* fails (no Raychaudhuri equation, no hypercubic BZ structure, no cubic group decomposition). Only *integer-polynomial growth* survives; by Gromov's theorem and statistical isotropy, the graph is quasi-isometric to $\mathbb{Z}^d$ for integer $d$. The four filters then select $d = 3$.
...
**Relation to the anthropic principle.** This is not selection from a landscape. The framework's axioms require an observer as a constitutive element (Axiom 3 defines the partition relative to an observer); the concordance calculation constrains $d$ from within, not by filtering a multiverse.
...
*Center independence is necessary for emergent QM.* Without center coupling, the update $x_i(t+1) = [h_{i-1}(t) + h_{i+1}(t) - x_i(t-1)] \bmod q$ does not contain $x_i(t)$, so the visible present cannot screen the visible past from the future — correlations bypass the present through the hidden sector, producing P-indivisibility. With center coupling, $x_i(t)$ appears explicitly; conditioning on $(X_V(t), X_V(t+1))$ generically determines $H(t)$, making the process Markov.
...
*Linearity is selected by four independent criteria:* (i) $q$-gauge invariance — nonlinear dynamics makes predictions $q$-dependent, contradicting the gauge freedom; (ii) maximum propagation speed — nonlinear CI dynamics over $\mathbb{R}$ has $v < 1$; (iii) maximum P-indivisibility — $\alpha = 1$ uniquely maximizes it among all linear CI dynamics; (iv) KMS thermality — only linear dispersion supports the Jacobson derivation (§5). All four converge on the wave equation.
...
*Step 3 (K > 2d forces δ ≥ 2).* Excess components beyond 2d have no dedicated spatial input and must depend on at least 2 channels through M: δ(K) ≥ 2.

*Uniqueness.* Steps 1–3 show δ = 1 iff K = 2d. $\square$

This is the per-site analog of the global factorization principle (§3.1). Coupling-degree minimization is the mathematical definition of "site," not a physical preference — $K = 2d = 6$ is a theorem about the wave equation on $\mathbb{Z}^3$, not a selection rule.
...
**Remark (robustness).** The multiplicities $(3, 2, 1)$ are determined by the topology of $d = 3$ rotations (§6: the graph is quasi-isometric to $\mathbb{Z}^3$), not by exact cubic symmetry. Since eigenvalue multiplicities are integers, small perturbations breaking exact $O$ symmetry to statistical isotropy cannot change them continuously — the decomposition is stable.
...
**Remark.** The checkerboard is not selected by hand — it is the bipartite structure of range-1 coupling on a hypercubic graph. The staggered-to-Dirac reconstruction identifies the two sublattices with left- and right-handed spinor components (a standard result [5, 12]). At macroscopic scales, any region contains equal even and odd sites; chirality follows from the bipartite dynamics, not from a choice of partition.
...
The wave function is derived, not fundamental; asking "does it collapse?" is asking about a compression artifact. Branching is forbidden: φ has exactly one trajectory from any initial state. Bell correlations arise from correlated hidden-sector configurations at preparation [1, §3.2], mediated by the coupling graph without superluminal influence.
...
The mass hierarchy and the Higgs naturalness problem both fall in the second category: the Higgs mass is an eigenvalue of M (a property of φ), not a radiative correction.

**Remaining open problems.** (i) *Lattice-continuum comparison* — the lattice is the fundamental description ($\epsilon = 2l_p$ is physical); lattice-continuum discrepancies are $(E/M_{\text{Pl}})^2 \sim 10^{-32}$ at LHC energies. The Yang-Mills mass gap problem does not affect OI predictions. (ii) *Generation identification* — the fermion count (three) is proved; the assignment to specific tastes follows from anomaly cancellation, not geometric derivation.
...
**Relation to computation.** The correspondence with a reversible Turing machine is structural: S is the tape, V is the head's access window, φ is a reversible transition function, C1–C3 characterize the architecture. The framework extends Turing's question: instead of asking what a machine can compute for an external observer, it asks what computation looks like to a component of the machine — and proves the answer is quantum mechanics.
...
**The incompleteness family.** The result belongs to a family of impossibility-with-structure theorems: Gödel (unprovable truths have rigid structure), Turing (undecidable problems have rigid structure), OI (the inaccessible state produces rigid emergent structure). The common pattern is self-reference under finite resources. The framework distinguishes *undecidable* quantities (the hidden state $h$ — definite, consequential, provably inaccessible; the structural consequence is QM) from *gauge* quantities ($q$, $|\mathcal{C}_D|$ — different values produce identical observables).
...
**Remark.** The reconstruction establishes uniqueness within finite deterministic systems with bounded coupling; it does not exclude continuum or stochastic alternatives. The lattice is the fundamental description ($\epsilon = 2l_p$ is physical, not a regulator), so lattice-continuum discrepancies at accessible energies are suppressed by $(E/M_{\text{Pl}})^2 \sim 10^{-32}$.
