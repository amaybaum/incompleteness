...
From the cosmological application [1]: dark energy evolution with $\nu_{\text{OI}} = (2.45 \pm 0.04) \times 10^{-3}$ (independently testable ratio $\nu/\Omega_\Lambda = 0.00358 \pm 0.00003$; DESI DR2 reports 2.7–4.2$\sigma$ evidence for evolving DE, with RVM fits at $\nu \sim \mathcal{O}(10^{-3})$); GW echoes with first-echo amplitude $A_1 = \Gamma \approx 0.45$, delay $\Delta t = (r_+/c)\ln(r_+/2l_p)$, and fitting factor FF = 0.19 against long-lived QNM searches (non-detection by Wu et al. 2025 is consistent with the prediction — a dedicated matched-filter search on GW250114 is the definitive test); dark matter from entropy displacement with $a_0 = cH/6$, the baryonic Tully-Fisher relation $v^4 = GM_B \cdot cH/6$, the simple interpolation function $\nu(y) = (1 + \sqrt{1+4/y})/2$ matching galaxy clusters (Coma to $< 1\%$), the Bullet Cluster gas-lensing offset (frozen boundary entropy with $\tau_{\text{relax}} \sim H^{-1} \gg t_{\text{collision}}$), and redshift evolution $a_0(z) = cH(z)/6$ (declining rotation curves confirmed by Genzel et al. 2017 at $> 3\sigma$; stellar Tully-Fisher no-evolution confirmed by McGaugh et al. 2024, predicted by gas fraction compensation).
