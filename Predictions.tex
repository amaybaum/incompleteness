...
We present twenty-one predictions spanning gauge couplings, quark mixing, lepton masses, neutrino mixing, the Higgs mass, and fermion mass ratios. Fifteen are purely geometric; two involve non-perturbative lattice dynamics at $\beta = 11.1$.
...
The mixing matrix element between generations $i$ and $j$ is the continuum fermion propagator at the inter-generation momentum: $|M_{ij}| = 1/|X_j - X_i| = 1/(\pi\sqrt{2})$. The $1/|q|$ form arises because the taste-changing transition preserves chirality: $\text{Tr}[\gamma \cdot S(q)] \propto 1/|q|$ in the massless limit. The $1/|q|^2$ form gives $\lambda^2 = m_d/m_s$ (the GST relation, from self-energy with two insertions).
...
The $T_1$ representation of SO(3) has $l = 1$ and Casimir $C_2 = 2$. The mass splitting is the unique $O$-invariant quadratic form on the generation space, given by $C_2$ normalized by the lattice bandwidth $d^2$:
\begin{equation}
...
\midrule
$\sin^2\theta_{12}$ & 0.3080 & $0.3092 \pm 0.0087$ (JUNO) & $0.14\sigma$ \\
...
\noindent\textbf{Experimental confirmation (JUNO).} The JUNO experiment reported the world's most precise $\sin^2\theta_{12}$ measurement in November 2025~\cite{JUNO}: $0.3092 \pm 0.0087$ (1.6$\times$ improvement over all previous measurements). The OI prediction matches at $0.14\sigma$. The updated global fit gives $0.3085 \pm 0.0073$~\cite{Capozzi}, matching at $0.07\sigma$.
...
Monte Carlo at $L = 16$--$64$, scanning 30 masses ($m = 0.005$--$0.50$), shows $Z_S(m)$ monotonically decreasing for $mL \gtrsim 3$. The completed $L = 32$ run (50 configs) confirms chiral condensate formation: $Z_S$ at small $m$ grows $8\times$ between $L = 16$ and $L = 32$, with the peak at $m = 0.024$ ($Z_S = 2.828 \pm 0.008$) tracking the finite-volume boundary $mL \sim 1$.

\begin{equation}
m_{\text{match}} = \lambda \times g_0^2 = \frac{1}{\pi\sqrt{2}} \times \frac{4\pi}{23.25} = \frac{4}{23.25\sqrt{2}} = 0.1217
\end{equation}
The taste-breaking amplitude $\lambda$ and the unified gauge coupling $g_0^2 = 4\pi\alpha_0$ jointly set the scale where the perturbative Yukawa description meets the non-perturbative condensate.
\begin{equation}
\boxed{\frac{m_b}{m_\tau} = \frac{4.28}{Z_S(\lambda g_0^2)} = \frac{4.28}{1.813} = 2.361}
\end{equation}
Observed: $2.352 \pm 0.017$ ($0.5\sigma$, $0.4\%$). All inputs --- $\lambda$, $g_0^2$, $R = 4.28$, and $Z_S$ (from $L = 32$, 50 configs) --- are determined by the lattice structure with zero free parameters.
...
\bibitem{JUNO} JUNO Collaboration, ``First measurement of reactor neutrino oscillations at JUNO,'' arXiv:2511.14593 (2025).
\bibitem{Capozzi} F.~Capozzi \textit{et al.}, ``Updated bounds on the (1,2) neutrino oscillation parameters after first JUNO results,'' arXiv:2511.21650 (2025).
\end{thebibliography}
