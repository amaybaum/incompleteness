...
*Proof.* For each Fourier mode $k$ the 2×2 block $B_k$ has characteristic polynomial $t^2 - \lambda_k t + 1$ with $\lambda_k = \zeta^k + \zeta^{-k}$, so its roots are $\zeta^{\pm k}$ and the diagonalizable blocks contribute orders dividing $L$. At parabolic modes ($\lambda_k = \pm 2$), the Jordan form gives
$$B_k^n = \alpha^n \begin{pmatrix} 1 & n\alpha^{-1} \\ 0 & 1 \end{pmatrix}$$
contributing order $q$ (or $2q$). Therefore $\text{ord}(F) = \text{lcm}(L, q) = qL$, which is Theorem B.3.2's decomposition $F = F_{\text{ss}} F_u$ read multiplicatively. At $q = 2$ the unipotent factor has order exactly $2$, and $\gcd(L, 2) = 1$ forces $L$ odd, so the parabolic contribution is not absorbed into $L$: $\text{ord}(F \bmod 2) = 2L$. $\square$
