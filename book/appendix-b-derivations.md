...
**Corollary 1.** *$\Phi$ is not entanglement-breaking if and only if $w < \kappa$.* Immediate from Lemma 2 and the Theorem with $t = s - \kappa + w$: entanglement breaking is invariant under composition with the unitary $\mathrm{Ad}_{V_A}$, so $\Phi$ is entanglement-breaking exactly when $\Phi_G$ is, and $t = s$ reads $w = \kappa$ (Lemma 2 gives $w \leq \kappa$, matching $t \leq s$).
