...
*Remark (failure of (C1)).* Condition (C1) does not suffice. For $|V| = |H| = 2$ and $\varphi(x,h) = (h,x)$ one has $T_{ij} = 1/2$ for all $i,j$, yet $\Phi(\rho) = I/2$ identically: a constant channel, hence measure-and-prepare. Exhaustive enumeration gives the same verdict for every (C1)-satisfying bijection at $|V| = |H| = 2$, and for 612 of 648 at $|V| = 2$, $|H| = 3$. The obstruction is structural: (C1) constrains the diagonal of $\Phi$, whereas entanglement-breaking is a property of $\mathrm{id} \otimes \Phi$ and is not fixed by the action on single-system inputs. The correct criterion is geometric.

**Definition.** For a visible region $R \subseteq \Lambda$, the inner and outer boundaries are $\partial^{-}R = \{x \in R : \exists\, z \notin R,\ z \sim x\}$ and $\partial^{+}R = \{y \notin R : \exists\, z \in R,\ z \sim y\}$. Write $\kappa = \dim \ker M_{HV}$ and $w = \mathrm{rank}\, M_{VH}$, where $M_{HV}$ and $M_{VH}$ are the blocks of the update carrying visible input to hidden output and hidden input to visible output.

**Lemma (boundary bound).** *For the update of §B.2, $\mathrm{rank}\, M_{HV} \leq |\partial^{+}R|$ and $\mathrm{rank}\, M_{VH} \leq |\partial^{-}R|$.*
...
One update step transports information one lattice site, so the sectors resolve one another only across the cut. The argument uses no property of the coefficient ring and holds over $\mathbb{Z}_q$; for a general nearest-neighbour bijection it yields the weaker statement that the hidden output depends on the visible input only through $\partial^{-}R$.

**Theorem (coherence criterion).** *$\Phi$ preserves coherence exactly between visible states differing by an element of $\ker M_{HV}$, and $\Phi$ is not entanglement-breaking if and only if $w < \kappa$.*

*Proof.* $\Phi(|i\rangle\langle i'|)$ vanishes unless $M_{HV}(i - i') = 0$, and on the surviving blocks the output is averaged over translations by $W = \mathrm{im}\, M_{VH}$. Hence $\Phi$ is a uniform random-Weyl channel over the group generated by the $2^{\,s-\kappa}$ phase operators dual to $\ker M_{HV}$ and the $2^{w}$ translations in $W$, with $s = \dim V$. The configurations $(0, v_R)$ lie in $\ker M_{HV}$, since the hidden output does not involve $v_R$, and $M_{VV}$ carries them onto the full $u'$-subspace, which contains $W$; the group is therefore abelian. A uniform mixture over an abelian group of Weyl operators is a stabilizer mixture, separable precisely when the group is maximal isotropic, that is when $(s - \kappa) + w \geq s$. $\square$

**Corollary.** *If $|\partial^{-}R| + |\partial^{+}R| < 2|R|$ then $\Phi$ is not entanglement-breaking.*

*Proof.* $\dim V = 2|R|$, so the Lemma gives $w + (\dim V - \kappa) \leq |\partial^{-}R| + |\partial^{+}R| < \dim V$. $\square$

*Remark.* For a $d$-dimensional region of linear size $\ell$, $|\partial R| = O(\ell^{d-1})$ against $2|R| = 2\ell^{d}$, so the hypothesis holds whenever $\ell > 2d$; in $d = 3$ this is roughly six sites across. The criterion carries no dependence on $|H|$: a hidden sector of $10^{122}$ states is no more decohering than a small one, since it reads only the boundary. The counterexample above is the degenerate case of the same statement — under $\varphi(x,h) = (h,x)$ every visible site lies on the cut, and the region has no bulk. Coherence is thus a bulk quantity and decoherence an area quantity, the accounting that reappears in the horizon entropy of Chapter 7.

*Remark (scope).* The Lemma and Corollary are established above. The separability threshold invoked in the Theorem is the standard criterion for stabilizer mixtures; the abelian hypothesis it requires is proved here, while the threshold itself has been checked against Choi-matrix ground truth in the configurations tested rather than derived. For non-linear nearest-neighbour rules the boundary factorization survives, but the sufficiency argument does not yet extend.
