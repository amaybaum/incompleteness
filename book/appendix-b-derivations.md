...
**Lemma 2 (surviving coherences).** *$\Phi(|i\rangle\langle i'|)$ vanishes unless $M_{HV}(i - i') = 0$, and $\Phi$ is the uniform mixture $|G|^{-1} \sum_{g \in G} g\,\cdot\,g^{\dagger}$ over an abelian group $G$ of Weyl operators with $|G| = 2^{\,s - \kappa + w}$.*

*Proof.* The hidden outputs of $i$ and $i'$ agree for some hidden input exactly when $M_{HV}(i-i') = 0$, and then for every hidden input, so the surviving coherences are those at displacements in $\ker M_{HV}$; killing the rest is the uniform mixture over the $2^{\,s-\kappa}$ phase operators dual to $\ker M_{HV}$. On the surviving blocks the output is averaged over translations by $W = \mathrm{im}\, M_{VH}$, a group of $2^{w}$ translation operators. The two families commute: the configurations $(0, v_R)$ lie in $\ker M_{HV}$, since the hidden output does not involve $v_R$, and $M_{VV}$ carries them onto the whole $u'$-subspace, which contains $W$; so $W$ lies in the image of $\ker M_{HV}$ and the symplectic form between the two families vanishes. $\square$
...
*Remark (scope).* Lemma 1 holds over any $\mathbb{Z}_q$ and, in weakened form, for arbitrary nearest-neighbour bijections. Lemma 2, the Theorem and both Corollaries are stated for $q = 2$; the symplectic and Clifford arguments carry over to prime $q$ with $2^{\bullet}$ replaced by $q^{\bullet}$, while composite $q$ requires module-theoretic treatment of rank and kernel and is not covered. All of this concerns the realization displayed above and is not a property of arbitrary finite bijections. The enumeration and the boundary bounds are reproduced by the deterministic tests in `papers/oi_lattice_code/coherence/`.
