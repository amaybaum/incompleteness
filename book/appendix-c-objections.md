...
**The framework's response.** Two routes reach the bridge's transition-statistics layer, and the dependency on any single recent result is correspondingly weaker — but neither route reaches the operational instrument algebra, which remains open ([Main §3.4]), so "either alone sufficient" would overstate what is established.
...
Either route alone establishes the transition-statistics layer, so a referee who rejects the Barandes correspondence must also reject Stinespring dilation to remove that layer — which would require identifying a specific error in the textbook operator algebra of the past seventy years. The framework's content is therefore robust against rejection of the Barandes correspondence specifically.
