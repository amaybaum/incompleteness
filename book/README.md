...
The book develops Observation Incompleteness (OI) from its foundational claim through physics and applications. The foundational result is an exact finite-horizon equivalence: finite stochastic laws, finite reversible deterministic realizations, and fixed-basis unitary/Born representations coincide. History readback then identifies the nontrivial OI sector by forcing hidden predictive memory and global indivisibility on a fixed finite recurrent representative. In the spatial theory, the coupling graph supplies exact causal cones for finite-range substrata. Bell then imposes a fork on the deterministic completion: retaining measurement independence means quantum Bell violation requires ontic parameter dependence. In the specific SM/GR branch the target realization is preparation-indexed adjacency in the existing state-dependent graph $G(x)$; H-Bell is the open metric/Ollivier--Ricci stability condition for those prepared graphs. The measurement-dependent alternative is recorded quantitatively but not adopted.

The manuscript therefore uses “equivalence of quantum mechanics and embedded observation” in a precise sense rather than as a slogan. The stochastic/quantum-representation equivalence is exact and bidirectional for finite observable laws, and finite quantum experimental envelopes have constructive finite OI realizations to arbitrary specified accuracy. The remaining qualification concerns full operational quantum mechanics: the entire coherent instrument/composite hierarchy must be represented by one compatible standard local quantum theory. That residual is kept explicit without weakening the equivalence already proved.
...
- [`ch01-observation.md`](ch01-observation.md) — **The Incompleteness of Observation.** The book's central equivalence theorem. Develops $(S, \varphi, V)$ from the standpoint of an observer with bounded access to a deterministic system and proves the finite-horizon equivalence $S\iff D\iff Q_{\mathrm{fb}}$: finite stochastic laws, finite reversible realizations, and fixed-basis unitary/Born representations are exactly intertranslatable. C4 supplies the nontrivial memory-bearing content and, for a fixed finite recurrent representative, forces global indivisibility. The spatial OI constitution supplies graph causal cones. The remaining full-QM qualification is narrow: one common coherent quantum instrument/composite representation. Bell is treated separately and explicitly: the deterministic completion keeps measurement independence, relinquishes ontic parameter independence for Bell violation, and preserves operational no-signaling.
...
- [`ch05-gauge-structure.md`](ch05-gauge-structure.md) — **The Emergence of Gauge Structure.** The minimal substratum, $d = 3$ from three independent self-consistency filters, the observer-level lattice-wave branch and its staggered factorization, the exact decomposition of the six geometric link directions into $(3,2,1)$, H-link/H-cust as the named bridges from that geometry to a single physical gauge carrier, and the resulting conditional $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ commutant construction, chirality from the trace-out (with explicit evasion of Nielsen-Ninomiya), and the six anomaly conditions forcing $Y_Q = 1/6, Y_u = 2/3, Y_d = -1/3, Y_L = -1/2, Y_e = -1$ with $|q_p| = |q_e|$ as a theorem.
...
- [`ch19-open-problems.md`](ch19-open-problems.md) — **Resolved and Open Problems in Fundamental Physics.** A status inventory of problems the framework resolves, dissolves, or reduces to named conditional bridges (quantum gravity, cosmological constant, dark matter and dark energy, measurement problem, BH information, strong CP, hierarchy, gauge group origin, generation puzzle, dark sector budget, Born rule). Five problems remaining genuinely open (fermion mass hierarchy, baryogenesis, inflation/initial conditions, Hubble tension, cosmological initial state).
