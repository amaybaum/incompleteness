...
**M — Mass-chain prediction.** The framework predicts mass relationships within a chain (e.g., $m_e$, $m_\mu$ via Koide from $m_\tau$) where one mass is empirical input and the others follow structurally. Mass-chain predictions provide *structural relationships* between empirical inputs.
...
**Layer 0 — Substratum.** Predictions at the substratum level: properties of the underlying bijection $\varphi$ on $S$, the substratum's dimensionality, the substratum's discreteness scale. Examples: dimensionality $d = 3$, finite cardinality, deterministic bijection structure. These predictions have *foundational* evidential weight — they are the framework's substratum-level commitments themselves.
...
**Layer 2(a) — Substratum-level mechanism-specific.** Predictions that require specific structural mechanism beyond Layer 1 generic structural content. Examples: $m_b/m_\tau$ requires the two-loop S$\chi$PT mechanism specialized to OI's 3D-slice cubic-group taste structure. Layer 2(a) predictions are *mechanism-specific* — they require more detailed structural content than Layer 1.
...
**Layer 3 — Mixed-level.** Predictions that mix substratum-level structural content with emergent-level computational content. Examples: $m_e, m_\mu$ via Koide from $m_\tau$ requires both substratum-level Koide relation (Layer 1) and emergent-level mass chain. Layer 3 predictions are *mixed* — they combine substratum and emergent content.
...
| 3 | $\delta_0$ from $U(1)$ row | 10.02 | E | 0 | Empirical input | Ch 5 §5.5 |
| 4 | $\alpha_2(M_Z), \alpha_3(M_Z)$ | 29.57, 8.47 | R | 0 | Retrodictions | Ch 5 §5.5 |
...
The neutrino sector provides the framework's sharpest parameter-free retrodiction. The JUNO measurement of $\sin^2\theta_{12}$ at 0.07$\sigma$ against the framework's structural value is a retrodictive match whose exact form converts JUNO's design-lifetime precision into the framework's cleanest pre-registered forward test.
...
| F8 | Cosmic recurrence | $\sim e^{e^{10^{122}}}$ timescale | Logically falsifiable only | Ch 18 §18.7 |
...
| # | Prediction | Quantitative content | Testability | Chapter |
|---|-----------|---------------------|-------------|---------|
| F13 | Cross-charged boost-LV decoupling | $\delta c \sim \Delta C_2\,(\alpha_0/4\pi)(M_X/M_{\text{Pl}})^2[c_L\ln(M_{\text{Pl}}/M_X)+c_0]$; the symmetric-point matching is certified ([SM §4.4]); the quadratic-log *form*, its scalar-/fermion-line and non-absorbed controls, and the constants $c_L \simeq -9.4 \pm 0.3$, $d \simeq -0.88$ were reported by a one-loop calculation whose driver and numerical output are not available for verification, and are not independently verified; on them the illustrative consistency window is $M_X \lesssim 3.9\times10^{9}$ GeV against the electron-sector SME coefficient and $M_X \lesssim 4\times10^{7}$ GeV against the GZK/Auger isotropic-$n{=}2$ bound (SM §3.1 (*Scope of the emergent-Lorentz claim*)), with the electron-SME and photon-sector ceilings ($7.5\times10^{9}$, $1.1\times10^{12}$ GeV) the looser sector-specific figures; the overall normalization $\Delta C_2$ remains open, so these are illustrative windows, not predictions or bounds | Multi-species LV bounds (matching certified; running reported, unverified; full reach pending $\Delta C_2$) | SM §3.1 (*Scope of the emergent-Lorentz claim*, point (iii)) |

The *rotational* Lorentz sector is deliberately **not** listed as a forward prediction. Unlike the boost sector (F13), the rotational $E_g$ anisotropy is a custodial-$\mathrm{U}(6)$ singlet with no power-law suppression ally, and its magnitude is undetermined — bifurcating, on the uncomputed smearing-kernel width $R/\varepsilon$, between a negligible dimension-$\geq 6$ residual ($\sim 10^{-32}$) and an unsuppressed dimension-$\leq 4$ operator already constrained by laboratory sidereal experiments. It is therefore the sharpest *open question* in the framework's Lorentz sector, treated as such in SM §3.1 (*Scope of the emergent-Lorentz claim*, point (iii)), not a predictive bet. Its one regime-independent feature — that any detectable residual must take the $\ell=4$ cubic-harmonic form aligned to the cosmic rest frame and correlated across species — is recorded there as a conditional falsification template, not here as a forward prediction.
...
- F6 (DESI running vacuum): DESI Year 5 confirmation of phantom-crossing dark energy would contradict the framework's Type II running-vacuum reading — the strongest available disconfirmation of its dark-energy account; the framework supplies the smooth $H^2-H_0^2$ bookkeeping but does not derive a distinctive Type II law.
...
- F13 (cross-charged decoupling): observation of an inter-species boost-Lorentz-violation that does *not* decouple with the heavy cross-charged mass — i.e. a non-decoupling (constant or pure-logarithmic) sector-differential limiting-speed difference — would falsify the framework's condensate-stabilizer mechanism for gauge content (Theorem 5, revised), under which, conditional on the symmetric-point matching under H-blind ([SM §4.4]), the differential is suppressed as $(M_X/M_{\text{Pl}})^2$ up to a logarithm. The *form* of this suppression and the coefficient $c_L \simeq -9.4 \pm 0.3$ were reported by a one-loop calculation whose driver and numerical output are not available for verification; on them the illustrative consistency window is $M_X \lesssim 3.9\times10^{9}$ GeV. The overall normalization $\Delta C_2$ is not fixed, and the window carries no weight as a prediction or bound until the calculation is independently reproduced.

## A.6 Framework's epistemic position summary

The framework's cumulative empirical record can be summarized as follows.

**Strongest current evidence.** The framework currently has no pre-registered forward confirmation. Its strongest empirical content is retrodictive: JUNO $\sin^2\theta_{12}$ at 0.07$\sigma$ (entry 12 / N1; the derivation postdates the November-2025 first results), Koide $Q$ at 0.02%, and Cabibbo at 0.04%. The genuinely pre-registered content is forward-looking — DESI Y5 ($\nu$), the anisotropy amplitude, BTFR at $z > 1$, the A2 lattice thresholds, and JUNO design-lifetime precision on the exact $\sin^2\theta_{12}$ value — documented in dated artifacts preceding their future data. The Bekenstein-Hawking coefficient (entry 24 / G4) is derived but has no direct empirical test — GW250114 constrains the classical area theorem, not the coefficient. The Rett wider-window result (entry B1) is a retrodiction, not counted among the forward confirmations.
...
**Highest-precision overall.** All twenty-two Standard Model predictions within approximately 1% or 1$\sigma$ — cumulative empirical content matching at observational precision across the full SM.
