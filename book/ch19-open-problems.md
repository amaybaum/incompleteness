...
**Framework status.** Open with a quantitative shortfall, but with a specific framework-internal mechanism identified for closing it. The framework's structural predictions provide ingredients for leptogenesis via the Weinberg operator $(c_{ij}/\Lambda) L_i L_j H H$: Majorana neutrinos (Chapter 6 §6.7), large PMNS phases, and hierarchical masses. The operator mediates $\Delta L = 2$ scattering in thermal equilibrium at $T > 10^{14}$ GeV and decouples at $T_{\text{dec}} \sim 10^{14}$ GeV, above the flavor threshold.

A precise calculation reveals a quantitative shortfall. The total unflavored CP asymmetry vanishes by CPT plus unitarity ($\sum_\alpha \epsilon_\alpha = 0$), and the surviving flavored asymmetry from thermal corrections is suppressed by $y_\tau^2 \sim 5 \times 10^{-5}$, yielding $\eta_B \sim 4 \times 10^{-11}$ — approximately $15 \times$ below the observed value of $6.1 \times 10^{-10}$.

This shortfall is the same problem faced by any framework with Majorana neutrinos but no heavy right-handed neutrinos: the Weinberg operator alone, without a UV completion providing additional CP-violating phases, cannot produce sufficient baryon asymmetry. The framework's emergent T-invariance (which resolves the strong CP problem, §19.2.7) constrains CP violation *at the emergent level* to the CKM and PMNS sectors, preventing new sources from appearing in the continuum SM effective theory.

**The substratum-emergent distinction applied to baryon number.** The framework's distinctive structural commitment is that operators at the substratum level and operators at the emergent level can be functionally distinct (Chapter 6 §6.3, Chapter 16 §16.10). Two operators that are identical at the emergent level can have different substratum content. The framework's resolution of the strong CP problem (§19.2.7) relies on this distinction: the emergent T-invariance is consistent with substratum-level structure that *would* produce a $\bar\theta$ at the emergent level if mapped naively.

Applied to baryon number, the framework's content is: the substratum-level baryon-number operator may have anomalies under the substratum's gauge equivalence $\mathcal{G}_{\text{sub}}$ that don't appear under the emergent $\mathrm{SU}(2)_L$. The standard SM gives $B+L$ violation through electroweak instantons (suppressed by $e^{-2\pi/\alpha_W} \sim 10^{-160}$ at low energies); the substratum-emergent operator distinction predicts *additional* baryon-number-violating processes at the substratum level whose suppression is not set by $\alpha_W$ but by the substratum's lattice cutoff scale $\epsilon = 2 l_p$.

**The mechanism.** Substratum-level instanton-like processes at the lattice cutoff have action $S \sim O(1)$ in natural units (the lattice spacing sets the action's natural scale), not $S = 2\pi/\alpha_W \sim 200$. The suppression factor is therefore $e^{-O(1)} \sim 10^{-1}$ rather than $e^{-200} \sim 10^{-87}$ for the EW instanton at any given temperature. The substratum-level processes contribute additional baryon-number-violating events without the exponential suppression that makes standard EW instantons negligible.

The framework's structural prediction is therefore: total baryon-number violation is the sum of the standard EW instanton contribution (negligible at temperatures relevant for baryogenesis) and the substratum-level contribution (operative at temperatures near $T \sim 1/\epsilon \sim 10^{19}$ GeV). The substratum-level processes are out of equilibrium at the very early universe and become inoperative below the lattice cutoff. The framework's content matches the three Sakharov conditions: baryon number violation (substratum-level instantons), CP violation (CKM phase plus substratum-level phases that don't appear in the emergent SM), out-of-equilibrium dynamics (substratum-level processes freeze out as the universe cools below $T \sim 1/\epsilon$).

**Quantitative content.** The factor of 15 needed to close the gap between the Weinberg-operator prediction ($\eta_B \sim 4 \times 10^{-11}$) and the observed value ($\eta_B = 6.1 \times 10^{-10}$) is within the range that substratum-level instanton processes plausibly produce. The required amplification is modest in the framework's terms: $e^{-O(1)}$ suppression rather than $e^{-2\pi/\alpha_W}$ suppression means the substratum-level contribution can be many orders of magnitude larger than the standard EW instanton contribution, with the specific magnitude depending on the lattice's structural details.

**Technical content required.** A full derivation requires four pieces of substratum-level analysis that the framework's existing technical content does not yet provide:

(i) *Explicit substratum-level baryon-number operator.* The substratum-level operator must be constructed from the staggered cubic-lattice fermion structure (Chapter 6 §6.2), with explicit content showing how the emergent baryon-number operator emerges from it under the trace-out.

(ii) *Anomaly structure under $\mathcal{G}_{\text{sub}}$.* The commutator of the substratum-level baryon-number operator with the substratum's gauge equivalence operators (cubic rotation group $O$, chirality, flavor structure) must be computed. Anomalies present under $\mathcal{G}_{\text{sub}}$ that don't reduce to the emergent $\mathrm{SU}(2)_L$ anomaly are the source of additional baryon-number violation.

(iii) *Substratum-level instanton action.* The Euclidean instanton-like configurations of $\varphi$ at the lattice cutoff must be characterized, with their action and CP-violating phase content computed.

(iv) *Baryogenesis yield calculation.* The standard baryogenesis Boltzmann equations must be extended to include the substratum-level processes, with the early-universe rates and freeze-out temperature derived from the framework's structural commitments.

These calculations have not been performed. The framework's commitments are consistent with substratum-level processes producing the missing factor of 15, but the explicit calculation is research-paper work — likely a substantial technical paper (30-50 pages) producing a specific quantitative prediction for $\eta_B$ that can be compared against observation.

**Assessment.** Baryogenesis is a genuine open problem, but the framework identifies a specific technical mechanism for closing the quantitative gap. The framework's substratum-emergent operator distinction, applied to baryon number, predicts additional baryon-number-violating processes at the substratum scale that are not captured by the continuum SM effective theory. The mechanism is consistent with the framework's other commitments (T-invariance at the emergent level resolves strong CP; substratum-level T-breaking provides baryogenesis-relevant phases) and the required quantitative amplification is within the natural range of substratum-level instanton effects. The framework's pre-registration for this open problem is therefore: the quantitative gap should be closeable through the substratum-emergent distinction applied to baryon number, with the explicit calculation deferred to follow-up technical work.
