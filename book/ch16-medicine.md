...
The framework's content in this chapter is its strongest non-physics *structural* case, but its empirical status must be stated carefully. The wider therapeutic window for Rett syndrome (Guy et al. 2007) is a *retrodiction*, not a confirmed prediction: the reversibility result was established in 2007, well before the framework, so the framework accounts for a known phenomenon rather than having predicted an unknown one. This is explicitly *not* the same evidential standard as the JUNO $\sin^2\theta_{12}$ prediction, which was registered before its measurement; conflating the two would misdescribe a postdiction as a forward prediction (cf. the honest treatment of retrodictive claims in Chapter 17). The framework's reach across medicine is therefore a matter of structural organization and consistency with established clinical biology, together with genuine forward predictions (below) that have not yet been tested.
...
The framework's strongest structural content in clinical biology is the *wider therapeutic window for reader-writer disorders*, which is consistent with the Rett-syndrome reversibility established by Guy et al. 2007. Because that result predates the framework, it is a retrodiction rather than a confirmed prediction — the framework organizes and accounts for it, but the evidential weight of a forward prediction (as with JUNO) is not claimed here.
...
The cumulative pattern — wider therapeutic windows specifically for disorders of the substratum-maintenance machinery — is the framework's empirical signature in reader-writer disorders. The Guy et al. 2007 Rett reversibility is one case the framework accounts for retrodictively; the broader wide-window pattern is a genuine forward prediction, testable across the reader-writer disorder class.
...
*Epigenetics.* Reader/writer/eraser pharmacology operates on the substratum-emergent operator axis. The wider therapeutic window for reader-writer disorders is framework-organized and consistent (retrodictively) with Rett syndrome; it remains a forward prediction for other reader-writer disorders.

**Three categories of empirical content.**

The chapter's content sorts into three empirical categories.

*Empirically confirmed.* The wider therapeutic window for Rett syndrome (Guy et al. 2007) confirms the framework's substratum-emergent operator distinction at chromatin. The cancer methylome classifier (Capper et al. 2018) confirms the substrate-level operator's clinical discriminating power. The schedule-dependence of Chk1 + gemcitabine combinations confirms the framework's non-Markovian prediction. The Rett case is a structural retrodiction (the phenomenon predates the framework); the Capper classifier and Chk1 scheduling are consistency results with established clinical biology. The framework's genuine forward predictions in this domain remain to be tested.
...
**The framework's epistemic position in medicine.** The framework's content in this chapter is its strongest non-physics *structural* case. The Rett syndrome result (Guy et al. 2007) is a retrodiction — the framework accounts for a reversibility phenomenon established before the framework existed — and should not be equated with JUNO's $\sin^2\theta_{12}$ at 0.07$\sigma$, which was a genuine forward prediction registered before measurement. The cumulative case across the chapter's content (cancer scheduling, neurodegeneration, antibiotic resistance, immunotherapy, cardiac pharmacology, autoimmune disease, epigenetic operator distinction) provides multiple converging lines of evidence for the framework's reach beyond fundamental physics.
