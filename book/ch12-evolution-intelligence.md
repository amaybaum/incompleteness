...
One feature of Deutsch's broader program differs from the framework's commitments and deserves explicit acknowledgment. Deutsch's *Beginning of Infinity* defends the many-worlds interpretation of quantum mechanics as a corollary of taking the wave function as fundamental ontology. The framework's substrate-emergent architecture provides a different reading. Within the framework, quantum mechanics is emergent — the post-trace-out description of how observation operates for embedded observers, not a description of fundamental reality. The wave function's apparent "branching" structure is a feature of the compressed (emergent) description rather than a literal claim about parallel ontologies; the Everettian measure problem dissolves because the branches are not ontological objects requiring measure assignment ([Main] §3.1). The framework's treatment is consistent with the broader pattern: the substratum-emergent distinction dissolves several classical interpretive puzzles in quantum foundations (Bell, Kochen-Specker, Pusey-Barrett-Rudolph, Frauchiger-Renner; see [Methodology] Part III and Appendix C of this volume) by relocating the contested structure from substratum to emergent description. The framework's divergence from many-worlds is therefore not disagreement at the epistemic-status level — where OI and BoI align — but at the question of what level of the substrate-emergent architecture the wave function's apparent ontology should be read at.
