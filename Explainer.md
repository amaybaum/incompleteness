...
Two recent independent analyses tighten the empirical context. Lu et al. (October 2025) report observational evidence for a "direct wave" component in GW250114 oscillating near twice the horizon angular frequency and decaying at the surface gravity, with matched-filter SNR $14.0^{+0.2}_{-0.1}$ in LIGO Hanford and $13.5^{+0.1}_{-0.2}$ in Livingston — the same SNR range the OI echo is predicted to occupy ($\sim 15$ for GW250114). The Lu et al. signal is a different proposed horizon effect with different morphology (a smooth damped sinusoid rather than the OI comb), but its detection establishes that horizon-scale structure beyond the standard QNM picture is detectable in current data at the predicted scale, and that the analysis infrastructure for non-standard horizon physics on GW250114 is now operational. Wang et al. (January 2026) independently detect six quadratic quasi-normal modes in the same ringdown at Bayes factor 74, demonstrating that the post-merger signal contains nonlinear structure requiring more than the standard linear modes — the standard ringdown analysis is incomplete. The LIGO-Virgo-KAGRA standard area-law confirmation (Abac et al., PRL 135, 111403) independently verifies the Bekenstein-Hawking entropy formula derived in Part II of the Main paper, including the factor of 1/4. None of these is the OI echo, but each tightens the empirical context: horizon physics beyond standard GR is now a tractable observational target, GW250114 has the SNR to detect it, and the predicted OI echo scale is comparable to what is being seen in adjacent analyses.
