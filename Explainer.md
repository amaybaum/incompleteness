...
**Dark gravity: a second application.** The same axioms and conditions that produce quantum mechanics independently predict a second, qualitatively distinct phenomenon. The boundary entropy — the $S_{\text{dS}}$ modes traced out to produce the quantum description — has thermal energy that, distributed over the Hubble volume, equals the critical density exactly. This is an algebraic consequence of the framework's thermal self-consistency. The paper proves that the gravitational influence of this entropy medium has no operator in the emergent QFT — the boundary modes are the degrees of freedom that were traced out, so they cannot appear as quantum observables. To the embedded observer, this appears as additional gravity with no visible source. Matter gravitationally displaces this entropy, and the displacement persists because the hidden sector's slow dynamics (condition C2) prevents re-equilibration — the same P-indivisibility that produces quantum mechanics.

These qualitative predictions — invisible gravity, onset at acceleration scales below $cH$, baryonic-field tracking, cosmologically evolving $a_0$ — are derived entirely from the framework's axioms. The quantitative force law is due to Verlinde, whose independent derivation from holographic entropy considerations gives $g_D = \sqrt{g_b \cdot cH/\beta}$ with $\beta \in [5, 9]$. This yields flat rotation curves, the Tully-Fisher relation $v^4 \propto M_b$, and a Milgrom acceleration scale consistent with the observed value. The framework provides the mechanism and axiomatic foundation; Verlinde provides the quantitative force law. Known tensions in Verlinde's program (galaxy clusters, CMB) attach to the specific force law, not to the framework's qualitative prediction.

The fertility of the framework — that axioms designed to derive quantum mechanics also produce a gravitational anomaly matching the qualitative profile of dark matter — is itself evidence. Part I does not logically require a dark gravity prediction. The boundary entropy might have had negligible gravitational effect, or the effect might have been captured within the emergent QFT. Neither holds. The second application is a non-trivial consequence the framework did not need to produce but did.

The conjunction of all three predictions — dark energy evolution in RVM form, gravitational wave echoes at the predicted timescale, and dark gravity with no particle-sector source — is distinctive. No competing framework predicts all three.
...
The framework predicts that dark matter is not a particle. The same axioms and conditions that produce quantum mechanics independently predict additional gravitational effects with no source in the emergent QFT. The boundary entropy medium — the $S_{\text{dS}}$ modes whose trace-out produces quantum mechanics — has thermal energy equal to the critical density. Matter displaces this entropy; the displacement persists because of the hidden sector's slow dynamics (condition C2 — the same condition responsible for P-indivisibility); and the displaced entropy gravitates because gravity operates at the classical level, before the trace-out. The emergent QFT has no operator for this gravitational effect because the boundary modes are the degrees of freedom that were traced out to produce the QFT in the first place. The qualitative prediction is derived from the framework's axioms; the quantitative force law is due to Verlinde's independent derivation from holographic entropy considerations. That the framework produces this second application — qualitatively distinct from quantum mechanics, yet emerging from identical premises — is evidence for observational incompleteness as a unifying principle.
...
The correlation between entanglement and geometry is real either way. The question is which direction the arrow of explanation points. The two orderings make different empirical predictions: if geometry emerges from entanglement, spacetime structure should break down at high energy; if quantum mechanics emerges from geometry, the quantum description should break down near the discreteness scale while the geometric substratum persists. The paper's gravitational wave echo prediction provides one test. And as discussed above, the cosmological constant dissolution is arguably existing evidence: the geometry-first ordering predicts the observed vacuum energy; the quantum-first ordering produces the $10^{122}$ discrepancy and has no solution.
...
That this framework also predicts a second, qualitatively distinct phenomenon — additional gravitational effects matching the profile of dark matter — from the same axioms that produce quantum mechanics is perhaps the strongest reason to take it seriously. Axioms designed to address one problem are not obligated to solve another. When they do, it suggests the framework is capturing real structure rather than fitting known results. Quantum weirdness and dark gravity are two faces of the same coin: P-indivisibility — the universe's memory of what we cannot see.
