...
**Dark gravity: a second application.** The same axioms and conditions that produce quantum mechanics independently predict a second, qualitatively distinct phenomenon. The boundary entropy — the $S_{\text{dS}}$ modes traced out to produce the quantum description — has thermal energy that, distributed over the Hubble volume, equals the critical density exactly. This is an algebraic consequence of the framework's thermal self-consistency. The paper proves that the gravitational influence of this entropy has no operator in the emergent QFT — the boundary modes are the degrees of freedom that were traced out, so they cannot appear as quantum observables. To the embedded observer, this appears as additional gravity with no visible source. Matter gravitationally displaces this entropy, and the displacement persists because the hidden sector's slow dynamics (condition C2) prevents re-equilibration — the same P-indivisibility that produces quantum mechanics.

These qualitative predictions — invisible gravity, persistent entropy displacements — are derived entirely from the framework's axioms. Dimensional reasoning further identifies the partition boundary's surface gravity $cH$ as the characteristic acceleration scale at which the effect becomes significant, but the precise onset profile and interpolation between regimes require a quantitative force law, deferred to future work.

The conjunction of these predictions — dark energy evolution in RVM form, gravitational wave echoes at the predicted timescale, and additional gravity with no particle-sector source — is distinctive. No competing framework predicts all three.
...
The framework predicts additional gravitational effects with no source in the emergent QFT. The boundary entropy — the $S_{\text{dS}}$ modes whose trace-out produces quantum mechanics — has thermal energy equal to the critical density. Matter displaces this entropy; the displacement persists because of the hidden sector's slow dynamics (condition C2 — the same condition responsible for P-indivisibility); and the displaced entropy gravitates because gravity operates at the classical level, before the trace-out. The emergent QFT has no operator for this gravitational effect because the boundary modes are the degrees of freedom that were traced out to produce the QFT in the first place. The invisibility and persistence are derived from the axioms; dimensional reasoning identifies the partition boundary's surface gravity $cH$ as the characteristic scale at which the effect becomes significant. The quantitative force law requires additional input beyond the axioms and is deferred to future work.
...
That this framework also predicts a second, qualitatively distinct phenomenon — additional gravitational effects with no particle-sector source — from the same axioms that produce quantum mechanics is perhaps the strongest reason to take it seriously. Axioms designed to address one problem are not obligated to produce consequences in another domain. When they do, it suggests the framework is capturing real structure rather than fitting known results.
