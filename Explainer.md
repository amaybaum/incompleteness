...
**What the framework does not determine.** The coupling matrix $M$ has three independent eigenvalues ($\mu_c$, $\mu_w$, $\mu_y$), constrained by the max-speed condition to have $\mu_y = 1$. The remaining two eigenvalues set the strong and weak coupling strengths at the lattice scale. The Yukawa couplings (which determine fermion masses and mixing angles) and the Higgs potential parameters are properties of the specific bijection $\varphi$ — the specific "universe" the observer inhabits. The framework determines the *form* of the laws of physics but not all the *constants*. However, an increasing number of "constants" turn out to be constrained by the structural content: the universal induced coupling $1/\alpha_0 = 23.25$ reproduces all three gauge couplings at $M_Z$ to $< 0.1\%$ (Fundamental, §9), and the tree-level Yukawa equality $y_b = y_\tau$ (from the staggered action being diagonal in gauge indices) combined with full SM RG running and non-perturbative SU(3) mass renormalization ($Z_m = 0.835 \pm 0.007$ from Monte Carlo) gives $m_b/m_\tau = 2.42 \pm 0.02$, matching the observed $2.35$ to $3\%$ — the first derived fermion mass ratio.
...
Combined with the earlier papers' predictions — dark energy evolution with $\nu_{\text{OI}} = (2.45 \pm 0.04) \times 10^{-3}$, gravitational wave echoes, dark matter from galaxies through clusters to the Bullet Cluster, gauge couplings at $M_Z$, and the bottom-to-tau mass ratio $m_b/m_\tau = 2.42 \pm 0.02$ (the first derived fermion mass ratio, matching the observed $2.35$ to $3\%$) — the framework makes a suite of falsifiable predictions that no competing theory makes jointly.
