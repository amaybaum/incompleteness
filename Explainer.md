...
The conjunction is distinctive: dark energy evolution in RVM form with $\nu_{\text{OI}} = (2.45 \pm 0.04) \times 10^{-3}$, gravitational wave echoes with a computed waveform, the gauge coupling strengths at $M_Z$, dark matter phenomenology from galaxies through clusters to colliding clusters — all from four axioms about embedded observation. No competing framework produces all of these. The parallel with the cosmological constant dissolution is exact. Part II identifies the $10^{122}$ discrepancy as the *information compression ratio* of the trace-out — how much the emergent description distorts the vacuum energy. The dark-sector corollary identifies the ~95% dark sector as the *gravitational occlusion fraction* — how much of the gravitational universe the trace-out hides from the emergent description. Together, they account for the two largest anomalies in modern cosmology as two aspects of a single phenomenon: the cost of observing the universe from within.
...
The framework derives dark matter phenomenology from the same boundary entropy that produces dark energy. Baryonic matter displaces boundary entropy from its vicinity — the Clausius relation applied to the de Sitter thermal reservoir gives the displacement. The displaced entropy redistributes, creating a gradient; the Jacobson mechanism converts the gradient into curvature — additional gravity beyond what Newton predicts. The crossover acceleration $a_0 = cH/6 \approx 1.2 \times 10^{-10}$ m/s² matches the empirical MOND scale. In the outer parts of galaxies, the rotation velocity flattens to $v^4 = GM_B \cdot cH/6$ — the baryonic Tully-Fisher relation, with no free parameters beyond the baryonic mass and the Hubble parameter. Dark matter is not a substance — it is the local shape of the observer's blind spot, the same entropy that appears as dark energy when uniform. The mechanism works across all scales: galaxies (BTFR, radial acceleration relation), high-redshift galaxies (declining rotation curves confirmed by Genzel+2017; stellar TF no-evolution predicted from gas fraction compensation), galaxy clusters (Coma matched to $< 1\%$ by the simple interpolation function), and the Bullet Cluster (lensing offset explained by frozen boundary entropy with relaxation time $\sim H^{-1} \gg t_{\text{collision}}$).

**"What about the Bullet Cluster?"**
The Bullet Cluster is often called the "smoking gun" for particle dark matter, because gravitational lensing peaks at the galaxy positions rather than at the X-ray gas (which contains 85% of the baryonic mass). In the OI framework, the dark gravity comes from entropy displaced at the cosmological horizon — and the horizon's response time is $\sim H^{-1} \approx 14$ Gyr. The cluster collision took $\sim 0.15$ Gyr. The boundary entropy is frozen at the pre-collision configuration, centered on the galaxies (which defined the gravitational wells for billions of years). The gas moved; the entropy didn't. This produces exactly the observed lensing morphology — without collisionless particles.
...
**What the framework does not determine.** The coupling matrix $M$ has three independent eigenvalues ($\mu_c$, $\mu_w$, $\mu_y$), constrained by the max-speed condition to have $\mu_y = 1$. The remaining two eigenvalues set the strong and weak coupling strengths at the lattice scale. These, along with the Yukawa couplings (which determine fermion masses and mixing angles) and the Higgs potential parameters, are properties of the specific bijection $\varphi$ — the specific "universe" the observer inhabits. The framework determines the *form* of the laws of physics but not the *constants*. However, the universal induced coupling $1/\alpha_0 = 23.25$ IS structural — it depends on $N_f = 6$ and $T(R) = 1/2$, both determined by the lattice, not on the eigenvalues of $M$. Combined with non-perturbative gauge self-energy corrections and SM RG running, this reproduces all three observed gauge couplings at $M_Z$ (Fundamental, §9).
...
Combined with the earlier papers' predictions — dark energy evolution with $\nu_{\text{OI}} = (2.45 \pm 0.04) \times 10^{-3}$, gravitational wave echoes, dark matter from galaxies through clusters to the Bullet Cluster, and gauge couplings at $M_Z$ — the framework makes a suite of falsifiable predictions that no competing theory makes jointly.
