...
\subsubsection{3.3 Interpretive Consequences}\label{interpretive-consequences}}
...
\hypertarget{scope-limitations-and-relation-to-prior-work}{%
\subsubsection{3.4 Scope, Limitations, and Relation to Prior Work}\label{scope-limitations-and-relation-to-prior-work}}
...
\emph{Where the correspondence enters.} The dependence is load-bearing but circumscribed. Partition-relativity, emergent stochasticity, and P-indivisibility (§§1.4, 2) do not invoke Barandes at all --- they follow from the axioms and conditions alone. The correspondence enters at a single step: the map from P-indivisible stochastic process to quantum mechanics (§3.1). Everything before that step --- including the result that the embedded observer cannot see classical physics and that information backflow produces non-factorizable correlations --- is independent of the correspondence.
...
\textbf{Assumptions and limitations of the theorem:} \emph{The finiteness condition} is an independent assumption in Part I. In Part II, the cosmological horizon's finite area provides finiteness through the partition geometry, so it is not an additional assumption in the physical application. The theorem is silent on systems where neither an intrinsic discreteness scale nor a finite-area partition boundary ensures finite-dimensionality. \emph{Barandes' stochastic-quantum correspondence} is the mathematical engine of the theorem, entering at a single step (§3.1). Its proof rests on the Stinespring dilation theorem and has been independently corroborated (§3.4).

\textbf{Assumptions and limitations of the application:} \emph{The derivation of \(\hbar = c^3 \epsilon^2 / (4G)\)} (§5.2, Steps 1--4) rests on four components, each placed on rigorous footing: phase uniqueness (D-gauge completeness theorem, §5.3), volume-independence (volume-independence lemma, §5.2), scale determination via thermodynamic equilibrium (which works within finite-dimensional quantum statistical mechanics, requiring no continuum QFT), and lattice robustness (corrections bounded by \(\mathcal{O}(10^{-122})\)). The remaining genericity assumption --- non-degenerate spectrum and non-vanishing configuration-basis overlaps --- holds for all but a measure-zero set of systems satisfying (C1)--(C3). \emph{The tensor product structure} of the emergent QFT (§7.1) follows from spatial locality of the dynamics and is standard for lattice Hamiltonian systems. A rigorous derivation showing that the Barandes correspondence preserves spatial locality is expected to hold generically but is not proved here. This step is required for the CC dissolution (§7) but not for the derivation of \(\hbar\) (§5). \emph{The echo prediction} relies on a kinematic argument; the exact reflection amplitude requires non-perturbative calculation. \emph{The coefficient \(\nu_{\text{OI}}\)} is constrained to be positive and small (\(\nu_{\text{OI}} \ll 1\)) by the framework's internal logic (§8.1), consistent with phenomenological fits at \(\mathcal{O}(10^{-3})\). Its exact value requires non-perturbative methods.
