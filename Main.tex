...
A standard objection is that tracing out a vast bath should immediately lead to classical decoherence---incoherent, memoryless noise that destroys quantum behavior. However, the trans-horizon sector is not a generic thermal bath. It possesses three co-occurring physical properties that each independently constrain the dynamics against classical divisibility, making CP-divisible evolution increasingly fragile:

\begin{enumerate}
  \item \textbf{Maximal trans-horizon entanglement:} In QFT, the vacuum state across a causal horizon takes the maximally entangled Thermofield Double (TFD) form. As established by Buscemi \cite{23}, initially entangled system-environment states strongly violate the product-state assumption required for complete positivity. For almost any entangled initial state, the set of joint unitaries generating CP-divisible reduced dynamics is measure-zero in the space of all unitaries.
  \item \textbf{Fast scrambling:} Causal horizons disperse information across all their degrees of freedom exponentially fast (Sekino and Susskind \cite{19}). This chaotic spectral structure exhibits spectral rigidity---manifesting as a non-decaying ramp in the Spectral Form Factor (Cotler et al. \cite{22})---which prevents bath correlation functions from decaying to zero. The non-decaying component is exponentially small in the entropy ($\sim e^{-S}$), but it persists to arbitrarily late times up to the Heisenberg time $t_H \sim e^S$.
  \item \textbf{Failure of the Born-Markov conditions:} Gravity is not weakly coupled to the vacuum, and the universe enforces strict macroscopic conservation laws. Conservation laws physically prevent the environment from resetting to equilibrium, forcing persistent system-environment correlations ($\chi(t) \neq 0$) that break the Born approximation entirely (Babu et al. \cite{21}).
\end{enumerate}

\textbf{Counterexamples and their scope.} An important body of work by Kaplanek and Burgess \cite{37,38} demonstrates that an accelerated Unruh-DeWitt detector---which couples to the TFD vacuum near a Rindler or Schwarzschild horizon---can evolve via a Markovian Lindblad equation at late times for sufficiently weak coupling. Their open effective field theory program shows that secular growth terms ($g^2 t$ contributions) can be resummed into a fully CP-divisible Lindblad master equation describing exponential thermalization at the Hawking/Unruh temperature. These results hold \textit{despite} the Hartle-Hawking state being a TFD and despite the black hole being a fast scrambler. The crucial enabling condition is \textbf{weak probe-field coupling}: the detector is a small subsystem whose backreaction on the field is perturbative.

These counterexamples constrain but do not eliminate the claim. They apply to a localized probe weakly coupled to its field environment in a horizon background. The cosmological setting of this framework differs in three respects: (i) the “system” is not a single qubit but the \textit{entire visible sector}, coupled gravitationally to the trans-horizon sector through the stress-energy tensor; (ii) the gravitational coupling is not perturbatively weak---it is precisely what produces the $10^{122}$ projection discrepancy of \S\,2.2; and (iii) cosmological observables are sensitive to IR modes with frequencies $\omega \sim H$, a regime where Kaplanek and Burgess themselves identified “critical slowing down” that restricts the Markovian validity window \cite{39}. In de Sitter spacetime with near-massless fields, the thermalization timescale diverges as $m/H \to 0$, and the Markovian approximation breaks down for precisely the modes most relevant to the cosmological constant.

We therefore state the central dynamical claim as a conjecture:

\begin{quote}
\textbf{The Trace-Out Conjecture.} Tracing out the trans-horizon sector in the cosmological setting---with its combination of maximal TFD entanglement, spectral rigidity from fast scrambling, and non-perturbative gravitational coupling at IR scales---forces the marginal dynamics of the visible sector into a CP-indivisible stochastic process. Any faithful mathematical representation of these dynamics is empirically equivalent to quantum mechanics. The Hilbert space formalism is one such representation; Barandes’ stochastic framework \cite{14,15} is another. The physical content --- indivisible, non-Markovian configuration-space dynamics --- is mandatory; the choice of mathematical language is not.
\end{quote}
The conjecture status is important to state precisely. What is \textit{established} is: (a) each of the three properties individually makes CP-divisibility more fragile and restricts its validity window; (b) the Kaplanek-Burgess Markovian results are confined to weak-coupling regimes that do not apply to the full cosmological trace-out; (c) if the dynamics are CP-indivisible, Barandes’ stochastic-quantum correspondence \cite{14,15} guarantees equivalence to unitary quantum mechanics; and (d) the framework’s falsifiable predictions (\S\,4.7, \S\,4.8) do not depend on the specific mechanism producing indivisibility, only on the fact of it. What remains \textit{open} is a rigorous proof that the three properties jointly force at least one canonical decay rate $\gamma_k(t) < 0$ at some time in the cosmological regime. The most promising avenue for such a proof appears to be the IR-sensitive sector of de Sitter QFT, where critical slowing down \cite{39} genuinely prevents Markovian resummation.

By Barandes’ stochastic-quantum correspondence \cite{14,15}, any indivisible stochastic process on configuration space is mathematically equivalent to a unitarily evolving quantum system. The Schrödinger equation is therefore the unique description an embedded observer must use to predict the marginal behavior of a system coupled to this bath, \textit{provided} the conjecture holds. The indivisible stochastic process recovers the quantum potential $Q = -\frac{\hbar^2}{2m} \frac{\nabla^2 \sqrt{\rho}}{\sqrt{\rho}}$ as the effective force on configuration-space probability flow, with diffusion coefficient $D = \hbar/2m$. The value of Planck’s constant is consistent with the ratio of the holographic entropy to the hidden-sector dimensionality, $\hbar \approx S_{\text{universe}}/N \approx 10^{-122}$ in dimensionless units---a non-trivial self-consistency check linking the macroscopic causal structure to the microscopic noise floor.
