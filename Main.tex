...
(\(V \subset neq S\)), and the observer registers changes (coupling
...
\(\log P_{\text{perm}} \sim -n_V n_H \log n_V\) as \(n_H \to \in fty\)
...
\[\Lambda(2,1) = T(2,0) \cdot [T(1,0)]^{-1} = P1\]
...
level: bijection $\leftrightarrow$ stochastic matrix via marginalization, both
...
\(V^{\circ}\), boundary \(\partial V\), and exterior \(V^c\), the next-step
state at sites in \(V^{\circ}\) depends only on current-step values within
\(V \cup \partial V\), so \(V^{\circ} \perp V^c \mid \partial V\) in the joint
...
\emph{The full biconditional (i) $\Leftrightarrow$ (ii) $\Leftrightarrow$ (iii) of the characterization
theorem below is similarly conditional on ETH for the (ii) $\Rightarrow$ (iii)
direction specifically. The other directions --- (i) $\Leftrightarrow$ (ii) from {[}11,
12{]}; (iii) $\Rightarrow$ (ii) from §2.3; C1 and C3 necessity from their own proofs
--- are unconditional.}
...
the observer partition \(V \subset neq S\) as a choice, not an absolute.
...
\(V_W \cup V_F \subset neq S\), and shared external hidden sector
...
von Neumann algebra of observables from type III to type II\(_\in fty\),
