...
continuum QM at scale \(E\) share the \((E/M_{\text{Pl}})^2\) scaling
that controls Lorentz-invariance violation (Substratum §6.4) and
trans-Planckian corrections (SM §8.3); no independent
continuous-spectrum prediction is needed or claimed. Calvo {[}13{]}
extends the Barandes correspondence to countable-infinite dimensions for
completeness, but the finite-dim correspondence is the load-bearing one
for the framework --- every result in §2--§3 runs on Lemma 1 directly.
...
probabilities, this is the appropriate scope. The reverse direction here
is what {[}Substratum, Theorem 23, Stage 1{]} invokes.
...
compatible with (C2); {[}GR, §§5--7{]} develops this quantitatively.
