...
\subsection{Embedded Inference and Complementary Projections}
...
Two physical couplings to the vacuum define two projection maps from $\Omega$ to observables. Localized matter (e.g., an atom subject to the Lamb shift) measures the unsigned total vacuum activity through the squared field operator:

\begin{equation}
\pi_Q : \Omega \to \mathbb{R}, \qquad \pi_Q(\omega) = \langle \hat{\phi}^2 \rangle_\omega
\end{equation}

Spacetime geometry measures the net signed stress-energy density sourcing Einstein's equations:

\begin{equation}
\pi_G : \Omega \to \mathbb{R}, \qquad \pi_G(\omega) = \langle T_{00} \rangle_\omega
\end{equation}

Both maps are many-to-one: distinct full-universe configurations that differ only in the hidden sector yield identical outputs. But their kernels are non-identical --- $\pi_Q$ discards sign information while summing magnitudes, whereas $\pi_G$ retains signs while summing algebraically. This is the origin of the discrepancy.

Wolpert's (2008) limits of inference \cite{4} provide the formal no-go theorem confirming that this discrepancy is structural. Wolpert proved that any physical subsystem whose state is a deterministic, many-to-one function of the total configuration constitutes an inference device subject to absolute limits: two such devices with non-identical surjective projections cannot fully reconstruct each other's conclusions from their own outputs alone (the Mutual Inference Impossibility). Both $\pi_Q$ and $\pi_G$ satisfy Wolpert's conditions --- each is an embedded proper subsystem of $\Omega$ with a surjective observation map --- and their non-identical kernels trigger the impossibility result. No single embedded subsystem can simultaneously access both the unsigned total and the signed net of the vacuum. The $10^{122}$ discrepancy between them is therefore a structural limit on embedded observation, not an error awaiting correction.


\section{OBSERVATIONAL INCOMPLETENESS}

\subsection{Quantifying the Discrepancy}

Having established that the $10^{122}$ cosmological constant discrepancy \cite{1,2,3} reflects a structural limit rather than a fine-tuning error (\S\,1.2), we now extract its quantitative content. A thermodynamic analogy clarifies the mechanism. When measuring a glass of water, a thermometer records the total unsigned kinetic energy of all molecules (a variance-type measurement), producing a massive reading because nothing cancels out. Conversely, a suspended dust speck is only moved by the net canceled-out mechanical push of molecules striking it from all sides (a mean-type measurement), leaving a tiny statistical residual.
