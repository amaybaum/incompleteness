...
\textbf{Status:} \textsc{Draft Pre-Print}\\
\textbf{Classification:} \textit{Theoretical Physics / Foundations}
...
\subsection{The Cosmological Discrepancy}\label{the-cosmological-discrepancy}}
...
Padmanabhan~[19] identified the central structural point: the QFT and gravitational descriptions of vacuum energy probe different statistical properties of the same underlying degrees of freedom. He argued that classical gravity probes vacuum \emph{fluctuations} rather than the mean energy density, arriving at the observed dark energy scale as the geometric mean of the Planck and Hubble energy densities: $\rho_{\text{vac}} \simeq \sqrt{\rho_{\text{UV}} \, \rho_{\text{IR}}}$. The present framework adopts Padmanabhan's core insight---that two distinct statistical operations yield the discrepancy---but \textbf{inverts the assignment} and grounds it in the impossibility results of \S2.4--2.5:

\textbf{Projection 1: Fluctuation statistics (QM).} The QFT vacuum energy sums zero-point energies mode by mode---each contributing $+\frac{1}{2}\hbar\omega$, a quantity proportional to the position and momentum variances of the quantum ground state. No cancellation is possible; every term is positive. This is a variance-type quantity:
\begin{equation}
V = \sum_{i=1}^{N} \frac{1}{2}\hbar\omega_i \propto N
\end{equation}

\textbf{Projection 2: Mean-field pressure (gravity).} The stress-energy tensor is not positive-definite: different field sectors (bosonic vs.\ fermionic, vacuum condensates) contribute with signs $s_i = \pm 1$, and the Einstein equations couple to the net signed aggregate. This is a mean-type quantity:
\begin{equation}
...
\subsection{Formal Statement}\label{formal-statement-via-wolperts-framework}}
...
\hypertarget{the-discrepancy-as-a-measurement}{%
\section{The Discrepancy as a Measurement}\label{the-discrepancy-as-a-measurement}}
...
\subsection{Extracting the Hidden-Sector Dimensionality}\label{extracting-the-hidden-sector-dimensionality}}
...
The \textbf{total fluctuation content} (quantum projection) scales as $V \propto N$. The \textbf{net mechanical effect} (gravitational projection) scales as $M \sim \sqrt{N}$. The $\sqrt{N}$ scaling of this variance-to-mean ratio, first identified in this context by Padmanabhan~[19], allows the $10^{120}$ discrepancy to be read as a measurement. Setting the ratio equal to the observed value:
\begin{equation}
