...
- **`lean-mathlib/`** — `OIBridge`, the Mathlib-based formal verification programme: 113 modules and,
  at this commit, 2,378 named results, each printing its axiom dependencies (`propext`, `Classical.choice`,
...
the inverse clause is carried out in `INVERSE-CLAUSE-AUDIT.md`, and the dagger-stable package
(`LieRankSource.carrier_general_oiPlusElem`) characterizes the same class
(`oiPlusPos_iff_oiPlusElem`); one optimization question is backlogged and does not affect the
equivalence: whether the elementary repertoire is minimal. A separate substratum-source audit
