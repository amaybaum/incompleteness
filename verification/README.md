...
- **Remaining, one item, sharply formulated:** `ReachabilitySeam.LocalReachabilityOfLieRank`
  ...
  unitary group (the orbit theorem of geometric control at the identity). Everything around
  it is internal: local reachability gives exact reachability by the open-subgroup theorem
  and the connectedness of the matrix unitary group (`exact_of_local`), exact reachability
  gives every unitary conjugation channel (`universalReachability_of_exact`), the global
  phase is invisible to channels, and every unitary is a phase times a special unitary. It is
  not a dependency of the completion classification, which assumes full control outright.
