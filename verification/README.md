...
- **`lean-mathlib/`** — `OIBridge`, the Mathlib-based formal verification programme: 115 modules and,
  at this commit, 2,416 named results, each printing its axiom dependencies (`propext`, `Classical.choice`,
...
would not be quantum mechanics (`target_not_qm`). Fourteen named results, each printing only
`propext`, `Classical.choice`, `Quot.sound`. Not claimed: that a countertheory exists, that the
substratum-generated theory is one, or that `DerivedOI` exhausts what bare OI entails beyond what
the kernel derives at this commit. Guard `R7-RB0`.
