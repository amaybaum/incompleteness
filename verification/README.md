...
- **`lean-mathlib/`** — `OIBridge`, the Mathlib-based formal verification programme: 91 modules and,
  at this commit, 1,660 named results, each printing its axiom dependencies (`propext`, `Classical.choice`,
...
- `*_probe.py` — twenty-nine probe files, all run by CI. `gauge_certificates`, `structural_core`,
  ...
  explicit finite data — `bohr_frequency_probe.py` carries the F-series (F1–F71, one per
...
- **The completion classification** (`ReferenceExtension`, `ReferenceSufficiency`,
  ...
  `CompletedOI`, `CarrierGeneralOIPlus`, `EmbeddedObservation`, `ImplementationLocality`, `MicroscopicReversibility`, `LieRankSource`, `SubstratumSource`): the five completion conditions defined one by one with the countermodel
...
property the abstract stabilities do not entail (`diagGen_not_quantumArchitectureGenerated`). The
