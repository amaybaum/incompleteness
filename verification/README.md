...
`RouteBTarget`, a two-state theory satisfying `DerivedOICore` with `rot` unavailable, has no
unconditional proof; a witness would separate the closure from phase-free richness (`target_separates`) and
