...
- **`lean-mathlib/`** — `OIBridge`, the Mathlib-based formal verification programme: 88 modules and,
  at this commit, 1,577 named results, each printing its axiom dependencies (`propext`, `Classical.choice`,
...
every nonempty finite carrier (`carrier_general_oiPlusLocal`).
...
- `*_probe.py` — twenty-nine probe files, all run by CI. `gauge_certificates`, `structural_core`,
  ...
  explicit finite data — `bohr_frequency_probe.py` carries the F-series (F1–F68, one per
...
- **The completion classification** (`ReferenceExtension`, `ReferenceSufficiency`,
  ...
  `CompletedOI`, `CarrierGeneralOIPlus`, `EmbeddedObservation`, `ImplementationLocality`): the five completion conditions defined one by one with the countermodel
