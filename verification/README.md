...
- **`lean-mathlib/`** — `OIBridge`, the Mathlib-based formal verification programme: 112 modules and,
  at this commit, 2,377 named results, each printing its axiom dependencies (`propext`, `Classical.choice`,
...
(`PositiveReachability.carrier_general_oiPlusPos`); one optimization question is backlogged and
does not affect the equivalence: whether the elementary repertoire is minimal. A separate substratum-source audit
...
(`INVERSE-CLAUSE-AUDIT.md`); the minimal control repertoire remains a backlog optimization that
can simplify the theorem but not change this conclusion.
