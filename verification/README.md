...
`infrastructure/round-v3-14-retirement/`). Its conformance corpus is
...
Round `V3-14` (`infrastructure/round-v3-14-retirement/`) retired the checks that re-derived the
...
amendment `V3-13` froze adds, and fourteen of its checks; the `V2` verifier with its gate step,
workflow job and conformance corpus, the control-plane base check and lint, and the verifier's
projection over the `V2` rows. `V3-13` (`infrastructure/round-v3-13-retirement/`) froze the same
retirement and halted under the specification's `S12`: its transformation deleted retained
statements of the guard, and its withdrawal restored every execution path. `V3-14`'s transformation
is checked against a frozen ledger of every change it makes, by a checker independent of it, and
was executed in CI before its freeze. The records of those rounds are kept instead by the release
gate's `legacy-records` step, `tools/legacy_records_check.py`: every record listed in
