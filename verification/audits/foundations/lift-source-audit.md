...
At review of the round at `f8167cf`, the owner found the headline "every formulation that lands
delivers configuration-level operations, both targets negative for each" stronger than the interface
facts the kernel proves for the channel-level formulations. For L1, `reversibleExtension_conj_monomial`
proves the reversible coherent members to be single-operator monomial implementations, and
`correlationExtension_preservesDiag` proves the nonreversible members to be diagonal-preserving
channels; the round does not classify the implementation classes of those nonreversible channels.
For L2, `stochasticChannel_kraus_monomial` exhibits one monomial matrix-unit Kraus realization, and
`stochastic_not_layerFlowExecutable` is conditional on a generating class of monomials and scaled
matrix units; neither proves that every Kraus realization, or every implementation of the same
channel, is configuration-level. The existence of a configuration-level realization of a named
channel is not read as the exclusion of every non-monomial realization. The verdict is stated, from
here on, in the form the kernel proves: no formulation that lands derives either target, a
non-monomial admissible operator or the gate flow of a layer; the named channels preserve diagonal
states; the reversible L1 implementation is monomial; the explicit L2 realization is
configuration-level; the generator reading does not imply executability; L3 stays underdetermined.
The preregistration above is unchanged, its T2, T3 and T6 admissible outcomes "Target A and Target B
negative" being read as "derives neither target"; no theorem changes; the verdict of
`EXEC-SOURCE-AUDIT.md` is unchanged; the guard pins the narrowed form and rejects the broader one in
the outcome, the README and the registry.
...
read under the scope amendment above: no formulation the corpus states that lands in the operational
interface derives either target, and the one that does not land is underdetermined. Neither target is
derived by any formulation; every test reached the outcome expected in advance, no proved statement differs from
...
|---|---|---|
| L1, the coherent completion on the configuration carrier | representational; derives neither target | lands as a channel; its reversible members are monomial conjugations and every member, reversible or not, preserves diagonal states (T2); adjoining every reversible lift to the substratum's operators gives the substratum class back, whose theory executes no layer flow; the implementation classes of the nonreversible members are not classified here |
| L2, the projected observer operator on visible distributions | representational; derives neither target | lands as classical maps on diagonal states; its explicit Kraus realization is by monomial operators and the channel preserves diagonal states (T3); a theory generated by a class of such operators over the stated access executes no layer flow; other realizations of the same channel are not classified here |
...
**The headline.** No formulation of the observer-level lift the corpus states that lands in the
operational interface derives either target. The stronger statements hold where the kernel proves
them, and there only. The coherent completion of a
...
whose explicit Kraus realization has monomial operators, the channel preserving diagonal states. A generator whose time-one
...
derived: no formulation that lands supplies a non-monomial admissible operator, so none supplies the
gate flow of a layer. The existence of a configuration-level realization of a named channel is not
the exclusion of every other realization, and the round claims no such exclusion. The verdict of `EXEC-SOURCE-AUDIT.md` stands unchanged, and its underdetermined entry
...
anything on the configuration carrier or its states, derive neither a non-monomial admissible
...
access, no formulation of the lift that lands deriving anything beyond it.
...
for the gate flow of a layer. That every implementation of a nonreversible coherent completion, or
every Kraus realization of the stochastic channel, is configuration-level: the kernel proves the
reversible coherent members monomial and exhibits one monomial realization of the stochastic
channel, and classifies neither the other members' implementation classes nor the channel's other
realizations. That Route A is closed in either direction. That quantum mechanics
...
operator and the generator reading, no formulation that lands deriving either target, the stronger
statements attached only where the kernel proves them;
