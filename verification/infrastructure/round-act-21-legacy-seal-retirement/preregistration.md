# Act 21 — the legacy seal retirement: PREREGISTRATION

**Control plane only.** This document fixes what Act 21 will do, what would count as each outcome,
...
## What Act 21 is, and what it deliberately is not
...
**Act 21 is the retirement round `SI-2` named.** It begins from a world in which the generic
...
**Act 21 is SEALING, `E` → `L` → `P`.** §A.37 as amended by `SI2-7` says so in terms: the legacy
...
**prospectively, here, and for exactly the inventory below**. `P` writes `verification/seals/A21.json`
with `kind: "sealed"`, `base` = the mandated execution base, `sealed_head` = `E`, `merge` = `L`, and
**no legacy constant**: no `_A21_BASE`, no `_A21_SEALED_HEAD`, no `_A21_MERGE`, not as `None`, not at
all, at any point in the round.

**Act 21 owns and supersedes what its deletions break, and says so before executing.** Amendment 2
...
**A naming note, so that no obligation is silently acquired.** Track B's act 20 freeze and result
speak of "act 21" as a fresh rigidity round written after act 20 lands, carrying an attestation-span
choice and a classification ladder reserved for that round's preregistration. The owner has assigned
the label *Act 21* to the retirement round. **This round takes up none of those Track B
obligations**: it states nothing about attestation spans, ladders, rungs or classifications, and
whichever round the owner opens for that work carries its own control plane. Nothing here is that
control plane.

## What Act 21 inherits, and only this
...
a verdict is not a rebinding; it is a result requiring adjudication, and `A21-7` catches it.
...
`U3` as its `prospective` argument by `_si2_manifest_verdicts`. While Act 21 executes it holds
exactly `{'A21': <mandated base>}`; `U3` classifies `A21` as `EXECUTION` and certifies its head by
act 10's strengthened check, and at `L` as `LANDED-PENDING-PIN`. **`P` removes the entry when it
writes the record**, so that from `P` on the round is classified from `A21.json` alone; a stem that
...
with the derived landing equal to the pinned one. For Act 21 the baseline is the twenty-three
records at the mandated base and the authorized additions are exactly `SI2` (at stage 1) and `A21`
(at `P`).
...
|---|---|---|---|
| 1 | `A21-1` | `SI2.json` added, `kind: "base-only"`, `base` = `df54b99dba99dc043b11752163d8d348c9e54472`; `R7-SI2`'s chronology clause becomes a keyed call to `U3` on that record; `R3` declared with that addition; the record set is **fixed** at twenty-four from here until `P` | — |
| 2 | `A21-2` | `R1` built; every read in the *other* class rebound or retired per the table; `R2` declared with Act 21's base | stage 1 |
| 3 | `A21-3` | the twenty-three shadow functions, the five comparators, the shadow-integrity recording and `R7-RNT`'s self-test block deleted; the twenty-three keyed calls lose `shadow=` | stage 2's dry run: every pre-existing tag `PASS` |
| 4 | `A21-4` | the sixty-two statements deleted; zero remain | stage 3's dry run: every pre-existing tag `PASS` |
| 5 | `A21-5` | the `SI-1` and `SI-2` live re-measurements that consumed the deleted machinery are converted to pinned historical artifacts, per the supersession table | stage 4 |
| 6 | `A21-6` | §A.37 amended: the legacy representation is retired, the prospective declaration and its removal at `P` stated, the standing prohibition stated, closed rounds' manifest-cardinality contracts read over the records they manifested | stage 4 |
| — | `A21-7` | the verdict map: the base's guard file is run and every one of its ninety-two tags returns the same verdict at the final head | everything above |
...
`A21-7` is measured at the commit that carries the whole retirement, and `P` adds a record and
...
**`A21-0` — locating controls.** The pinned blobs and the preconditions below hold at the mandated
base. Outcomes: `HOLD` / `DRIFTED`. `DRIFTED` stops the round.

**`A21-1` — `SI-2`'s record, first and then fixed.** Exactly one record is added before any other
...
**`A21-2` — every remaining read goes through the accessor.** After this stage, no genuine
...
**`A21-3` — the shadows are gone and nothing gated on them.** Zero `_<stem>_legacy_ancestry`
...
**`A21-4` — the representation is gone.** Zero module-level `_<STEM>_(BASE|SEALED_HEAD|MERGE)`
...
**`A21-5` — the archived measurements.** The `SI-1` and `SI-2` contracts named in the supersession
table as *live re-measurement retired* are, at the final head, pins of the artifacts they produced:
`SI-1`'s `census.json` at `7a3e6288c5f532a849a07beb9a8e5757cdf79d04` and `SI-2`'s at
`d6b2e505da8bb816179a822258d6672b79670155`, each with a one-byte drift control, each labelled in its
guard as **a certified historical measurement whose old side no longer exists and which is not
re-measured**. Every `U3`-side measurement those guards make — the schema, the three states, the
negative suites that exercise the validator, the tag-map comparisons that run historical guard files
from git, `#140` and `#141`'s restatements — keeps running live. Outcomes: `ARCHIVED-AS-PINNED` /
`ARCHIVE-PARTIAL`.

**`A21-6` — the protocol.** §A.37's *Sealing through the manifest* subsection is amended,
prospectively, so that it states: that the legacy representation is **retired** from Act 21's
...
`A21-4` keeps; how a sealing round carries its base while it executes — the prospective declaration
...
**`A21-7` — the verdict map is preserved.** The base's guard file — `B0`'s, at blob
...
Act 21's head one run of the file executes the guard file **eight** times: the head; `R7-SI1`'s
base; `R7-SI2`'s base and, inside it, its own `SI-1` base; and `R7-A21`'s base and, inside it, that
...
| `R7-SI2` | `SI2-1`'s record-count contracts: twenty-three records, `U5` delta empty against `SI-2`'s stage-1 set | Act 21 adds `SI2` and then `A21` | **scoped to the twenty-three records `SI-2` manifested**, `_SI2_MANIFESTED`, exactly as Amendment 1 scoped `SI-1`'s to its twenty-two; `U5` moves to `R3`'s baseline |
| `R7-SI2` | `SI2-2`'s negative case 13, the N13 mutation control | N13 is gone | **retired** with N13; cases 1–8, 11 and 12 keep running |
| `R7-SI2` | `SI2-3`'s measurement, `_si2_authority_measured` and `_si2_clause_gates_on_u3`: the `shadow=` form required, shadow-forced controls, the in-vivo rebinding that flips eighteen shadows | there are no shadows | **replaced** by `A21-3`'s shadowless form, measured statically the same way; the static count of clauses gating on `U3` stays twenty-three and the count gating on a legacy constant stays zero |
| `R7-SI2` | `SI2-4`'s live census reconstruction, `_si2_census` and `_si2_control_census` | the old side reads the constants | **live re-measurement retired**; `census.json` pinned at `d6b2e505…` as a certified historical measurement |
...
| `R7-SI2` | `SI2-6`(b), `LEGACY-INTACT` | the round deletes the inventory it protects | **retired**; `A21-4`'s zero is the successor contract; the extraction it used is kept as the detector |
...
|---|---|---|
| `A21-0` | `HOLD` | HIGH |
| `A21-1` | `RECORD-AS-AUTHORIZED` | HIGH |
| `A21-2` | `REBOUND` | HIGH |
| `A21-3` | `SHADOWS-RETIRED` | MEDIUM |
| `A21-4` | `RETIRED` | HIGH |
| `A21-5` | `ARCHIVED-AS-PINNED` | HIGH |
| `A21-6` | `PROTOCOL-UPDATED` | HIGH |
| `A21-7` | `MAP-PRESERVED` | MEDIUM |

`A21-3` and `A21-7` are MEDIUM for the same reason: `SI-2` measured that the shadows gate nothing
and that the map is preserved, and this round removes them. If a verdict moves, `SI-2`'s measurement
missed a read, and that is the informative outcome. `A21-4` is HIGH because it is a count.
...
1. One legacy statement of any shape re-added at the final head — **fails `A21-4`**.
...
4. A genuine identifier read of any of the sixty names surviving outside the deleted functions —
   **fails `A21-2`**; the same name inside a string literal does not.
5. `SI2.json` absent, or carrying `sealed_head` or `merge` even as null — **fails `A21-1`**.
6. A twenty-fifth record before `P`, or any record other than `A21` at `P` — **fails `U5` under
   `R3`** as an unauthorized addition.
7. The prospective declaration present at the same time as `A21.json` — **fails** as
   declared-and-recorded.
8. A head descending from Act 21's landing `L` before `P` — **fails as seal pending** under `U3`'s
   prospective path; the resolved target being `L` itself — **`LANDED-PENDING-PIN`, permitted**.
9. At `P`: `A21.json` with `sealed_head` ≠ `L`'s second parent, or `merge` ≠ the unique landing
...
11. One byte changed in a pinned archived census — **fails `A21-5`'s drift control**.
...
5. Any statement about Track B's act-20 successor, its attestation span, ladder or classification.
...
with a fake value. `A21-4` counts statements, and the self-test block is deleted as a block.
...
error in a record would surface here as a changed verdict. `A21-7` catches it and the round stops;
...
validator it installed. Act 21 installs no validator; it is certified by `U3` as `SI-2` left it,
through the prospective path `SI-1` built. What Act 21 does touch is the wiring — `R2` handed to
...
time at Act 21's own `L`. Negative cases 8 and 9 exist for this, and the result note records what the
live classification said at `L` and at `P`.

**H6 — the name.** Two different things are called act 21 in this repository. The naming note above
is the whole of what this round says about the other one.

**H7 — the archive read as live.** A pinned census is a certified statement about a past head under
a past rule. `A21-5` labels each pin as such in the guard, and non-licence 4 exists so that no later
...
1. `R7-A21` does not appear in `verification/lean/edge_rigidity_probe.py`. Verified at `B0`: 0
   occurrences.
2. `_A21` does not appear in that file. Verified at `B0`: 0 occurrences.
...
7. `AGENTS.md` §A.37 at `B0` still says the legacy constants "remain **PROTECTED HISTORICAL SEAL
   STATE**" until the retirement round — the sentence `A21-6` supersedes prospectively.
...
Fresh tag **`R7-A21`**, stem **`_A21_`** for its own helpers and **no legacy-shaped name**: nothing
matching `_A21_(BASE|SEALED_HEAD|MERGE)` exists at any commit of the round. It carries this
preregistration pinned **by blob** with a one-byte drift control; its chronology clause is a keyed
call to `U3` on stem `A21`, which `R2` makes `EXECUTION` while the round runs, `LANDED-PENDING-PIN`
at `L`, and `A21.json` makes `ARCHIVED` at `P`; the locating controls; the inventory measurements of
`A21-2`, `A21-3` and `A21-4` by tokenization and extraction; the pins of `A21-5`; the protocol
contract of `A21-6`; the base run of `A21-7`; and content contracts holding the result note to the
distinctions this freeze makes — sealing and `E` → `L` → `P`; the inventory as counts; the
supersession table as the authorization; the archived censuses as history and not live evidence;
`SI-1`'s and `SI-2`'s recorded outcomes not edited; the naming note. Each contract is
mutation-tested against the exact failure it exists to catch.

## The landing shape

`E` → `L` → `P`, on the execution pull request, under §A.37 as amended by `SI2-7`:

- **`E`** is the final head after stage 6 and `A21-7`, certified by exact-head CI.
- **`L`** has current green `main` as its first parent and exactly `E` as its second. At `L` the
  guard classifies `A21` as `LANDED-PENDING-PIN`, permitted, "the seal record is still owed".
- **`P`** writes `verification/seals/A21.json` — `{"round": "A21", "kind": "sealed", "base":
  <the mandated execution base>, "sealed_head": <E>, "merge": <L>}` — removes the `A21` entry from
  `R2`, and touches nothing else. From `P` the guard classifies `A21` as `ARCHIVED`, re-derives `L`
