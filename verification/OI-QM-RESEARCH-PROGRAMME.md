...
### 3.16 Source C's construction does not select the intermediate candidate

Track B act 4 is closed at **`MP2` — the construction maps, and consumes data our side does not determine**. `verification/BARANDES-DILATION-MAPPING-RESULT.md` records it. It is an **audit determination, not a theorem**: act 1's evidence levels 1 and 3, never level 2, and no Lean was written.

**The question.** Act 3 left it open whether the missing candidate selection was already supplied by the external correspondence. Against **Source C's theorem construction** — the only one this round adjudicates — it is not.
...
**Why that does not settle the candidate.** Every object it builds is indexed `(t ← 0)`, and the dilated process's conditioning-time set is declared the singleton `{0}` (§5.8). So the construction's output contains **no propagator from an intermediate time**, and hence no visible intermediate candidate. The only route from its outputs to one is the relative operator, which Source C does not form.

**And along that route the answer turns on a choice.** The source states the non-uniqueness itself: eq (72) is "an **identity**, not a postulate", and `Θ` is a "**non-unique** potential matrix" fixed per target time. Right-multiplying `Θ(t ← 0)` by a diagonal unitary preserves every constraint the construction imposes, and act 4 exhibits two admissible choices whose relative operators give the **identity** and the **swap** — different visible propagators from the same `Γ`. Operator non-uniqueness is therefore not merely formal here; it reaches the candidate. Establishing that, rather than assuming it, is what separates this outcome from the one where the correspondence supplies the selection.

**The named data**, which is `MP2`'s deliverable: the per-target-time phase freedom in `Θ` beyond `|Θ|²`, and the Stinespring completion, whose added columns "can always be chosen" and whose dimension is only bounded.
...
**What this does not say.** Not that the external framework requires an additional physical principle; act 3's frozen licence forbids it and this round does not earn it. Not that the selection is unavailable — **Source A's earlier construction is out of scope** and forms relative operators in its own development, so it may yet supply what Source C's does not. Nothing about the backward obligation, which was not attempted. And `CU1a` is untouched: it is a theorem about **our** bridge, and no external audit revises it.
...
Track B act 4, **the dilation mapping obligation**, is executed and closed at **`MP2`** (§3.16): Source C's construction does not select the intermediate candidate from our datum, and the data it would need are named. That **narrows** the mapping obligation rather than discharging it — **Source A's earlier construction**, which forms relative operators in its own development, is now the sharpest open question on this route, and is what act 5 would take. The alternative remains to select the candidate on this side from a stated principle and say plainly that it is an addition to the merged bridge; the two are different claims and should not be conflated. Two further continuations stand as before: a `BarandesTuple` instantiation lemma making "our processes instantiate the tuple" checkable rather than argued, for which act 2's `rootedMap_zero` supplies the trivialization condition; and the question `BD3` leaves open — what `PIndivisibleWithin` adds *on top of* class membership, which act 1 located in the external framework's interference term. Whether one of those, or Arc D round 2, or Arc E is on the critical path is the decision the programme now faces.
