...
Three Track B continuations are now available and none is executed. The first is new and act 3 put it there: **the mapping obligation** — show that our representation instantiates the external dilation construction, which would *supply* the candidate selection rather than add to it. The alternative to it is to select the candidate on this side from a stated principle, and say plainly that this is an addition to the merged bridge; the two are different claims and should not be conflated. The other two stand as before: a `BarandesTuple` instantiation lemma making "our processes instantiate the tuple" checkable rather than argued, for which act 2's `rootedMap_zero` supplies the trivialization condition; and the question `BD3` leaves open — what `PIndivisibleWithin` adds *on top of* class membership, which act 1 located in the external framework's interference term. Whether one of those, or Arc D round 2, or Arc E is on the critical path is the decision the programme now faces.
