...
Track B act 2 is closed at **RT1 — kernel-closed**. `verification/BARANDES-TRANSPOSE-BRIDGE-RESULT.md` records it; `OIBridge/TransposeBridge.lean` carries it in eight named results, each printing only `propext`, `Classical.choice`, `Quot.sound`.
...
Track B act 3 is closed at **`CU1a` — the named rules are underdetermined**. `verification/BARANDES-CANDIDATE-SELECTION-RESULT.md` records it; `OIBridge/CandidateSelection.lean` carries it in seven named results, each printing only `propext`, `Classical.choice`, `Quot.sound`.
...
Track B act 4 is closed at **`MP4` — unresolved at the frozen interface**. `verification/BARANDES-DILATION-MAPPING-RESULT.md` records it. It is an **audit determination, not a theorem**: act 1's evidence levels 1 and 3, never level 2, and no Lean was written.
...
**The object, and the dichotomy underneath it.** Source A contains two relative-time objects. `Γ̃(t ← t′) ≡ Γ(t ← 0)Γ⁻¹(t′ ← 0)` at eq (37), p. 13, is computed from the visible data alone, and Source A rules it out: the inverse of a stochastic matrix is stochastic only for permutation matrices, so `Γ̃` is "not generically stochastic". `Γ_ik(t ← t′) ≡ |U_ik(t ← t′)|²` at eq (42), p. 14, built from the relative time-evolution operator `U(t ← t′) ≡ U(t ← 0)U†(t′ ← 0)` at (39), p. 13, **is** "manifestly unistochastic" — a genuine visible candidate. So the object computable from the visible data is not a candidate, and the object that is a candidate is not computable from the visible data.
...
**Why the candidate is not fixed by the visible data.** Its inputs are `U(t ← 0)` and `U(t′ ← 0)` — the unitary, not `Γ`. Eq (30), p. 11 determines `Γ` from `U` and not conversely, and Source A names the resulting freedom itself: `Θ(t ← 0)` is "guaranteed to exist, although it is **not unique**" (p. 7), and footnote 6, p. 7 calls this "a previously unrecognized form of **gauge invariance**", changing entries "by arbitrary, time-dependent phase factors" in a way that alters "the dynamics" while leaving "all empirical results unchanged"; p. 9 adds that `Θ`, `ρ`, `Ψ` and `A` "are not uniquely defined by `C` or by `Γ(t ← 0)`".

**Exhibited at the visible level.** With `U(t ← 0) = U(t′ ← 0) = H` the real Hadamard, so that `Γ(t ← 0) = Γ(t′ ← 0)` is the uniform bistochastic matrix, the unitarity-preserving part of that gauge freedom gives `Γ(t ← t′) = |H D H†|²`: at `D = 𝟙` the **identity**, at `D = diag(1, −1)` the **swap**. Two admissible choices, the same transition data, different visible candidates — displayed as candidates, after the readout, not as operators.

**The named datum** is therefore the choice of unitary representatives, equivalently the relative gauge between the two times. It is a freedom **internal to Source A's construction**, not a datum missing from our side.

**The freedoms, at the scope each has earned.** The gauge freedom is examined and exhibited. The choice of number system for `Θ`, representatives outside the `E U D` orbit, and the Stinespring dilation freedom are recorded **not examined** rather than absent — the last as representation freedom, not named as a selection datum, exactly as act 4 recorded its counterpart. None bears on which of A5's three answers holds, since the exhibition already rules out *unique* and fixing the representatives rules out *unresolved*; what stays open is whether the parameter list is complete.
...
**Together these two acts locate the missing selection rather than supplying it**, and they discharge the mapping obligation as far as auditing external constructions can. What remains on this route is no longer a question about the sources: it is whether a selection principle should be adopted on our side and said plainly to be an addition to the merged bridge, or whether one of the family Source A's construction names can be argued canonical on grounds act 5 did not examine. Those are different claims and should not be conflated, and neither has been adopted.

The **`BarandesTuple` instantiation lemma** therefore returns to the front of the queue, and now has a second reason to exist beyond making "our processes instantiate the tuple" checkable rather than argued: act 5 deliberately deferred to it the containment question of whether our datum supplies Source A's inputs — in particular whether our visible data is unistochastic at all. Act 2's `rootedMap_zero` supplies the trivialization condition. Behind it stand the question `BD3` leaves open — what `PIndivisibleWithin` adds *on top of* class membership, which act 1 located in the external framework's interference term — along with Arc D round 2 and Arc E. Which of those is on the critical path is the decision the programme now faces.
