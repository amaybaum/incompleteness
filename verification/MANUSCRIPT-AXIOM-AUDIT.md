...
shown here to be the only one. Route A's next test is thereby fixed: non-configuration-level
sourcing is necessary, and the question is whether a derived sourcing, the observer-level lift
first, makes the required continuous mixing executable for the embedded observer — a
one-parameter family of transitions at the relevant levels, in the form `PhaseFreeRichness`
asks for, `∀ t, flow (transition a b) t` available at every level — and thereby yields
`PhaseFreeRichness`, the exchanges and closure the substratum already supplies then giving the
rest (`derivedOI_qm_iff_phaseFree`). A single executable non-monomial gate is not enough: it
gives neither phase-free richness nor quantum mechanics, and a theory generated by the
permutations, the phases and one discrete non-monomial gate would be a further non-quantum
theory, not a closure of Route A. A3–A6 may well be what such a derivation consumes; the pass
does not say they are not.
...
That an executable non-monomial gate would close Route A: what closes it is an executable
continuous one-parameter mixing family yielding `PhaseFreeRichness`, and a discrete
non-monomial gate is a different, weaker thing.
...
## The third item of the missing interface, decided for the formulations that land, recorded after the round

The lift-source audit (`LIFT-SOURCE-AUDIT.md`, `OIBridge/LiftSource.lean`) tests the observer-level
...
a map to the carrier would have to supply recorded there. The non-configuration-level sourcing the
bound requires is therefore supplied by none of the lift's stated formulations; neither audit decides
whether the lift can be derived.
