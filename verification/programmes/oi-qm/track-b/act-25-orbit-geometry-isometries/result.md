...
landed by `iso1_family_acts` with its two instantiations; `ISO2` landed by `iso2_classes_single`,
...
`claude/act-25-execution`, rooted at `B`, and its first-parent chain from `B` is, in order:

| # | commit | content |
| --- | --- | --- |
| 1 | `24ff53cccd44b7a92c8f1604b7d9295ff26303e9` | **stage A**: the two declarations set to `B`, in the guard file only |
| 2 | `5bca42f0df2f38a299baf8019a2e98765263449c` | **the module commit**: Section A, the eight shared lemmas, plus the import line; no definition, no verdict |
| 3 | `d4b7738aafeb3ad06b4ab3bb86c1000f59bef204` | `ISO1`: `iso1_family_acts`, `iso1_single_carrier`, `iso1_product_carrier` and twelve supporting results |
| 4 | `f239305c3b5bf013fd8868da8f9c0672c90434ed` | `ISO2`: `iso2_classes_single` and seven supporting results |
| 5 | `bbf87156d71e8ed2dcd96850d33c4aa8dde63d03` | `ISO3`: `fourier_circle_metric` and three supporting results, the route's step 1; no classification theorem |
...
**`24ff53cccd44b7a92c8f1604b7d9295ff26303e9`.** `git show --stat` lists one file,
...
**`5bca42f0df2f38a299baf8019a2e98765263449c`**, the first commit at which the module is present. It
...
| --- | --- | --- |
| `ISO1` | `d4b7738aafeb3ad06b4ab3bb86c1000f59bef204` | `relabel2_realizable`, `relabel2_gramPhaseEquiv`, `relabel2_isometry`, `conj_isometry`, `transpose_single_valued`, `transpose_unitary`, `transpose_descends`, `transpose_isometry`, `relabel2_dilation`, `conj_dilation`, `conj_relabel2`, `relabel2_relabel2`, `iso1_family_acts`, `iso1_single_carrier`, `iso1_product_carrier` |
| `ISO2` | `f239305c3b5bf013fd8868da8f9c0672c90434ed` | `row_forms`, `core_row_norm`, `perm_of_rows`, `rows_injective`, `core_nonreal`, `core_real`, `classify_dephased`, `iso2_classes_single` |
| `ISO3` | `bbf87156d71e8ed2dcd96850d33c4aa8dde63d03` | `fourier_entry_uniform`, `fourier_coord_uniform`, `fourier_coord_diff_norm`, `fourier_circle_metric` |
...
`git diff 5bca42f0df2f38a299baf8019a2e98765263449c <E> -- verification/lean-mathlib/OIBridge/OrbitGeometryIsometries.lean | grep -c -E '^[-+](def |abbrev |structure |class |instance |axiom |opaque )'`
...
the packaging commit, which does not touch the module, and re-run by `R7-OGC` on every head from
...
**What the execution did in this span**, bearing on any target not yet closed at its end: verified the frozen blob at `B` by `git hash-object` and `git rev-parse` and every other pinned blob by `git rev-parse`; wrote and committed the stage-A edits and ran the guard at that commit; read act 12's, act 21's, act 23's and act 24's merged modules at their pinned blobs — `FibreGram`, `GramPhaseEquiv`, `RealizableGram`, `fibreGram_apply`, `sh1_necessity`, `sh1_sufficiency`, `AdmissibleDilationAt`, `RelabelTransition`, `hadamard_z_admissible`, `mixedTriple`, `mixedTriple_star`, `realizable_conj`, `conj_gramPhaseEquiv`, `conj_conj`, `geo2_relabel_isometry` — to reuse their proof shape; wrote the eight shared lemmas of Section A and built them, the builds revealing elaboration details only (the unitary-group rewrite for the transposed product, `star z` against `(starRingEnd ℂ) z` and conjugated numerals in the Fourier entry cases, `simp` turning `x = -(1/4)` into `-x = 1/4`, `linear_combination` refusing products of hypotheses) and then that the lemmas compile, and nothing else. No proof, search or experiment about any generator's action, any realizable class's description, any isometry or any transition family was attempted or run, no build output revealed any, and the execution reasoned to no fact bearing on any target beyond what the freeze and the pinned blobs state.

**The freeze-supplied facts that were in front of the execution in this span**, listed so that the
owner can weigh them: the freeze's proof route for every target — the coordinate identities of the three generators, the row-phase argument, the transpose's admissibility and descent, the six relations, the dephasing with its phases, the antipodal identity, the shape of the case analysis, the monomial coordinates of the Fourier tuple and the three steps of the classification route — each recorded there as the freeze's reading and not as a finding; the consumption of `ISO2` by `ISO3` and of `ISO3` by `ISO4`; acts 12's, 21's, 23's and 24's result notes at their pinned blobs, which record every consumed fact; the merged modules. Reading those is reading the freeze.
...
**What the execution did in this span**, bearing on any target not yet closed at its end: read the freeze's `ISO1` target; wrote and built the fifteen named results of Section B — the realizability, descent and isometry of an independent relabelling, the isometry of the conjugation at an arbitrary carrier, the transpose's single-valuedness on classes, its descent and its isometry, the relabelled and the conjugated dilations with their transposes, the two exact relations, the assembled `iso1_family_acts` with its fifteen conjuncts, and its two instantiations; the builds revealed elaboration details only (`PosSemidef.submatrix` needing the scoped `ComplexOrder`, `AdmissibleDilationAt` needing `DecidableEq A`, the Mathlib name `Unitary.star_mem`, `Prod.map_apply` needed to reduce the relabelled dilation's entries, `simp` closing goals before `ring`) and nothing about any later target. `ISO1-R`'s finite check on a concrete tuple against the images `(π, τ) C ^ e G` was not carried out, as recorded in §5. No realizable class was described as a relabelled Fourier class, no isometry was classified, and no transition family was tested.

**The freeze-supplied facts that were in front of the execution in this span**, listed so that the
owner can weigh them: the freeze's `ISO1` target with its conjuncts (i)–(xi), the route of each — the coordinate permutations and the reindexing along `(π, π, π) × (τ, τ, τ)` and along `p ↦ ((p.2.2.1, p.2.2.2, p.2.1), p.1)`, the row-phase lemma, the two-sided phase bookkeeping — the accounting act 24 asked for as a control, and the observation sub-question `ISO1-R` with its three outcomes.
...
**What the execution did in this span**, bearing on any target not yet closed at its end: read the freeze's `ISO2` target and the class-description route; wrote and built the eight named results of Section C — the three row forms from the antipodal lemma, the row norms of the core matrix, the permutation from an injective row-type map, the injectivity of the row types from orthogonality, the non-real case, the real case, the classification of the dephased matrix, and the assembled `iso2_classes_single` with its two directions as separate conjuncts; the builds revealed elaboration details only (`simp` normalising `star z` to `(starRingEnd ℂ) z` in the row-norm cases, `norm_num` making partial progress inside `first`, the transport lemma's existential over `z` re-wrapped) and nothing about any later target. `ISO2-P` was not attempted, the freeze naming no route. No isometry of the normalized space was classified or tested, no intrinsic-metric fact was computed, and no transition family was tested.

**The freeze-supplied facts that were in front of the execution in this span**, listed so that the
owner can weigh them: the freeze's `ISO2` target with its two directions, the five steps of the class-description route as the freeze's reading — the dilation, the dephasing, the antipodal lemma, the case analysis with its three orthogonality equations, the converse — and the frozen obstruction of `ISO2-P`.
...
**What the execution did in this span**, bearing on any target not yet closed at its end: read the freeze's `ISO3` target and the classification route; ran the route in the frozen order 1, 3, 2: wrote and built the four named results of Section D that close step 1 — the uniform monomial form of every entry and of every coordinate of the Fourier tuple, the norm identity of a coordinate difference, and `fourier_circle_metric`; attempted step 3, the affine-extension lemma `ISO3-L`, and did not obtain it; attempted step 2, the special classes and the finiteness argument, and did not obtain it, the obstructions recorded in §7; the builds revealed elaboration details only (a nested `by` inside `exact` recovering with `sorry` and defeating `first`, `ring` falling back to `ring_nf` without failing so that `ring1` is needed inside `first`, the exponent bounds `0 ≤ 1` being `zero_le_one`, the sign of a factorization coefficient) and nothing about `ISO4`. No candidate isometry outside the family was found or sought, no theorem quantifying over an isometry of the normalized space was stated, and `ISO3-P` was not attempted. No transition family was tested against anything.

**The freeze-supplied facts that were in front of the execution in this span**, listed so that the
owner can weigh them: the freeze's `ISO3` target, its three hypotheses and four-shape conclusion, the three steps of the route in the order 1, 3, 2 as the freeze's reading — the intrinsic metric of the Fourier circle through the integer exponents, the optional extension lemma, the special classes and the cross distances — the frozen statement that no complete route to the four-shape conclusion is in hand, and the frozen obstructions of `ISO3-L` and `ISO3-P`.
...
**No execution defect is recorded; one hygiene item is disclosed.** The hypothesis `hw : star w * w = 1`
...
| --- | --- | --- | --- |
| `ISO2` | `ISO1-FAMILY-ACTS`, by `iso1_family_acts` at `d4b7738aafeb` | **opened** | — |
| `ISO3` | `ISO2-CLASSIFIED`, by `iso2_classes_single` at `f239305c3b5b` | **opened**: the route was run | **not applied** — `ISO2` classified internally, so the route was run |
| `ISO4` | `ISO3-UNDECIDED`, the route having been run, at `bbf87156d71e` | **closed**: `ISO4-NOT-EXECUTED` | — |
...
statement, quantifying over every finite carrier `W` with `[Fintype W] [DecidableEq W]`, every ancilla
...
declaration, named in no statement. **Thirty-five named results.**
...
with zero errors and one linter warning, the unreferenced binder name of §3.7.1.
