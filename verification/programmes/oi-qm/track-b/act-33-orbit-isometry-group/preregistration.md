...
- **`D`** = `db82376dfc0e2ae561ec541fd0d3a915f082beda`: act 32's receipt commit `Q` and the head of `main` after act 32's landing. It
...
| `bridge_of_tuple_isometry`, `tuple_isometry_of_bridge`, `eqOn_affineSpan_of_agree`, `exists_affineIsometryEquiv_of_isSurjIsometryOn` | `the same file` | 213, 251, 408, 415 |
| `a26_0_affine_extension`, `a26_1_circle_count`, `rigid_motion_of_tuple_isometry` | `the same file` | 444, 2160, 2309 |
| `mixedTriple_relabel2`, `fibreGram_unique`, `mixedTriple_transpose`, `relabel2_realizable` | `verification/lean-mathlib/OIBridge/OrbitGeometryIsometries.lean` | 72, 82, 94, 273 |
| `relabel2_isometry`, `conj_isometry`, `iso1_single_carrier`, `iso2_classes_single` | `the same file` | 300, 313, 770, 1081 |
...
| `dist_eq_norm_toLp`, `geo1_class_invariant`, `geo1_equiv_of_zero_single`, `fourier_dist_le` | `the same file` | 439, 471, 497, 727 |
| `a32_shared_separation`, `a32_shared_star_mul_self`, `a32_shared_relabel`, `a32_shared_fourier_star` | `verification/lean-mathlib/OIBridge/OrbitIsometryClassification.lean` | 347, 397, 406, 416 |
| `a32_shared_coord_inj`, `a32_shared_fourier_inj`, `a32_shared_pair`, `a32_shared_classify` | `the same file` | 427, 435, 648, 667 |
| `a32_control_overlap`, `a32_shared_exists`, `a32_shared_isometry`, `a32_not_rigid` | `the same file` | 692, 768, 804, 1049 |
...
| `FibreGram`, `GramPhaseEquiv`, `RealizableGram`, `sh1_necessity` | `verification/lean-mathlib/OIBridge/TwoSidedGauge.lean` | 95, 102, 108, 168 |
...
| `verification/lean-mathlib/OIBridge.lean` | `4a6ff5d03fa4ec01b60fd8d3e014804cdd89f48b` |
| `verification/lean-manuscript-census.json` | `40deafe0a4fed772c4824eba852b827e7179001f` |
| `verification/ROADMAP.md` | `b1f6f9f728d07c0361302ed1994d43e848522372` |
...
| 36258396274 | `baf76aa327a8cea603fc1fff9516a0effc006be1` | **the twenty frozen propositions of this file**, exactly as frozen, in their final organisation, and nothing else | all three jobs green. All twenty `#check`s elaborate and no error is reported; the release gate passes all 21 steps with eight receipts holding |
...
- **`A33-NOT-CLASSIFIED`:**
  ...
  > At the single-carrier configuration, the classification of the surjective isometries of the normalized space by the automorphisms of the circles' incidence graph `K₃,₃` and nine conjugation bits fails, by a witness exhibited in the kernel. `P0`'s threading part is untouched, no isometry of the normalized space is adopted as a physical symmetry, principle or law, and nothing here names, endorses or excludes a selection principle.
...
- `aa98c620d859c1e07ef013d76fbf82d0203284ef` (`A33-CLASSIFIED`);
- `5cbcbe2f85fb5cf034475a6d1ac14fdaf540fb14` (`A33-NOT-CLASSIFIED`).
...
**`7f67ac4346f543f5fb9d0677572942d64d2cd709`**, is written before `F` and added by the execution with exactly this blob.
...
- it checks the duality and single source of the frozen texts, together with 7 duality
...
- it applies 64 mutation controls, each of which must fail with its named code.

Run at `D` beside this file, it prints:

```text
controls: the two verdict propositions are duals and every shared text has one source; 7 duality mutations fail as required
controls: 3 rows hold as frozen, 64 mutation controls fail as required
