...
Executed from `main` at `7c209241384017069d556572b74733f82ea09e9a`, the merge of the verification
layout migration (PR #577). The obligation this round takes up was named by act 7 layer 1, merged at
`e0c0c709620db0d114dbd7061975b6747cb7aabc` (PR #574).
Act 7's own freeze is commit `06ea197f02e736dbeb4247c0ecd0f57f9b381a73`, blob
...
**What each answer would mean.** `CE1` reopens act 7 at its `D4a`/`D4b` adjudication — the signs
unset — and the `OI → QM` route resumes from there. `CE4` would say the OI class and Source
...
2. **The rooted family alone does not supply a full two-time family, absent an established
   composition or selection theorem** — so the two-time family is a **further selection**, and the
   round says so.

   **The reason is the absence of such a theorem, and NOT class membership.** Act 1's `BD3` settled
   that Source A's "indivisible stochastic process" is a **tuple/class definition**, and that class
   membership neither entails nor is entailed by failure of divisibility — Markov chains are
   members. So this round may not argue that the two-time family is undetermined *because* the
   process is indivisible, or treat non-composition as the defining feature of the class. It may
   only record that nothing merged or cited supplies the composition, which is a statement about the
   available theorems.
...
**Only `CE1` reopens act 7, and it reopens at the ADJUDICATION, not at an answer.** On `CE1`, and
only then, `D2` is repaired and act 7 resumes at the **`D4a`/`D4b` adjudication** — those two
questions asked and answered on the source, as act 7 froze them. The signs are **not** preselected
here.

**If** that adjudication returns `D4a` positive and `D4b` negative, act 7's frozen stop table then
routes to the **readback-amendment path** at **reduced strength**: layer 2 pauses for a merged
append-only amendment fixing the readback map before any layer-2 formalization or witness
calculation. Any other combination routes as that same table says, and this round fixes none of it.

**Nothing in this round promotes act 7's counterfactual to a result, and writing the signs into the
restart rule would have done exactly that.** Act 7's own note records `D4a`/`D4b` as **not reached**
and says of its conditional `D4a`-positive/`D4b`-negative reading that it "settles nothing in this
round". So that reading is carried here as a **prediction and a source reading only** — the position
act 7 is likely to resume to, never the position it resumes *at*. It may not be reported as a
finding, restated without its conditional, or treated as something `CE1` established. **`CE1`
establishes that a testable object exists — not what the test returns.**
...
15. **Only `CE1` reopens act 7, and at the `D4a`/`D4b` ADJUDICATION**, not at preselected signs.
    Act 7's conditional `D4a`-positive/`D4b`-negative reading is carried as a **prediction only**;
    if the adjudication returns those answers, act 7's frozen stop table routes to the readback
    amendment at reduced strength. The counterfactual is never promoted to a result.
...
family) which no merged or cited composition theorem determines from the first — stated that way,
and **not** as something indivisibility guarantees, per act 1's `BD3` — and that is where this freeze
...
- **Two PRs, in order, and exactly two.** Control-plane PR carrying **this file alone**, reviewed,
  frozen and merged before any execution; then exactly one execution/result PR from the resulting
  `main`.
- **No execution in the control-plane PR**: no witness search, no construction, no Lean, no probe
  guard, no roadmap edit.
- **`verification/ROADMAP.md`'s `P0` row is propagated by the EXECUTION PR, not by a third PR.** The
  roadmap lags this freeze by design — its own rule is that a status change "belongs in that linked
  artifact first; this file follows it" — so between the two PRs the row may still read `OPEN` while
  a merged freeze governs the obligation. That lag is accepted deliberately, for two reasons. It
  keeps the two-PR rule literally true rather than granting an administrative PR a silent exemption.
  And the execution PR **knows the outcome**, so it sets the row's settled status in one edit, where
  a propagation PR merged first would set `ACTIVE` and then have to change it again.
...
15. explicitly: whether act 7 reopens, and under `CE1` that it reopens at the **`D4a`/`D4b`
    adjudication** with the signs unset — act 7's conditional reading cited as a prediction and not
    promoted, and the readback-amendment path at reduced strength named as where its frozen stop
    table routes **if** that adjudication returns `D4a` positive and `D4b` negative;
    that `UB2` and `CE1` are existential and are not restated as classifications; that
