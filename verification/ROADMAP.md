...
| --- | --- | --- | --- | --- |
| **P0** | What additional structure determines the relative quantum evolution OI leaves free | OI→QM / Track B | **OPEN**, and now LOCALIZED: act 11's `GL2` proved the visible family does not fix the relative evolution; act 12 classified the per-slice lift freedom exactly — the two-sided uniform gauge generated by the separately maximal left and right factors has orbits determined by the fibre-Gram data modulo anchored phases (`TG2`), the visible law is exactly that data's diagonal and the residual freedom exactly its off-diagonal part (`SH1`), and act 11's open target is closed positively as right-only insufficiency carried by left moves (`RO1`) — so the open question is what selects or constrains the fibre-Gram trajectory **across time** | the conditional OI ⇔ QM statement; `P0a`/`P0b` closed, acts 11–12 landed |
...
**What this changes about `P0`.** The per-slice freedom is now known exactly, so the open question
is no longer its shape but **what additional condition selects or constrains the fibre-Gram
trajectory across time**. That is the next frontier, and it is not act 12's.
