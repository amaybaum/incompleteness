...
| --- | --- | --- | --- | --- |
| **P0** | What additional structure determines the relative quantum evolution OI leaves free | OI→QM / Track B | **OPEN**, and now LOCALIZED: act 11's `GL2` proved the visible family does not fix the relative evolution; act 12 classified the per-slice quotient by the defined two-sided action exactly — the two-sided uniform gauge generated by the separately maximal left and right factors has orbits determined by the fibre-Gram data modulo anchored phases (`TG2`), the visible law is exactly that data's diagonal and the residual freedom exactly its off-diagonal part (`SH1`), and act 11's open target is closed positively as right-only insufficiency carried by left moves (`RO1`) — so what remains is **two-part**: what selects or constrains the Gram/orbit trajectory **across time**, and what determines the cross-time threading within those orbits, which act 11's `GL2` shows is not fixed even by the full Gram trajectory | the status of dynamical selection within the already-established finite quantum representation; whether the emergence determines a unique quantum history; `P0a`/`P0b` closed, acts 11–12 landed |
