...
for _t in ('What the substratum does determine is a path',
           ...
           'That path is not a one-parameter group, the two layers not commuting, and it exhibits no generator',
           'whether one time-independent finite-range interaction has the update as its unit-time map is open, and nothing here decides it',
...
ok_ctn &= ('an exact norm-continuous path of local $*$-automorphisms of the quasilocal algebra from the '
           'identity to the update\'s Heisenberg action, which is not a one-parameter group and exhibits '
           'no generator') in _ctn_files['papers/Main.md']
_ctn_sum = ('while the update is joined to the identity by an exact norm-continuous path of local '
            '$*$-automorphisms of that algebra, not a one-parameter group and exhibiting no generator, a '
            'continuous-time Hamiltonian law remains additional structure')
for _rel in ('papers/Main.md', 'papers/Explainer.md', 'book/ch01-observation.md',
             'book/ch19-open-problems.md', 'book/The-Incompleteness-of-Observation-FULL.md'):
    ok_ctn &= _ctn_sum in _ctn_files[_rel]
for _rel in ('papers/Main.tex', 'papers/Explainer.tex', 'book/The-Incompleteness-of-Observation-FULL.tex'):
    ok_ctn &= 'not a one-parameter group and exhibiting no generator' in _ctn_files[_rel]
ok_ctn &= 'driveQ_isContinuousPath' in _ctn_files['papers/GR.tex'].replace('\\_', '_')
# no manuscript promotes the path to a group or a generator, or closes CT3
for _rel, _t in _ctn_files.items():
    ...
    for _bad in ('the update is generated by', 'generated by a time-independent',
                 ...
                 'the update is the exponential of'):
...
check('R7-CTN', ok_ctn,
      ...
      'and not a one-parameter group, with no generator exhibited and CT3 stated open, citing the '
      'SecondOrder modules; Main 2.3 and the quasilocal summaries in Main, the Explainer and both '
      'book mirrors carry the same clause, in source and generated forms; no manuscript promotes '
      'the path to a group or a generator or closes CT3; the kernel file asserts no group law and '
