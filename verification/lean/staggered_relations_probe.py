...
S1 Every field of the `Gens` structure holds EXACTLY (int64) for the concrete staggered
   ...
   and commutes with those of later axes; shifts on distinct axes commute in all four
   combinations. The Lean structure is therefore inhabited by the physical operators, so its
...
assert nthm>=15 and 'e : Nat → R' in src
