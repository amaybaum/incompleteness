#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""l3_core_relations_probe (b408) — semantic mirror for OI_L3_Core.lean.

R1 susskind3 instance obligations, EXACT on the lattice: for d = 3 and 4 (L = 4), the
   staggered summands A_mu = eta_mu (T_mu - T_mu^{-1}) pairwise anticommute and satisfy
   A_mu^2 = S_mu^2 in int64 — the hypotheses of the Lean theorem hold for the manuscript's
   concrete operators, so its conclusion IS the Theorem-2 identity (b406-verified).
R2 Generator relations (pre-mirroring the next file's derivation): eta^2 = 1, etas commute,
   eta_mu anti/commutes with earlier/later shifts per the sign pattern, shifts commute —
   all exact.
R3 Operator-level M1: random U with idempotent P — the mz_identity operator form
   P U^{t+1} = A (P U^t) + B K_t + B D^t Q holds to machine for t <= 6 with the file's K
   recursion; kernel commutation with an equivariant R; non-equivariant control breaks it.
R4 M3 statement forms: {D + m*eps + p0, eps} = (m+m) I + (p0 eps + p0 eps) and
   (D + m*eps)^2 = D^2 + m^2, int-exact.
R5 S9 in the c-general form over exact rationals.
R6 Lint: OI_L3_Core.lean has no sorry/admit, zero imports, >= 30 theorems; unchecked
   kernel-targeted .lean count = 2 (pilot + this file), both behind one gate.
"""
...
src=open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'OI_L3_Core.lean'),encoding='utf-8').read()
...
assert leans==['OI_B397_Pilot.lean','OI_L3_Core.lean']
print(f"R6 PASS: no sorry/admit; zero imports; {nthm} theorems; unchecked kernel-targeted files = 2, one gate")
