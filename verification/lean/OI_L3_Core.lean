/-
OI_L3_Core.lean — core-only Lean 4 formalization of the L3-SPEC-1 targets that need no
Mathlib: M1 (Theorem 1a, SM:238), M2.1-abstract (the Susskind factorization's algebraic
core, SM:282), M3.1/M3.2 (Theorem 3's chirality algebra, SM:296), S9.1 (the b401
boost-Ward identity).

STATUS: authored b408 under owner-directed continuation of the Lean audit; the L1 gate has
not yet run, so unchecked-file count is now TWO, both zero-import core-only (uniform risk
class; same class/calc patterns as the kernel-targeted pilot). Gate (see GATE.md):
    lean OI_B397_Pilot.lean && lean OI_L3_Core.lean
Acceptance mirror: l3_chain_mirror_probe.py (conclusions) + l3_core_relations_probe.py
(hypothesis instantiation on the concrete lattice operators, operator-level M1, lint).

Design notes. M1 is proved at OPERATOR level (identities in any ring generated by U, P
with P idempotent) — strictly stronger than the manuscript's state-applied display; the
mirror maps the two. M2 is proved ABSTRACTLY: from the generator relations (eta
involutions, commuting shifts, the sign pattern eta_mu T_nu = -T_nu eta_mu for nu < mu),
the summands anticommute pairwise and D^2 = sum (T - T^{-1})^2; the concrete d=3 lattice
operators satisfy every hypothesis (mirror-certified, int64-exact), so the manuscript
identity is the instance. d = 4 follows the same pattern (next file).
-/
...
/-! ## Part 4 (S9.1): the b401 quadratic boost-Ward identity, over any commutative ring
