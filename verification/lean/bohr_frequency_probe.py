...
check("F81", ok81,
      ...
      "(states_untouched, over EVERY consistent density family rather than only the reference one, "
      "and dynamics_untouched). Nothing frozen is weakened -- every finite-region ENDOMORPHIC "
      "Kraus instrument the Level-II theory supplies inside this fixed-carrier interface is "
      "available and conversely; Level-II's typed attachment and discard change the carrier, are "
      "not expressible by this predicate, and stay separately frozen -- and the theory is closed "
      "under the identity, composition on the union of regions, outcome relabelling, outcome "
      "coarse-graining and the frozen OI-induced dynamics. It withholds exactly one thing: the "
      "all-sites phase map is the total map of no available operation, at any finite outcome "
      "index. Hence the current frozen structure together with finite-support quasilocal "
      "availability does not entail genuinely infinite-support availability, within this "
      "fixed-algebra interface, and Q5 sharpens -- an extension requiring them needs some "
      "additional principle, an addition rather than a consequence, with operational completion a "
      "proposed name rather than a uniquely forced one. Verified "
      ...
      "impossibility; that AvailFS models the whole Level-II typed availability relation; that "
