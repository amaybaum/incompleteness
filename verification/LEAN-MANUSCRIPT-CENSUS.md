...
| instrument realization: one-instrument provenance | 1 | verification-only | the replacement of branch-wise realization by an inductive predicate with five constructors and no sum, sound for the branch-wise notion, generating a finite operational theory for every architecture, with the implementation-locality stack re-established under instrument names and inverse accessibility under phase saturation; the protocol invariant on the all-ones vector survives measurement and feed-forward, so provenance removes the replication of a contractive branch in every ones-fixing class; `Realized` and `ImplementationGenerated` are unchanged and no manuscript narrates the primitive |
