/-
  ...
  [SHARPENED IN ROUND FORTY-NINE BY `ReachabilitySeam.lean`: the one remaining item is the
  single analytic lemma `LocalReachabilityOfLieRank` — with `controlLie H U ⊇ su(D)`, the
  subgroup generated by the flows, the controls and the phases is a neighbourhood of `1` in
  the unitary group. The open-subgroup step, the connectedness of the matrix unitary group,
