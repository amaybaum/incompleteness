...
/-! ### A fixed two-dimensional sector refutes entanglement breaking

The converse direction of the separability threshold needs only this, and it is not about Weyl
operators at all: **a linear map that fixes the whole `2×2` matrix algebra generated by two
non-proportional vectors is not entanglement breaking.** The refutation is a single explicit vector,

    w (i, a)  =  x̄ᵢ ȳₐ − ȳᵢ x̄ₐ,

on which the partially transposed Choi matrix evaluates to `−Σ_{a,b} |yₐ x_b − xₐ y_b|²`, which is
`−2(‖x‖²‖y‖² − |⟨x,y⟩|²)` and so `−2‖x‖²‖y‖²` for orthogonal `x`, `y`.

THE CONJUGATION IS AN ACCEPTANCE GATE, like the Kraus transpose and the Choi index order. The
witness is `x̄ ⊗ ȳ − ȳ ⊗ x̄`, and the three obvious variants are wrong: conjugating only the ancilla
slot or only the output slot makes the form POSITIVE, the symmetric combination makes it positive,
and the unconjugated `x ⊗ y − y ⊗ x` gives the wrong value and can vanish outright. All four are
checked numerically against a genuinely complex plane, since on real vectors they coincide.

Note what is NOT needed: no eigenvalue, no spectral theorem, no Schmidt decomposition, no PPT
characterization, and no normalization of `x` or `y`. -/

/-- A rank-one operator expanded in matrix units. -/
theorem vecMulVec_eq_sum (u v : n → ℂ) :
    vecMulVec u v = ∑ i : n, ∑ j : n, (u i * v j) • Matrix.single i j (1 : ℂ) := by
  refine Matrix.ext fun k l => ?_
  rw [Matrix.sum_apply, Finset.sum_eq_single k]
  · rw [Matrix.sum_apply, Finset.sum_eq_single l]
    · simp [Matrix.vecMulVec_apply]
    · intro j _ hj; simp [hj]
    · intro hc; exact absurd (Finset.mem_univ _) hc
  · intro i _ hi
    refine (Matrix.sum_apply _ _ _ _).trans (Finset.sum_eq_zero fun j _ => ?_)
    simp [hi]
  · intro hc; exact absurd (Finset.mem_univ _) hc

/-- **The quadratic form of the partially transposed Choi matrix, regrouped.** Each pair of output
indices contributes the map's value on one rank-one operator built from the witness. This is where
linearity is used, and the only place. -/
theorem qform_ptranspose_choi (Φ : Matrix n n ℂ → Matrix n n ℂ)
    (hadd : ∀ ρ σ, Φ (ρ + σ) = Φ ρ + Φ σ) (hsmul : ∀ (c : ℂ) ρ, Φ (c • ρ) = c • Φ ρ)
    (w : n × n → ℂ) :
    qform (ptranspose (choi Φ)) w
      = ∑ a : n, ∑ b : n,
          Φ (vecMulVec (fun k => star (w (k, a))) (fun l => w (l, b))) b a := by
  have hΦ0 : Φ 0 = 0 := by
    have h := hsmul 0 0
    rwa [zero_smul, zero_smul] at h
  have hsum : ∀ (s : Finset n) (f : n → Matrix n n ℂ), Φ (∑ i ∈ s, f i) = ∑ i ∈ s, Φ (f i) := by
    intro s
    induction s using Finset.cons_induction with
    | empty => intro f; simpa using hΦ0
    | cons a s ha ih => intro f; rw [Finset.sum_cons, hadd, ih, Finset.sum_cons]
  have key : ∀ a b : n,
      Φ (vecMulVec (fun k => star (w (k, a))) (fun l => w (l, b))) b a
        = ∑ i : n, ∑ j : n, star (w (i, a)) * w (j, b) * Φ (Matrix.single i j (1 : ℂ)) b a := by
    intro a b
    have hL : Φ (∑ i : n, ∑ j : n, (star (w (i, a)) * w (j, b)) • Matrix.single i j (1 : ℂ))
        = ∑ i : n, ∑ j : n, (star (w (i, a)) * w (j, b)) • Φ (Matrix.single i j (1 : ℂ)) := by
      rw [hsum]
      exact Finset.sum_congr rfl fun i _ => by
        rw [hsum]
        exact Finset.sum_congr rfl fun j _ => hsmul _ _
    rw [vecMulVec_eq_sum, hL, Matrix.sum_apply]
    exact Finset.sum_congr rfl fun i _ => (Matrix.sum_apply _ _ _ _).trans
      (Finset.sum_congr rfl fun j _ => rfl)
  rw [Finset.sum_congr rfl fun a (_ : a ∈ Finset.univ) =>
    Finset.sum_congr rfl fun b (_ : b ∈ Finset.univ) => key a b, qform]
  simp only [ptranspose_apply, choi_apply, Fintype.sum_prod_type]
  rw [Finset.sum_comm]
  refine Finset.sum_congr rfl fun a _ => ?_
  rw [Finset.sum_congr rfl fun i (_ : i ∈ Finset.univ) => Finset.sum_comm, Finset.sum_comm]
  exact Finset.sum_congr rfl fun b _ => Finset.sum_congr rfl fun i _ =>
    Finset.sum_congr rfl fun j _ => by ring

theorem mul_star_self_pos {z : ℂ} (hz : z ≠ 0) : 0 < (z * star z).re := by
  have h : z * star z = ((Complex.normSq z : ℝ) : ℂ) := Complex.mul_conj z
  rw [h]
  simpa using Complex.normSq_pos.2 hz

/-- **A map fixing the matrix algebra of a two-dimensional sector is not entanglement breaking.**
The hypothesis `hne` is exactly non-proportionality of `x` and `y`, stated as one non-vanishing
`2 × 2` minor, which is all the witness needs. -/
theorem not_eb_of_fixed_plane {Φ : Matrix n n ℂ → Matrix n n ℂ}
    (hadd : ∀ ρ σ, Φ (ρ + σ) = Φ ρ + Φ σ) (hsmul : ∀ (c : ℂ) ρ, Φ (c • ρ) = c • Φ ρ)
    (x y : n → ℂ)
    (hfix : ∀ α β γ δ : ℂ,
      Φ (vecMulVec (α • x + β • y) (star (γ • x + δ • y)))
        = vecMulVec (α • x + β • y) (star (γ • x + δ • y)))
    {a₀ b₀ : n} (hne : x a₀ * y b₀ ≠ x b₀ * y a₀) :
    ¬ EntanglementBreaking Φ := by
  classical
  set w : n × n → ℂ :=
    fun p => star (x p.1) * star (y p.2) - star (y p.1) * star (x p.2) with hwdef
  -- the plane vector attached to an output index
  set u : n → (n → ℂ) := fun a => (y a) • x + (-(x a)) • y with hudef
  have hu1 : ∀ a : n, (fun k => star (w (k, a))) = u a := by
    intro a
    funext k
    show star (star (x k) * star (y a) - star (y k) * star (x a))
        = y a * x k + -(x a) * y k
    rw [star_sub, star_mul', star_mul', star_star, star_star, star_star, star_star]
    ring
  have hu2 : ∀ b : n, (fun l => w (l, b)) = star (u b) := by
    intro b
    funext l
    show star (x l) * star (y b) - star (y l) * star (x b)
        = star (y b * x l + -(x b) * y l)
    rw [star_add, star_mul', star_mul', star_neg]
    ring
  have hentry : ∀ a b : n,
      Φ (vecMulVec (fun k => star (w (k, a))) (fun l => w (l, b))) b a
        = -((y a * x b - x a * y b) * star (y a * x b - x a * y b)) := by
    intro a b
    rw [hu1, hu2, hudef]
    rw [hfix (y a) (-(x a)) (y b) (-(x b)), Matrix.vecMulVec_apply]
    show ((y a) • x + (-(x a)) • y) b * star (((y b) • x + (-(x b)) • y) a) = _
    show (y a * x b + -(x a) * y b) * star (y b * x a + -(x b) * y a) = _
    rw [show y b * x a + -(x b) * y a = -(y a * x b - x a * y b) by ring, star_neg]
    ring
  have hval : qform (ptranspose (choi Φ)) w
      = -∑ a : n, ∑ b : n, ((y a * x b - x a * y b) * star (y a * x b - x a * y b)) := by
    rw [qform_ptranspose_choi Φ hadd hsmul w,
      Finset.sum_congr rfl fun a (_ : a ∈ Finset.univ) =>
        Finset.sum_congr rfl fun b (_ : b ∈ Finset.univ) => hentry a b]
    simp only [Finset.sum_neg_distrib]
  have hpos : 0 < (∑ a : n, ∑ b : n,
      ((y a * x b - x a * y b) * star (y a * x b - x a * y b))).re := by
    rw [Complex.re_sum]
    refine Finset.sum_pos' (fun a _ => ?_) ⟨a₀, Finset.mem_univ _, ?_⟩
    · rw [Complex.re_sum]
      exact Finset.sum_nonneg fun b _ => (mul_star_self _).1
    · rw [Complex.re_sum]
      refine Finset.sum_pos' (fun b _ => (mul_star_self _).1) ⟨b₀, Finset.mem_univ _, ?_⟩
      refine mul_star_self_pos fun hc => hne ?_
      linear_combination -hc
  refine not_eb_of_neg_witness (w := w) ?_
  rw [hval, Complex.neg_re, neg_lt_zero]
  exact hpos
...
#print axioms vecMulVec_eq_sum
#print axioms qform_ptranspose_choi
#print axioms not_eb_of_fixed_plane
