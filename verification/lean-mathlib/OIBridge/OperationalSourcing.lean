/-
  ...
  and 2, and consequence 3 as narrowed by Amendment 1.  It also carries **S3a, the trivialization
  of representation-augmented access**, which consumes consequence 3.

  S1, S2 and S3b are independent, as the frozen dependency structure records: none is proved on the
  strength of another, and S3b's route is the access semantics rather than S1 or S2.  S3a is the one
  target that is not independent: it consumes S1's consequence 3 by construction, at exactly one
  place (`repAugmented_allUnitaries_nonempty`), and says so where it is stated.
  ...
  **inherited**, and nothing here re-proves it, restates it more strongly, or counts it as this
  ...
  WHAT IS AND IS NOT PROVED.  The S3b package proves class-extensionality of availability, together
  ...
  equal `permClass`; that is the augmentation question, and S3a answers it below rather than there.  The
...
/-! ### S3a — the trivialization of representation-augmented access

The frozen target: let the augmented class be the implementation class generated by the stated
access together with every unitary occurring in some representation of some OI-realizable family;
show that it is an `Architecture`, that it is not `OnesFixing`, and that its generated theory has
`PhasesAvailable` — and report the **disqualification**, not the availability.

The generated class is constructed here rather than assumed: `archGen 𝓙` is the inductive closure
of a base class under exactly the six `Architecture` fields, so `archGen_arch` is a theorem and not
a hypothesis.  The augmented class is then `repAugmented := archGen repBase`, with `repBase` the
stated access `permClass` together with `RepUnitary`.

**S3a consumes S1's consequence 3** — that is the one dependence between targets in this round, and
it enters at exactly one place, `repAugmented_allUnitaries_nonempty`.  Amendment 1's split is
carried explicitly: nonempty carriers come from consequence 3, the empty carrier from the stated
access itself (`archGen_of_isEmpty`) with no appeal to representation.  A single uniform argument
does not cover every finite carrier, and the statements say so. -/

section S3a

/-- **A REPRESENTATION UNITARY**: an operator that occurs, up to the relabelling implementation
classes are already invariant under, as the unitary of a representation of an OI-realizable
family. -/
def RepUnitary (S : Type) [Fintype S] [DecidableEq S] (K : Matrix S S ℂ) : Prop :=
  ∃ (W : Type) (fW : Fintype W) (dW : DecidableEq W),
    letI := fW
    letI := dW
    ∃ Γ : ℕ → Matrix W W ℝ, FiniteRootedRealizable Γ ∧
      ∃ Q : QfbData W, Q.IsLaw ∧ Q.PositiveRootMass
        ∧ (∀ (a : W) (t : ℕ) (j : W), Γ t a j = Q.rooted t a j)
        ∧ ∃ e : Q.Bas ≃ S, Matrix.reindex e e Q.U = K

/-- **THE BASE OF THE AUGMENTATION**: the stated access, together with every representation
unitary.  This is the set the frozen S3a augments by, before any closure. -/
def repBase : ImplementationClass := fun S _ _ K => permClass S K ∨ RepUnitary S K

/-- **THE ARCHITECTURE GENERATED BY A CLASS**: the inductive closure of `𝓙` under exactly the six
operations an `Architecture` is closed under.  Nothing else is added, and the closure is a
construction rather than a hypothesis, so "the class generated by" denotes something. -/
inductive archGen (𝓙 : ImplementationClass) :
    ∀ (S : Type) [Fintype S] [DecidableEq S], Matrix S S ℂ → Prop
  | base {S : Type} [Fintype S] [DecidableEq S] {K : Matrix S S ℂ} (h : 𝓙 S K) : archGen 𝓙 S K
  | one (S : Type) [Fintype S] [DecidableEq S] : archGen 𝓙 S 1
  | mul {S : Type} [Fintype S] [DecidableEq S] (K L : Matrix S S ℂ)
      (hK : archGen 𝓙 S K) (hL : archGen 𝓙 S L) : archGen 𝓙 S (K * L)
  | smul {S : Type} [Fintype S] [DecidableEq S] (a : ℂ) (K : Matrix S S ℂ)
      (ha : ‖a‖ ≤ 1) (hK : archGen 𝓙 S K) : archGen 𝓙 S (a • K)
  | proj (S : Type) [Fintype S] [DecidableEq S] (m : ℕ) (k : Fin m) :
      archGen 𝓙 (S × Fin m) (Matrix.diagonal fun r => if r.2 = k then 1 else 0)
  | block {S : Type} [Fintype S] [DecidableEq S] (m : ℕ)
      (K : Matrix (S × Fin m) (S × Fin m) ℂ) (f e : Fin m)
      (hK : archGen 𝓙 (S × Fin m) K) : archGen 𝓙 S (AncillaClosure.ancBlock K f e)

/-- The generated class is an architecture.  Each field is one constructor, which is what makes the
inductive the right closure and not merely a superset of one. -/
theorem archGen_arch (𝓙 : ImplementationClass) : Architecture (archGen 𝓙) where
  one := fun S _ _ => archGen.one S
  mul := fun _ _ _ K L hK hL => archGen.mul K L hK hL
  smul := fun _ _ _ a K ha hK => archGen.smul a K ha hK
  proj := fun S _ _ m k => archGen.proj S m k
  block := fun _ _ _ m K f e hK => archGen.block m K f e hK

/-- **THE GENERATED CLASS IS THE SMALLEST ONE**: any architecture containing the base contains the
closure.  This is what makes `archGen 𝓙` *the* class generated by `𝓙`, rather than merely some
architecture lying above it. -/
theorem archGen_le {𝓙 𝓘 : ImplementationClass} (arch : Architecture 𝓘)
    (hbase : ∀ (S : Type) [Fintype S] [DecidableEq S] (K : Matrix S S ℂ), 𝓙 S K → 𝓘 S K)
    {S : Type} [Fintype S] [DecidableEq S] {K : Matrix S S ℂ} (h : archGen 𝓙 S K) : 𝓘 S K := by
  induction h with
  | base hb => exact hbase _ _ hb
  | one S => exact arch.one S
  | mul K L _ _ ihK ihL => exact arch.mul _ K L ihK ihL
  | smul a K ha _ ih => exact arch.smul _ a K ha ih
  | proj S m k => exact arch.proj S m k
  | block m K f e _ ih => exact arch.block _ m K f e ih

/-- **THE REPRESENTATION-AUGMENTED ACCESS.** -/
def repAugmented : ImplementationClass := archGen repBase

theorem repAugmented_arch : Architecture repAugmented := archGen_arch repBase

/-- The stated access is contained in the augmented access, as an augmentation must be. -/
theorem permClass_le_repAugmented (S : Type) [Fintype S] [DecidableEq S] (K : Matrix S S ℂ)
    (h : permClass S K) : repAugmented S K :=
  archGen.base (Or.inl h)

/-- **AMENDMENT 1'S SPLIT, THE EMPTY HALF.**  At the empty carrier there is exactly one matrix, so
it is the identity, and every generated architecture contains the identity.  The case is settled by
the stated access, with no appeal to representation — which is what Amendment 1 requires, because
the frozen universal form of consequence 3 is false there. -/
theorem archGen_of_isEmpty (𝓙 : ImplementationClass) (S : Type) [Fintype S] [DecidableEq S]
    [IsEmpty S] (K : Matrix S S ℂ) : archGen 𝓙 S K := by
  have hK : K = 1 := by
    ext i
...
/-- **AMENDMENT 1'S SPLIT, THE NONEMPTY HALF — WHERE CONSEQUENCE 3 IS CONSUMED.**  Every unitary on
every nonempty finite carrier is in the augmented access, because consequence 3 exhibits it as the
unitary of a representation of an OI-realizable family. -/
theorem repAugmented_allUnitaries_nonempty (S : Type) [Fintype S] [DecidableEq S] [Nonempty S]
    (K : Matrix S S ℂ) (hK : K ∈ Matrix.unitaryGroup S ℂ) : repAugmented S K := by
  obtain ⟨Γ, hΓ, Q, hQ, hQr, hrep, e, he⟩ := consequence_augmentedAll S K hK
  exact archGen.base (Or.inr ⟨Fin 1, inferInstance, inferInstance, Γ, hΓ, Q, hQ, hQr, hrep, e, he⟩)

/-- **THE AUGMENTED ACCESS CONTAINS EVERY UNITARY ON EVERY FINITE CARRIER**, by the split
Amendment 1 requires and not by one uniform argument. -/
theorem repAugmented_allUnitaries (S : Type) [Fintype S] [DecidableEq S] (K : Matrix S S ℂ)
    (hK : K ∈ Matrix.unitaryGroup S ℂ) : repAugmented S K := by
  rcases isEmpty_or_nonempty S with _ | _
  · exact archGen_of_isEmpty repBase S K
  · exact repAugmented_allUnitaries_nonempty S K hK

/-- **THE AUGMENTED ACCESS IS NOT ONES-FIXING**: it contains the quarter phase, which moves the
all-ones ray.  This is the property the merged `permClass` has and the augmented class loses. -/
theorem repAugmented_not_onesFixing : ¬ OnesFixing repAugmented := by
  intro hf
  have hmem : phaseGate (0 : Fin 2) ∈ Matrix.unitaryGroup (Fin 2) ℂ :=
    Matrix.mem_unitaryGroup_iff'.mpr (phaseGate_unitary (0 : Fin 2))
  obtain ⟨z, hz⟩ := hf (Fin 2) (phaseGate 0)
    (repAugmented_allUnitaries (Fin 2) _ hmem) (phaseGate_unitary 0)
  exact phaseGate_moves_ones (by simp) 0 z hz

/-- **THE AUGMENTED ACCESS SUPPLIES THE QUARTER PHASE AT EVERY LEVEL.** -/
theorem repAugmented_phasesAvailable (A : Type) [Fintype A] [DecidableEq A] :
    PhasesAvailable (genTheory repAugmented repAugmented_arch A) := by
  intro n a
  have hmem : phaseGate a ∈ Matrix.unitaryGroup (A × Fin n) ℂ :=
    Matrix.mem_unitaryGroup_iff'.mpr (phaseGate_unitary a)
  exact InstAvail.op (phaseGate a) (repAugmented_allUnitaries (A × Fin n) _ hmem)
    (phaseGate_unitary a)

/-- **S3a — THE TRIVIALIZATION.**  Every unitary conjugation, at every carrier and every level, is
available under the representation-augmented access.

What the round reports is the **disqualification** and not the availability: a criterion that
returns *Sourced* for every resource expressible in an admissible operator has no discriminating
power, so representation-augmentation is not a route to sourcing.  This is what makes the frozen
disposition criterion non-vacuous, and it is why S1 outranks S3 in the frozen outcome precedence. -/
theorem repAugmented_everyUnitaryAvailable (A : Type) [Fintype A] [DecidableEq A] (n : ℕ)
    (K : Matrix (A × Fin n) (A × Fin n) ℂ) (hK : Kᴴ * K = 1) :
    (genTheory repAugmented repAugmented_arch A).availExt n Unit
      (fun _ => MonoidalCompletion.conjChannel K) :=
  InstAvail.op K
    (repAugmented_allUnitaries (A × Fin n) K (Matrix.mem_unitaryGroup_iff'.mpr hK)) hK
...
#print axioms OIBridge.OperationalSourcing.archGen_arch
#print axioms OIBridge.OperationalSourcing.archGen_le
#print axioms OIBridge.OperationalSourcing.repAugmented_arch
#print axioms OIBridge.OperationalSourcing.permClass_le_repAugmented
#print axioms OIBridge.OperationalSourcing.archGen_of_isEmpty
#print axioms OIBridge.OperationalSourcing.repAugmented_allUnitaries_nonempty
#print axioms OIBridge.OperationalSourcing.repAugmented_allUnitaries
#print axioms OIBridge.OperationalSourcing.repAugmented_not_onesFixing
#print axioms OIBridge.OperationalSourcing.repAugmented_phasesAvailable
#print axioms OIBridge.OperationalSourcing.repAugmented_everyUnitaryAvailable
